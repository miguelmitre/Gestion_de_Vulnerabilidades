           ...
           COMMAND "Ingresa" "Ingresa transacciones por folio"
              ...
              DISPLAY HOY USING "dd-mm-yyyy" AT 5,67 ATTRIBUTE(REVERSE)
                   ...
                   IF (g_reg.proceso_cod = "00041" OR 
                       ...
                       g_reg.proceso_cod = "00055" OR
                       g_reg.proceso_cod = "00056") THEN
...
-----<erm 25-Oct-2005
      ...
      AFTER FIELD vfolio
          ...
          --AND    proceso_cod <> '00008'

          GROUP BY 1
          IF STATUS <> NOTFOUND THEN

              IF vproceso = "00055" OR
                 vproceso = "00056" THEN
              ...
              IF vproceso = "00008" THEN
                 DISPLAY "Fecha de Proceso        " AT 16,16
                 NEXT FIELD vfecha
              END IF
{
     ...
     OR vpregunta = "s"  THEN
         ...
         ELSE
             CASE vproceso
                 ...
                 WHEN "00056"
                     CALL bono_pension_salida()         ---erm 21 Nov 2008
                 WHEN "00060"
                     CALL retiros_diversos()         #rd
                 WHEN "00061"
                     CALL retiros_diversos()         #rd
                 ...
                 WHEN "00072"
                     CALL retiros_diversos()         #rd
                 WHEN "00107" CALL interes_transito_issste()
                 WHEN "00161" CALL retiros_diversos()
                 WHEN "00170" CALL retiros_diversos()
                 #erm <= 
           ...
           #fhh se anexo 02-Dic-2004
             ...
             WHEN 90093
               LET  principal = " SELECT ROUND(SUM(a.monto_en_acciones),2),",
                                ...
                                " AND  (subcuenta in(","'", subcta,"'",")",
                                " OR   subcuenta in(","'", subcta1,"'","))",
                                " AND    a.tipo_movimiento in(1,4,220,240,248,271,295,293,294) ",
...
#rp----------------------------
    ...
    IF (g_reg5.proceso_cod = "00016" OR
        ...
        g_reg5.proceso_cod = "00055" OR
        g_reg5.proceso_cod = "00056") THEN
...
WHILE TRUE # Se mantiene dentro de un ciclo a menos que el usuario aborte la
    ...
    INPUT BY NAME reg.* WITHOUT DEFAULTS
      ...
      BEFORE FIELD vfolio
            ...
            IF (reg.vproceso >= "00001"  AND reg.vproceso <= "00004") OR
               reg.vproceso = "00008" THEN
...
#ia---------------------------
    ...
    DEFINE  xfolio        INTEGER
	   ...
	   LET xfolio = vfolio
        END IF
-->CPL_1502 CAMBIA EL SELECT * POR EL SELECT... SUM(importe)
        SELECT folio,
               fecha_emision,
               fecha_valor,
               identificador,
...
#ia---------------------------
    ...
    DEFINE vsum,vimp_ent2,vimp_frac,
           vcomp_frac     DECIMAL(15,2),
           vimp_ent       DECIMAL(15,0)
    DEFINE vsief          SMALLINT
...
#arc
    ...
    IF g_reg5.proceso_cod = '00039' THEN
       DECLARE cur_sum CURSOR FOR
          SELECT SUM(importe),siefore
          FROM  con_transaccion
          WHERE proceso_cod     = '00039'
          AND   fecha_emision = '10082010'
          AND   identificador   = 2
          AND   transaccion_cod NOT IN(99991)
          AND   siefore = g_reg5.siefore
          GROUP BY 2
          ORDER BY 2
       FOREACH cur_sum INTO vsum,vsief
          IF vsum > 0 THEN 

             IF vsief = g_reg5.siefore THEN
                LET vimp_ent   = vsum + (1 * 1)
                LET vimp_frac  = vimp_ent - vsum   #pesos
                LET vcomp_frac = vimp_frac * vprecio
                LET vcomp_frac = vcomp_frac * -1
                LET g_reg5.importe = vcomp_frac
                LET g_reg5.identificador   = 1

                INSERT INTO con_transaccion VALUES(g_reg5.*)

                LET vimp_frac = vimp_frac * -1
                LET g_reg5.importe = vimp_frac
                LET g_reg5.identificador   = 2
...
FUNCTION recaudacion_issste()
    ...
    LET g_reg.estado          = 10
     ...
     DECLARE con_rec_issste CURSOR FOR
         SELECT transaccion_cod,subcuenta,subcuenta1
         FROM   tab_transaccion
         WHERE  proceso_cod = g_reg.proceso_cod
         AND    descripcion_1 NOT MATCHES "*FRACCION"

     FOREACH con_rec_issste INTO vtransaccion,subcta,subcta1
         ...
         LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                ...
                WHEN 29024
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " WHERE  a.folio =","'", g_reg.folio,"'",
                                   " AND  (a.subcuenta in(","'", subcta,"'",")",
                                   " OR   a.subcuenta in(","'", subcta1,"'","))",
                                   --" AND    a.tipo_movimiento in (1,2,3,4) ",
                                   " AND    a.tipo_movimiento not in(888,999) ",
                                   " GROUP BY 3,4 "
                    PREPARE rec_iss_29024 FROM  principal
                    DECLARE rec_isss29024 CURSOR FOR rec_iss_29024
                    FOREACH rec_isss29024 INTO total_pesos,total_acciones,f_conversion,vsiefore
...
--->subcuentas 34 y 35
                  ...
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento in (1,2,3,4) ",
                                   " GROUP BY 3,4 "
                    PREPARE rec_iss_29029 FROM  principal
                    DECLARE rec_isss_29029 CURSOR FOR rec_iss_29029
                    FOREACH rec_isss_29029 INTO total_pesos,total_acciones,f_conversion,vsiefore
                    ...
                    LET  principal_2 = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                          ...
                                          " AND siefore = ",vsiefore,
                                          " GROUP BY 3,4 "
                          PREPARE rec_is_290292 FROM  principal_2
                          DECLARE rec_is_290293 CURSOR FOR rec_is_290292
                          FOREACH rec_is_290293 INTO total_pesos_com,total_acciones_com,f_conversion,vsiefore
...
---<subcuentas 34 y 35
                 ...
                 WHEN 22616  #comisiones
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   " a.fecha_conversion, a.siefore ",
                                   " FROM   dis_cuenta a ",
                                   " WHERE  a.folio =","'", g_reg.folio,"'",
                                   " AND    a.subcuenta =","'", subcta,"'",
                                   " AND    a.tipo_movimiento BETWEEN 100 AND 119 ",
                                   " GROUP BY 2,3 "
                ...
                WHEN 29022
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento not in(888,999) ",
                                   " GROUP BY 3,4 "
                    PREPARE rec_iss_29022 FROM  principal
                    DECLARE rec_isss29022 CURSOR FOR rec_iss_29022
                    FOREACH rec_isss29022 INTO total_pesos,total_acciones,f_conversion,vsiefore
                ...
                WHEN 29032
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento not in(888,999) ",
                                   " GROUP BY 3,4 "
                    PREPARE rec_iss_29032 FROM  principal
                    DECLARE rec_isss29032 CURSOR FOR rec_iss_29032
                    FOREACH rec_isss29032 INTO total_pesos,total_acciones,f_conversion,vsiefore
...
FUNCTION bono_pension_salida()
#revalorizacion al día en que salieron (cedidos o redimidos) 
    DEFINE vtransaccion    INTEGER
    DEFINE f_conversion    DATE
    #DEFINE subcta          CHAR(05)
    DEFINE vsiefore        SMALLINT
    ...
    LET g_reg.estado          = 20
     ...
     FOREACH bono_sal_2 INTO vtransaccion,subcta,subcta1,subcta2
            ...
            CASE vtransaccion 
               WHEN  29023
                    LET principal = " SELECT ROUND(SUM(mov_dia_710401),2), ",
                                    " fecha_actualiza ",
                                    --" FROM   act_dia_bono ",
                                    " FROM   act_dia_his_bono ",
                                    " WHERE  fecha_actualiza = ","'",vfecha,"'",
                                    " GROUP BY 2 "
                    PREPARE bono_sal1 FROM  principal
                    DECLARE bono_pen_sal CURSOR FOR bono_sal1
                    FOREACH bono_pen_sal INTO total_acciones,f_conversion
               ...
               WHEN  29024
                    LET principal = " SELECT ROUND(SUM(mov_dia_710402),2), ",
                                    ...
                                    " WHERE  fecha_actualiza = ","'",vfecha,"'",
                                    " GROUP BY 2 "
                    PREPARE bono_sal2 FROM  principal
                    DECLARE bono_pen_sal1 CURSOR FOR bono_sal2
                    FOREACH bono_pen_sal1 INTO total_acciones,f_conversion
               ...
               WHEN  29025
                    LET principal = " SELECT ROUND(SUM(mov_dia_710403),2), ",
                                    ...
                                    " WHERE  fecha_actualiza = ","'",vfecha,"'",
                                    " GROUP BY 2 "
                    PREPARE bono_sal3 FROM  principal
                    DECLARE bono_pen_sal2 CURSOR FOR bono_sal3
                    FOREACH bono_pen_sal2  INTO total_pesos,f_conversion
               ...
               WHEN  29026
                    LET principal = " SELECT ROUND(SUM(mov_dia_710404),2), ",
                                    ...
                                    " WHERE  fecha_actualiza = ","'",vfecha,"'",
                                    " GROUP BY 2 "
                    PREPARE bono_sal4 FROM  principal
                    DECLARE bono_pen_sal3 CURSOR FOR bono_sal4
                    FOREACH bono_pen_sal3 INTO total_pesos,f_conversion
...
FUNCTION unificacion_sar_issste()
    DEFINE vtransaccion    INTEGER
    DEFINE f_conversion    DATE
    DEFINE f_proceso       DATE
    DEFINE vsiefore        SMALLINT
    ...
    LET g_reg.estado          = 10
     ...
     DECLARE con_uni_sariss CURSOR FOR
         SELECT transaccion_cod,subcuenta,subcuenta1
         FROM   tab_transaccion
         WHERE  proceso_cod = g_reg.proceso_cod
         AND    descripcion_1 NOT MATCHES "*FRACCION"

     FOREACH con_uni_sariss INTO vtransaccion,subcta,subcta1
            ...
            CASE vtransaccion
                WHEN 90093
                  ...
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento in (1,4) ",
                                   " GROUP BY 3,4 "
                    PREPARE rec_unisar FROM  principal
                    DECLARE rec_unisars2 CURSOR FOR rec_unisar
                    FOREACH rec_unisars2 INTO total_pesos,total_acciones,f_conversion,vsiefore
                ...
                WHEN 29024
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento not in(888,999) ",
                                   " GROUP BY 3,4 "
                    PREPARE rec_unisar_29024 FROM  principal
                    DECLARE rec_unisar29024 CURSOR FOR rec_unisar_29024
                    FOREACH rec_unisar29024 INTO total_pesos,total_acciones,f_conversion,vsiefore
...
--->subcuentas 34 y 35
                  ...
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento in (1,4) ",
                                   " GROUP BY 3,4 "
                    PREPARE rec_unisar_29029 FROM  principal
                    DECLARE rec_unisars_29029 CURSOR FOR rec_unisar_29029
                    FOREACH rec_unisars_29029 INTO total_pesos,total_acciones,f_conversion,vsiefore
...
---<subcuentas 34 y 35
                 ...
                 WHEN 22616  #comisiones
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento BETWEEN 100 AND 119 ",
                                   " GROUP BY 2,3 "
                     PREPARE rec_unisar2 FROM  principal
                     DECLARE rec_unisars3 CURSOR FOR rec_unisar2
                     FOREACH rec_unisars3 INTO total_pesos,f_conversion,vsiefore
                        LET g_reg.importe       = total_pesos
                        LET g_reg.fecha_valor   = f_conversion
                        LET g_reg.fecha_emision = f_conversion
                        LET g_reg.siefore       = vsiefore
                 ...
                 WHEN 20115  #comisiones
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento BETWEEN 100 AND 119 ",
                                   " GROUP BY 2,3 "
                     PREPARE rec_unisar3 FROM  principal
                     DECLARE rec_unisars4 CURSOR FOR rec_unisar3
                     FOREACH rec_unisars4 INTO total_pesos,f_conversion,vsiefore
                        LET g_reg.importe       = total_pesos
                        LET g_reg.fecha_valor   = f_conversion
                        LET g_reg.fecha_emision = f_conversion
                        LET g_reg.siefore       = vsiefore
                 ...
                 WHEN 21615  #comisiones
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento BETWEEN 100 AND 119 ",
                                   " GROUP BY 2,3 "
                     PREPARE rec_unisar4 FROM  principal
                     DECLARE rec_unisars5 CURSOR FOR rec_unisar4
                     FOREACH rec_unisars5 INTO total_pesos,f_conversion,vsiefore
                        LET g_reg.importe       = total_pesos
                        LET g_reg.fecha_valor   = f_conversion
                        LET g_reg.fecha_emision = f_conversion
                        LET g_reg.siefore       = vsiefore
                ...
                WHEN 29022
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento not in(888,999) ",
                                   " GROUP BY 3,4 "
                    PREPARE rec_unisar_29022 FROM  principal
                    DECLARE rec_unisars29022 CURSOR FOR rec_unisar_29022
                    FOREACH rec_unisars29022 INTO total_pesos,total_acciones,f_conversion,vsiefore
                ...
                WHEN 29032
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento not in(888,999) ",
                                   " GROUP BY 3,4 "
                    PREPARE rec_unisar_29032 FROM  principal
                    DECLARE rec_unisars29032 CURSOR FOR rec_unisar_29032
                    FOREACH rec_unisars29032 INTO total_pesos,total_acciones,f_conversion,vsiefore
                ...
                WHEN 29025
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_pesos),2), ",
                                   ...
                                   " AND    a.tipo_movimiento not in(888,999) ",
                                   " GROUP BY 3,4 "
                    PREPARE rec_unisar_29025 FROM  principal
                    DECLARE rec_unisars29025 CURSOR FOR rec_unisar_29025
                    FOREACH rec_unisars29025 INTO total_pesos,total_acciones,f_conversion,vsiefore
...
FUNCTION interes_transito_issste()
    ...
    FOREACH int_t1_iss INTO vtransaccion,subcta,subcta1,subcta2
            ...
            CASE vtransaccion
                WHEN 90093
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_acciones),2),",
                                 ...
                                 " GROUP BY 3,4 "
                    PREPARE int_t2_iss FROM  principal
                    DECLARE int_t3_iss CURSOR FOR int_t2_iss
                    FOREACH int_t3_iss INTO total_pesos,total_pesos1,f_conversion,
                                        vsiefore
                        LET g_reg.importe       = total_pesos
                        LET g_reg.fecha_valor   = f_conversion
                        LET g_reg.fecha_emision = f_conversion
                ...
                WHEN 29024
                  LET  principal = " SELECT ROUND(SUM(a.monto_en_acciones),2),",
                                  ...
                                  " AND    a.tipo_movimiento in (2,4) ",
                                 " GROUP BY 3,4 "
                    PREPARE int_t4_iss FROM  principal
                    DECLARE int_t5_iss CURSOR FOR int_t4_iss
                    FOREACH int_t5_iss INTO total_pesos,total_pesos1,f_conversion,
                                        vsiefore
                        LET g_reg.importe       = total_pesos
                        LET g_reg.fecha_valor   = f_conversion
                        LET g_reg.fecha_emision = f_conversion
                 ...
                 WHEN 29027
                    LET  principal = " SELECT ROUND(SUM(monto_en_acciones),2),",
                                   ...
                                   " WHERE  folio =","'", g_reg.folio,"'",
                                   " AND    subcuenta =","'", subcta,"'",
                                   " AND    tipo_movimiento in (2,4)  ",
                                 " GROUP BY 3,4 "
                     PREPARE int_t6_iss FROM  principal
                     DECLARE int_t7_iss CURSOR FOR int_t6_iss
                     FOREACH int_t7_iss INTO total_pesos,total_pesos1,f_conversion,
                                             vsiefore
                        LET g_reg.importe       = total_pesos
                        LET g_reg.fecha_valor   = f_conversion
                        LET g_reg.fecha_emision = f_conversion
                 ...
                 WHEN 29029
                    LET  principal = " SELECT ROUND(SUM(monto_en_acciones),2),",
                                   ...
                                   " AND    tipo_movimiento in (2,4)  ",
                                   " GROUP BY 3,4 "
                     PREPARE int_t8_iss FROM  principal
                     DECLARE int_t9_iss CURSOR FOR int_t8_iss
                     FOREACH int_t9_iss INTO total_pesos,total_pesos1,f_conversion,
                                             vsiefore
                        LET g_reg.importe       = total_pesos
                        LET g_reg.fecha_valor   = f_conversion
                        LET g_reg.fecha_emision = f_conversion
