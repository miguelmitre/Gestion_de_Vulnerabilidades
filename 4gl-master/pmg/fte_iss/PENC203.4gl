...
MAIN
    ...
    CALL ERRORLOG("SE EJECUTA PROGRAMA DE OP72 VERSION "||VERSION)
...
FUNCTION f_captura_fechas()
    ...
    LET lr_fechas.inicio    = MDY(MONTH(HOY),"01",YEAR(HOY))  --INV-4730
    LET lr_fechas.fin       = f_ultimo_dia_mes(HOY)             --INV-4730
...
FUNCTION f_genera_detalle(pr_fechas)
  ...
  --DEFINE lr_biometricos         RECORD LIKE tbl_biometricos_sellos_retiro_total.*      --INV-4731 -- SE QUITA LA "S" POR INDICACION DEL AFORE INV-4947
    ...
    FOREACH cur_det INTO lr_sol_pmg.*, ls_id_pri_pago
        ...
        LET ld_monto_vigente = f_obten_pago_mensual_actual()

        IF ld_monto_vigente = 0 OR ld_monto_vigente IS NULL THEN      
           --como se venia realizando                                 
           LET ld_monto_vigente = lr_sol_pmg.pago_mensual          
        END IF 
    
        IF ls_num_mensualidad > 1 AND lr_sol_pmg.tipo_retiro = 'G' THEN 
...
FUNCTION f_inserta_pago_saldo(pr_solicitud, pr_fechas, ps_id_pago)
   ...
   FOREACH cur_total INTO ld_subcuenta, ls_siefore, ld_mto_acciones
      ...
      IF ld_subcuenta <> 35 THEN
          LET ls_edo_viv      = NULL
          LET ldt_fecha_val   = HOY --TENUA NULL
      ELSE
