...
MAIN
	...
	OPEN WINDOW ventana_1 AT 2,2 WITH FORM "COMC0231" ATTRIBUTE( BORDER)
	DISPLAY " COMC023   REPORTE DE PAGO DE COMISIONES RELIQUIDACION DETALLE                       " AT 3,1 ATTRIBUTE(REVERSE) 
   ...
   PREPARE  query_1    FROM     g_grep_01
    ...
    ELSE
      ...
      LET cla_sel ="SELECT a.n_folio,               ",
             ...
             --    " AND   a.estado_comision = yy ",     --quitar
             " and a.coduni_n1 in ('00102','00901') "    --quitar
    ...
    ERROR "Preparando la informacion para imprimir..."
       ...
       INITIALIZE mandar.* TO NULL
         ...
         --Buscamos el monto de comision y salario anterior (Adelanto,salario)
         EXECUTE  query_1 USING nuevo.coduni_n1,
                                nuevo.codven,
                                nuevo.nss,
                                xfecha_corte2
         ...
         --Llamo a la funcion que trae el nivel superior
...
REPORT rpt_cuenta_imp(lnuevo,val_porcen,vcausa_edo,
 ...
 OUTPUT
      ...
      ORDER BY niv_super,lnuevo.niv_sup,
               lnuevo.coduni_n1,lnuevo.codven,
               lnuevo.cod_esq_comision desc   --v2
  FORMAT
     PAGE HEADER

      PRINT '\033e\033(10U\033&l1O\033&k2S\033&l12d'
      PRINT COLUMN 02,'\033(s7B',"COMC023                                                                                                                                     ",today USING "dd-mm-yyyy",'\033(s0B'
      SKIP 2 LINE
      PRINT COLUMN 80,'\033(s7B',"REPORTE DE PAGO DE COMISIONES",'\033(s0B'
      PRINT COLUMN 80,'\033(s7B',"RELIQUIDACION DETALLE     ",'\033(s0B'
      SKIP 1 LINE
    ...
    BEFORE GROUP OF lnuevo.niv_sup
      ...
      PRINT COLUMN 05,"FOLIO        NSS        NOMBRE DEL TRABAJADOR                      TIPO SOL   SALA_ADELAN   SALA_RELIQ    SAMS     %CELDA        ADELANTO     COMIS_RELIQ      RELIQUI"
    ...
    ON EVERY ROW
      ...
      PRINT COLUMN 004,lnuevo.n_folio              USING "&&&&&&&&&",
            ...
            COLUMN 077,lnuevo.salario_adelanto     USING "#########&&.&&",
            COLUMN 093,lnuevo.salario_base_comis   USING "#########&&.&&",
            COLUMN 109,lnuevo.num_sm               USING "####&.&&",
            COLUMN 119,val_porcen                  USING "###&.&&",'%', --v2
       COLUMN 128,lnuevo.adelanto                  USING "#########&&.&&",
       column 144,lnuevo.comis_pagada              USING "#########&&.&&",
       column 159,lnuevo.monto_comision            USING "#########&&.&&"
...
--    VALUES(lnuevo.codven,lnuevo.nombre_pro,lnuevo.coduni_n1,lnuevo.n_folio,lnuevo.nss,lnuevo.nombre_cli,lnuevo.tipo_solicitud,lnuevo.salario_base_comis,lnuevo.num_sm,val_porcen,lnuevo.monto_comision)
      ...
      SKIP  1 LINES
      PRINT COLUMN 077,'____________________________TOTALES POR GERENCIA________________________________________________________________'
      PRINT COLUMN 077,GROUP SUM(lnuevo.salario_base_comis) USING "#########&&.&&","  ",GROUP SUM(lnuevo.salario_base_comis) USING "#########&&.&&","  ",GROUP SUM(lnuevo.num_sm) USING "####&.&&","           ",GROUP SUM(lnuevo.adelanto) USING "#########&&.&&","  ",GROUP SUM(lnuevo.comis_pagada) USING "#########&&.&&"," ",GROUP SUM(lnuevo.monto_comision) USING "#########&&.&&"," ",GROUP SUM(lnuevo.comis_pagada) USING "#########&&.&&"
      PRINT COLUMN 077,'    SALA_ADEL.        SALA_RELI     SAMS            SUBT_ADELANTO       COMIS_RELI.       RELIQUI.  COMISI_EJECU.'
      SKIP  2 LINES

      ----@@@@
      AFTER GROUP OF lnuevo.codven
      PRINT COLUMN 077,'________________________________________________________________________________________________________________'
      PRINT COLUMN 077,GROUP SUM(lnuevo.salario_adelanto) USING "#########&&.&&","  ",GROUP SUM(lnuevo.salario_base_comis) USING "#########&&.&&","  ",GROUP SUM(lnuevo.num_sm) USING "####&.&&","           ",GROUP SUM(lnuevo.adelanto) USING "#########&&.&&","  ",GROUP SUM(lnuevo.comis_pagada) USING "#########&&.&&"," ",GROUP SUM(lnuevo.comis_pagada) USING "#########&&.&&"," ",GROUP SUM(lnuevo.comis_pagada) USING "#########&&.&&"
      PRINT COLUMN 077,'    SALA_ADEL.        SALA_RELI     SAMS            TOTA_ADELANTO       COMIS_RELI.      RELIQUI.   COMISI_EJECU.'
      ...
      PRINT COLUMN 077,'________________________________________GRAN TOTAL :____________________________________________________________'
      PRINT COLUMN 077,'________________________________________________________________________________________________________________'
      PRINT COLUMN 077,SUM(lnuevo.salario_adelanto) USING "#########&&.&&","  ",SUM(lnuevo.salario_base_comis) USING "#########&&.&&","   ",SUM(lnuevo.num_sm) USING "####&.&&","           ",SUM(lnuevo.adelanto) USING "#########&&.&&","  ",SUM(lnuevo.comis_pagada) USING "#########&&.&&"," ",SUM(lnuevo.comis_pagada) USING "#########&&.&&"," ",SUM(lnuevo.comis_pagada) USING "#########&&.&&"
      PRINT COLUMN 077,'    SALA_ADEL.        SALA_RELI.     SAMS           TOTA_ADELANTO       COMIS_RELI.      RELIQUI.   COMSI_EJECU.'
      PRINT COLUMN 077,'________________________________________________________________________________________________________________'
      PRINT COLUMN 077,'________________________________________________________________________________________________________________'
...
FUNCTION Busca_afiliados(vcodven,vcoduni_n1)
   ...
   OPEN WINDOW ventana_2 AT 9,2 WITH FORM "COMC0232" 
   ...
   LET cla_sel2="SELECT n_folio,",
            ...
            --  "' AND estado_comision = yy ",         --quitar
                "' and coduni_n1 in ('00102','00901') ", --quitar
                "  GROUP BY 1,2,3 ",
     	        "  ORDER BY nss" CLIPPED
...
FUNCTION Mostrar_nombre2(vnss)
   ...
   OPEN WINDOW ventana001 AT 09,2 WITH FORM "COMC0234" 
