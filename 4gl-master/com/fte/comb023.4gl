...
MAIN
   ...
   LET ejecuta = "nohup time fglgo COMB024.4gi ",hoy," &"
   RUN ejecuta
...
FUNCTION Calculo_afiliacion()
   ...
   DATABASE safre_tmp
   DROP TABLE cont_afi2
   CREATE TABLE cont_afi2
      (codven char(10),
       cuantos integer);
   CREATE index cont_afi2_1 on cont_afi2(codven);
   DATABASE safre_af
   WHENEVER ERROR STOP
   ...
   WHERE  estado_comision = 100
   AND    nivel = 1
   GROUP  BY 1
   ORDER  BY 1

   FOREACH curcont INTO vcodven,vcontafi
      INSERT INTO safre_tmp:cont_afi2 values(vcodven,vcontafi)
   ...
   FROM   com_comis_detalle a,
          com_esq_comis b,
          safre_tmp:cont_afi2 c
   WHERE  a.estado_comision  = 100
   ...
   FOREACH cursor_1 INTO g_reg.*,vcriterio,vcont_afi
	 ...
	 WHERE  MONTH(fecha_recepcion) = vmes_recep
         ...
         IF STATUS = NOTFOUND THEN
            LET vult_salario_diario = 0
            LET vestado_comision = 140 --s/c TC bim num_sm>=8 no encont 2ra rec
            LET l_monto_comision = 0   --sin generar ya nada
         ELSE
            SELECT monto_sm
            INTO   l_sm
            ...
            IF g_reg.salario_base_comis > vult_salario_diario THEN
               LET vestado_comision = 120 --c/c TC bim negativa numsm< 8 2ra rec
               LET l_monto_comision = g_reg.monto_comision * -1
            ELSE
               LET vestado_comision = 130 -- c/c TC bim numsm>=8 en 2ra rec
...
FUNCTION Comision_afiliacion()
   ...
   LET cla_sel = "UPDATE com_comis_detalle ",
                ...
                " AND    estado_comision = 100 " CLIPPED

   PREPARE claexe FROM cla_sel
   EXECUTE claexe

   IF vestado_comision <> 140 THEN
