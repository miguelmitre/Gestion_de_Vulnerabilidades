...
#                  => DESCUENTO POR NO APORTE CONSOLIDADO                      #
#Fecha             => 02 Feb  2005.        	                                  #
...
MAIN
	...
	OPEN WINDOW ventana_1 AT 2,2 WITH FORM "COMC0181" ATTRIBUTE( BORDER)
	DISPLAY " COMC018  PAGO DE COMISIONES A GERENTES DESCUENTO POR NO APORTE          " AT 3,1 ATTRIBUTE(REVERSE) 
 ...
 OUTPUT
  ...
  FORMAT
     PAGE HEADER
      ...
      PRINT COLUMN 02,'\033(s7B',"COMC018                                                                                                                                     ",today USING "dd-mm-yyyy",'\033(s0B'
      SKIP 2 LINE
      PRINT COLUMN 80,'\033(s7B',"REPORTE DE PAGO DE COMISIONES A GERENTES",'\033(s0B'
      PRINT COLUMN 80,'\033(s7B',"DESCUENTO POR NO APORTE CONSOLIDADO",'\033(s0B'
      SKIP 1 LINE
    ...
    BEFORE GROUP OF lnuevo.niv_sup
      ...
      PRINT COLUMN 01,'\033(s7B',"COD PROMOTOR  NOMBRE PROMOTOR                    GRP VENTA   FECHA CORTE           MASA SALARIAL   SAMS          ADELANTO     DESCUENTO  ",'\033(s0B'
    ...
    ON EVERY ROW
      ...
      PRINT COLUMN 002,lnuevo.codven CLIPPED,
            ...
            COLUMN 102,lnuevo.num_sm * 0           USING "####&.&&",
            column 113,lnuevo.adelanto             USING "#########&&.&&",
            COLUMN 129,lnuevo.monto_comision       USING "#########&&.&&"
...
--    VALUES(lnuevo.codven,lnuevo.nombre_pro,lnuevo.coduni_n1,lnuevo.n_folio,lnuevo.nss,lnuevo.nombre_cli,lnuevo.tipo_solicitud,lnuevo.salario_base_comis,lnuevo.num_sm,val_porcen,lnuevo.monto_comision)
      ...
      SKIP  1 LINES
      PRINT COLUMN 085,'_________________________________________TOTALES POR GERENCIA_______________________________________________'
      PRINT COLUMN 085,GROUP SUM(lnuevo.salario_base_comis) USING "#########&&.&&","   ",GROUP SUM(lnuevo.num_sm) * 0 USING "####&.&&","   ",GROUP SUM(lnuevo.adelanto) USING "#########&&.&&","  ",GROUP SUM(lnuevo.monto_comision) USING "#########&&.&&"," ",GROUP SUM(lnuevo.comis_pagada) USING "#########&&.&&"
      PRINT COLUMN 085,'    SUBT_SALA.  SUBT_SAMS      SUBT_ADELANTO   SUBT_DESCUENTO    DESCUE_EJECU.'
      SKIP  2 LINES
      ...
      PRINT COLUMN 085,'_______________________________________________GRAN TOTAL :_________________________________________________'
      PRINT COLUMN 085,'____________________________________________________________________________________________________________'
      PRINT COLUMN 085,SUM(lnuevo.salario_base_comis) USING "#########&&.&&","   ",SUM(lnuevo.num_sm) * 0 USING "####&.&&","   ",SUM(lnuevo.adelanto) USING "#########&&.&&","   ",SUM(lnuevo.monto_comision) USING "#########&&.&&","  ",SUM(lnuevo.comis_pagada) USING "#########&&.&&"
      PRINT COLUMN 085,'     TOTA_SALA.    TOTA_SAMS        TOTA_ADELA.    TOTA_DESCUE.  DESCUE_EJECU.'
      PRINT COLUMN 085,'____________________________________________________________________________________________________________'
      PRINT COLUMN 085,'____________________________________________________________________________________________________________'
...
FUNCTION Busca_promotores(vcoduni_n1)
   ...
   OPEN WINDOW ventana_2 AT 9,2 WITH FORM "COMC0182" 
...
LET      g_grep_01  =
         'SELECT  MAX(a.fecha_corte),SUM(a.monto_comision) ', --quitar sum
...
FUNCTION Mostrar_nombre2(vcodven0)
   ...
   OPEN WINDOW ventana001 AT 09,2 WITH FORM "COMC0184" 
