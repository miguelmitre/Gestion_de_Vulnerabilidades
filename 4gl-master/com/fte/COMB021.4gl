...
-- Propietario  => E.F.P.                                                    --
-- Programa     => COMB021                                                   --
-- Descripcion  => Calcula comision TIPO C en com_comis_detalle 1ra rec      --
-- Sistema      => COM                                                       --
...
MAIN
   ...
   LET ejecuta = "nohup time fglgo COMB022.4gi ",hoy," &"
   RUN ejecuta
...
FUNCTION Calculo_afiliacion()
   ...
   WHERE  a.estado_comision  in (0,40) --se agrego estado cero p/pagar en recau
   ...
   FOREACH cursor_1 INTO g_reg.*,vcriterio,vcont_afi
	 ...
	 WHERE  MONTH(fecha_recepcion) = vmes_recep
         ...
         IF STATUS = NOTFOUND THEN
            LET vult_salario_diario = 0
            LET vestado_comision = 100 --s/c TC bim num_sm>=8 no encont 1ra rec
...
--jerry      IF g_reg.salario_base_comis > vult_salario_diario THEN
--jerry         LET vestado_comision = 80 --c/c TC bim negativa numsm< 8 1ra rec
--jerry            LET l_monto_comision = g_reg.monto_comision * -1
--jerry         ELSE
            LET vestado_comision = 90 -- c/c TC bim numsm>=8 en 1ra rec
...
--jerry            END IF
      ...
      IF l_monto_comision IS NOT NULL THEN
         CALL Comision_afiliacion()
          ...
          IF vestado_comision_ori = 0 AND vestado_comision = 90 THEN
             LET g_reg.cod_esq_comision = 10 --esquema tipo A
             CALL Cantidad()RETURNING l_monto_comision
             IF l_monto_comision = 0 THEN --mto 0 por sm < 8 ptt esq tipo B
...
FUNCTION Comision_afiliacion()
   ...
   LET cla_sel = "UPDATE com_comis_detalle ",
                ...
                " AND    estado_comision in (0,40) " CLIPPED
...
{
      IF vestado_comision = 80 THEN   --jerry nuevo requerimiento
         LET g_reg.cod_esq_comision = 20 --esquema tipo B
         LET vestado_comision = 85   --estado com pagada tipo B en 1ra rec
         CALL Cantidad()RETURNING l_monto_comision
         INSERT INTO com_comis_detalle VALUES(
            g_reg.codven,             #codven
            g_reg.nivel,              #nivel
