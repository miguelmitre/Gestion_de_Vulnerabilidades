...
#   OJO            => (v5) Se cambio cod_promotor por codven y comis.codven por#
#   OJO            => (v6) Se cambio cod_promotor por codven y comis.codven por#
#                  => num_interno.
#                  => Me falta verificar v3,v4 no estan
--------------------------------------------------------------------------------
#Modify.           => Alejandro Ramirez 5 de may 2005                          #
#Descr.            => (v7) Piden no mostrar los que tengan monto = 0           #
################################################################################
DATABASE safre_af
GLOBALS
   DEFINE
   ...
   g_reg4 ARRAY[6] OF RECORD
      codven         LIKE pro_mae_promotor.codven,   --v5
...
MAIN
        ...
        OPEN WINDOW ventana_1 AT 2,2 WITH FORM "COMC0161" ATTRIBUTE( BORDER)
        DISPLAY " COMC016  PAGO DE COMISIONES A GERENTES ADELANTO DEL 80% CONSOL.         " AT 3,1 ATTRIBUTE(REVERSE)
   ...
   DISPLAY " [Ctrl-C] Salir " AT 1,63 ATTRIBUTE(REVERSE)
      ...
      CONSTRUCT cla_where
       ...
       END IF
         ...
         CALL primer_row()
   DISPLAY " [Ctrl-I]Imprime  [Ctrl-P]Promotores del gerente" AT 7,01 ATTRIBUTE(REVERSE)
   ...
   DEFINE val_porcenx    DECIMAL(12,2)
    ...
    ELSE
     ...
     SELECT  codven,coduni_n1,    --v5
             ...
             uni_superior_n1
     FROM    tmp_union
     WHERE   monto_comision <>0     --v7
     GROUP BY 1,2,7,8
     ORDER BY 1,2
     FOREACH cursor_12 INTO  nuevo.codven,   --v6
                             nuevo.coduni_n1,
...

{
   ...
   LET columnas = "COD PROMOTOR'|'NOM PROMOTOR'|'CENTRO COSTOS'|'FOLIO CLIEN'|'NSS CLIE'|',",                                                                                     "NOM CLIEN'|'FEC CERTIF'|'SALAR BASE COMI'|'SAMS'|'MONTO COMIS'|'ESTADO COMIS'|'DESCRIP EDO'|'DESCRIP CAUSA"
...
REPORT rpt_cuenta_imp(lnuevo,val_porcen, niv_super,
 ...
 OUTPUT
      ...
      ORDER BY niv_super,lnuevo.niv_sup,
               lnuevo.coduni_n1,lnuevo.codven, --v6
               lnuevo.cod_esq_comision desc   --v2
  FORMAT
     PAGE HEADER

      PRINT '\033e\033(10U\033&l1O\033&k2S\033&l12d'
      PRINT COLUMN 02,'\033(s7B',"COMC016                                                                                                                                     ",today USING "dd-mm-yyyy",'\033(s0B'
      SKIP 2 LINE
    ...
    -- PRINT COLUMN 002,'Centro de costos:',lnuevo.niv_sup
       ...
       PRINT COLUMN 002,'ESQUEMA:',desc_esquema
      ...
      PRINT COLUMN 01,'\033(s7B',"NOMINA PROM   NOMBRE PROMOTOR                         GRP VENTA  FECHA CORTE      MASA SALARIAL   SAMS         ESQUEMA               COMISION      ADELANTO   ",'\033(s0B'
    ...
    ON EVERY ROW
      ...
      PRINT COLUMN 002,lnuevo.codven CLIPPED,   --v6
            ...
            COLUMN 095,lnuevo.num_sm               USING "####&.&&",
            COLUMN 111,desc_esquema CLIPPED,
            COLUMN 126,lnuevo.comis_pagada         USING "####,###,#&&.&&",
            column 141,lnuevo.monto_comision       USING "####,###,#&&.&&"
...
--      PRINT COLUMN 080,'    ESQUEMA:',desc_esquema
      ...
      PRINT COLUMN 080,'______________________________________GRAN TOTAL :___________________________________________'
...
FUNCTION Busca_promotores(vcoduni_n1)
   ...
   OPEN WINDOW ventana_2 AT 9,2 WITH FORM "COMC0162"
   ...
   LET mto_comi    = 0
      ...
      LET cla_sel2="SELECT a.codven,SUM(a.salario_base_comis), ", --v6
                          "SUM(a.num_sm), ",
                          "SUM(a.comis_pagada), ",
                   "a.cod_esq_comision ",
                   "FROM   com_comis_detalle a ",  --quitar
                   "WHERE  a.coduni_n1 = '",vcoduni_n1 CLIPPED,
                   "' AND a.fecha_corte = '",xfecha_corte2 CLIPPED,
                   "' AND a.nivel = 1 ",
                   "  AND a.comis_pagada <> 0 ",   --v7
                   "  GROUP BY 1,5 ",
                   " ORDER BY a.codven " CLIPPED  --v6
...
FUNCTION Mostrar_nombre2(vcodven0)
   ...
   OPEN WINDOW ventana001 AT 09,2 WITH FORM "COMC0164"
   ...
   LET cla_sel2="SELECT a.codven,",    --v5
                       "a.paterno,",
                       "a.materno,",
                       "a.nombres ",
                "FROM   pro_mae_promotor a ",
                "WHERE  a.codven  = '",vcodven0 CLIPPED,  --v5
                "'"
         ...
         FOREACH cursor_cte2 INTO g_reg4[pos].*
            LET g_reg5[pos].codven   = g_reg4[pos].codven    --v5
