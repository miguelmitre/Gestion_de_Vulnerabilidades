...
#Owner             => E.F.P.        					       #
#Programa          => COMC012                                                  #
#Descripcion       => REPORTE DE PRODUCCION PREMIOS                            #
#Fecha             => 29 Sep   2004.     				       #
...
MAIN
	...
	DEFER INTERRUPT
      ...
      --LET g_param_dis.ruta_rescate="/safre/com/fte" --ojo
	...
	OPEN WINDOW ventana_1 AT 2,2 WITH FORM "COMC0121" ATTRIBUTE( BORDER)
	DISPLAY " COMC012            CONSULTA  DE  PRODUCCION  PREMIOS                         " AT 3,1 ATTRIBUTE(REVERSE) 
    ...
    
{
       ...
       LET cla_sel ="SELECT a.fecha_corte,",
			   "a.codven ",
                    "FROM   com_comis_detalle a ",
                   " WHERE ",cla_where CLIPPED,
                   " ORDER  BY 2" CLIPPED
...
--                                ,"_",hora CLIPPED
  ...
  --FOR i=1 TO 300 
   ...
   WHERE  cod_promotor = g_reg[i].codven
  ...
  FORMAT
     PAGE HEADER
      PRINT COLUMN 001,"Num promotor |",
            ...
            COLUMN 032,"Centro costos|",
            COLUMN 046,"Fecha corte|",
            COLUMN 058,"Total Afi|", 
            COLUMN 068,"Total sm|",
            COLUMN 077,"Prom  sm|",
            COLUMN 086,"Comision |",
            COLUMN 096,"Estado|"
      ...
      PRINT COLUMN 001,nuevo.codven CLIPPED,                    "|",
            ...
            COLUMN 076,nuevo.coduni_n1,                         "|",
            COLUMN 090,nuevo.fecha_corte  USING 'DD/MM/YYYY',   "|",
            COLUMN 101,nuevo.total_afiliados,                   "|",
            COLUMN 112,nuevo.total_sm,                          "|",
            COLUMN 124,nuevo.promedio_sm,                       "|",
            COLUMN 146,nuevo.total_comision,                    "|",
            COLUMN 160,nuevo.estado_comision,                   "|"
 ...
 OUTPUT
  ...
  FORMAT
     PAGE HEADER
      ...
      PRINT COLUMN 02,'\033(s7B',"COMC012                                                                                                                                     ",today USING "dd-mm-yyyy",'\033(s0B'
      SKIP 2 LINE
      PRINT COLUMN 80,'\033(s7B',"Reporte de Produccion premios",'\033(s0B'
      SKIP 1 LINE
      ...
      PRINT COLUMN 02,'\033(s7B',"Num promotor  Nombre promotor                   Grp Venta   Fecha corte   Total afiliaciones  Total Salario       Total  sm       Tot comi  Estado",'\033(s0B'
      PRINT COLUMN  002,'__________________________________________________',
                        '__________________________________________________',
                        '__________________________________________________',
                        '______________________________________'
      ...
      PRINT COLUMN 02,nuevo.codven CLIPPED,
            ...
            COLUMN 60,nuevo.coduni_n1 CLIPPED,
            COLUMN 70,nuevo.fecha_corte USING 'DD-MM-YYYY',
            COLUMN 080,nuevo.total_afiliados,
            COLUMN 100,nuevo.total_sm USING "#######&.&&",
            COLUMN 125,nuevo.promedio_sm USING "#####&.&&", 
            COLUMN 135,nuevo.total_comision USING "###########&.&&",
            COLUMN 145,nuevo.estado_comision CLIPPED
      ...
      ON LAST ROW
       ...
       SKIP 1 LINE
       PRINT COLUMN 002,"TOTAL DE REGISTROS:       ",cont_de_registros,"                                               ",sumtot_afil USING "####&.&&","        ",sumtot_sm USING "#####&.&&","           ",sumtot_promsm USING "########&.&&"," ",sumtot_comi USING "###########&.&&" 
...
FUNCTION ver_arreglo()
   ...
   LET cla_sel2="SELECT a.codven,",              --codven
                       "a.coduni_n1,",           --coduni_n1
                       "a.fecha_corte,",         --fecha_corte
                       "a.porcent_comision,",    --vtot_afil
                       "a.salario_base_comis,",  --vsum_sm
                       "a.num_sm,",              --vprom_sm 
                       "a.monto_comision,",      --vmonto_comi
                       "estado_comision ",       --vedo
                "FROM   com_comis_detalle a ",
                "WHERE  ",cla_where CLIPPED,
          --    " AND estado_comision=170 ",
                " AND estado_comision=290 ",
---     	       " GROUP BY 1,2,3,4,8 " CLIPPED,
...
FUNCTION Busca_afiliados(vcodven,vcoduni_n1,vfec_corte,vestado)
   ...
   OPEN WINDOW ventana_2 AT 9,2 WITH FORM "COMC0123" 
   ...
   LET cla_sel2="SELECT a.n_folio,",
		       ...
		       "a.estado_comision ",
                ...
                " AND a.fentcons between '09/01/2004' and '10/31/2004' ",
                " AND a.estado_comision in (260,280)  " CLIPPED,
-------                " AND a.cod_esq_comision = 30 " CLIPPED,
                " AND a.nivel = 1 " CLIPPED,
	 ...
	 FOREACH cursor_pend INTO g_reg3[pos].*
		 LET registros2 = registros2 + 1
                 ...
                 AND    rango_desde <= g_reg3[pos].num_sm 
                 AND    rango_hasta >= g_reg3[pos].num_sm 
                 IF vmonto_bono IS NULL THEN
                    LET vmonto_bono = 0
                 END IF
                 LET g_reg3[pos].monto_comision = vmonto_bono
...
FUNCTION Busca_estado(vestado)
  ...
  DEFINE l_record   ARRAY[300] OF RECORD
         ...
         IF (pos-1) >= 1 THEN
            CALL  SET_COUNT(pos-1)
                                  -- r,c
            OPEN WINDOW ventana_2 AT 5,3 WITH FORM "COMC0125" ATTRIBUTE( BORDER)
...
FUNCTION Mostrar_nombre(vcodven)
   ...
   OPEN WINDOW ventana001 AT 09,2 WITH FORM "COMC0124" 
   ...
   LET cla_sel2="SELECT a.seguro,",
                       "a.paterno,",
                       "a.materno,",
                       "a.nombres ",
                "FROM   pro_mae_promotor a ",
                "WHERE  a.cod_promotor  = ",vcodven
	 ...
	 ELSE
	    ...
	    ERROR "NO EXISTEN DATOS CON ESTAS CONDICIONES"
            ...
            ELSE
                 LET g_reg5[pos].seguro ='10000000000'
...
FUNCTION Mostrar_nombre2(vnss)
   ...
   OPEN WINDOW ventana001 AT 09,2 WITH FORM "COMC0125" 
