...
#Owner             => E.F.P.        		                                     #
#Programa          => COMC017                                                  #
#Descripcion       => GENERACION DEL REPORTE DE PAGO DE COMISIONES A GERENTES  #
#                  => RELIQUIDACION CONSOLIDADO                                #
#Fecha             => 09 Feb  2005.        	                                  #
...
-----------------------------------ADELANTO ------------------------------------
#                  => estado_comision =  xx  (S/2da Recau)                     #
#                  => monto_comision  =  1345.03 ejmpl                         #
-----------------------------------DESCUENTO------------------------------------
#                  => estado_comision =  yy  (S/2da Recau)                     #
#                  => monto_comision  =  -1345.03 ejmpl                        #
################################################################################
DATABASE safre_af
GLOBALS
   DEFINE 
      ...
      g_reg3 ARRAY[1000] OF RECORD
        ...
        salario_base_comis LIKE com_comis_detalle.salario_base_comis,
	     adelanto           LIKE com_comis_detalle.comis_pagada,
...
MAIN
	...
	OPEN WINDOW ventana_1 AT 2,2 WITH FORM "COMC0171" ATTRIBUTE( BORDER)
	DISPLAY " COMC017  PAGO DE COMISIONES A GERENTES RELIQUIDACION CONSOLIDADO        " AT 3,1 ATTRIBUTE(REVERSE) 
	 ...
	 ON    a.fecha_corte,
       ...
       END IF 
         ...
         LET cla_sel ="SELECT a.nivel,",
                           "a.coduni_n1,'",xfecha_corte2,
                           "', a.codven ",
                    "FROM   safre_tmp:com_comis_detalle a ",  --quitar
                    "WHERE a.coduni_n1='",xcoduni_n1 CLIPPED,
                    "' AND nivel=2 ",
                    "  GROUP BY 1,2,4 ", 
                    " ORDER BY 2"
         ...
         CALL primer_row()
   DISPLAY " [Ctrl-I]Imprime  [Ctrl-P] Promotores de la gerencia" AT 7,01 ATTRIBUTE(REVERSE)
...
FUNCTION imprime(ban_agreg,ycoduni_n1)
   ...
   mandar RECORD
      ...
      niv_sup            LIKE com_nivel1.uni_superior_n1,
      nombre_pro         CHAR(45),
      adelanto           LIKE com_comis_detalle.monto_comision,
      salario_adelanto   LIKE com_comis_detalle.salario_base_comis
   END RECORD,
   ...
   DEFINE val_porcenx    DECIMAL(12,2)
    ...
    --Forma la estructura del query indepen de su uso
    CALL Forma_select()
    ...
    ELSE
      ...
      IF ban_solo_admon = 1 THEN  --todo
         LET cla_sel="SELECT a.codven,a.coduni_n1,SUM(a.salario_base_comis), ",
                      ...
                      "FROM   safre_tmp:com_comis_detalle a,", --quitar
                      "outer  com_nivel1 b ",
                      "WHERE  a.fecha_corte = '",xfecha_corte2 CLIPPED,
                      "' AND a.nivel = 1 ",
              " and a.coduni_n1 in ('00101','00901') ",    --quitar
                      "  AND a.coduni_n1=b.coduni_n1 ",
                  --  "  AND a.estado_comision in (yy) ",  --quitar
                      "  GROUP BY 1,2,7 ",
      ...
      ELSE
         LET cla_sel="SELECT a.codven,a.coduni_n1,SUM(a.salario_base_comis), ",
                      ...
                      "FROM   safre_tmp:com_comis_detalle a,",  --quitar
                      " outer com_nivel1 b ",
                      "WHERE  a.coduni_n1 = '",ycoduni_n1 CLIPPED,
     ...
     FOREACH cursor_12 INTO  nuevo.codven,
                             nuevo.coduni_n1,
       ...
       INITIALIZE mandar.* TO NULL
         ...
         --Buscamos el monto de comision anterior (Adelanto)
         EXECUTE  query_1 USING nuevo.coduni_n1,nuevo.codven,xfecha_corte2
         INTO     fec,mandar.adelanto,mandar.salario_adelanto

         IF      fec  IS NULL  THEN
                 LET mandar.adelanto = 0
                 LET mandar.salario_adelanto = 0
         END IF
   ...
   OUTPUT TO REPORT rpt_cuenta_imp(mandar.*,
...
REPORT rpt_cuenta_imp(lnuevo, niv_super,
                              nom_ger,
                              unidad_ger,
                              esq_comi_ger,
                              niv_super2,
 ...
 OUTPUT
      ...
      ORDER BY niv_super,lnuevo.niv_sup,
               lnuevo.coduni_n1,lnuevo.codven desc
  FORMAT
     PAGE HEADER

      PRINT '\033e\033(10U\033&l1O\033&k2S\033&l12d'
      PRINT COLUMN 02,'\033(s7B',"COMC017                                                                                                                                     ",today USING "dd-mm-yyyy",'\033(s0B'
      SKIP 2 LINE
      PRINT COLUMN 80,'\033(s7B',"REPORTE DE PAGO DE COMISIONES A GERENTES",'\033(s0B'
      PRINT COLUMN 80,'\033(s7B',"RELIQUIDACION CONSOLIDADO          ",'\033(s0B'
      PRINT COLUMN 80,'\033(s7B',"FECHA DE CORTE:",'\033(s0B',xfecha_corte2
      SKIP 1 LINE
    ...
    BEFORE GROUP OF lnuevo.niv_sup
      ...
      PRINT COLUMN  001,'__________________________________________________',
       ...
       PRINT COLUMN 002,'Gerencia'          
       PRINT COLUMN 002,'Centro de costos:',lnuevo.niv_sup
       PRINT COLUMN 002,'Nombre de la unidad:',unidad_ger
      ...
      PRINT COLUMN 01,'\033(s7B',"COD PROMOTOR  NOMBRE PROMOTOR                    GRP VENTA      MS_ADELANTO    MS_SALARIAL   SAMS     ESQUEMA                      ADELANTO  COMIS_RELIQ.   RELIQU.",'\033(s0B'
    ...
    ON EVERY ROW
      ...
      PRINT COLUMN 002,lnuevo.codven CLIPPED,
        ...
        --  COLUMN 067,xfecha_corte2 USING "DD-MM-YYYY",
            COLUMN 067,lnuevo.salario_adelanto     USING "#########&&.&&",
            COLUMN 082,lnuevo.salario_base_comis   USING "#########&&.&&",
            COLUMN 097,lnuevo.num_sm               USING "####&.&&",
            COLUMN 106,desc_esquema[1,23], 
            column 120,lnuevo.adelanto             USING "#########&&.&&",
            COLUMN 135,lnuevo.comis_pagada         USING "#########&&.&&",
            COLUMN 150,lnuevo.monto_comision       USING "#########&&.&&"
...
--    VALUES(lnuevo.codven,lnuevo.nombre_pro,lnuevo.coduni_n1,lnuevo.n_folio,lnuevo.nss,lnuevo.nombre_cli,lnuevo.tipo_solicitud,lnuevo.salario_base_comis,lnuevo.num_sm,val_porcen,lnuevo.monto_comision)
      ...
      LET sams_ger = sams_ger + GROUP SUM(lnuevo.num_sm) * 0
      ...
      SKIP  1 LINES
      PRINT COLUMN 067,'_________________________________________TOTALES POR GERENCIA_________________________________________________________'
      PRINT COLUMN 067,GROUP SUM(lnuevo.salario_adelanto) USING "#########&&.&&"," ",GROUP SUM(lnuevo.salario_base_comis) USING "#########&&.&&","  ",GROUP SUM(lnuevo.num_sm) USING "####&.&&","                     ",GROUP SUM(lnuevo.adelanto) USING "#########&&.&&"," ",GROUP SUM(lnuevo.comis_pagada) USING "#########&&.&&"," ",GROUP SUM(lnuevo.monto_comision) USING "#########&&.&&"," ",GROUP SUM(lnuevo.comis_pagada) USING "#########&&.&&"
      PRINT COLUMN 067,'      MS_ADELAN.   MS_SALARIAL    SAMS                          SUBT_ADELANTO   COMIS_RELIQ.     RELIQU.    DESCUE_EJECU.'
      SKIP  2 LINES
      ...
      PRINT COLUMN 067,'_______________________________________________GRAN TOTAL :___________________________________________________________'
      PRINT COLUMN 067,'______________________________________________________________________________________________________________________'
      PRINT COLUMN 067,SUM(lnuevo.salario_adelanto) USING "#########&&.&&","  ",SUM(lnuevo.salario_base_comis) USING "#########&&.&&","  ",SUM(lnuevo.num_sm) USING "####&.&&","                   ",SUM(lnuevo.adelanto) USING "#########&&.&&"," ",SUM(lnuevo.comis_pagada) USING "#########&&.&&"," ",SUM(lnuevo.monto_comision) USING "#########&&.&&"," ",SUM(lnuevo.comis_pagada) USING "#########&&.&&"
      PRINT COLUMN 067,'      MS_ADELAN.    MS_SALARIAL   TOTA_SAMS                      TOTA_ADELA.    COMIS_RELIQ.   RELIQU.  DESCUE_EJECU.'
      PRINT COLUMN 067,'______________________________________________________________________________________________________________________'
      PRINT COLUMN 067,'______________________________________________________________________________________________________________________'
   ...
   DEFINE
      ...
      ban_agregados SMALLINT 
       ...
       IF ban_solo_admon = 1 THEN  --todo 
          LET cla_sel2=" SELECT dat.cod_resp_uni,  ",
                       ...
                       " WHERE  dat.cod_uni      = niv.coduni_n1  ",
               " AND    niv.coduni_n1    in ('00101','00901') ",   --quitar
                       " AND    dat.nivel        = 1  ",
       ...
       ELSE
          ...
          LET cla_sel2=" SELECT dat.cod_resp_uni,  ",
                       ...
                       "        com_dat_uni_com   dat,  ",
                       "        com_respon_unidad res   ",
                       " WHERE  niv.coduni_n1 = '",ycoduni_n1, 
	 ...
	 IF (pos-1) >= 1 THEN
	    DISPLAY ARRAY g_reg TO scr_1.*
               ...
               ON KEY (CONTROL-P)
                 LET i = ARR_CURR()
	         CALL Busca_promotores(g_reg[i].coduni_n1)
...
FUNCTION Busca_promotores(vcoduni_n1)
   ...
   OPEN WINDOW ventana_2 AT 9,2 WITH FORM "COMC0172" 

   DISPLAY "                         PROMOTORES DE LA GERENCIA                            " AT 2,1 ATTRIBUTE(REVERSE) 


   DISPLAY "             [Ctrl-C] Salir                          [Enter] Nombre Promotor   " AT 04,1 ATTRIBUTE(REVERSE)
   ...
   --Forma la estructura del query indepen de su uso
   CALL Forma_select()
   PREPARE  query_2    FROM     g_grep_01
   ...
   LET mto_comi    = 0
      ...
      LET cla_sel2="SELECT codven,SUM(salario_base_comis), ",
   		          "0,",
                   "SUM(comis_pagada), ",
                   "cod_esq_comision ",
                   "FROM   safre_tmp:com_comis_detalle ",  --quitar
                   "WHERE  coduni_n1 = '",vcoduni_n1 CLIPPED,
                   "' AND fecha_corte = '",xfecha_corte2 CLIPPED,
     	           "' AND nivel = 1 ",
              --   "  AND  estado_comision in (yy) ", --quitar
                   "  GROUP BY 1,5 ",
                   " ORDER BY codven " CLIPPED
	 ...
	 LET reg1 = reg1 + 1
         ...
         --Buscamos el monto de comision anterior (Adelanto)
         EXECUTE  query_2 USING vcoduni_n1, vcodven, xfecha_corte2
         INTO     fec,g_reg3[pos].adelanto

         IF      fec  IS NULL   THEN
                 LET g_reg3[pos].adelanto = 0
         END IF
    ...
    DISPLAY mto_comi TO scr_1.mto_comi
	 ...
	 IF (pos-1) >= 1 THEN
	    DISPLAY ARRAY g_reg3 TO scr_0.*
               ON KEY (CONTROL-M)
                  LET i = ARR_CURR()
                  CALL Mostrar_nombre2(g_reg3[i].codven)
               ON KEY (INTERRUPT)
                  EXIT DISPLAY
	    END DISPLAY
	 ELSE
...
#-----------------------------------------------------------------------------
# Obtengo el mto de la comision pasada para obtener el mto del adelanto
#-----------------------------------------------------------------------------
FUNCTION Forma_select()

LET      g_grep_01  =
         'SELECT  MAX(a.fecha_corte),        ',
         'sum(a.monto_comision),             ', --quitar sum
         'sum(a.salario_base_comis)          ', --quitar sum
         'FROM    safre_tmp:com_comis_detalle a ',
         'WHERE   a.coduni_n1            = ? ',
         'AND     a.codven               = ? ',
         'AND     a.nivel                = 2 ',   --ojo
         'AND     a.fecha_corte          < ? ' 
   --    'GROUP   BY 2,3 '                      --quitar
   --    'AND     a.estado_comision      = xx '   --quitar
...
#-----------------------------------------------------------------------------
# Muestra el nombre de los promotores               
#-----------------------------------------------------------------------------

FUNCTION Mostrar_nombre2(vcodven0)
   DEFINE vcodven0 CHAR(10),
          pos     INTEGER

   OPEN WINDOW ventana001 AT 09,2 WITH FORM "COMC0174" 
   ...
   LET cla_sel2="SELECT a.cod_promotor,",
                       "a.paterno,",
                       "a.materno,",
                       "a.nombres ",
                "FROM   pro_mae_promotor a ",
                "WHERE  a.cod_promotor  = '",vcodven0 CLIPPED,
                "'"
	 ...
	 FOREACH cursor_cte2 INTO g_reg4[pos].*
            LET g_reg5[pos].cod_promotor   = g_reg4[pos].cod_promotor
