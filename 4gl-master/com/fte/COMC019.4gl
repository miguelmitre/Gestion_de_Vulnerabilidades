...
MAIN
	...
	OPEN WINDOW ventana_1 AT 2,2 WITH FORM "COMC0191" ATTRIBUTE( BORDER)
	DISPLAY " COMC019  PAGO DE COMISIONES A SUBDIRECTORES RELIQUI. CONSOLIDADO             " AT 3,1 ATTRIBUTE(REVERSE) 
   ...
   DEFINE val_porcenx    DECIMAL(12,2)
    ...
    ELSE
      ...
      IF ban_solo_admon = 1 THEN  --todo
         ...
         LET cla_sel="SELECT a.coduni_n1,SUM(a.salario_base_comis), ",
                     "SUM(a.num_sm), ",
                     "SUM(a.comis_pagada), ",
     ...
     FOREACH cursor_12 INTO  nuevo.coduni_n1,
       ...
       INITIALIZE mandar.* TO NULL
         ...
         EXECUTE  query_1 USING nuevo.coduni_n1,xfecha_corte2
         ...
         --Obtengo el porcentaje por promotor
...
REPORT rpt_cuenta_imp(lnuevo, niv_super,
 ...
 OUTPUT
      ...
      RIGHT MARGIN  0
      PAGE LENGTH   90
      ORDER BY niv_super,lnuevo.niv_sup,lnuevo.coduni_n1 desc   --v2
  FORMAT
     PAGE HEADER

      PRINT '\033e\033(10U\033&l1O\033&k2S\033&l12d'
      PRINT COLUMN 02,'\033(s7B',"COMC019                                                                                                                                     ",today USING "dd-mm-yyyy",'\033(s0B'
      SKIP 2 LINE
    ...
    BEFORE GROUP OF lnuevo.niv_sup
      ...
      PRINT COLUMN 01,'\033(s7B',"COD GERENTE   NOMBRE GERENTE                     GRP VENTA   FECHA CORTE    MS_ADELANTO    MS_SALARIAL      SAMS           ADELANTO       COMIS_RELIQ.     RELIQU.",'\033(s0B'
    ...
    ON EVERY ROW
      ...
      PRINT COLUMN 002,0,
            ...
            COLUMN 079,lnuevo.salario_adelanto     USING "#########&&.&&",
            COLUMN 095,lnuevo.salario_base_comis   USING "#########&&.&&",
            COLUMN 112,lnuevo.num_sm               USING "####&.&&",
            COLUMN 123,lnuevo.adelanto             USING "#########&&.&&",
            column 139,lnuevo.comis_pagada         USING "#########&&.&&",
            column 155,lnuevo.monto_comision       USING "#########&&.&&"
...
--    VALUES(lnuevo.codven,lnuevo.nombre_pro,lnuevo.coduni_n1,lnuevo.n_folio,lnuevo.nss,lnuevo.nombre_cli,lnuevo.tipo_solicitud,lnuevo.salario_base_comis,lnuevo.num_sm,val_porcen,lnuevo.monto_comision)
      ...
      SKIP  1 LINES
      PRINT COLUMN 079,'_________________________________________TOTALES POR SUBDIRECCION___________________________________________'
      PRINT COLUMN 079,GROUP SUM(lnuevo.salario_adelanto) USING "#########&&.&&","  ",GROUP SUM(lnuevo.salario_base_comis) USING "#########&&.&&","   ",GROUP SUM(lnuevo.num_sm) USING "####&.&&","   ",GROUP SUM(lnuevo.adelanto) USING "#########&&.&&","  ",GROUP SUM(lnuevo.comis_pagada) USING "#########&&.&&","  ",GROUP SUM(lnuevo.monto_comision) USING "#########&&.&&"," ",GROUP SUM(lnuevo.comis_pagada) USING "#########&&.&&"
      PRINT COLUMN 079,'    MS_ADELANTO    MS_SALARIAL      SAMS        SUBT_ADELANTO    COMIS. RELIQ.    RELIQU.    COMISI_EJECU.'
      SKIP  2 LINES
      ...
      PRINT COLUMN 079,'_______________________________________________GRAN TOTAL :_________________________________________________'
      PRINT COLUMN 079,'____________________________________________________________________________________________________________'
      PRINT COLUMN 079,SUM(lnuevo.salario_adelanto) USING "#########&&.&&"," ",SUM(lnuevo.salario_base_comis) USING "#########&&.&&","   ",SUM(lnuevo.num_sm) USING "####&.&&","   ",SUM(lnuevo.adelanto) USING "#########&&.&&","   ",SUM(lnuevo.comis_pagada) USING "#########&&.&&","  ",SUM(lnuevo.monto_comision) USING "#########&&.&&","  ",SUM(lnuevo.comis_pagada) USING "#########&&.&&"
      PRINT COLUMN 079,'    MS_ADELAN.      MS_SALARIAL    TOTA_SAMS     TOTA_ADELA.      COMIS_RELIQ.    RELIQU.   COMISI_EJECU.'
      PRINT COLUMN 079,'____________________________________________________________________________________________________________'
      PRINT COLUMN 079,'____________________________________________________________________________________________________________'
      ...
      ban_agregados SMALLINT 
       ...
       IF ban_solo_admon = 1 THEN  --todo 
          LET cla_sel2=" SELECT dat.cod_resp_uni,  ",
               ...
               " AND    niv.coduni_n2    in ('0000000100') ",   --quitar
                       " AND    dat.nivel        = 2  ",
...
FUNCTION Busca_gerentes(vcoduni_n1)
   ...
   OPEN WINDOW ventana_2 AT 9,2 WITH FORM "COMC0192" 
   ...
   LET cla_sel2="SELECT coduni_n1,SUM(salario_base_comis), ",
                ...
                "                     FROM   com_nivel1 ",
                "                     WHERE  uni_superior_n1 = '",
                                      vcoduni_n1 CLIPPED,"')",
                "  AND fecha_corte = '",xfecha_corte2 CLIPPED,
                "' AND nivel = 1 ",
                "  GROUP BY 1 ",
                "  ORDER BY coduni_n1 " CLIPPED
   ...
   DECLARE cursor_pend2 CURSOR FOR claexe_pend
	...
	FOREACH cursor_pend2 INTO g_reg3[pos].*
       ...
       --Buscamos el monto de comision anterior (Adelanto)
       EXECUTE  query_2 USING vcoduni_n1,xfecha_corte2
   ...
   DISPLAY mto_comi TO scr_1.mto_comi
	...
	IF (pos-1) >= 1 THEN
	    DISPLAY ARRAY g_reg3 TO scr_0.*
               ON KEY (CONTROL-M)
                  LET i = ARR_CURR()
                  CALL Mostrar_nombre2(g_reg3[i].coduni_n1)
               ON KEY (INTERRUPT)
                  EXIT DISPLAY
	    END DISPLAY
	ELSE
...
FUNCTION Mostrar_nombre2(vcoduni_n1)
   ...
   OPEN WINDOW ventana001 AT 09,2 WITH FORM "COMC0194" 
