...
--TOTAL POR SIEFORE
   DECLARE cur_siefore CURSOR FOR
      ...
      WHERE  {a.nss IN (SELECT nss
                       FROM safre_tmp:cta_formato_nss_1)
...
FUNCTION consulta_detalle()
   ...
   DECLARE cur_subcta CURSOR FOR
      ...
      AND    a.siefore   = gar_siefore[arr_c].cod_siefore
      GROUP BY 1
      ORDER BY 1
...
FUNCTION reporte(li_totnss, ld_fecha_corte)
   ...
   LET lc_ejecuta = "nohup time fglgo CTAB034.4gi " CLIPPED,
   ...
   LET lc_msg = "Folio: ",li_folio_proceso CLIPPED, " Proc Cod: CTAB034"
