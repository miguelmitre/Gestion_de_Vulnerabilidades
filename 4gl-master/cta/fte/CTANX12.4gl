...
################################################################################
FUNCTION proceso()
   OPEN WINDOW ventana_1 AT 2,2 WITH FORM "CTANX121" ATTRIBUTE(BORDER)
           #-----------------------------------------------------------------------------
   DISPLAY "CTANX12       SALDOS CUENTAS ISSSTE POR REGIMEN   < Ctrl-C > SALIR     " AT 3,1 ATTRIBUTE(REVERSE)
   ...
   MENU "SALDOS CTAS ISSSTE"
      ...
      MENU "Archivos"
        COMMAND "Archivo de Saldos" "Reporte de Saldos ISSSTE"
           CALL lee_paso(4)
...
FUNCTION lee_paso(li_paso)
   ...
   CASE li_paso
      ...
      WHEN 4
      	 #Archivo
      	 LET gc_mensaje1 = "ESTE PROCESO GENERARA EL REPORTE DE SALDOS"
   END CASE
...
FUNCTION borra_informacion(ls_tipo, ls_paso)
   ...
   WHENEVER ERROR CONTINUE
      IF ls_tipo = 0 OR
         ...
         DROP TABLE safre_tmp:tmp_saldo_issste
      END IF

      IF ls_tipo = 0 OR
        (ls_tipo = 1 AND
         ls_paso = 3) THEN --cuota
         DROP TABLE safre_tmp:tmp_cuota_issste
      END IF
   WHENEVER ERROR STOP
   ...
   IF ls_tipo = 0 OR
      ...
      CREATE TABLE safre_tmp:tmp_cuota_issste
     (nss              CHAR(11),
  ...
  IF ls_tipo = 0 OR
     ...
     CREATE TABLE safre_tmp:tmp_saldo_issste
     (
       nss               CHAR(11),
       subcuenta         SMALLINT,
       siefore           SMALLINT,
   ...
   CASE li_paso
      ...
      WHEN 2 --Saldo
      	 CALL inserta_paso(li_paso, gd_fecha_corte, "Inicia generacion de saldos")
      	    LET lc_comando = "nohup time ./eje_saldo_issste.sh ",
      	                     gd_fecha_corte," ",
                             GUSER CLIPPED,
                             " 1> ", GUSER CLIPPED, ".issste_saldo.salida ",
                             " 2> ", GUSER CLIPPED, ".issste_saldo.error &"
            ERROR "EJECUTANDO PROCESO POR NOHUP."
            SLEEP 2
            ERROR ""
            RUN   lc_comando
            ERROR "VERIFIQUE ARCHIVOS: ", GUSER CLIPPED, ".issste_saldo.salida Y ", GUSER CLIPPED ,".issste_saldo.error"
            SLEEP 3
      WHEN 3 --Cuota
         ...
         CALL inserta_paso(li_paso, gd_fecha_corte, "Inicia Identificacion de Cuota")
      	 LET lc_comando = "nohup time ./eje_cuota_issste.sh ",
      	                  gd_fecha_corte," ",
                          GUSER CLIPPED,
                          " 1> ", GUSER CLIPPED, ".issste_cuota.salida ",
                          " 2> ", GUSER CLIPPED, ".issste_cuota.error &"
         ERROR "EJECUTANDO PROCESO POR NOHUP."
         SLEEP 2
         ERROR ""
         RUN   lc_comando
         ERROR "VERIFIQUE ARCHIVOS: ", GUSER CLIPPED, ".issste_cuota.salida Y ", GUSER CLIPPED,".issste_cuota.error"
         SLEEP 3
      WHEN 4 --Archivo
      	 CALL inserta_paso_reporte(gd_fecha_corte, "CTANX13") RETURNING li_totnss, li_folio_proceso

      	 LET lc_comando = "nohup time fglgo CTANX13.4gi " CLIPPED,
                           li_folio_proceso CLIPPED, " ",
                           gd_fecha_corte   CLIPPED, " ",
                           li_totnss        CLIPPED, " ",
                           "1> ", GUSER CLIPPED, ".issste_rpt.salida ",
                           "2> ", GUSER CLIPPED, ".issste_rpt.error &"

         ERROR "EJECUTANDO PROCESO POR NOHUP."
         SLEEP 2
         RUN lc_comando
         ERROR "VERIFIQUE ARCHIVOS: ", GUSER CLIPPED, ".issste_rpt.salida Y ", GUSER CLIPPED, ".issste_rpt.error"
         SLEEP 3
         ERROR ""

         LET lc_msg = "Folio: ",li_folio_proceso CLIPPED, " Proc Cod: CTANX13"
...
FUNCTION despliega_pasos(ld_fecha_corte)
   ...
   OPEN WINDOW  ventana_2 AT 6,4 WITH FORM "CTANX122" ATTRIBUTES(BORDER)
