...
DATABASE safre_tmp
    ...
    DEFINE v_listado          CHAR(150)
    DEFINE v_listado1         CHAR(150)
    DEFINE v_imprime          CHAR(150)

    DEFINE reg_datos_afore RECORD LIKE safre_af:tab_afore_local.*

    DEFINE r_nss_oss RECORD
           nss         CHAR(11),
           cve_subcta  SMALLINT,
           siefore     SMALLINT,
           ind_edad    SMALLINT
...
#flr--------------------
    ...
    CREATE TABLE fmto_a (  nss                 CHAR(11),
                           ind_edad            SMALLINT,
                           sie_regimen         SMALLINT,
                           siefore             SMALLINT,
                           grupo_regimen       SMALLINT,
                           monto_en_acciones   DECIMAL(18,6))

    CREATE TABLE nss_oss ( nss                 CHAR(11),
                           cve_subcta          SMALLINT,
                           siefore             SMALLINT,
                           siefore_desc        CHAR(8),
                           ind_edad            SMALLINT,
    ...
    CREATE TABLE tmp_saldo_grupo ( nss                  CHAR(11),
                                   grupo_regimen        SMALLINT,
                                   siefore              SMALLINT,
...

{
    ...
    --- genera tabla con registros con ordenes de selección de siefores para rubro elegidos
    INSERT INTO nss_oss
    SELECT a.nss,
           a.cve_subcta,
           c.codigo_siefore,
           ...
           d.ind_edad,
           a.fecha_operacion
      FROM safre_af:cta_det_sie_acep a,
           safre_af:afi_mae_afiliado b,
           safre_af:tab_siefore_local c,
           safre_af:cta_ctr_cuenta d
     WHERE a.fecha_operacion > '03282008'
       AND a.nss = b.n_seguro
       AND a.nss = d.nss
       AND b.finicta <= a.fecha_operacion
       AND a.diagnostico1 = '000'
       AND c.razon_social = a.siefore_rec

     --- agrupa saldos por grupo_regimen
     INSERT INTO tmp_saldo_grupo
     SELECT c.nss,
            a.grupo_regimen,
            c.siefore,
            c.fecha_conversion,
       ...
       FROM safre_af:tab_grupo_regimen a,
            safre_af:tab_agrupa_subcta_regimen b,
            safre_tmp:tmp_saldo_corte c
      WHERE a.grupo_regimen = b.grupo_regimen
        --AND a.ind_actualiza = 1
        AND b.subcuenta = c.subcuenta
        AND a.grupo_regimen NOT IN(5,6,12,13)
     GROUP BY 1,2,3,4 ;

    CREATE INDEX tmp_saldo_grupo1 ON tmp_saldo_grupo ( nss );
    CREATE INDEX tmp_saldo_grupo2 ON tmp_saldo_grupo ( grupo_regimen );
    CREATE INDEX tmp_saldo_grupo3 ON tmp_saldo_grupo ( monto_en_acciones );

    UPDATE STATISTICS FOR TABLE tmp_saldo_grupo ;

     --- elimina registros con saldo en acciones en 0
     DELETE
       FROM tmp_saldo_grupo
    ...
    SELECT d.nss          ,
      ...
      FROM tmp_saldo_grupo d
     GROUP BY 1,2,3,4,5

    CREATE INDEX fmto_a_1 ON fmto_a (nss, ind_edad, siefore)
    CREATE INDEX fmto_a_2 ON fmto_a (ind_edad, siefore)
    CREATE INDEX fmto_a_3 ON fmto_a (ind_edad, sie_regimen, grupo_regimen)
    CREATE INDEX fmto_a_4 ON fmto_a (nss, grupo_regimen)

    UPDATE STATISTICS FOR TABLE fmto_a

    --- actualiza ind_edad y siefore de los registros con ordenes de seleccion de siefore (elegidos)
    DECLARE cur_oss CURSOR FOR
    SELECT o.nss       ,
           o.cve_subcta,
           o.siefore
      FROM nss_oss o
    ...
    FOREACH cur_oss INTO r_nss_oss.*
        UPDATE fmto_a
           SET ind_edad    = r_nss_oss.ind_edad,
               sie_regimen = r_nss_oss.siefore
         WHERE nss = r_nss_oss.nss
           AND grupo_regimen = r_nss_oss.cve_subcta
    END FOREACH
    ...
    FOR i = 1 TO 8
     ...
     ### IDENTIFICACION DE LOS TITULOS ###
        ...
        SELECT a.grupo_regimen   ,
          ...
          FROM fmto_a a
         WHERE a.ind_edad = a.sie_regimen
           AND a.siefore  = i
         GROUP BY 1
         ORDER BY 1
       ...
       PAGE HEADER
        ...
        IF sie.cod_siefore = 1 THEN    #SB 1, SIN SIEFORE ADICIONAL#
            SELECT COUNT(UNIQUE a.nss)
              ...
              FROM fmto_a a
             WHERE (    a.ind_edad = a.sie_regimen
                    AND a.sie_regimen  = sie.cod_siefore)
