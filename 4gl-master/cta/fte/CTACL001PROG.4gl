...
MAIN
	...
	DISPLAY "  FECHA     MOVIMIENTO DESCRIPCION    SIEFORE   MONTO PESOS    MONTO ACCIONES  " AT 15,1 ATTRIBUTE(REVERSE)
        ...
        ELSE 
	      ...
	      AND r_reg[j].f_pago <= "05/01/2002" THEN
	         ...
	         AND  ct.folio            = r_reg[j].folio
                ...
                #AND  ct.consecutivo_lote = r_reg[j].consecutivo
              ELSE
		 let errorx = "SELECT {+INDEX(dis_cuenta dis_cuenta_1)} ct.folio ," ,
		 "ct.folio_sua, hp.periodo_pago, ct.fecha_valor, ct.fecha_conversion, hp.ult_salario_diario/100 , ct.precio_accion, ct.dias_cotizados, ct.id_aportante ",
	         "FROM   dis_cuenta ct, dis_det_aporte hp ",
	         "WHERE  ct.nss = ",'\'',enc.n_seguro, '\'' ,
	         " AND ct.subcuenta = ", x_subct CLIPPED ,
	         " AND ct.tipo_movimiento = ", r_reg[j].tipo_mvto CLIPPED ,
                 " AND ct.fecha_conversion = ",'\'', r_reg[j].f_pago, '\'',
	         " AND ct.folio = ",r_reg[j].folio CLIPPED ,
                 " AND ct.folio_sua = ",'\'',r_reg[j].folio_sua CLIPPED, '\'',
                 " AND ct.monto_en_acciones = ",r_reg[j].mto_accion CLIPPED ,
                 " AND ct.consecutivo_lote = ",r_reg[j].consecutivo CLIPPED ,
	         " AND hp.folio = ct.folio AND hp.n_seguro = ct.nss AND hp.folio_pago_sua = ct.folio_sua AND hp.consec_reg_lote = ct.consecutivo_lote" CLIPPED 

		PREPARE finalxx FROM errorx
		EXECUTE finalxx INTO final.*
