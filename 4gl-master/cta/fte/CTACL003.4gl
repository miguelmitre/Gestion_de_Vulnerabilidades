...
#Owner             => E.F.P.
#Programa CTACL003 => CONSULTA DE SALDOS POR CUENTA INDIVIDUAL HISTORICO
#Fecha actualiz.   => 04 NOVIEMBRE 1998
#Actualizacion     => HECTOR M. FERNANDEZ A.
#Actualizacion     => ARMANDO RODRIGUEZ CASTROPAREDES
#Fecha actualiz.   => 27 noviembre 08
#Sistema           => CTA
#Req:MLM-754       => JCPV 09/08/2011. Identificar subctas 36,37(31)
##############################################################################
DATABASE safre_af
GLOBALS
   ...
   DEFINE det1 ARRAY[15] OF RECORD
       subct_prueba     CHAR(1),
       subct_cod        SMALLINT,
       subct_corta      CHAR(5),
...
---solo XXI
	...
	DISPLAY " CTACL003   CONSULTA HISTORICA DE SALDOS DE CUENTA INDIVIDUAL                  " AT 3,1 ATTRIBUTE(REVERSE)
	DISPLAY "SUBCUENTA        PESOS           SIEFORE         PESOS          ACCIONES       " AT 6,1 ATTRIBUTE(REVERSE)
	DISPLAY "  FECHA     MOVTO DESC  ID APORTANTE   SIEFORE  MONTO PESOS    MONTO ACCIONES  " AT 15,1 ATTRIBUTE(REVERSE)                   #754
...
#c-----------------
   ...
   WHENEVER ERROR CONTINUE
      ...
      DROP TABLE cuenta_individual
      CREATE TEMP TABLE cuenta_individual(
...
#dpi----------------------------
	...
	DEFINE det2 ARRAY[5] OF RECORD
 	...
 	END RECORD
      ...
      DEFINE vfecha_part         DATE
      DEFINE vfecha_udi          DATE
      DEFINE vprecio_part        DECIMAL(16,14)
      DEFINE vprecio_part12      DECIMAL(16,14)
      DEFINE vprecio_udi         DECIMAL(16,14)
      DEFINE xprecio             DECIMAL(16,14)
      DEFINE xacciones           DECIMAL(16,6)
      DEFINE xpesos              DECIMAL(16,6)
      DEFINE rpesos_viv          DECIMAL(12,2)
      DEFINE xhis_cuenta         CHAR(12)
      DEFINE xcarga_tabla        CHAR(300)
      ...
      ---carga historia
{
      ---- carga antes del 2000
      DECLARE cta_his9 CURSOR FOR
      select tabname
      from   systables
      where tabname matches  "dis_cuenta9*"
      FOREACH cta_his9 INTO xhis_cuenta
          LET xcarga_tabla = " INSERT INTO cuenta_individual ",
                             " SELECT * ",
                             " FROM  ", xhis_cuenta ,
                             " WHERE  nss = ","'",enc.n_seguro,"'",
                             " AND    tipo_movimiento <> 999 "
          PREPARE cta_his1 FROM xcarga_tabla
          EXECUTE cta_his1 
      END FOREACH

      ---- carga 2000 an adelante
      LET xcarga_tabla = ""
      DECLARE cta_his CURSOR FOR
      select tabname
      from   systables
      where tabname matches  "dis_cuenta0*"
      FOREACH cta_his INTO xhis_cuenta
          LET xcarga_tabla = " INSERT INTO cuenta_individual ",
                             " SELECT * ",
                             " FROM  ", xhis_cuenta ,
                             " WHERE  nss = ","'",enc.n_seguro,"'",
                             " AND    tipo_movimiento <> 999 "
          PREPARE cta_his2 FROM xcarga_tabla
          EXECUTE cta_his2 
      END FOREACH
}
      DECLARE cta_his CURSOR FOR
      ...
      and   tabname not matches  "dis_cuenta_*"
      FOREACH cta_his INTO xhis_cuenta
          LET xcarga_tabla = " INSERT INTO cuenta_individual ",
                             " SELECT * ",
                             " FROM  ", xhis_cuenta ,
   ...
   -- solo xxi
   LET xdias = DAY(vfecha_accion) -1
   LET vfecha_part = vfecha_accion - xdias UNITS DAY
   LET vfecha_udi  = vfecha_part - 1

   SELECT precio_del_dia
   INTO   vprecio_part
   FROM   glo_valor_accion
   WHERE  codigo_siefore = 11
   AND    fecha_valuacion = vfecha_part

   SELECT precio_del_dia
   INTO   vprecio_part12
   FROM   glo_valor_accion
   WHERE  codigo_siefore = 12
   AND    fecha_valuacion = vfecha_part
   ...
   AND    fecha_valuacion = vfecha_udi
...
---carga precio del dia
   ...
   IF SQLCA.SQLCODE <> 0 THEN
      ...
      UPDATE precio_dia
      SET      precio_dia.precio = vprecio_part
      WHERE    precio_dia.codigo = 11
      ...
      IF STATUS = NOTFOUND THEN
         INSERT INTO precio_dia VALUES (vprecio_part12,12)
      ELSE
         UPDATE   precio_dia
         SET      precio_dia.precio = vprecio_part12
         WHERE    precio_dia.codigo = 12
      END IF
      UPDATE precio_dia
      SET      precio_dia.precio = vprecio_udi
      WHERE    precio_dia.codigo = 13
   END IF
   ----saldos totales
...
#tel----------------
   ...
   WHERE  n_seguro = enc.n_seguro
	...
	END DISPLAY
   CLOSE WINDOW v16
	DISPLAY "  FECHA     MONTO DESC  ID APORTANTE   SIEFORE  MONTO PESOS    MONTO ACCIONES  " AT 15,1 ATTRIBUTE(REVERSE)                   #754
END FUNCTION
...
#d-----------------------
   ...
   DEFINE c10f_valor       CHAR(10)
   DEFINE c8f_valor        CHAR(08)
   DEFINE xafo_recep       CHAR(03)
      ...
      IF x_subct = 7  OR
	   ...
	   AND    ct.fecha_conversion = r_reg[j].f_pago
        ...
        ELSE 
	      ...
	      AND r_reg[j].f_pago <= "05/01/2002" THEN
	         ...
	         SELECT ct.folio,
	                ...
	                ct.id_aportante
	         INTO   final.*
	         FROM   cuenta_individual ct, dis_det_aporte hp
	         WHERE  ct.nss               = enc.n_seguro
	         AND    ct.subcuenta         = x_subct
	         AND    ct.tipo_movimiento   = r_reg[j].tipo_mvto
                 AND    ct.fecha_conversion  = r_reg[j].f_pago 
        ...
        END IF
                  ...
                  IF x_subct = 36 THEN
                     DECLARE rbono CURSOR FOR
                        SELECT fecha_reden
                        --INTO   c8f_valor
                        FROM   dis_det_bono
                        WHERE  n_seguro = enc.n_seguro
                        --AND    folio    = final.folio
                     FOREACH rbono INTO c8f_valor
                        EXIT FOREACH
                     END FOREACH
                     IF  c8f_valor IS NULL THEN
                        ...
                        AND    folio = final.folio
                     ELSE
                        LET  c10f_valor = "01/01/",c8f_valor[1,4] CLIPPED
                     END IF
                     LET  final.fecha_valor = c10f_valor
                  END IF
                  IF x_subct = 5 THEN
		      ...
		      END IF
                  ...
                  IF x_subct = 30 OR
                     ...
                     x_subct = 33 THEN
                     SELECT periodo_pago,
                            sueldo_base_cot_rcv/100,
                            sum(dias_cot_bimestre - dias_incap_bimestre
                            - dias_ausen_bimestre),
                            dias_cot_bimestre
                     INTO   final.periodo_pago,
                            final.salario_diario,
                            dias_cs_acum,
                            final.dias_cotizados
                     FROM   dis_det_issste
                     WHERE  n_seguro = enc.n_seguro
                  ...
                  END IF
                  IF r_reg[j].tipo_mvto =220 OR
                     r_reg[j].tipo_mvto =225 OR
                     r_reg[j].tipo_mvto =226 OR
                     r_reg[j].tipo_mvto =247 OR
                     r_reg[j].tipo_mvto =291 OR
                     r_reg[j].tipo_mvto =293 OR
                     r_reg[j].tipo_mvto =294 OR
                     r_reg[j].tipo_mvto =295 OR
                     r_reg[j].tipo_mvto =297 THEN
                     IF (final.id_aportante[4,6] IS NULL OR
                         final.id_aportante[4,6] = "  ") THEN
                        SELECT ident_lote_solici[3,5]
                        INTO   xafo_recep
                        FROM   taa_cd_det_cedido
                        WHERE  n_seguro = enc.n_seguro
                        AND    folio = final.folio
                        AND    estado = 103
                        LET final.id_aportante = "TR-", xafo_recep CLIPPED
                     END IF
                  END IF
		  ...
		  DISPLAY " Periodo Pago                  ",
                  ...
                  IF x_subct = 36 THEN
		  DISPLAY " Fecha Redencion de Bono       ",
