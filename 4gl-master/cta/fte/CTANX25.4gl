...
#Owner               => E.F.P.                                                 #
#Programa CTANX25    => ESTADO DE CUENTA                                       #
#Sistema             => CTA                                                    #
...
GLOBALS
 ...
 DEFINE v_sb,i,j           SMALLINT
   ...
   DEFINE gar_fechas ARRAY[2]  OF RECORD
   	      f_proceso DATE,
   	      f_precio  DATE
   END RECORD
END GLOBALS
...
FUNCTION inicializa()
   ...
   #Fecha antes
   LET gar_fechas[1].f_proceso = '08/16/2012'
   LET gar_fechas[1].f_precio  = '08/16/2012'

   #Fecha despues
   LET gar_fechas[2].f_proceso = '08/17/2012'
   LET gar_fechas[2].f_precio  = '08/17/2012'
...
FUNCTION crea_tablas()
   ...
   (nss              CHAR(11)     ,
    ...
    id_sector        SMALLINT     ,
    id_sector_nvo    SMALLINT     , --nuevo layout
    tipo_trabajador  SMALLINT     , --nuevo_layout
    actividad        SMALLINT     , --nuevo_layout
    sexo             SMALLINT     , --nuevo_layout
    fecha_ult_liq    DATE         , --nuevo_layout
    salario_di       DECIMAL(22,6), --nuevo_layout
    peiodo_pago      CHAR(06)     , --nuevo_layout
    actividad_liq    SMALLINT     , --nuevo_layout
    criterio         SMALLINT     ,
    siefore          SMALLINT     ,
    fecha_corte      DATE         ,
    factualiza       DATE         ,
   ...
   IF gi_afore = 562 THEN --Invercap
      SELECT UNIQUE nss
      FROM   tes_solicitud
      WHERE  estado          = 103
      AND    tipo_traspaso   = 13
      AND    fecha_solicitud = "08/16/2012"
      INTO TEMP tmp_aceptados_regimen
   END IF
   ...
   CREATE INDEX ix1_tmp_cuota_trasferencia ON safre_tmp:tmp_cuota_trasferencia (nss,estatus_cuenta);
   CREATE INDEX ix2_tmp_cuota_trasferencia ON safre_tmp:tmp_cuota_trasferencia (tipo_solicitud,estatus_cuenta);
   UPDATE STATISTICS FOR TABLE safre_tmp:tmp_cuota_trasferencia;
END FUNCTION
################################################################################
  ...
  END SQL
   ...
   FOR li_estatus_cuenta = 2 TO 5
         LET lc_nomarch = lc_nomarch_ini CLIPPED ,
          ...
          #/ruta/YYYYMMDD_AF_afore_edad_016_00000.004
   ...
   LET lc_totales = gr_seg_modulo.ruta_envio CLIPPED,"/",
                    "anexo_sumario_edad_", gd_fecha_corte USING "DDMMYYYY"
...
FUNCTION archivo_cpl()
  ...
  END SQL
   ...
   FOR li_estatus_cuenta = 1 TO 13 --TIPO SOLICITUD
      FOR li_rango_edad = 1 TO 5  --SIEFORE POR EDAD
      	 ...
      	 IF SQLCA.SQLCODE = 0 THEN
            ...
            #/ruta/anexo71_edad_ts00_re1_DDMMYYYY
            #/ruta/anexo71_edad_enc_DDMMYYYY
            #/ruta/anexo71_edad_det03_DDMMYYYY
            #/ruta/YYYYMMDD_AF_afore_edad_016_00000.004
  ...
  END SQL
   ...
   FOR li_estatus_cuenta = 2 TO 5
      FOR li_rango_edad = 1 TO 5
          ...
          #/ruta/anexo71_edad_sc0_re1_DDMMYYYY
          #/ruta/anexo71_edad_enc_DDMMYYYY
          #/ruta/anexo71_edad_det03_DDMMYYYY
          #/ruta/YYYYMMDD_AF_afore_edad_016_00000.004
      ...
      AFTER GROUP OF lr_monto_nss.nss
         ...
         IF li_saldo = 200 THEN
            ...
            #Obtener siefore del trabajador
         	  ...
         	  #Si la fecha es antes
         	  IF gd_fecha_corte = gar_fechas[1].f_proceso THEN
            	  LET li_cod_siefore = li_cod_siefore + 1
            END IF
...
FUNCTION detalle03(lc_detalle03)
   ...
   IF gd_fecha_corte = gar_fechas[1].f_proceso THEN
   	  #Si la fecha es antes
   	  LET ld_ultimo_habil = gar_fechas[1].f_precio
   ELSE
   	  #Si la fecha es después
   	  IF gd_fecha_corte = gar_fechas[2].f_proceso THEN
   	     LET ld_ultimo_habil = gar_fechas[2].f_precio
   	  END IF
   END IF
   ...
   WHERE  codigo_siefore NOT IN (0,10)
   ORDER  BY 1
