...
#Owner               => E.F.P.                                               #
#Programa CTAB009    => ESTADO DE CUENTA                                     #
#Sistema             => CTA                                                  #
...
FUNCTION prepara_nss()
   ...
   LET v_arch = g_parametro.ruta_envio CLIPPED,"/",x_tipo_informe USING "&&",
                hoy USING "ddmmyy",".",x_folio USING "<<<<<<<<<"
   ...
   OUTPUT TO REPORT r_report(genera_encabezado,229,1)
   ...
   FOREACH cur_fechas INTO g_reg.fecha_ini,
      ...
      FOREACH cur_nss INTO x_tipo_informe,
         ...
         OUTPUT TO REPORT r_report(genera_detalle,229,2)
      END FOREACH
...
FUNCTION prepara_nss_semestral()
   ...
   LET g_reg.fecha_ini = "01/01/2005"
   LET g_reg.fecha_top = "06/30/2005"
...
display rendimiento_siefore2
   ...
   IF x_tipo_informe >= 0 THEN
      LET rendimiento_siefore1 = 9.48
      LET rendimiento_siefore2 = 9.63
   END IF
...
FUNCTION datos_trabajador()
   ...
   IF x_tipo_informe = 3 THEN
      ...
      LET xx_fecha_reingreso = xx_fec_liq_tras + 366 UNITS DAY 
...
FUNCTION saldo_dia(pnss,
   ...
   ELSE
      ...
      WHERE  b.fecha_valuacion = pfecha_saldo
      ...
      IF num_siefore_t <>  num_siefore_g THEN
         DISPLAY "Verifique siefores y precios de accion: ",pfecha_saldo,
                 " Vivienda: ",pfecha_saldo
      END IF

      SELECT c.precio_del_dia
      INTO   v_precio_viv
      FROM   glo_valor_accion c
      WHERE  c.fecha_valuacion = pfecha_saldo
...
-----------------------------------------------------------------------
   ...
   FOREACH c_saldo1 INTO f_subcuenta,
      ...
      CASE
         ...
         WHEN f_subcuenta = 2 OR f_subcuenta = 6 OR f_subcuenta = 9
            LET cv_fin  = cv_fin  + f_monto_pes
         WHEN f_subcuenta = 5
            LET cs_fin  = cs_fin  + f_monto_pes
         WHEN f_subcuenta = 7
         ...
         WHEN f_subcuenta = 4
            LET viv_97_imss_fin = viv_97_imss_fin + f_monto_pes
         WHEN f_subcuenta = 8
...
--
   ...
   SELECT (SUM(a.monto_en_pesos))*(-1)
   INTO   comircv
   FROM   tmp_cuenta a
   WHERE  a.subcuenta        IN (1,2,5,6,9)
   AND    a.tipo_movimiento  BETWEEN 100 AND 109
   ...
   SELECT SUM(ROUND(b.monto_en_pesos,2))*(-1)
   INTO   comisal
   FROM   tmp_cuenta b
   WHERE  b.subcuenta        IN (1,2,3,5,6,7,9,10,11,12,13)
   AND    b.tipo_movimiento  = 110
   ...
   LET total_comisiones = comisal + comivol + comircv
...
FUNCTION obtiene_retiros()
   ...
   SELECT a.subcuenta,a.id_aportante,
   ...
   FOREACH cursor_retiros INTO r_subcuenta,
      ...
      CASE 
         ...
         WHEN r_subcuenta = 2 OR r_subcuenta = 6 OR r_subcuenta = 9
            LET cv_retiros  = cv_retiros  + r_monto_pes

         WHEN r_subcuenta = 5
            LET cs_retiros  = cs_retiros  + r_monto_pes
         ...
         WHEN r_subcuenta = 4
            LET viv_97_imss_retiros = viv_97_imss_retiros + r_monto_pes
   ...
   LET sub_retiros_rcv = retiro97_retiros +
                         sar_imss_retiros +
                         sar_issste_retiros +
                         cv_retiros +
                         cs_retiros
...
FUNCTION aportes_netos()
   ...
   LET retiro97_netas        = retiro97_fin - retiro97_ini + (retiro97_retiros * -1)
   LET cv_netas              = cv_fin - cv_ini + (cv_retiros * -1)
   LET cs_netas              = cs_fin - cs_ini + (cs_retiros * -1)
   ...
   LET sub_netas_rcv         = retiro97_netas +
                               sar_netas_imss +
                               sar_netas_issste +
                               cv_netas +
                               cs_netas

   LET viv_97_imss_netas     = viv_97_imss_fin - viv_97_imss_ini + (viv_97_imss_retiros * -1)
...
--IF g_reg.status = 50 THEN
   SELECT "X"
   FROM   tmp_cuenta
   WHERE  subcuenta        IN (1,2,3,5,6,7,9,10,11,12,13)
   AND    tipo_movimiento  = 10
   AND    fecha_conversion >= g_reg.fecha_ini
   AND    fecha_conversion <= g_reg.fecha_top
   GROUP BY 1
   ...
   IF  STATUS <> NOTFOUND THEN
      ...
      LET dia       = DAY(hoy)-1
      LET mes       = MONTH(hoy)-1
      LET vfecha_fiscal    = hoy - dia UNITS DAY
      ...
      SELECT subcuenta,
             ...
             tipo_movimiento
      FROM   tmp_fiscal
      WHERE  subcuenta IN (3,7,10,11,12)
...
FUNCTION ren_fiscal(x_nss,vfecha_fiscal)
   ...
   FOREACH cur_col USING x_nss,
      ...
      LET v_precio_fin = precio_siefore (2,gr_prom.fecha_liq)
...
FUNCTION sumarios()
   ...
   DEFINE com_1 ARRAY [20] OF RECORD
          afore_cod    SMALLINT,
          afore_desc   CHAR(30),
   ...
   DEFINE com_1x ARRAY [20] OF RECORD
          porcentaje   CHAR(5)
   END RECORD
   ...
   IF STATUS = NOTFOUND THEN
      SELECT MAX(fecha)
      INTO   xc_fecha
      FROM   safre_tmp:tab_comparativo
   ELSE
   ...
   SELECT a.afore_cod,
          ...
          a.porcentaje,
          a.fecha
   FROM   safre_tmp:tab_comparativo a ,OUTER tab_afore b
   ...
   FOR cont_com = 1 TO 20
      IF com_1[cont_com].afore_desc IS NULL THEN
         LET genera_sum97 = "|",
                            "|",
                            "|"

         OUTPUT TO REPORT r_report(genera_sum97,229,97)
      ELSE
   ...
   FOREACH com2 INTO com_1[cont_com].*
      ...
      IF com_1[cont_com].porcentaje = 0.0 THEN
         LET com_1x[cont_com].porcentaje = "N/A"
      ELSE
   ...
   FOR cont_com = 1 TO 20
      IF com_1[cont_com].afore_desc IS NULL THEN
         ...
         OUTPUT TO REPORT r_report(genera_sum98,229,98)
      ELSE
   ...
   OUTPUT TO REPORT r_report(genera_sum99,229,99)
