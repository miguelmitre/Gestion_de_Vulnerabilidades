...
################################################################################
FUNCTION proceso()
   OPEN WINDOW ventana_1 AT 2,2 WITH FORM "CTANX011" ATTRIBUTE(BORDER)
           #-----------------------------------------------------------------------------
   DISPLAY "CTANX01                A N E X O     7 1          < Ctrl-C > SALIR     " AT 3,1 ATTRIBUTE(REVERSE)
   ...
   MENU "ANEXO 71"
      ...
      MENU "Consultas"
         COMMAND "Consulta" "Consulta de saldos"
            CALL consulta(1)
         COMMAND "Regresar" "Regresar al menu anterior"
            EXIT MENU
      END MENU}
      ...
      MENU "Archivos"
        COMMAND "Archivo Procesar" "Reporte del anexo 71 Procesar"
           CALL lee_paso(4)

        COMMAND "Archivo Consar" "Reporte del anexo 71 Consar"
           CALL lee_paso(5)
...
{               	 IF valida_dia(gd_fecha_corte) != 0 THEN
         ...
         IF li_paso = 5 THEN
            ...
            IF SQLCA.SQLCODE <> 0 THEN
            	 ERROR "DEBE EJECUTAR PRIMERO EL ARCHIVO DE PROCESAR"
            	 SLEEP 3
            	 ERROR ""
            	 EXIT INPUT
            END IF
...
FUNCTION borra_informacion()
   ...
   WHENEVER ERROR CONTINUE
      DROP TABLE safre_tmp:cta_cuota_anexo71_nss
      DROP TABLE safre_tmp:cta_estatus_saldos_anexo71
      DROP TABLE safre_tmp:cta_estatus_total_sie_anexo71

      DROP TABLE safre_tmp:glo_valor_accion

      DROP TABLE safre_tmp:tmp_saldo_anexo71
   WHENEVER ERROR STOP

   CREATE TABLE safre_tmp:cta_cuota_anexo71_nss
  (nss              CHAR(11),
  ...
  CREATE TABLE safre_tmp:cta_estatus_total_sie_anexo71
  (estatus_cuenta    SMALLINT,
   fecha_corte       DATE,
   siefore           SMALLINT,
   total_nss         INTEGER,
  ...
  CREATE TABLE safre_tmp:cta_estatus_saldos_anexo71
  (estatus_cuenta    SMALLINT,
   fecha_corte       DATE,
   siefore           SMALLINT,
   subcuenta         SMALLINT,
  ...
  CREATE TABLE safre_tmp:tmp_saldo_anexo71
  (
    nss               CHAR(11),
    subcuenta         SMALLINT,
    siefore           SMALLINT,
...
FUNCTION valida_proceso (li_paso, ld_fecha_corte)
   ...
   IF li_paso = 4 OR      -- LIQUIDACION
   	  li_paso = 5 OR      -- ARCHIVO 2 SIEFORES
   ...
   CASE li_paso
      ...
      WHEN 2 --Genera Saldo Corte
      	 CALL inserta_paso(li_paso, gd_fecha_corte, "Inicia generacion de saldos Anexo71")
      	    LET lc_comando = "nohup time ./eje_saldo_anexo71.sh ",
      	                     gd_fecha_corte," ",
                             GUSER CLIPPED,
                             " 1> anexo71_saldo.salida ",
                             " 2> anexo71_saldo.error &"
            ERROR "EJECUTANDO PROCESO POR NOHUP."
            SLEEP 2
            ERROR ""
            RUN   lc_comando
            ERROR "VERIFIQUE ARCHIVOS: anexo71_saldo.salida Y anexo71_saldo.error"
            SLEEP 3
      WHEN 3 --Identifica NSS
         ...
         CALL inserta_paso(li_paso, gd_fecha_corte, "Inicia Identificacion de NSS Anexo71")
      	 LET lc_comando = "nohup time ./eje_anexo71_nss.sh ",
      	                  gd_fecha_corte," ",
                          GUSER CLIPPED,
                          " 1> anexo71_nss.salida ",
                          " 2> anexo71_nss.error &"
         ERROR "EJECUTANDO PROCESO POR NOHUP."
         SLEEP 2
         ERROR ""
         RUN   lc_comando
         ERROR "VERIFIQUE ARCHIVOS: anexo71_nss.salida Y anexo71_nss.error"
         SLEEP 3
         --END IF
      WHEN 4 --Reporte Procesar
      	 CALL inserta_paso_reporte(gd_fecha_corte, "CTANX02") RETURNING li_totnss, li_folio_proceso

      	 LET lc_comando = "nohup time fglgo CTANX02.4gi " CLIPPED,
                           li_folio_proceso CLIPPED, " ",
                           gd_fecha_corte   CLIPPED, " ",
                           li_totnss        CLIPPED, " ",
                           "1> anexo71_rpt_prc.salida ",
                           "2> anexo71_rpt_prc.error &"

         ERROR "EJECUTANDO PROCESO POR NOHUP."
         SLEEP 2
         RUN lc_comando
         ERROR "VERIFIQUE ARCHIVOS: anexo71_rpt_prc.salida Y anexo71_rpt_prc.error"
         SLEEP 3
         ERROR ""

         LET lc_msg = "Folio: ",li_folio_proceso CLIPPED, " Proc Cod: CTANX02"
         PROMPT lc_msg CLIPPED FOR CHAR lc_ok ATTRIBUTE (REVERSE)

      WHEN 5 --Reporte Consar
      	 CALL inserta_paso_reporte(gd_fecha_corte, "CTANX03") RETURNING li_totnss, li_folio_proceso

      	 LET lc_comando = "nohup time fglgo CTANX03.4gi " CLIPPED,
                           li_folio_proceso CLIPPED, " ",
                           gd_fecha_corte   CLIPPED, " ",
                           li_totnss        CLIPPED, " ",
                           "1> anexo71_rpt_cns.salida ",
                           "2> anexo71_rpt_cns.error &"

         ERROR "EJECUTANDO PROCESO POR NOHUP."
         SLEEP 2
         RUN lc_comando
         ERROR "VERIFIQUE ARCHIVOS: anexo71_rpt_cns.salida Y anexo71_rpt_cns.error"
         SLEEP 3
         ERROR ""

         LET lc_msg = "Folio: ",li_folio_proceso CLIPPED, " Proc Cod: CTANX03"
         PROMPT lc_msg CLIPPED FOR CHAR lc_ok ATTRIBUTE (REVERSE)
...
FUNCTION despliega_pasos(ld_fecha_corte)
   ...
   OPEN WINDOW  ventana_2 AT 6,4 WITH FORM "CTANX012" ATTRIBUTES(BORDER)
...
FUNCTION consulta(li_tipo_consulta)
   ...
   OPEN WINDOW ventana_3 AT 2,2 WITH FORM "CTANX013" ATTRIBUTE(BORDER)
              #------------------------------------------------------------------------------
   DISPLAY    "<CTANX01>           C O N S U L T A     D E     S A L D O S                   " AT 1,1 ATTRIBUTE(REVERSE)
      ...
      IF li_consultar = 1 THEN
      	 ...
      	 CASE li_tipo_consulta
      	    ...
      	    LET lc_sql = "SELECT estatus_cuenta, ",
                         ...
                         "FROM   safre_tmp:cta_estatus_total_sie_anexo71 ",
                         "WHERE  fecha_corte = '", gd_fecha_corte, "' ",
                         lc_where CLIPPED,
                         "GROUP BY 1,3 ",
                         "ORDER BY 1,3 "
            ...
            AT 3,1 ATTRIBUTE(REVERSE)
      	    LET lc_sql = "SELECT estatus_cuenta, ",
                         ...
                         "SUM(total_nss) NssTot,          ",
                         "SUM(monto_en_acciones) Acciones ",
                         "FROM   safre_tmp:cta_estatus_total_sie_anexo71_est ",
                         "WHERE  fecha_corte = '", gd_fecha_corte, "' ",
                         lc_where CLIPPED,
                         "GROUP BY 1,3 ",
                         "ORDER BY 1,3 "
...
FUNCTION muestra_subcuentas(li_siefore, li_estatus, ld_fecha_corte, li_tipo_consulta)
   ...
   CASE li_tipo_consulta
         ...
         LET lc_query = "SELECT b.subct_cns, "           ,
                        ...
                        "FROM   safre_tmp:cta_estatus_saldos_anexo71 a, "  ,
      ...
      WHEN 2 --CONSULTA FUTURA
         LET lc_query = "SELECT b.subct_cns, "           ,
                              "SUM(a.monto_en_acciones) ",
                        "FROM   safre_tmp:cta_estatus_saldos_anexo71_est a, "  ,
                               "safre_af:tab_subcuenta b "        ,
                        "WHERE  a.siefore        = ", li_siefore,  " ",
                        "AND    a.estatus_cuenta = ", li_estatus,  " ",
                        "AND    a.subcuenta      = b.subct_cod ",
...
FUNCTION consulta_liquidacion()
   ...
   OPEN WINDOW ventana_4 AT 2,2 WITH FORM "CTANX014" ATTRIBUTE(BORDER)
              #------------------------------------------------------------------------------
   DISPLAY    "<CTANX01>      C O N S U L T A    D E    L I Q U I D A C I O N                " AT 1,1 ATTRIBUTE(REVERSE)
   DISPLAY    "<ESC> CONSULTAR                                              <CTRL-C> CANCELAR" AT 2,1
...
FUNCTION consulta_regimen()
   ...
   OPEN WINDOW ventana_5 AT 2,2 WITH FORM "CTANX015" ATTRIBUTE(BORDER)
              #------------------------------------------------------------------------------
   DISPLAY    "<CTANX01>         C O N S U L T A    D E    R E G I M E N                     " AT 1,1 ATTRIBUTE(REVERSE)
...
INTEGER
   ...
   FIRST PAGE HEADER
      ...
      SKIP 1 LINE
      PRINT COLUMN 001, "PROGRAMA: CTANX01"
