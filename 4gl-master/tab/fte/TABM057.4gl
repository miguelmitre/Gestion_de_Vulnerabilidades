...
#Owner             => E.F.P.                                       #
#Programa TABM057  => Catalogo de Contabilidad del Origen          #
#Fecha             => 12/10/2001  .                                #
...
FUNCTION proceso()
   LET HOY = TODAY
   OPEN WINDOW ventana_1 AT 3,3 WITH FORM "TABM0571" ATTRIBUTE( BORDER)
...
FUNCTION Agrega()
   ...
   INPUT BY NAME  g_reg.*
         ...
         AFTER FIELD origen_cod
               IF g_reg.origen_cod IS NULL THEN
                  ERROR "Codigo NO puede ser nulo"
                  NEXT FIELD  origen_cod
               END IF

               SELECT "X" 
               FROM   tab_origen
               WHERE  origen_cod = g_reg.origen_cod
         ...
         BEFORE FIELD descripcion
               IF g_reg.origen_cod IS NULL OR  g_reg.origen_cod = 0 THEN
                  ERROR "Codigo NO puede ser nulo"
                  NEXT FIELD  origen_cod
               END IF 
         ...
         ON KEY ( ESC )
               ...
               INSERT INTO tab_origen VALUES ( g_reg.* ) 
...
FUNCTION Consulta()
   ...
   IF (pos-1) >= 1 THEN
      ...
      LET sel_where = "SELECT * FROM tab_origen WHERE ",cla_where CLIPPED,
                      "ORDER BY 1,2 "
   ...
   IF (pos-1) >= 1 THEN
      ...
      IF (pos-1) >= 1 THEN
          ...
          DISPLAY ARRAY l_record TO scr_1.* 
             ...
             ON KEY (CONTROL-M)
                LET pos = ARR_CURR()
                LET g_reg.origen_cod = l_record[pos].codigo
...
FUNCTION impresion(pos)
   ...
   FOR i=1 TO (pos+1)
       LET g_reg.origen_cod  = l_record[i].codigo
...
REPORT rpt_taborigen(g_reg)
   ...
   FORMAT
      ...
      ON EVERY ROW
         ...
         PRINT COLUMN 12,g_reg.origen_cod USING "#&&",
