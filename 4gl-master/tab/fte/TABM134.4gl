...
GLOBALS
    ...
    END RECORD
       ...
       DEFINE g_reg1  ARRAY[1000] OF RECORD 
              fecha_valuacion          LIKE glo_valor_acc_fov.fecha_valuacion,
              codigo_siefore           LIKE glo_valor_acc_fov.codigo_siefore,
              razon_social             LIKE tab_siefore_local.razon_social,
              precio_del_dia           CHAR(20), 
            #  monto_comis_saldos       LIKE glo_valor_acc_fov.monto_comis_saldos,
              rowid                    INTEGER 
       END RECORD

       DEFINE g_reg2  RECORD 
              acc_circ_cap_fijo        LIKE glo_valor_acc_fov.acc_circ_cap_fijo,
              acc_circ_res_esp         LIKE glo_valor_acc_fov.acc_circ_res_esp,
              acc_circ_inv_perm        LIKE glo_valor_acc_fov.acc_circ_inv_perm,
              acc_circ_traba           LIKE glo_valor_acc_fov.acc_circ_traba,
              acc_circ_traba_vol       LIKE glo_valor_acc_fov.acc_circ_traba_vol,
              acc_circ_inv_afore       LIKE glo_valor_acc_fov.acc_circ_inv_afore,
                acc_circ_totales         LIKE glo_valor_acc_fov.acc_circ_totales,
              rowid                    INTEGER 
       END RECORD
       ...
       DEFINE g_reg3  RECORD 
              codigo_siefore           LIKE glo_valor_acc_fov.codigo_siefore,
              razon_social             LIKE tab_siefore_local.razon_social,
              precio_del_dia           LIKE glo_valor_acc_fov.precio_del_dia, 
              fecha_valuacion          LIKE glo_valor_acc_fov.fecha_valuacion,
              rowid                    INTEGER 
       END RECORD
...
#----------------
         ...
         INPUT BY NAME g_reg.*
            BEFORE FIELD codigo_siefore
               IF modo = "1" OR modo = "2" OR modo = "3" OR modo = "4" THEN
                  LET g_reg.codigo_siefore = 21
                  LET g_reg.razon_social = "AIVs FOVISSSTE"
...
{               IF g_reg.codigo_siefore IS NULL THEN
               ...
               IF g_reg.codigo_siefore = 21 THEN 
                 IF modo = "1" THEN
                  CALL despliega_a21()
                  DISPLAY BY NAME g_reg.codigo_siefore
                  EXIT INPUT
      ...
      IF g_reg.codigo_siefore = 21 AND modo = "1" THEN
            CALL INICIALIZA()
      ELSE
         CALL Carga1()
      END IF
...
#----------------
        ...
        LET g_reg3.razon_social = "AIVs FOVISSSTE"
...
#pp-------------------------
    ...
    OPEN WINDOW ventana_21 AT 3,2 WITH FORM "TABM0345" ATTRIBUTE(BORDER)
    ...
    INPUT BY NAME g_reg21.generar
        AFTER FIELD generar
        IF g_reg21.generar IS NULL THEN
...
DECLARE cur_12 CURSOR FOR
    ...
    FOREACH cur_12 INTO g_regg.*
         ...
         SELECT 'X'
           FROM glo_valor_acc_fov
          WHERE codigo_siefore  = 21
            AND fecha_valuacion = g_regg.fecha_valuacion
          GROUP BY 1
         IF STATUS = NOTFOUND THEN

            INSERT INTO glo_valor_acc_fov 
            VALUES(21, g_regg.precio_del_dia, g_regg.fecha_valuacion,
...
#impr--------------
    ...
    START REPORT rpt_val_part TO G_IMPRE
       ...
       FOR ii=1 TO (i)
          ...
          LET g.codigo_siefore       = g_regi[ii].codigo_siefore
          LET g.fecha_valuacion      = g_regi[ii].fecha_valuacion
          LET g.precio_del_dia       = g_regi[ii].precio_del_dia
      ...
      PAGE HEADER
        ...
        PRINT '\033e\033(10U\033&l1O\033&k2S\033&l12d'
             ...
             PRINT COLUMN  2, "SIEFORE",
              ...
              COLUMN 44, "PARTISIPACION DEL DIA"
