...
#Modificacion      => DMR 04/JULIO/2014   CPL-1675 Se calculan los pesos con#
#                  => las acciones redondeadas a 2 dec, no con las acciones #
#                  => del saldo que son a 6 dec.                            #
#############################################################################
DATABASE safre_af
...
MAIN
   ...
   CALL STARTLOG(FGL_GETENV("USER")||".ACRB002.log")
   CALL inicio()             #i
...
#pp-------------------------
   ...
   ELSE
      ...
      LET cat = "cat ",g_seg_modulo.ruta_envio CLIPPED,"/CTA ",
                       ...
                       "TRANS_ACRED.",hoy USING "MMDD","-",
                       hora[1,2],hora[4,5]

      RUN cat

      DISPLAY "ARCHIVO GENERADO EN RUTA DE ENVIO Y PROCESO FINALIZADO " AT 19,1
      ...
      FOREACH cur_1 INTO reg_det_tra_ctas.tipo_transferencia ,
         ...
         ELSE
            ...
            IF sdo_part_v97 IS NULL OR sdo_part_v97 < 0.01 THEN
               ...
               LET sdo_pe97_2d  = 0
            ELSE
               LET sdo_pa97_2d = sdo_part_v97                    #CPL-1675
               LET sdo_apo_v97 = sdo_pa97_2d * vprecio_accion
            ...
            IF sdo_part_v92 IS NULL OR sdo_part_v92 < 0.01 THEN
               ...
               LET sdo_pe92_2d  = 0
            ELSE
               LET sdo_pa92_2d = sdo_part_v92                    #CPL-1675
               LET sdo_apo_v92 = sdo_pa92_2d * vprecio_accion
         ...
         INSERT INTO safre_tmp:sdo_acr_af
         VALUES(g_reg.*)
...
#2---------------------------------
   ...
   ON EVERY ROW 
      PRINT
         COLUMN 1,reg_det_tra_ctas.tipo_registro        ,
                  cont_reg                              USING "&&&&&&&&&&",
...
#3---------------------------------
  ...
  ON EVERY ROW 
     PRINT
     COLUMN 1,reg_sum_tra_ctas.tipo_registro      ,
   ...
   FORMAT
      PAGE HEADER
         ...
         PRINT COLUMN  2,"NOMBRE ARCHIVO A ENVIAR : ",
                         g_seg_modulo.ruta_envio CLIPPED,"/",
                         "TRANS_ACRED.",hoy USING "MMDD","-",hora[1,2],hora[4,5]
         SKIP 2 LINE
