############################################################################
#Propietario       => E.F.P.                                               #
#Programa ACRC011  => RECIBE ARCHIVOS RECHAZOS PROCESAR CRED. GARANTIA     #
...
GLOBALS
    ...
    DEFINE reg_cza_cred_rech RECORD
    ...
    DEFINE reg_det_cred_rech RECORD
    ...
    DEFINE reg_sum_cred_rech RECORD
         tipo_registro        CHAR(02)      ,
...
#pp-------------------------
   ...
   DISPLAY " ACRC011  RECIBE ARCHIVO RECHAZOS PROCESAR CRED GARANTIA                       " AT 3,1 ATTRIBUTE(REVERSE)
...
#ct-------------------
    ...
    WHENEVER ERROR CONTINUE
        DROP TABLE cza_cred_rech
        DROP TABLE det_cred_rech
        DROP TABLE sum_cred_rech
    WHENEVER ERROR STOP

    CREATE TABLE cza_cred_rech
       (
        tipo_registro         CHAR(02) ,
        ident_servicio        CHAR(02) ,
        ident_operacion       CHAR(02) ,
...
#lap------------------------
    ...
    FOREACH cur_1 INTO carga_reg
        ...
        IF carga_reg[5,6] = "09" THEN
            ...
            LET reg_cza_cred_rech.tipo_registro      = carga_reg[001,002] 
            LET reg_cza_cred_rech.ident_servicio     = carga_reg[003,004]
            LET reg_cza_cred_rech.ident_operacion    = carga_reg[005,006]
            LET reg_cza_cred_rech.tipo_ent_origen    = carga_reg[007,008]
            LET reg_cza_cred_rech.cve_ent_origen     = carga_reg[009,011]
            LET reg_cza_cred_rech.tipo_ent_destino   = carga_reg[012,013]
            LET reg_cza_cred_rech.cve_ent_destino    = carga_reg[014,016]
            LET reg_cza_cred_rech.ent_fed_envio_lote = carga_reg[017,019]
            LET c_paso_cza                           = carga_reg[020,027]
            LET reg_cza_cred_rech.consec_lote_dia    = carga_reg[028,030]
            LET reg_cza_cred_rech.cve_mod_recepcion  = carga_reg[031,032]
            LET reg_cza_cred_rech.cod_result_operac  = carga_reg[033,034]
            LET reg_cza_cred_rech.rechazo            = carga_reg[035,043]

            LET c_fecha_cza = c_paso_cza[5,6],"/",
                              c_paso_cza[7,8],"/",
                              c_paso_cza[1,4]

            LET reg_cza_cred_rech.fecha_presentacion = c_fecha_cza

            INSERT INTO safre_tmp:cza_cred_rech VALUES(reg_cza_cred_rech.*)
        END IF

                #---DETALLE DE SOLICITUD DE TRANSFERENCIA---#
        IF carga_reg[1,2] = "02" AND c2_ident_operacion = "01" THEN
            LET reg_det_cred_rech.tipo_registro      = carga_reg[001,002]
            LET reg_det_cred_rech.cont_servicio      = carga_reg[003,012]
            LET reg_det_cred_rech.tipo_recep_cuenta  = carga_reg[013,014]
            LET reg_det_cred_rech.cve_recep_cuenta   = carga_reg[015,017]
            LET reg_det_cred_rech.tipo_ced_cuenta    = carga_reg[018,019]
            LET reg_det_cred_rech.cve_ced_cuenta     = carga_reg[020,022]
            LET reg_det_cred_rech.tipo_transferencia = carga_reg[023,024]
            LET c_paso_det                           = carga_reg[025,032]
            LET reg_det_cred_rech.n_unico_infonavit  = carga_reg[041,058]
            LET reg_det_cred_rech.nss_infonavit      = carga_reg[059,069]
            LET reg_det_cred_rech.rfc_infonavit      = carga_reg[085,097]
            LET reg_det_cred_rech.paterno_infonavit  = carga_reg[098,137]
            LET reg_det_cred_rech.materno_infonavit  = carga_reg[138,177]
            LET reg_det_cred_rech.nombres_infonavit  = carga_reg[178,217]
            LET reg_det_cred_rech.ident_lote_devol   = carga_reg[240,255]
            LET reg_det_cred_rech.nss_afore          = carga_reg[271,281]
            LET reg_det_cred_rech.rfc_afore          = carga_reg[282,294]
            LET reg_det_cred_rech.paterno_afore      = carga_reg[325,364]
            LET reg_det_cred_rech.materno_afore      = carga_reg[365,404]
            LET reg_det_cred_rech.nombres_afore      = carga_reg[405,444]
            LET reg_det_cred_rech.ult_apor_viv_97    = carga_reg[490,504]
            LET reg_det_cred_rech.cod_result_operac  = carga_reg[583,584]
            LET reg_det_cred_rech.diag_proceso       = carga_reg[585,599]
            LET reg_det_cred_rech.nombre_imss        = carga_reg[600,649]
            LET reg_det_cred_rech.num_cred_infonavit = carga_reg[650,659]

            LET c_fecha_det = c_paso_det[5,6],"/",
                              c_paso_det[7,8],"/",
                              c_paso_det[1,4]
            LET reg_det_cred_rech.fecha_presentacion  = c_fecha_det

            INSERT INTO safre_tmp:det_cred_rech VALUES(reg_det_cred_rech.*,0)

            UPDATE acr_det_tra_cred
            SET    estado = 20,
                   cod_result_operac = reg_det_cred_rech.cod_result_operac,
                   diag_proceso = reg_det_cred_rech.diag_proceso
            WHERE  nss_afore = reg_det_cred_rech.nss_afore
            AND    fecha_presentacion = reg_det_cred_rech.fecha_presentacion

            DELETE
            FROM   cta_act_marca
            WHERE  nss = reg_det_cred_rech.nss_afore
            AND    marca_cod = 237
        ...
        IF carga_reg[1,2] = "09" AND c2_ident_operacion = "01" THEN
            LET c2_ident_operacion = ""
            LET reg_sum_cred_rech.tipo_registro       = carga_reg[001,002]
            LET reg_sum_cred_rech.cant_reg_det        = carga_reg[003,011]
            LET reg_sum_cred_rech.sum_ult_apor_viv97  = carga_reg[057,071]

            INSERT INTO safre_tmp:sum_cred_rech VALUES(reg_sum_cred_rech.*)
        END IF
...
FUNCTION impresion_reporte()
  ...
  LET G_IMPRE = g_param_taa.ruta_listados CLIPPED,"/",g_usuario CLIPPED,
              ".RECH_CRED.",HOY USING "DD-MM-YYYY"

  START REPORT det_cred_rech_imp TO  G_IMPRE

  OUTPUT TO REPORT det_cred_rech_imp(g.*)

  FINISH REPORT det_cred_rech_imp

  LET gimpresion = "lp ",G_IMPRE
  RUN gimpresion
END FUNCTION

REPORT det_cred_rech_imp(g)
   DEFINE g     RECORD
     total         INTEGER,
     diag_proceso  CHAR(3)
   END RECORD
   ...
   FORMAT
     PAGE HEADER
       ...
       PRINT COLUMN 10,"PROCESO CREDITOS EN GARANTIA."
       PRINT COLUMN 05,"------------------------------------------"
