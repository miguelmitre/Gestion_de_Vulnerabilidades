...
#Propietario       => E.F.P.                                          #
#Programa ACRB006  => INCORPORAR REGISTROS DE DEVOLUCION ACRED        #
...
#Sistema           => ACR                                             #
#Modificacion      => DMR  CPL-1495  STARTLOG Personalizado           #
#                  => Se idento codigo y se coloco mensaje de ERROR   #
#                  => para folio incorrecto o vacio                   #
#######################################################################
DATABASE safre_af
...
GLOBALS
   ...
   DEFINE
      cza RECORD LIKE acr_cza_devuelto.*,
      det RECORD LIKE acr_det_devuelto.*,
      sum RECORD LIKE acr_sum_devuelto.*
END GLOBALS
...
MAIN
   ...
   CALL STARTLOG(FGL_GETENV("USER")||".ACRB006.log")

   DISPLAY "              INCORPORAR REGISTROS DE DEVOLUCIONES          " AT 12,10 ATTRIBUTE(REVERSE)

   WHILE TRUE 
      PROMPT " [T]ransferencia devolucion saldos acreditados =>  " FOR opc
...
#pp-------------------------
   ...
   IF TODOS > 0 THEN
      ...
      UPDATE acr_cza_devuelto
      SET    estado = 2
      WHERE  folio = vfolio
      AND    tipo_registro = "01"
      AND    estado = 1

      UPDATE acr_det_devuelto
      SET    estado = 2
      WHERE  folio = vfolio
      AND    tipo_registro = "02"
      AND    estado = 1

      UPDATE acr_sum_devuelto
      SET    estado = 2
      WHERE  folio= vfolio
      AND    tipo_registro = "09"
      AND    estado = 1

      UPDATE acr_devolucion
      SET    estado = 2
      WHERE  folio= vfolio
      AND    estado = 1
   ELSE
      ERROR " FOLIO SIN REGISTROS o INCORRECTO !!! "     #CPL-1495
      SLEEP 4
      ERROR ""
   END IF
END FUNCTION
...
#d----------------------------
   DEFINE
      x_historico  RECORD LIKE acr_det_devuelto.*,
      g_sie        RECORD LIKE cta_regimen.*,       #antes afi_mae_siefore
      ...
      CASE i
         ...
         WHEN 2
            ...
            FOREACH cursor_8 USING reg_prov.subcuenta INTO g_sie.*
               ...
               LET reg_prov.monto_en_acciones = x_historico.partic_v92
               LET valor_en_pesos             = reg_prov.monto_en_acciones
                                                * vprecio_accion
               LET vpesos_cta                 = valor_en_pesos
               LET reg_prov.monto_en_pesos    = vpesos_cta
