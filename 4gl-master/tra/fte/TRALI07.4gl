##############################################################################
#Owner             => E.F.P.
#Programa TRALI07  => RESUMEN DE LA RECEPCION DE TRASPASOS ICEFA-AFORE
...
GLOBALS
	...
	DEFINE  g_lista               CHAR(100) , 
        ...
        END RECORD,
                ...
                over_size       SMALLINT,
                reg_total       RECORD
                totalsar92      LIKE tra_det_trasp_sal_issste.saldo_sar_92,
                totalviv92      LIKE tra_det_trasp_sal_issste.saldo_viv_92,
                cuenta          INTEGER
        END RECORD                
...
DISPLAY FORM f_tral0071 
    ...
    DISPLAY " TRALI07     RESUMEN RECEPCION DE TRASPASO ICEFA-AFORE ISSSTE                  " AT 3,1 ATTRIBUTE(REVERSE)
...
#vi---------------
      ...
      PROMPT "DESEA IMPRIMIR REPORTE S/N? " ATTRIBUTE (REVERSE) FOR CHAR enter
		...
		START REPORT rpt_1 TO g_lista
                IF reg_fol.opc = 1 THEN
                        LET titulo = "APORTACIONES SAR Y VIVIENDA 92"  
		ELSE
                        LET titulo = "INTERESES  SAR  Y  VIVIENDA 92" 
		END IF 
	...
	OUTPUT TO REPORT rpt_1(reg_trasp[i].*,titulo,desc_banco[i].banco)
		...
		END FOR 
                FINISH REPORT rpt_1
                RUN comando
                DISPLAY "ARCHIVO GENERADO EN: ", g_lista CLIPPED AT 18,2 ATTRIBUTE(REVERSE)
                EXIT WHILE
            ELSE
...
DISPLAY "                             <Ctrl-c> Salir                                    " AT 1,1 ATTRIBUTE(REVERSE)
DISPLAY " TRALI07    GENERA REPORTE DE SALDO TRASPASO ICEFA-AFORE ISSSTE                " AT 3,1 ATTRIBUTE(REVERSE)
DISPLAY HOY USING "DD-MM-YYYY" AT 3,65 ATTRIBUTE(REVERSE) 
...
WHEN 2
	LET txt = "SELECT cve_ced_cuenta, sum(int_sar_92), sum(int_viv_92),",
        " count(*) from   tra_det_trasp_int_issste ",
...
WHEN 1
     ...
     " count(*) from   tra_det_trasp_sal_issste ",
     "where  folio = ",reg_fol.folio CLIPPED,
     "group by cve_ced_cuenta order by cve_ced_cuenta" CLIPPED     
