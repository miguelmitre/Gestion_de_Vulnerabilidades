##############################################################################
#Owner             => E.F.P.
#Programa TRAM100  => MANTENIMIENTO DE ICEFAS
#Fecha creacion    => 23 ENERO DE 1998   
#By                => FRANCO ESTEBAN ULLOA VIDELA
#Modificado  By    => JESUS DAVID YAÑEZ MORENO    
#Fecha de Mod      => 10 NOVIEMBRE DEL 2005
...
######
    ...
    DISPLAY " TRAM100   MANTENIMIENTO SOLICITUDES TRASPASO ICEFA-AFORE ISSSTE               " AT 3,1 ATTRIBUTE(REVERSE,BOLD) 
...
#a---------------
    ...
    OPEN WINDOW tram1001 AT 2,2 WITH FORM "TRAM10I1" ATTRIBUTE( BORDER)
	    ...
	    LET reg_2.descripcion     = NULL
            ...
            INPUT BY NAME reg_2.n_folio_tra      ,
			  ...
			  reg_2.desc_sector      ,
                          ...
                          reg_2.icefa_cod        ,
                          reg_2.des_icefa        ,
                          reg_2.nro_int_cta        WITHOUT DEFAULTS
		...
		WHERE a.cve_ced_cuenta_org = reg_2.icefa_cod
         ...
         AFTER FIELD nro_int_cta
                ...
                ON KEY(F4) 
                   ...
                   IF reg_2.cve_sector = 2 THEN

                      CALL TRAB012 (reg_1.n_seguro        ,
                                    reg_2.nss        ,
                                    reg_2.rfc        ,
                                    reg_2.icefa_cod  ,
                                    reg_2.nro_int_cta)
...
############
		...
		END CASE
			  ...
			  LET i_corr = i_corr + 1
                     ...
                     UPDATE tra_det_aut_issste   #TABLA VIEJA                 
                     SET    estado = 1,
                            fecha_edo       = TODAY
                     WHERE  n_seguro        = reg_1.n_seguro
                     AND    n_seguro_ent    = reg_2.nss
                     AND    rfc_ent         = reg_2.rfc
                     AND    cve_ced_cuenta  = reg_2.icefa_cod
                     AND    nro_ctrl_icefa  = reg_2.nro_int_cta
                     AND    estado          IN (161,164)

                     UPDATE tra_det_atm_issste   #TABLA NUEVA 
                     SET    estado = 1,
                            fecha_edo       = TODAY
                     WHERE  n_seguro        = reg_1.n_seguro
                     AND    n_seguro_ent    = reg_2.nss
...
#c-----------------
  ...
  ELSE 
    DISPLAY " [Ctrl-c]Salir [F1]Confirmar [F2]Aportes [F3]Devoluciones "
            AT 1,1 ATTRIBUTE(BOLD)
...
END FOR
    ...
    OPEN WINDOW tram1009 AT 11,16 WITH FORM "TRAM1009" ATTRIBUTE(BORDER)
    DISPLAY "          CONSULTA APORTES SAR ISSSTE Y VIV ISSSTE               " 
...
CLOSE WINDOW tram1009
  ...
  ON KEY (F3)
    ...
    OPEN WINDOW tram1000 AT 13,12 WITH FORM "TRAM1000" ATTRIBUTE(BORDER)
...
CLOSE WINDOW tram1000
 ...
 ON KEY(F4)
   ...
   IF arr_1_c[g].cve_sector = 2 THEN
    ...
    CALL TRAB012 (reg_1.n_seguro        ,
                  ...
                  arr_1_c[g].rfc        ,
                  arr_1_c[g].icefa_cod  ,
                  arr_1_c[g].nro_int_cta)
    RETURNING     band_conf

   END IF
ON KEY(F1)
     LET g = ARR_CURR()
     IF arr_1_c[g].estado = 20 THEN 
                 ...
                 WHILE TRUE
                   ...
                   IF enter MATCHES "[sSnN]" THEN
                    IF enter MATCHES "[sS]" THEN
                        ...
                        SET  tra_mae_icefa_issste.status          = 1,
			     tra_mae_icefa_issste.fecha_solic_tra  = TODAY,
			     tra_mae_icefa_issste.fecha_comp_icefa = TODAY
                        WHERE tra_mae_icefa_issste.correlativo =
                              arr_1_c[g].correlativo

                        UPDATE tra_det_aut_issste 
                        SET    estado = 1
                        WHERE  n_seguro        = reg_1.n_seguro
                        AND    n_seguro_ent    = arr_1_c[g].nss
                        AND    rfc_ent         = arr_1_c[g].rfc
                        AND    cve_ced_cuenta  = arr_1_c[g].icefa_cod
                        AND    nro_ctrl_icefa  = arr_1_c[g].nro_int_cta
                        AND    estado          = 20

# MODIFICACION  PARA ACTUALIZAR LA TABLA NUEVA

                        UPDATE safre_af:tra_det_atm_issste 
                        SET    estado = 1
                        WHERE  n_seguro        = reg_1.n_seguro
                        AND    n_seguro_ent    = arr_1_c[g].nss
                        AND    rfc_ent         = arr_1_c[g].rfc
                        ...
                        DISPLAY arr_1_c[g].fecha_comp_icefa 
			TO scr_1.fecha_comp_icefa
...
#m-----------------
    ...
    OPEN WINDOW tram1005 AT 2,2 WITH FORM "TRAM10I5" ATTRIBUTE( BORDER)
    DISPLAY " TRAM100             DATOS DEL REGISTRO DE AFILIACION                          " AT 3,1 ATTRIBUTE(REVERSE) 
 ...
 AFTER FIELD nro_int_cta
        ...
        ON KEY (ESC)
	       ...
	       LET reg_2_m.nombres2 = " "
            ...
            IF reg_2_m.origen_traspaso IS NULL THEN
                ERROR " ORIGEN TRASPASO NO PUEDE SER NULO "
                NEXT FIELD origen_traspaso
            END IF
...
#di------------------------
 ...
 OPEN WINDOW vent_1 AT 05,12 WITH FORM "TRAM1010" ATTRIBUTE(BORDER)
...
#s------------------
    ...
    OPEN WINDOW tram1003 AT 2,2 WITH FORM "TRAM10I3" ATTRIBUTE( BORDER)
    DISPLAY " TRAM100             DATOS DEL REGISTRO DE AFILIACION                          " AT 3,1 ATTRIBUTE(REVERSE) 
    DISPLAY HOY USING "dd-mm-yyyy" AT 3,68 ATTRIBUTE(REVERSE)
    DISPLAY "" AT 1,1
    DISPLAY "" AT 2,1
    ...
    SELECT A.n_seguro         ,
           ...
           A.fecha_genera     ,
           A.lote_genera
    FROM   tra_mae_icefa_issste A, OUTER tab_icefa B
    WHERE  A.n_seguro       = reg_1.n_seguro
    AND    A.tipo_solicitud = reg_1.tipo_solicitud
    AND    A.icefa_cod      = B.icefa_cod
    ...
    FOREACH cur_33 INTO c11_n_seguro   ,
        ...
        SELECT descripcion 
        INTO   arr_m[i].descripcion
        FROM   tra_tab_tiptra a
        WHERE  origen_traspaso = arr_m[i].origen_traspaso
...
#a--------------
    ...
    OPEN WINDOW tram1004 AT 05,12 WITH FORM "TRAM1004" ATTRIBUTE(BORDER)
...
DEFINE cadena CHAR(100)
  ...
  OPEN WINDOW tram10020 AT 12,20 WITH FORM "TRAM10020" ATTRIBUTE(BORDER)
