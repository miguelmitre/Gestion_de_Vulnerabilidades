...
EXECUTE qr    
    ...
    DISPLAY " TRAC005         LIQUIDACION DE TRASPASO ICEFA-AFORE IMSS                      " AT 3,1 ATTRIBUTE(REVERSE)
 ...
 FOREACH cur_2 INTO x_reg.*
	...
	INTO g_siefore_rcv
     ...
     LET valor_accion = 0
      ...
      WHERE  a.fecha_valuacion = fecha2
        AND  a.codigo_siefore  = x_reg.siefore
  ...
  LET u3 = uu[7,8]
     ...
     IF STATUS = NOTFOUND  THEN
        CALL f_desmarca_cuenta(x_reg.nss,250,vv_corr)
     END IF
...
LET u3 = uu[7,8]
     ...
     FROM v_viv A
     WHERE A.n_seguro     = x_reg.nss
     AND   A.cont_servcio = x_reg.consecutivo_lote
...
#s------------------
 ...
 ON EVERY ROW
            LET l_tipo_proc_tes   = 6
            LET l_medio           = 10
            LET x_reg.etiqueta    = 0
            LET cont              = cont + 1
            LET d16_actualiz_udis = 0

IF x_reg.subcuenta = 7 THEN
	    ...
	    WHERE  nss             = x_reg.nss
	    AND    fecha_solicitud = x_reg.fecha_conversion
	    AND    tipo_traspaso   = 6      
            AND    grupo_regimen   = 7
	    AND    folio_origen    = x_reg.folio
...

{
    ...
    END IF
     ...
     UPDATE tra_mae_icefa 
     SET    tra_mae_icefa.usuario   = usuario,
	    tra_mae_icefa.status    = 8
     WHERE  tra_mae_icefa.n_seguro  = reg_llave.n_seguro
     and    tra_mae_icefa.nss       = reg_llave.n_seguro_ent
     and    tra_mae_icefa.rfc       = reg_llave.rfc_ent
     and    tra_mae_icefa.icefa_cod = reg_llave.cve_ced_cuenta
...
INTO sal_sar ,
     sal_viv 
FROM  safre_af:tra_det_trasp_sal a 
WHERE  a.folio = ffol
