##############################################################################
#Owner             => E.F.P.
#Programa TRABI02  => PROVISIONA TRASPASOS ICEFA - AFORE    
...
DEFINE sub_inv SMALLINT
DEFINE indicador_inv SMALLINT
...
MAIN
    ...
    CALL STARTLOG("TRABI02.log")

    CALL init() #i
    OPEN WINDOW trab0021 AT 4,4 WITH FORM "TRAB0021" ATTRIBUTE(BORDER)
    DISPLAY "                            < CTRL-C> Salir                                " AT 1,1 ATTRIBUTE(REVERSE)

    DISPLAY" TRABI02     PROVISIONA TRASPASOS ICEFA-AFORE  ISSSTE                          " AT 3,1 ATTRIBUTE(REVERSE)   
...
#ff
     ...
     CALL dispersa_sal(c10_coduni_n1        ,
       ...
       SET    tra_mae_icefa_issste.saldo_sar_92 = tra_mae_icefa_issste.saldo_sar_92 + 
               ...
               det_sal.saldo_viv_92
       WHERE  tra_mae_icefa_issste.n_seguro     = det_sal.n_seguro
       AND    tra_mae_icefa_issste.nss          = det_sal.n_seguro_ent
       AND    tra_mae_icefa_issste.rfc          = det_sal.rfc_ent
       AND    tra_mae_icefa_issste.icefa_cod    = det_sal.cve_ced_cuenta
       AND    tra_mae_icefa_issste.nro_int_cta  = det_sal.nro_ctrl_icefa
...
END CASE
   ...
   SET    tra_mae_icefa_issste.saldo_sar_92 = tra_mae_icefa_issste.saldo_sar_92 + det_int.int_sar_92,
       ...
       WHERE  tra_mae_icefa_issste.n_seguro     = det_int.n_seguro
   AND    tra_mae_icefa_issste.nss          = det_int.n_seguro_ent_ced
   AND    tra_mae_icefa_issste.rfc          = det_int.rfc_ent_ced
   AND    tra_mae_icefa_issste.icefa_cod    = det_int.cve_ced_cuenta
   AND    tra_mae_icefa_issste.nro_int_cta  = det_int.nro_ctrl_icefa
       AND    tra_mae_icefa_issste.status       in (7,8,41,17)
...
#ds---------------------------------------------------------------------
 ...
 valor_en_pesos         DECIMAL(16,6)
    ...
    DEFINE val_pes_viv  DECIMAL(16,2) #agrego 08102008
...
#-----TABLA NUEVA----
        ...
        SET    tra_det_atm_issste.estado       = reg_3.estado_nuevo ,
               tra_det_atm_issste.fecha_edo    = TODAY     
        WHERE  tra_det_atm_issste.n_seguro     = reg_3.n_seguro         
        AND  tra_det_atm_issste.n_seguro_ent   = reg_3.nss
        AND  tra_det_atm_issste.rfc_ent        = reg_3.rfc
        AND  tra_det_atm_issste.cve_ced_cuenta = reg_3.icefa_cod
        AND  tra_det_atm_issste.nro_ctrl_icefa = reg_3.nro_int_cta
        AND  tra_det_atm_issste.estado         = reg_3.estado_actual # ENVIADA     
#---------------------
                    RETURN
                ELSE
