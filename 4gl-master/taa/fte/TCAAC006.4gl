...
# CPL-1442         => JLNJ                                               #
# CPL-2971         => DMR 19/02/2019 Se corrigio codigo fuente,identacion#
# CPL-3154         => 17/03/2020 Se revisa procedimiento para verificar  #
#                     que la existencia de la cuenta                     #
##########################################################################
DATABASE safre_af
...
#   LET     nom_archivo                 =  "arch_ced"
   ...
   LOAD FROM ejecuta
   INSERT INTO taa_cd_archivo_cedente
...
#tc-----------------
   ...
   LOAD FROM ejecuta
   INSERT INTO taa_cd_cza_cedente
...
# armo archivo de comando cabecera
   ...
   LET ejecuta = "cd ",p_param_modulo.ruta_rescate CLIPPED,               
                 "/;dbload -d safre_tmp -c ",p_tablayout.nom_arch CLIPPED,t_cza USING "&",
...
#atcf------------------------
      ...
      IF l_dia_semana > 0  AND
         ...
         IF STATUS = NOTFOUND THEN
            IF g_tipo_traspaso = 1 OR
               ...
               IF l_dia_var = 7 THEN
                  LET reg_ctr_folio.fecha_envio_saldos = l_fecha_var 
               END IF  
   ...
   LET l_mes_envio = reg_ctr_folio.fecha_envio_saldos USING "MM"

   LET l_mes_liqui = reg_ctr_folio.fecha_liquidacion USING "MM"
   ...
   ELSE
      DISPLAY "FOLIO DE OPERACION         : ",reg_ctr_folio.folio USING  "#######"
...
# armo archivo de comando de detalle
   ...
   LET ejecuta = "cd ",p_param_modulo.ruta_rescate CLIPPED,               
                 "/;dbload -d safre_tmp -c ",p_tablayout.nom_arch CLIPPED,t_det USING "&",
...
# armo archivo de sumario
   ...
   LET ejecuta = "cd ",p_param_modulo.ruta_rescate CLIPPED,               
                 "/;dbload -d safre_tmp -c ",p_tablayout.nom_arch CLIPPED,t_sum USING "&",
...
#s--------------------------------------------
   ...
   LET nom_archivo_cmd = p_param_modulo.ruta_rescate CLIPPED, "/",
                         p_tablayout.nom_arch CLIPPED,v_tipo_reg USING "&"
...
#vl----------------------
   ...
   START REPORT f_250_imprime_detalle TO g_lista
      ...
      LET ejecuta       = "EXECUTE PROCEDURE sp_cierra_marca(","?,?)"
      LET ejecuta       = ejecuta  CLIPPED
      ...
      FOREACH cur_prueba INTO reg_soli.*
         ...
         ELSE
            IF (reg_soli.n_seguro IS NULL OR reg_soli.n_seguro = "           ") THEN
               SELECT COUNT(*)
               INTO  l_existe
               FROM  afi_mae_afiliado
               WHERE n_unico = reg_soli.n_unico;
         ...
         IF reg_soli.estado = 101 THEN
            ...
            IF l_ind_cierre_marca = 1 THEN
               EXECUTE desmara_unificacion USING reg_soli.n_seguro, reg_soli.tipo_traspaso
            END IF

            IF reg_soli.ident_lote_solici [3,5] = reg_soli.cve_ced_cuenta THEN
               LET reg_soli.estado = 107
            END IF

            IF reg_soli.estado = 101 THEN
               DECLARE cur_marca_cuenta CURSOR FOR spl_marca_cuenta
               OPEN    cur_marca_cuenta USING reg_soli.n_seguro,
                                              marca_entra,     
...
#ir-----------------------
   ...
   DEFINE
      g_afore     RECORD LIKE safre_af:tab_afore_local.*
