...
GLOBALS
   ...
   DEFINE
            ...
            g_acciones                           DEC(16,6),
            g_pesos                              DEC(16,6),
            g_pesos_traspaso                     DEC(16,6),
            g_pesos_rendi_dia                    DEC(20,10),
   ...
   DEFINE   g_valor_acc       ARRAY[40]     OF    RECORD
     	    nom_siefore                          CHAR(008),
            precio_accion                        DEC(11,6)
   END   RECORD
   DEFINE   g_c_flujo      ARRAY[40,35]     OF    RECORD
            acciones                             DEC(16,6),
            pesos                                DEC(16,6)
   END   RECORD
   DEFINE   g_c_saldo      ARRAY[40,35]     OF    RECORD
            acciones                             DEC(16,6),
            pesos                                DEC(16,6)
   END   RECORD
   DEFINE   g_resarcir      ARRAY[40,35]     OF    RECORD
            acciones                             DEC(16,6),
            pesos                                DEC(16,6)
   END   RECORD
   DEFINE   g_t_nss        ARRAY[40,35]     OF    RECORD
            acciones                             DEC(16,6),
            pesos                                DEC(16,6)
   END   RECORD
   DEFINE   g_t_rep        ARRAY[40,35]     OF    RECORD
            acciones                             DEC(16,6),
            pesos                                DEC(16,6)
   END   RECORD
   ...
   DEFINE   g_reg_factor_comi      RECORD  LIKE  safre_af:taa_cd_factor_comi.*
END GLOBALS
...
FUNCTION    F_022_precio_acc_parti()
   ...
   DISPLAY  g_valor_acc[5].precio_accion       TO  g_precio_accion_b5
...
FUNCTION    F_100_proceso()
   ...
   FOREACH  cur_nss_ced       INTO  g_nss,g_tipo_rendimiento
            ...
            SELECT   MAX(A.fecha_conversion)
              ...
              FROM   safre_af:taa_cd_his_indebidos  A
             WHERE   A.nss                        =  g_nss
               AND   A.tipo_movimiento           IN(777,778,779);
...
FUNCTION    F_150_resarcimiento()
   ...
   DEFINE   l_tipo_comi                  SMALLINT

   LET  l_txt =  
   " SELECT a.* ",
   " FROM   taa_cd_factor_comi a ",
   " WHERE  a.ind_tipo_comi = ?  ",
   " ORDER BY a.fecha_desde      "

   PREPARE qry_resarci FROM l_txt

   DECLARE cur_1_resarci CURSOR FOR qry_resarci
   ...
   FOREACH  cur_scta         INTO  g_scta 
         ...
         WHILE   g_fecha_recep_incre      <=  g_fecha_trasp_cta
               ...
               FOREACH cur_1_resarci USING l_tipo_comi INTO g_reg_factor_comi.*
                   IF ( g_fecha_recep_incre >= g_reg_factor_comi.fecha_desde AND
                        g_fecha_recep_incre <= g_reg_factor_comi.fecha_hasta    ) THEN
                           LET g_factor_comision   = 1 + (g_reg_factor_comi.factor_comision / 365)
                           LET g_factor_porcentaje = g_reg_factor_comi.factor_comision
                   ELSE 
                       IF (g_reg_factor_comi.fecha_hasta IS NULL) THEN 
                           LET g_factor_comision   = 1 + (g_reg_factor_comi.factor_comision / 365)
                           LET g_factor_porcentaje = g_reg_factor_comi.factor_comision
                       ELSE 
                           CONTINUE FOREACH
...
FUNCTION    F_155_max_siefore()
   ...
   CASE    g_tipo_rendimiento
           WHEN     1
                    LET      l_txt_cur      =  l_txt_cur  CLIPPED,
                             ...
                             "  OR a.afore_cod     =  ",g_afo_recupera_cta,")"
                    EXIT CASE
           OTHERWISE
                    EXIT CASE
   END CASE
   ...
   FOREACH lcur_mr1      USING  l_fecha_recep_incre 
                          INTO  reg_inicio.*
         IF       reg_inicio.precio_accion IS NULL THEN
                  CONTINUE  FOREACH
         END IF
...
{
IF l_fecha_recep_incre >= '10/01/2007' THEN

   display reg_inicio.afore_cod
   display reg_inicio.siefore_cod
   display reg_inicio.precio_accion
END IF
}
         SELECT   a.* 
           INTO   reg_fin.*
