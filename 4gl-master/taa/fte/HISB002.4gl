...
DATABASE safre_tmp 
DROP TABLE det_mov_trab_uni

create table det_mov_trab_uni 
  (
...
create index idx_dmtu on det_mov_trab_uni (cont_serv) 
create index idx_dmtu2 on det_mov_trab_uni (subcuenta)
create index idx_dmtu3 on det_mov_trab_uni 
(cont_serv,subcuenta,iden_proc)
...
DATABASE safre_af 
   ...
   IF reg_cza_hist_trab.nro_nss_asoc > 0 THEN 
      LET lanza_uni = 'fglgo HISB003 ',reg_cza_hist_trab.nss," ",
   ...
   WHERE a.codigo_siefore = reg_beta.siefore
     ...
     IF ((reg_beta.fecha_conversion >= v_fecha_traspaso) AND 
	 (reg_beta.tipo_movimiento = 220 OR 
	  reg_beta.tipo_movimiento = 240 OR 
	  reg_beta.tipo_movimiento = 247 OR 
	  reg_beta.tipo_movimiento = 290 )) THEN 
        CONTINUE  FOREACH
     END IF
...
LET lanza = "nohup fglgo HISL001 ",vv_fecha_ini CLIPPED," ",
...
LET v_comis = 0
              ...
              AND    A.tipo_movimiento  in (100,101,102,103,104,105,
	  ...
	  LET reg_det_mov_trab.iden_mov = " "
          ...
          LET reg_det_mov_trab.iden_proc = "03"
                      ...
                      WHEN "MQ" 
                        ...
                        LET reg_det_mov_trab.iden_mov[13,15]= "568"
		         ...
		         EXIT CASE
                      ...
                      WHEN "TR-"
                          ...
                          LET reg_det_mov_trab.iden_mov[13,15] = 
                              reg_beta.id_aportante[4,6]
                          LET reg_det_mov_trab.iden_mov[16,17] = "01"
                          LET reg_det_mov_trab.iden_mov[18,20] = "568"
                          EXIT CASE
                      WHEN "TC-" 
                          LET reg_det_mov_trab.iden_mov[9,10]  = "08"
                          LET reg_det_mov_trab.iden_mov[11,12] = "01"
...
#tm3rcv-----------------
		   ...
		   reg_beta.fecha_pago USING"YYYYMMDD"
               ...
               LET reg_det_mov_trab.iden_mov[45,47] = "568"
...
DEFINE vcve char(003)
	    ...
	    END IF
	       ...
	       END IF 
               ...
               LET reg_det_mov_trab.iden_mov[11,13]   = '568'
               LET reg_det_mov_trab.iden_mov[14,16]   = '568'
	       IF (vma = 241 OR vma = 242) THEN
                  LET reg_det_mov_trab.iden_mov[17,19]   = '568'
	       END IF
	       IF (vma = 243 OR vma = 244) THEN
...
#tmunif---------- 
		 ...
		 EXIT CASE
               WHEN "AL"
		  CASE reg_beta.id_aportante[4,6] 
		  WHEN "001" 
                   LET reg_det_mov_trab.iden_mov[9,10]   = '16' 
		   EXIT CASE
		  WHEN "051" 
                   LET reg_det_mov_trab.iden_mov[9,10]   = '16' 
		   EXIT CASE
		  ...
		  WHEN "152" 
		   ...
		   EXIT CASE
		  WHEN "201" 
                   LET reg_det_mov_trab.iden_mov[9,10]   = '03' 
		   EXIT CASE
		  WHEN "202" 
                   LET reg_det_mov_trab.iden_mov[9,10]   = '03' 
		   EXIT CASE
		  ...
		  WHEN "402" 
		   ...
		   EXIT CASE
		  WHEN "403" 
                   LET reg_det_mov_trab.iden_mov[9,10]   = '13' 
		   EXIT CASE
		  OTHERWISE 
		   EXIT CASE
	         END CASE	    
