...
GLOBALS 
define cons_unif smallint
define reg_uni_unificado RECORD LIKE safre_af:uni_unificado.*
define vnss_uni CHAR(011)
...
DEFINE reg_beta RECORD LIKE safre_af:dis_cuenta.*
...
CREATE TEMP TABLE e_fecha_uni(fecha date)

DATABASE safre_tmp
DROP TABLE taa_his_comis_uni
DATABASE safre_af

CREATE TEMP TABLE taa_his_comis_uni
  (
    ...
    LET vnss_uni                    = ARG_VAL(1) 
    LET reg_cza_hist_trab.cont_serv = ARG_VAL(2) 
    LET cons_unif = 0

    DECLARE uni1 CURSOR FOR 

	SELECT unique a.* 
	FROM uni_unificado a 
	WHERE  a.nss_uni =  vnss_uni
	AND    a.estado  = 100

    FOREACH uni1 INTO reg_uni_unificado.*
    LET reg_det_mov_trab_uni.consec_asoc = cons_unif + 1
    LET reg_det_mov_trab_uni.nss_unif = reg_uni_unificado.nss_cta1
    LET reg_det_mov_trab_uni.nss_afore = vnss_uni
    LET reg_det_mov_trab_uni.curp_unif = reg_uni_unificado.curp_cta1

    LET vv_siefore = "METLIFE"

    LET reg_det_mov_trab_uni.consecutivo = 0

          SELECT  A.fentcons
          INTO    v_fecha_apertura
          FROM    safre_af:afi_mae_afiliado    A         
          WHERE   A.n_seguro = reg_uni_unificado.nss_cta1
							 
          SELECT "ok"
          FROM safre_af:dis_cuenta A
          WHERE  A.nss =  reg_uni_unificado.nss_cta1
...
--     IF reg_beta.fecha_conversion >= reg_uni_unificado.fliquida THEN 
--        CONTINUE  FOREACH
--     END IF
...
#idmDIS-------------------------
    ...
    DEFINE reg_cur1_tipo_mov1dis RECORD
     ...
     DEFINE v_fecha_pago          CHAR(008),
          ...
          LET reg_det_mov_trab_uni.iden_mov = " "
          ...
          AND    A.n_seguro          = reg_uni_unificado.nss_cta1
	  ...
	  --AND    A.fech_valor_rcv    = v_fecha_valor
          ...
          END FOREACH
               ...
               LET reg_det_mov_trab_uni.cont_serv     = 
                   reg_cza_hist_trab.cont_serv

               LET reg_det_mov_trab_uni.curp = 
		   reg_uni_unificado.curp_cta1

               LET reg_det_mov_trab_uni.nss_afore = 
		   vnss_uni

               LET reg_det_mov_trab_uni.iden_proc = "01"

               LET reg_det_mov_trab_uni.consecutivo   =
                   reg_det_mov_trab_uni.consecutivo + 1
                    
               LET reg_det_mov_trab_uni.iden_mov[1,8]   = 
                   reg_cur2_tipo_mov1dis.fech_valor_rcv
 
               LET reg_det_mov_trab_uni.iden_mov[9,16]  = 
                   reg_beta.fecha_conversion USING"YYYYMMDD"

               LET reg_det_mov_trab_uni.iden_mov[17,24]   = 
                   reg_cur2_tipo_mov1dis.fech_valor_viv

               LET reg_det_mov_trab_uni.iden_mov[25,32]  = 
                   reg_cur2_tipo_mov1dis.fech_pago

               LET reg_det_mov_trab_uni.iden_mov[33,38] =
                   reg_cur2_tipo_mov1dis.periodo_pago
 
               LET reg_det_mov_trab_uni.iden_mov[39,44] = 
                   reg_cur2_tipo_mov1dis.folio_pago_sua USING"&&&&&&"

               LET reg_det_mov_trab_uni.iden_mov[45,47] = 
                   reg_cur2_tipo_mov1dis.cve_ent_receptora USING"&&&"

               LET reg_det_mov_trab_uni.iden_mov[48,58] = 
                   reg_cur2_tipo_mov1dis.reg_patronal_imss

               LET reg_det_mov_trab_uni.iden_mov[59,71] = 
                   reg_cur2_tipo_mov1dis.rfc_patron

               LET reg_det_mov_trab_uni.iden_mov[72,73] = 
                   reg_cur2_tipo_mov1dis.dias_cotz_bimestre USING"&&"

               LET reg_det_mov_trab_uni.iden_mov[74,75] = 
                   reg_cur2_tipo_mov1dis.dias_incap_bimest USING"&&"

               LET reg_det_mov_trab_uni.iden_mov[76,77] = 
                   reg_cur2_tipo_mov1dis.dias_ausent_bimest USING"&&"

               LET reg_det_mov_trab_uni.iden_mov[78,85] =
                   reg_cur2_tipo_mov1dis.fecha_estado USING "YYYYMMDD"

               LET reg_det_mov_trab_uni.siefore =  "METLIFE"

               LET reg_det_mov_trab_uni.fecha_valuacion = 
                   reg_beta.fecha_conversion USING "YYYYMMDD"

               LET reg_det_mov_trab_uni.acciones = 
                   reg_beta.monto_en_acciones
#prompt reg_beta.precio_accion for char enter
               LET reg_det_mov_trab_uni.precio_accion = 
                   reg_beta.precio_accion
#prompt reg_det_mov_trab_uni.precio_accion for char enter
              ...
              IF v_cuantos = 5 THEN
               LET reg_det_mov_trab_uni.iden_mov[86,87] = "02"
              END IF
              IF v_cuantos = 6 THEN
               LET reg_det_mov_trab_uni.iden_mov[86,87] = "02"
              END IF
              IF v_cuantos = 10 THEN 
               LET reg_det_mov_trab_uni.iden_mov[86,87] = "01"
              END IF 
              IF v_cuantos = 12 THEN 
               LET reg_det_mov_trab_uni.iden_mov[86,87] = "01"
              END IF 

              LET reg_det_mov_trab_uni.iden_mov[88,94] = 
...
DELETE FROM taa_his_comis_uni
CASE v_conver 
WHEN 1997
              INSERT INTO taa_his_comis_uni
              SELECT A.*
              FROM   safre_af:dis_cuenta97 A
...
LET v_comis = 0
              ...
              INTO   v_comis
              FROM   taa_his_comis_uni A
              WHERE  A.nss              = reg_beta.nss
              AND    A.subcuenta        = reg_beta.subcuenta
              AND    A.tipo_movimiento  in (100,101,102,103,104,105,106)
              ...
              LET reg_det_mov_trab_uni.iden_mov[95,108] = 
              v_comis  USING"&&&&&&&&&&&&&&"

              IF reg_beta.monto_en_pesos < 0 THEN
                 LET reg_det_mov_trab_uni.tipo_mov = "C" 
              ELSE 
                 LET reg_det_mov_trab_uni.tipo_mov = "A"
              END IF
              ...
              CASE reg_beta.subcuenta
               ...
               WHEN 1
                  CASE reg_beta.tipo_movimiento 
                   WHEN 1
                  LET reg_det_mov_trab_uni.subcuenta = "01" EXIT CASE
                   WHEN 2
                  LET reg_det_mov_trab_uni.subcuenta = "25" EXIT CASE
                   WHEN 3
                  LET reg_det_mov_trab_uni.subcuenta = "18" EXIT CASE
                   OTHERWISE
                  LET reg_det_mov_trab_uni.subcuenta = "01" EXIT CASE
                  END CASE
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
                  INSERT INTO safre_tmp:det_mov_trab_uni values (reg_det_mov_trab_uni.*)
                  EXIT CASE
               ...
               WHEN 2 
                  ...
                  CASE reg_beta.tipo_movimiento 
                   WHEN 1
                  LET reg_det_mov_trab_uni.subcuenta = "02" EXIT CASE
                   WHEN 2
                  LET reg_det_mov_trab_uni.subcuenta = "27" EXIT CASE
                   WHEN 3
                  LET reg_det_mov_trab_uni.subcuenta = "20" EXIT CASE
                   OTHERWISE
                  LET reg_det_mov_trab_uni.subcuenta = "27" EXIT CASE
                  END CASE
                  LET reg_det_mov_trab_uni.importe =
               ...
               WHEN 3 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "13"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos 
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
               ...
               WHEN 4 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "04"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
               ...
               WHEN 5 
                 CASE reg_beta.tipo_movimiento 
                 WHEN 3
                  LET reg_det_mov_trab_uni.subcuenta = "19"
                  EXIT CASE
                 OTHERWISE
                  LET reg_det_mov_trab_uni.subcuenta = "03"
                  EXIT CASE
                 END CASE
               ...
               WHEN 6 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "06"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos 
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
               ...
               WHEN 9 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "07"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos  
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
                ...
                END CASE
{
        ELSE 
              ...
              CASE reg_beta.subcuenta
               ...
               WHEN 1
                  LET reg_det_mov_trab_uni.subcuenta = "25"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
               ...
               WHEN 2 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "27"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
               ...
               WHEN 5 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "29"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
               ...
               WHEN 6 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "31"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos 
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
               ...
               WHEN 9 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "33"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos  
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
...
#tm1MQ------------------
	  ...
	  LET reg_det_mov_trab_uni.iden_mov = " "
          ...
          LET reg_det_mov_trab_uni.iden_proc = "03"
                      ...
                      WHEN "MQ" 
                        LET reg_det_mov_trab_uni.iden_mov[9,10] = "12"
                        LET reg_det_mov_trab_uni.iden_mov[11,12]= "01"
                        LET reg_det_mov_trab_uni.iden_mov[13,15]= "590"
                        LET reg_det_mov_trab_uni.iden_mov[16,17]= "08"
                        EXIT CASE
                      WHEN "MC"
                        LET reg_det_mov_trab_uni.iden_mov[9,10] = "12"
                        LET reg_det_mov_trab_uni.iden_mov[11,12]= "01"
                        ...
                        EXIT CASE
                      WHEN "MA" 
                        LET reg_det_mov_trab_uni.iden_mov[9,10] = "15"
                        LET reg_det_mov_trab_uni.iden_mov[11,12]= "01"
                        LET reg_det_mov_trab_uni.iden_mov[13,15]= "590"
                        LET reg_det_mov_trab_uni.iden_mov[16,17]= "08"
                        EXIT CASE
                      WHEN "MS" 
                        ...
                        EXIT CASE
                      WHEN "UN"
                        LET reg_det_mov_trab_uni.iden_mov[9,10] = "14"
                        LET reg_det_mov_trab_uni.iden_mov[11,12]= "01"
                        LET reg_det_mov_trab_uni.iden_mov[13,15]= "590"
                        LET reg_det_mov_trab_uni.iden_mov[16,17]= "08"
                        EXIT CASE
                      ...
                      LET reg_det_mov_trab_uni.iden_mov[18,20] = 
                      ...
                      IF (reg_beta.id_aportante[1,2] = "MQ" OR 
                         ...
                         LET reg_det_mov_trab_uni.iden_mov[21,22] = "01"
                      ELSE
                         LET reg_det_mov_trab_uni.iden_mov[21,22] = "02"
                      END IF
                      ...
                      END IF
{
			 ...
			 WHERE nss_afo_recep = reg_beta.nss
                       ...
                       LET reg_det_mov_trab_uni.iden_mov[23,27] = aa1 USING"&&&&&"
                       LET reg_det_mov_trab_uni.iden_mov[28,32] = aa2 USING"&&&&&"

	              END IF
}
                      LET reg_det_mov_trab_uni.siefore =  vv_siefore
                      ...
                      LET reg_det_mov_trab_uni.fecha_proceso = 
                          TODAY 
              ...
              CASE reg_beta.subcuenta
               ...
               WHEN 1
                  IF reg_beta.tipo_movimiento = 4 THEN
                     LET reg_det_mov_trab_uni.subcuenta = "18"
                  ELSE 
                     LET reg_det_mov_trab_uni.subcuenta = "01"
                  END IF
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos
                  LET reg_det_mov_trab_uni.fecha_conversion = 
               ...
               WHEN 2 
                  IF reg_beta.tipo_movimiento = 4 THEN
                     LET reg_det_mov_trab_uni.subcuenta = "20"
                  ELSE 
                     LET reg_det_mov_trab_uni.subcuenta = "02"
                  END IF
               ...
               WHEN 3 
                 IF reg_beta.tipo_movimiento = 4 THEN
                  LET reg_det_mov_trab_uni.subcuenta = "23"
                 ELSE 
                  LET reg_det_mov_trab_uni.subcuenta = "13"
                 END IF
                  LET reg_det_mov_trab_uni.importe =
               ...
               WHEN 4 
                  IF reg_beta.tipo_movimiento = 4 THEN
                      LET reg_det_mov_trab_uni.subcuenta = "12"
                  ELSE 
                      LET reg_det_mov_trab_uni.subcuenta = "04"
                  END IF
                  LET reg_det_mov_trab_uni.importe =
               ...
               WHEN 7 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "08"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
               ...
               WHEN 8 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "09"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
		   ...
		   vnss_uni
               ...
               LET reg_det_mov_trab_uni.iden_mov[1,8]   = 
                   reg_beta.fecha_conversion USING"YYYYMMDD"
                      ...
                      CASE reg_beta.id_aportante[1,3]
                      WHEN "AF-"
                        IF reg_beta.tipo_movimiento = 520 THEN
                          LET reg_det_mov_trab_uni.iden_mov[9,10]  = "07"
                          LET reg_det_mov_trab_uni.iden_mov[11,12] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[13,15] = "590"
                          LET reg_det_mov_trab_uni.iden_mov[16,17] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[18,20] = 
                              reg_beta.id_aportante[4,6]
                        ELSE
                          LET reg_det_mov_trab_uni.iden_mov[9,10]  = "06"
                          LET reg_det_mov_trab_uni.iden_mov[11,12] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[13,15] = "590"
                          LET reg_det_mov_trab_uni.iden_mov[16,17] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[18,20] = 
                      ...
                      WHEN "AC-" 
                          ...
                          EXIT CASE
                      WHEN "UR-" 
                          LET reg_det_mov_trab_uni.iden_mov[9,10]  = "10"
                          LET reg_det_mov_trab_uni.iden_mov[11,12] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[13,15] = "590"
                          LET reg_det_mov_trab_uni.iden_mov[16,17] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[18,20] = 
                      ...
                      WHEN "US-" 
                          ...
                          EXIT CASE
                      WHEN "AA-" 
                          LET reg_det_mov_trab_uni.iden_mov[9,10]  = "11"
                          LET reg_det_mov_trab_uni.iden_mov[11,12] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[13,15] = "590"
                          LET reg_det_mov_trab_uni.iden_mov[16,17] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[18,20] = 
                      ...
                      WHEN "AS-" 
                          ...
                          EXIT CASE
                      WHEN "TR-"
                          LET reg_det_mov_trab_uni.iden_mov[9,10]  = "08"
                          LET reg_det_mov_trab_uni.iden_mov[11,12] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[13,15] = 
                              reg_beta.id_aportante[4,6]
                          LET reg_det_mov_trab_uni.iden_mov[16,17] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[18,20] = "590"
                          EXIT CASE
                      WHEN "TC-" 
                          LET reg_det_mov_trab_uni.iden_mov[9,10]  = "08"
                          LET reg_det_mov_trab_uni.iden_mov[11,12] = "01"
                          ...
                          EXIT CASE
                      WHEN "ACR" 
                          LET reg_det_mov_trab_uni.iden_mov[9,10]  = "05"
                          LET reg_det_mov_trab_uni.iden_mov[11,12] = "04"
                          LET reg_det_mov_trab_uni.iden_mov[13,15] = "002"
                          LET reg_det_mov_trab_uni.iden_mov[16,17] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[18,20]  = "590"
                          EXIT CASE
                      WHEN "UT-"
                      ...
                      WHEN "UC-"
                          ...
                          EXIT CASE
                      WHEN "DI-"
                          LET reg_det_mov_trab_uni.iden_mov[9,10]  = "02"
                          LET reg_det_mov_trab_uni.iden_mov[11,12] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[13,15] = "590"
                          LET reg_det_mov_trab_uni.iden_mov[16,17] = "07"
                          LET reg_det_mov_trab_uni.iden_mov[18,20] = 
                              reg_beta.id_aportante[4,6]
                          EXIT CASE
                      WHEN "UY-"
			  ...
			  reg_beta.id_aportante[1,2]= "UY" ) THEN
                      ...
                      END IF
                       ...
                       IF reg_beta.id_aportante[1,3] = "ACR" THEN
                         LET reg_det_mov_trab_uni.iden_mov[21,22] = "  "
                       END IF
			...
			CASE reg_beta.subcuenta
			 WHEN 7
                         ...
                         CASE  vorig
                         WHEN "08"
                            LET reg_det_mov_trab_uni.iden_mov[21,22]="05"
                         EXIT CASE
                         WHEN "09"
                            LET reg_det_mov_trab_uni.iden_mov[21,22]="04"
                         EXIT CASE
                         WHEN "06"
	                    LET reg_det_mov_trab_uni.iden_mov[21,22]="06"
	                 EXIT CASE
	                 WHEN "07"
	                    LET reg_det_mov_trab_uni.iden_mov[21,22]="07"
	                 EXIT CASE
	                 OTHERWISE
                            LET reg_det_mov_trab_uni.iden_mov[21,22]="05"
			 EXIT CASE
			 ...
			 EXIT CASE
			   ...
			   END CASE
	              END IF
                      ...
                      LET reg_det_mov_trab_uni.iden_mov[23,27] = "00000"
                      LET reg_det_mov_trab_uni.iden_mov[28,32] = "00000"
              ...
              CASE reg_beta.subcuenta
               ...
               WHEN 8 
              IF reg_beta.tipo_movimiento = 235 THEN
                  LET reg_det_mov_trab_uni.subcuenta = "14"
              ELSE 
                  LET reg_det_mov_trab_uni.subcuenta = "09"
              END IF
                  LET reg_det_mov_trab_uni.importe =
		   ...
		   vnss_uni
               ...
               LET reg_det_mov_trab_uni.iden_mov[1,8]   = 
                   reg_beta.fecha_conversion USING"YYYYMMDD"
                          ...
                          LET reg_det_mov_trab_uni.iden_mov[9,10]  = "04"
                          LET reg_det_mov_trab_uni.iden_mov[11,12] = "01"
                          LET reg_det_mov_trab_uni.iden_mov[13,15] = "590"
                          LET reg_det_mov_trab_uni.iden_mov[16,17] = "04"
                          LET reg_det_mov_trab_uni.iden_mov[18,20] = "002"
...
#idm1IC------------------
		   ...
		   vnss_uni
               ...
               LET reg_det_mov_trab_uni.consecutivo   =
                   ...
                   LET reg_det_mov_trab_uni.iden_mov[9,10]  = "01"
                   ...
                   WHEN "TI"  
                       IF (reg_beta.tipo_movimiento = 3 OR 
                          reg_beta.tipo_movimiento = 5 )THEN
                           LET reg_det_mov_trab_uni.iden_mov[21,22] = "03"
                       ELSE
                           LET reg_det_mov_trab_uni.iden_mov[21,22] = "01"
                       END IF
                       EXIT CASE 
              ...
              CASE reg_beta.subcuenta
               ...
               WHEN 7
                 ...
                 WHEN 1
                  ...
                  EXIT CASE
                 WHEN 4
                  LET reg_det_mov_trab_uni.subcuenta = "24"
                  EXIT CASE
                 WHEN 3
                  LET reg_det_mov_trab_uni.subcuenta = "08"
                  EXIT CASE
                 ...
                 END CASE
                  ...
                  LET reg_det_mov_trab_uni.precio_accion = 
                      reg_beta.precio_accion
                  LET reg_det_mov_trab_uni.fecha_proceso = TODAY
                  LET reg_det_mov_trab_uni.fecha_conversion = 
                      reg_beta.fecha_conversion
                  INSERT INTO safre_tmp:det_mov_trab_uni values (reg_det_mov_trab_uni.*)
                  EXIT CASE
...
#tm3rcv-----------------
		   ...
		   reg_beta.fecha_pago USING"YYYYMMDD"
               ...
               LET reg_det_mov_trab_uni.iden_mov[45,47] = "590"

               LET reg_det_mov_trab_uni.iden_mov[72,73] = "00"
               LET reg_det_mov_trab_uni.iden_mov[74,75] = "00"
               LET reg_det_mov_trab_uni.iden_mov[76,77] = "00"
               LET reg_det_mov_trab_uni.iden_mov[86,87] = "03"
               LET reg_det_mov_trab_uni.iden_mov[88,108] = 
...
#---------------------
		   ...
		   vnss_uni
                ...
                LET reg_det_mov_trab_uni.iden_proc = "06"
                ...
                IF reg_beta.id_aportante = "INFONAVIT" THEN
                    LET reg_det_mov_trab_uni.iden_mov[1,2] = '01'
		ELSE 
	            LET reg_det_mov_trab_uni.iden_mov[1,2] = '02'
                END IF
              ...
              CASE reg_beta.subcuenta
               ...
               WHEN "4"
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "12"
                  ELSE
                  LET reg_det_mov_trab_uni.subcuenta = "15"
                  END IF 
                  INSERT INTO safre_tmp:det_mov_trab_uni values (reg_det_mov_trab_uni.*)
                  EXIT CASE
               ...
               WHEN 8
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "14"
                  ELSE
                  LET reg_det_mov_trab_uni.subcuenta = "16"
                  END IF 
                  INSERT INTO safre_tmp:det_mov_trab_uni values (reg_det_mov_trab_uni.*)
                  EXIT CASE
		  ...
		  USING "&&&&&&&"
              ...
              CASE reg_beta.subcuenta
               ...
               WHEN 6 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "10"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos 
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
               ...
               WHEN 9 
                  ...
                  LET reg_det_mov_trab_uni.subcuenta = "11"
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos  
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
...
#tm1AF-------------------- 
		   ...
		   vnss_uni
                      ...
                      LET reg_det_mov_trab_uni.iden_proc = "02"
                      ...
                      CASE reg_beta.tipo_movimiento 
                      WHEN 401 
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="IM"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="IN"
                        EXIT CASE
                      WHEN 402 
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="IM"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="VI"
                        EXIT CASE
                      WHEN 403 
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="IM"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="VO"
                        EXIT CASE
                      WHEN 404 
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="IM"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="OR"
                        EXIT CASE
                      WHEN 405 
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="IM"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="AS"
                        EXIT CASE
                      WHEN 408 
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="RT"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="IP"
                        EXIT CASE
                      WHEN 409 
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="RT"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="VI"
                     ...
                     END CASE 
                       ...
                       LET reg_det_mov_trab_uni.iden_mov[21,22] = "01"
                       LET reg_det_mov_trab_uni.iden_mov[46,47] = "01"
...
#tm1AF---------------------- 
		   ...
		   vnss_uni
                      ...
                      END IF
                       ...
                       LET reg_det_mov_trab_uni.iden_mov[46,47] = "02"
...
#tm1AF-------------------- 
		   ...
		   vnss_uni
                      ...
                      WHEN 468
                        ...
                        EXIT CASE
                      WHEN 471
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="IV"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="VE"
                        EXIT CASE
                      WHEN 472
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="IV"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="CE"
                        EXIT CASE
                      WHEN 475
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="TJ"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="IP"
                        EXIT CASE
                      WHEN 476
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="TJ"
                      ...
                      WHEN 481
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="TJ"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="RE"
                        EXIT CASE
                      WHEN 482
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="TJ"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="VE"
                     ...
                     END CASE 
                       ...
                       LET reg_det_mov_trab_uni.iden_mov[21,22] = "03"
                       LET reg_det_mov_trab_uni.iden_mov[46,47] = "03"
...
#tm1AF-------------------- 
		   ...
		   vnss_uni
                      ...
                      WHEN 430
                        ...
                        EXIT CASE
                      WHEN 433
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="PP"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="  " 
                        EXIT CASE 
                      WHEN 436
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="IV"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="IP"
                      ...
                      WHEN 441
                        ...
                        EXIT CASE
                      WHEN 442
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="CV"
                          LET reg_det_mov_trab_uni.iden_mov[19,20]="VE"
                        EXIT CASE
                      WHEN 443
                          LET reg_det_mov_trab_uni.iden_mov[17,18]="CV"
                     ...
                     END CASE 
                       ...
                       LET reg_det_mov_trab_uni.iden_mov[21,22] = "05"
                       END IF
                       LET reg_det_mov_trab_uni.iden_mov[46,47] = "04"
...
#tm1AF-------------------- 
		   ...
		   vnss_uni
                    ...
                    IF reg_beta.id_aportante = "PATRON" THEN 
                       LET reg_det_mov_trab_uni.iden_mov[46,47] = "07"
                    ELSE 
                       LET reg_det_mov_trab_uni.iden_mov[46,47] = "08"
                    END IF
              ...
              CASE reg_beta.subcuenta
               ...
               WHEN 3
                   IF reg_beta.id_aportante = "PATRON" THEN   
                      LET reg_det_mov_trab_uni.subcuenta = "13"
                    ELSE
                      LET reg_det_mov_trab_uni.subcuenta = "05"
                   END IF
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos
                  LET reg_det_mov_trab_uni.fecha_conversion = 
...
#tmven -----------------
        ...
        LET reg_det_mov_trab_uni.iden_proc = "09"
...
#idmDIS-------------------------
    ...
    DEFINE reg_cur1_tipo_movex RECORD
     ...
     DEFINE v_fecha_pago          CHAR(008),
          ...
          WHERE  A.folio             = reg_beta.folio
          AND    A.nss               = reg_uni_unificado.nss_cta1
          AND    A.folio_pago_sua    = reg_beta.folio_sua
          ...
          DECLARE cur_movex2 CURSOR FOR
           ...
           END FOREACH
           DELETE FROM e_fecha_uni

	   INSERT INTO e_fecha_uni
	   ...
	   FROM e_fecha_uni a
           ...
           WHERE  a.folio = reg_cur2_tipo_movex.folio
               ...
               LET reg_det_mov_trab_uni.iden_proc = "14"

               LET reg_det_mov_trab_uni.consecutivo   =
                   reg_det_mov_trab_uni.consecutivo + 1

               LET reg_det_mov_trab_uni.iden_mov[1,11]   =
                   reg_cur2_tipo_movex.reg_patronal_imss

               LET reg_det_mov_trab_uni.iden_mov[12,22] =
                   reg_beta.nss

               LET reg_det_mov_trab_uni.iden_mov[23,28] =
                   reg_cur2_tipo_movex.periodo_pago

               LET reg_det_mov_trab_uni.iden_mov[29,34] =
                   reg_cur2_tipo_movex.folio_pago_sua USING"&&&&&&"

               LET reg_det_mov_trab_uni.iden_mov[35,42]  =
                   reg_cur2_tipo_movex.fecha_pago USING "YYYYMMDD"

               LET reg_det_mov_trab_uni.iden_mov[43,50]  =
                   reg_cur2_tipo_movex.fecha_valor_viv USING"YYYYMMDD"

               LET reg_det_mov_trab_uni.iden_mov[51,58]  =
                   reg_cur2_tipo_movex.fecha_valor_rcv USING"YYYYMMDD"

               LET reg_det_mov_trab_uni.iden_mov[59,66]  =
                   ex_fecha_conversion USING"YYYYMMDD"

               LET reg_det_mov_trab_uni.iden_mov[67,74]  =
                   ex_fecha_conversion USING"YYYYMMDD"

               LET reg_det_mov_trab_uni.iden_mov[75,77] =
                   reg_cur2_tipo_movex.clave_ent_recep USING"&&&"

               LET reg_det_mov_trab_uni.iden_mov[78,79] =
                   reg_cur2_tipo_movex.dias_cotz_bimestre USING"&&"

               LET reg_det_mov_trab_uni.iden_mov[80,87] =
                   ex_fecha_creac USING"YYYYMMDD"

               LET reg_det_mov_trab_uni.iden_mov[88,92] =
                   reg_cur2_tipo_movex.consec_reg_lote USING"&&&&&"

               LET reg_det_mov_trab_uni.iden_mov[93,100] =
                   reg_beta.fecha_conversion USING"YYYYMMDD"

               LET reg_det_mov_trab_uni.iden_mov[101,108] =
                   reg_beta.fecha_conversion  USING"YYYYMMDD"
...
DEFINE vcve char(003)
		   ...
		   vnss_uni
               ...
               LET reg_det_mov_trab_uni.iden_proc = "08"

               LET reg_det_mov_trab_uni.consecutivo   =
                   reg_det_mov_trab_uni.consecutivo + 1


               IF reg_beta.tipo_movimiento = 240 THEN
                  LET reg_det_mov_trab_uni.iden_mov[1,2]   = '01'
	       END IF 
               IF reg_beta.tipo_movimiento = 241 THEN
                  LET reg_det_mov_trab_uni.iden_mov[1,2]   = '01'
	       END IF 
               IF reg_beta.tipo_movimiento = 242 THEN
                  LET reg_det_mov_trab_uni.iden_mov[1,2]   = '02'
	       END IF 
               IF reg_beta.tipo_movimiento = 243 THEN
                  LET reg_det_mov_trab_uni.iden_mov[1,2]   = '01'
	       END IF 
               IF reg_beta.tipo_movimiento = 244 THEN
                  LET reg_det_mov_trab_uni.iden_mov[1,2]   = '02'
	       END IF 

               LET reg_det_mov_trab_uni.iden_mov[3,10]   = reg_beta.fecha_conversion
               USING "YYYYMMDD"

               LET reg_det_mov_trab_uni.iden_mov[11,13]   = '590'
               LET reg_det_mov_trab_uni.iden_mov[14,16]   = '590'
               LET reg_det_mov_trab_uni.iden_mov[17,19]   = '590'
              ...
              CASE reg_beta.subcuenta
               ...
               WHEN 1 
		 IF (reg_beta.tipo_movimiento = 240 OR 
		     reg_beta.tipo_movimiento = 241 OR 
		     reg_beta.tipo_movimiento = 242) THEN
                  LET reg_det_mov_trab_uni.subcuenta = "01"
		 END IF 
		 IF (reg_beta.tipo_movimiento = 243 OR 
		    reg_beta.tipo_movimiento = 244) THEN
                  LET reg_det_mov_trab_uni.subcuenta = "18"
		 END IF
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos
		 ...
		 END IF
                  LET reg_det_mov_trab_uni.importe =
                      reg_beta.monto_en_pesos 
                       LET reg_det_mov_trab_uni.fecha_conversion = 
                           reg_beta.fecha_conversion
                  INSERT INTO safre_tmp:det_mov_trab_uni
		  values (reg_det_mov_trab_uni.*)
                  EXIT CASE
...
#tmunif---------- 
		   ...
		   vnss_uni
               ...
               LET reg_det_mov_trab_uni.iden_proc = "13"
