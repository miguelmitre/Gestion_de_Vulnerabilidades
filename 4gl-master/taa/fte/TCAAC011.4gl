...
FUNCTION    f_100_proceso()
   ...
   DISPLAY  " TCAAC011    CARGA MASIVA DE PRECIOS DE ACCION DE TODAS LAS AFORES  ",
            "                            "   AT   3,1   ATTRIBUTE(REVERSE)
   DISPLAY  "              PARA CALCULO DE RENDIMIENTOS POR TRASPASO INDEBIDO",
            "                            "   AT   4,1   ATTRIBUTE(REVERSE)
   DISPLAY  " << SEPARAR EL REGISTRO CON COMAS(,) Y DEBE LLEVAR UNA AL FINAL >>"
            ,"                            "   AT   5,1
   DISPLAY  " El Archivo lo toma de /safre_prc/taa/rescate           " AT 7,1
...
FUNCTION    f_110_carga_precios()
   ...
   CREATE   TEMP  TABLE       fecha_valuacion ( fecha   DATE,sie  SMALLINT );
   LET      g_ejecuta                    =  g_seg_modulo.ruta_rescate  CLIPPED,
                                           "/",nom_archivo             CLIPPED
   ...
   FOREACH  cur_valuacion     INTO    l_reg_valuacion
        ...
        INSERT   INTO   fecha_valuacion  VALUES(g_fecha_valuacion,g_siefore);
   ...
   END FOREACH
   SELECT   COUNT(*)
    INTO    g_total_grabados
    FROM    safre_af:tab_siefore_sistema a,fecha_valuacion  b
    WHERE   fecha_valuacion           =  fecha
      AND   siefore_cod               =  sie;
   DISPLAY "   PRECIOS DE ACCION GRABADOS :",
             g_total_grabados  USING  "###,###,##&"  AT 15,2 ATTRIBUTE(REVERSE)
   LET      g_ejecuta                 =  g_seg_modulo.ruta_listados  CLIPPED,
                                "/",g_usuario CLIPPED,".TCAAC011.",
                                g_siefore USING "&#" CLIPPED,".",TIME CLIPPED
   UNLOAD   TO   g_ejecuta
   SELECT   a.*
    FROM    safre_af:tab_siefore_sistema a,fecha_valuacion  b
    WHERE   fecha_valuacion           =  fecha
      AND   siefore_cod               =  sie
    ORDER   BY  1,2,3;
  DISPLAY  "Archivo con Precios Cargados:",g_ejecuta       AT  17,2
