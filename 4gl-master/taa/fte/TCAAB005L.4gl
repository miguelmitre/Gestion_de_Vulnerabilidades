...
FUNCTION    F_131_2_fecha_redencion()
   ...
   IF       g_fecha_redencion       IS    NULL          OR
            ...
            END IF
   END IF
   LET      l_fecha            =  MDY(g_fecha_redencion[5,6],
                                      g_fecha_redencion[7,8],
                                      g_fecha_redencion[1,4])
   IF       l_fecha           IS  NULL    OR
            l_fecha            <  reg_ctr_folio.fecha_envio_saldos  THEN
            DISPLAY  g_current," :ERROR FECHA REDENCION NSS: ",
                     reg_solicitud.n_seguro,"  FECHA: ",l_fecha
            LET      g_fecha_redencion         =  "        "
   END IF
END FUNCTION
...
## fecha de ultimo retiro o primera aportacion  largo plazo
   DEFINE   l_fecha                         DATE,
            l_scta                          SMALLINT
   LET      g_aport_l_plazo              =  NULL
   IF       g_saldos[26].pesos           =  cero               OR
            g_saldos[26].pesos           <  g_monto_minimo     THEN
            LET      l_fecha             =  NULL
   ELSE
            LET      l_scta              =  26
            EXECUTE  sql_48 
...
## fecha de ultimo retiro o primera aportacion de perspectiva  largo plazo
   DEFINE   l_fecha                         DATE,
            l_scta                          SMALLINT
   LET      g_perspect_l_plazo           =  NULL
   IF       g_saldos[28].pesos           =  cero                OR
            g_saldos[28].pesos            <  g_monto_minimo     THEN
            LET      l_fecha             =  NULL
   ELSE
            LET      l_scta              =  28
            EXECUTE  sql_48 
...
FUNCTION    F_140_inserta_dis_provision()
   ...
   IF r_scta = 39 THEN LET r_scta = 31 END IF
