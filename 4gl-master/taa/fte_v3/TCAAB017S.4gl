...
FUNCTION  arma_querys_TCAAB017S()
   ...
   IF g_periodo_carga = 1 THEN
      LET      g_sql_06               =
            ' SELECT   COUNT(*) ',
            ' FROM     safre_af:taa_cd_det_cedido a ',
            ' WHERE    a.estado                  =  103 ',
            ' AND      a.fecha_trasp            >=  ?  ',
            ' AND      a.tipo_traspaso          IN (12,20,24,25,51,57,83,84,85); '
   ELSE
      LET      g_sql_06               =
            ' SELECT   COUNT(*) ',
            ' FROM     safre_af:taa_cd_det_cedido a ',
            ' WHERE    a.estado                  =  103 ',
            ' AND      a.fecha_trasp            >=  ?  ',
            ' AND      a.tipo_traspaso       NOT IN (12,20,24,25,51,57,83,84,85); '
   END IF
   ...
   IF g_periodo_carga = 1 THEN
      ...
      IF       g_fecha_trasp_desde            =   g_today      THEN
               ...
               '    AND   b.tipo_traspaso             IN (12,20,24,25,51,57,83,84,85) ',
               ' ORDER    BY  b.n_seguro                      ;'
      END IF
   ELSE
      LET      g_sql_14               =
      ...
      IF       g_fecha_trasp_desde            =   g_today      THEN
               ...
               '    AND   b.estado                     =  103        ',
               '    AND   c.marca_cod                  =  120        ',        --Inhabilitado ACS-Sep2011
               '    AND   b.tipo_traspaso     NOT  IN (12,20,24,25,51,57,83,84,85) ',
               ' ORDER    BY  b.n_seguro                      ;'
      END IF
   END IF
