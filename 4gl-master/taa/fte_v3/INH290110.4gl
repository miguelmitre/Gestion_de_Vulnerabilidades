#####################################################################
#  Modifica Fechas de inhabilitación  por traspaso   de Enero 2010  #
#####################################################################
DATABASE   safre_af
GLOBALS
...
MAIN
   ...
   OPEN     WINDOW    PREPBIM        AT  2,2
            WITH      FORM     "INH290110"  ATTRIBUTE(BORDER)
   DISPLAY  "<<INH290110>>         TRASPASOS  AFORE-AFORE (CEDENTE)       ",
   ...
   SELECT   COUNT(*)
     ...
     FROM   cta_act_marca
    WHERE   marca_cod         =  120
      AND   fecha_ini         =  "05/26/2010"
      AND   nss   IN(SELECT   n_seguro
                       FROM   taa_cd_det_cedido
                      WHERE   fecha_trasp      =  "01/29/2010"
                        AND   estado           =  103);
   DISPLAY  "                                  "  AT 16,5
   ...
   FOREACH  C_FECHA    INTO   act.*
        ...
        LET    his.fecha_ini         =  "01/29/2010"
        LET    his.fecha_causa       =  "01/29/2010"
        INSERT    INTO       cta_his_marca
                  VALUES    (his.*)
        UPDATE    cta_act_marca
           SET    fecha_ini          =  "01/29/2010",
                  fecha_causa        =  "01/29/2010"
         WHERE    nss                =  act.nss
           AND    marca_cod          =  120
           AND    fecha_ini          =  "05/26/2010";
