...
#Propietario       => E.F.P.                      
#Programa TCAAB017 => PANTALLA LANZADORA DEL PROCESO TCAAB017(AF-AF COMP)
#Sistema           => TCAA                     
...
GLOBALS "TCAAB017S.4gl"  #  Definicion  de  SQL

MAIN
    OPTIONS INPUT WRAP,
    PROMPT LINE LAST,
    ACCEPT KEY CONTROL-I
    CALL     STARTLOG("TCAAB017.log")
    CALL     F_010_inicio()
END MAIN
...
#i----------------------
    ...
    DISPLAY  "TCAAB017            TRASPASOS  AFORE-AFORE (CEDENTE)                          " AT  2,2 ATTRIBUTE(REVERSE)   
    ...
    IF       g_cuentas_pendientes       >  0   THEN
            ...
            END      WHILE
    END IF
    LET    lanza_proceso = "nohup fglgo TCAAB017L ",
...
FUNCTION  F_020_muestra_folio_comple()
    ...
    LET       g_folio                   =  NULL
    LET       g_periodo_carga           =  NULL
    
    INPUT BY NAME g_folio,g_periodo_carga,g_fecha_trasp_desde    WITHOUT  DEFAULTS
       
       AFTER  FIELD  g_folio               
               IF g_folio IS NULL THEN 
                       ERROR "FOLIO NO VALIDO... TECLEAR FOLIO VALIDO ..."
                       NEXT FIELD g_folio
               END  IF
               ...
               IF     SQLCA.SQLCODE <> 0     THEN
                       ERROR "  FOLIO NO VALIDO PARA TOMAR LAS FECHAS PARA EL PROCESO  "
                       NEXT FIELD g_folio
               END IF
      ...
      # BEFORE  FIELD  g_fecha_trasp_desde
       ...
       AFTER FIELD g_periodo_carga
          IF g_periodo_carga  IS  NULL   THEN
             ERROR     " Periodo de carga : 1=Mensual,2=Bimestral"
             NEXT FIELD g_periodo_carga
          END IF 
