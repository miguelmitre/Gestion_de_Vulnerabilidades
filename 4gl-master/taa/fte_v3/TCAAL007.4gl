...
#Objetivo        => Genera reporte de Provision  trasp.(normal y comple)    #
#Propietario     => E.F.P.                                                  #
#Fecha           => 20 DE ENERO DEL 2004 .                                  #
#Sistema         => TCAA                                                    #
#Programa        => TCAAL007(ESTE PROGAMA SE  INTEGRA CON EL OBJ GLOB_REPS) #
#############################################################################
DATABASE safre_af
...
#### VAR PARA ARREGLO
    ...
    DEFINE  reg_1        ARRAY[200]  OF   RECORD
            folio                        INTEGER        ,
...
GLOBALS   "GLOB_REPS.4gl"    ###  Programa  para  la  Definicion  de  Querys

MAIN
   CALL     STARTLOG("TCAAL007.log")
...
#pp-------------------------
    OPEN WINDOW ventana_21 AT 2,2 WITH FORM "TCAAL0072" ATTRIBUTE(BORDER)
    DISPLAY " <<TCAAL007>>          TRASPASOS  AFORE - AFORE  CEDENTE                               " AT  1,2 ATTRIBUTE(REVERSE)
    DISPLAY g_hoy USING "DD-MM-YYYY" AT 1,64 ATTRIBUTE(REVERSE)
    DISPLAY "              FOLIOS  PROVISIONADOS   ( CONSULTAS  Y  REPORTES )                               " AT  2,2 ATTRIBUTE(REVERSE)
...
#C-----------------
    ...
    PREPARE  qry_consul_07   FROM g_sql_07
    DECLARE  cursor_c_07     CURSOR FOR   qry_consul_07

    LET      i_arr_1      =  0
    FOREACH  cursor_c_07     INTO  g_reg_1.*,g_desc_estado
    ...
    IF     (i_arr_1)        >=  1    THEN
           ...
           DISPLAY  ARRAY reg_1 TO scr_1.*
    	        ...
    	        ON KEY ( RETURN )
                     ...
                     LET lanza_proceso   = "fglgo GLOB_CON ",
                                            g_folio CLIPPED," ",
                                            g_tot_ctas CLIPPED," '",
                                            g_tip_des1 CLIPPED,"' '",
                                            g_tip_des2 CLIPPED,"' ",
                                            g_nombre_progr CLIPPED," '",
                                            g_titulo_pant CLIPPED,"' ","'",
                                            g_tblprovliq CLIPPED,"' "," ",
                                            g_repprovliq CLIPPED
          ...
          END DISPLAY
   ELSE 
          PROMPT " NO HAY FOLIOS PROVISIONADOS <Enter> para Salir " FOR g_enter
          EXIT  PROGRAM
   END IF 
...
FUNCTION prep_var_argval()
    ...
    LET    g_tip_des1                    =         "PROVISION AF-AF POR TIPO MOV"," ",g_nor_o_comple CLIPPED

    LET    g_tip_des2                    =         "PROVISION AF-AF POR SUBCUENTA"," ",g_nor_o_comple CLIPPED

    LET    g_nombre_progr                =         "TCAAL007"

    LET    g_titulo_pant                 =         "Consulta de Provision  Afore Cedente AF-AF"

    LET    g_tblprovliq                  =         "dis_provision b" 

    LET    g_repprovliq                  =         "PROV"
