...
#Propietario       => E.F.P.                                              #
#Programa TCAAB005L=> GENERA ARCHIVO DE TRASPASO NORMAL-INTERNET(CEDENTE) #
#Sistema           => TCAA                                                #
...
FUNCTION    F_010_inicio()
   ...
   CALL STARTLOG (FGL_GETENV("USER")||".TCAAB005L.log")
   ...
   CALL     F_012_prepara_querys_TCAAB005L()
   LET      g_hora                     =  TIME
   LET      g_num_registro             =  cero
   LET      INT_FLAG                   =  TRUE
   DISPLAY  " "
   DISPLAY  ".1"
   DISPLAY  g_current,": TCAAB005L: "
...
FUNCTION    F_012_prepara_querys_TCAAB005L()
...
FUNCTION    F_015_verifica_rechazo_convive()
   ...
   DEFINE   l_fecha_causa                    ,
            l_today                          DATE
   LET      l_today                 =  TODAY
   IF       l_today       >  reg_ctr_folio.fecha_envio_saldos   THEN
            RETURN
   END  IF
   DECLARE  cur_convivencia    CURSOR   FOR      sql_49
   OPEN     cur_convivencia     USING   reg_ctr_folio.folio
   FOREACH  cur_convivencia      INTO   reg_solicitud.*
            INITIALIZE  l_marca_entra        TO  NULL
            EXECUTE  sql_50     USING   reg_solicitud.tipo_traspaso
 		                 INTO   l_marca_entra
            IF       l_marca_entra           IS  NULL    THEN 
                     DISPLAY  " TIPO TRASPASO : ",
                              reg_solicitud.tipo_traspaso, 
            ...
            OPEN     cursor_marca        USING   
                     ...
                     l_marca_entra                , # marca
                     reg_ctr_folio.folio   , #envia folio como correlati
...
FUNCTION    F_030_despliega_folio()
   ...
   #CPL-1337 Se inicializa l_registros
   LET      l_registros      = cero
...
FUNCTION    F_100_proceso()
   ...
   FOREACH  cur_nss_ced           INTO   reg_solicitud.*
            ...
            END IF
#CPL-1635 INI
           LET l_folio = 0  LET l_sql_ok = 100
           SELECT   MAX(folio) INTO l_folio
            FROM   safre_af:taa_cd_det_cedido
           WHERE   n_seguro =  reg_solicitud.n_seguro
            AND   estado IN (99,103)

           LET l_sql_ok = SQLCA.SQLCODE
             INITIALIZE mr_folio.* TO NULL 
             LET mr_folio.fecha_envio_saldos = "12/31/1899"
             IF  l_sql_ok = 0 THEN
...
FUNCTION    F_120_arma_datos_generales()
   ...
   LET      g_id_credito_fovi             =  ' '
   EXECUTE  sql_12_2    USING   reg_solicitud.n_seguro
                         INTO   g_tipo_solicitud,g_finicta,g_n_unico
...
FUNCTION    F_130_arma_saldo_viv()
   DEFINE   l_resta_credito                     ,
            l_grupo                             SMALLINT
   LET      l_grupo                         =  4
   LET      g_fecha_redencion               =  "        "
...
FUNCTION    F_131_2_fecha_redencion()
   DEFINE   l_fecha                             DATE
   DEFINE   l_ferror                            SMALLINT
   
   LET      g_fecha_redencion        =   NULL
   LET      l_fecha                  =   NULL
   LET      l_ferror                 =   TRUE
   
   EXECUTE  sql_38         USING   g_n_unico
                            INTO   g_fecha_redencion
   
   LET g_fecha_redencion = g_fecha_redencion CLIPPED # CPL-1418 Se agrega la validación por si no existe la fecha en dis_det_bono o en taa_viv_recepcion
   
   IF  g_fecha_redencion IS    NULL          OR
       g_fecha_redencion =  ""               OR
       g_fecha_redencion =  "00010101"       THEN
            LET      g_fecha_redencion            =  "        "
            EXECUTE  sql_38_1       USING    reg_solicitud.n_seguro
                                     INTO   l_fecha
                                     
            IF       l_fecha           IS  NOT  NULL  THEN
                     LET      g_fecha_redencion   =  l_fecha  USING  "YYYYMMDD"
                     LET      l_ferror = FALSE
            END IF
   ELSE 
      LET      l_ferror = FALSE
   ...
   IF  l_ferror THEN
      DISPLAY  g_current," :ERROR, SIN FECHA REDENCION ENCONTRADA NSS: ", reg_solicitud.n_seguro
      LET      l_fecha = NULL
   ELSE
      LET      l_fecha = MDY(g_fecha_redencion[5,6], g_fecha_redencion[7,8], g_fecha_redencion[1,4])
   END IF
   
   IF  l_fecha           IS  NULL    OR
       l_fecha            <  reg_ctr_folio.fecha_envio_saldos  THEN
         DISPLAY  g_current," :ERROR FECHA REDENCION NSS: ", reg_solicitud.n_seguro,"  FECHA: ",l_fecha
         LET      g_fecha_redencion         =  "        "    
   END IF
END FUNCTION
...
----------------------------------
   ...
   DEFINE   l_pesos_36   DECIMAL(16,2)
   DEFINE   l_pesos_37   DECIMAL(16,2)
   ...
   FOREACH  c_saldo_rcv   USING  reg_solicitud.n_seguro,
            ...
            ---
            ---- omision de saldo por veriicar
            ---
            IF      (g_scta                  =  37      OR
                     g_scta                  =  38 )    THEN
        
                     IF        l_sin_esep    =  0    THEN
                               EXECUTE  sql_62       USING   
                                        reg_ctr_folio.folio,
                                        reg_solicitud.n_seguro,
                                        reg_ctr_folio.tipo_traspaso
                               LET      l_sin_esep       =  1
                     END IF
        
                    # CONTINUE  FOREACH         #CPL-1635 Se tratan las subcuentas como cuaquier otra, para incluir en el archivo de saldos
            END IF
            ---
            ---- fin omision de saldo por veriicar
            ---
            ...
            IF ((l_acc_36 + l_acc_37) > g_acciones) AND g_scta = 31 THEN
               DISPLAY "POSIBLE QUEBRANTO NSS: ",reg_solicitud.n_seguro, " CURP:", reg_solicitud.n_unico,
                       ...
                       " SALDO SUB39: ",l_acc_37
                            ...
                            LET ind_quebranto = TRUE
                            LET l_pesos = g_pesos
                            LET l_acc_36 = g_acciones
            END IF
            
            FOR l_i = 1 TO g_veces 
                    ...
                    #CPL-1418 se cambia variable l_pesos por g_pesos
                    IF       g_pesos              >=  g_monto_minimo    THEN
                             CALL     F_140_inserta_dis_provision()
                             LET      g_imp_2_dec                       =  g_pesos
...
FUNCTION    F_140_inserta_dis_provision()
   ...
   IF r_scta = 39 AND g_veces = 2 THEN LET r_scta = 31 END IF
  ...
  #########     Arma  registro  2 de  detalle 05  de  rcv    ############
   IF     g_tot_bloques_rcv                  >  8   THEN
    ...
    IF        g_por_curp            THEN
              LET       reg_solicitud.n_seguro            =
                        reg_solicitud.n_seguro_cedente
              LET       reg_solicitud.n_seguro_cedente    =
                        reg_solicitud.n_seguro
     END IF
END REPORT
...
FUNCTION    F_200_fin()
   ...
   DISPLAY  g_current," :FIN DE PROGRAMA       : TCAAB005L"
END FUNCTION
