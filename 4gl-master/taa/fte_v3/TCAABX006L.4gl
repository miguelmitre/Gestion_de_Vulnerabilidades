...
GLOBALS  "TCAABX006S.4gl"   #  ARMA LOS QUERYS PARA EL PROCESO
   ...
   FOREACH  c_cedidos       INTO  l_nss
            SELECT   MAX(folio) INTO   l_folio
             ...
             WHERE   n_seguro          =  l_nss
               AND   estado            =  103
               AND folio IN (20910,20939)  #folio fijos -->ACS
...
{DESCOMENTAR}
            ...
            END IF 
            IF       l_leidos                 =  500    THEN
                     LET      l_regs_proc     =  l_regs_proc   +  500
                     LET      l_leidos        =  cero
                     LET      g_current       =  CURRENT
...
FUNCTION    F_200_fin()
   ...
   RUN      g_comando
   LET      g_current             =  CURRENT
   DISPLAY  g_current," :FIN DE PROGRAMA       : TCAABX006"
END FUNCTION
