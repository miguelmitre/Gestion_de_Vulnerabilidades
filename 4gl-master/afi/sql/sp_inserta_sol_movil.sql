...
LET lc_n_rfc            = NULL; --pc_curp_ahorrador[1,10]; -- CPL-3612 Si el RFC llega nulo así se mantiene
LET lc_hora             = "";
LET ls_cont_tel         = 0;
...
-- se asigna usuario y hora
LET lc_usuario_safre = USER;
LET lc_hora = TO_CHAR(extend (current, hour to second),'%H:%M:%S');
...
LET ld_registro = seq_afi_solicitud_movil.NEXTVAL;
...
IF pc_aplicacion_origen IS NULL OR pc_aplicacion_origen = "00" OR pc_aplicacion_origen = " " THEN
   ...
   RETURN ls_cod_rech    ,
          lc_desc_rechazo;
ELSE
   -- CPL-4009 Se valida que la aplicacion origen corresponda a 01, 02 o 07
   IF NOT (pc_aplicacion_origen = "01" OR pc_aplicacion_origen = "02" OR pc_aplicacion_origen = "07") THEN
      LET lc_desc_rechazo = "APLICACION ORIGEN NO VALIDO ";
      LET ls_cod_rech       = 1;
...
-- se valida si la aplicacion origen corresponde a 07-REGISTRO PORTAL WEB
IF (pc_aplicacion_origen = "07") THEN
   -- se validan los tipos de solicitud correspondientes a la aplicacion origen
   IF ps_tipo_solicitud NOT IN (38,39,40,41) THEN
...
-- se valida si la aplicacion origen corresponde a 01 APP GENERICA o 02 APP PERSONALIZADA
IF (pc_aplicacion_origen IN ("01","02")) THEN -- AFORE MOVIL
   -- se validan los tipos de solicitud correspondientes a la aplicacion origen
   IF ps_tipo_solicitud NOT IN (23,24,26,30) THEN
       LET lc_desc_rechazo = "EL TIPO DE SOLICITUD NO CORRESPONDE A AFORE MOVIL";
       LET ls_cod_rech     = 1;
...
IF ps_tipo_solicitud IS NULL THEN
   ...
   RETURN ls_cod_rech    ,
          lc_desc_rechazo;
ELSE
   IF NOT (ps_tipo_solicitud IN (23,24,26,30,38,39,40,41)) THEN
...
-- CPL-4009 Ocupacion y Nivel de Estudios NO ES OBLIGATORIO ps_tipo_solicitud = 40
IF NOT (ps_tipo_solicitud IN (26,40)) THEN
   IF pc_ocupacion IS NULL OR pc_ocupacion = ' ' THEN
      LET lc_desc_rechazo = "OCUPACION NULA";
      LET ls_cod_rech     = 1;
   ...
   IF pc_nivel_estudios IS NULL OR pc_nivel_estudios = ' ' THEN
      LET lc_desc_rechazo = "NIVEL DE ESTUDIOS NULO";
      LET ls_cod_rech     = 1;
...
IF pc_curp_benef IS NOT NULL AND pc_curp_benef[1] <> " " THEN
   ...
   IF (ld_val_porcentaje + pd_porcentaje_benef ) > 100.00 THEN
      LET lc_desc_rechazo = "PORCENTAJE BENEF1 MAYOR A 100";
      LET ls_cod_rech       = 1;
...
IF pc_curp_benef2 IS NOT NULL AND pc_curp_benef2[1,1] <> "" THEN
   ...
   IF (ld_val_porcentaje2 + pd_porcentaje_benef2 ) > 100.00 THEN
      LET lc_desc_rechazo = "PORCENTAJE BENEF2 MAYOR A 100";
      LET ls_cod_rech       = 1;
...
IF pc_curp_benef3 IS NOT NULL AND pc_curp_benef3[1,1] <> "" THEN
   ...
   IF (ld_val_porcentaje3 + pd_porcentaje_benef3 ) > 100.00 THEN
      LET lc_desc_rechazo = "PORCENTAJE BENEF3 MAYOR A 100";
      LET ls_cod_rech       = 1;
...
IF pc_curp_benef4 IS NOT NULL AND pc_curp_benef4[1,1] <> "" THEN
   ...
   IF (ld_val_porcentaje4 + pd_porcentaje_benef4 ) > 100.00 THEN
      LET lc_desc_rechazo = "PORCENTAJE BENEF4 MAYOR A 100";
      LET ls_cod_rech       = 1;
...
IF pc_curp_benef5 IS NOT NULL AND pc_curp_benef5[1,1] <> "" THEN
   ...
   IF (ld_val_porcentaje5 + pd_porcentaje_benef5 ) > 100.00 THEN
      LET lc_desc_rechazo = "PORCENTAJE BENEF5 MAYOR A 100";
      LET ls_cod_rech       = 1;
...
-- si el valor de Ocupacion no es nulo ni vacio se valida que exista en catálogo
IF pc_ocupacion IS NOT NULL AND pc_ocupacion <> ' ' THEN
   LET v_existe = 0;
...
-- si el valor de Nivel de Estudios no es nulo ni vacio se valida que exista en catálogo
IF pc_nivel_estudios IS NOT NULL AND pc_nivel_estudios <> ' ' THEN
   LET v_existe = 0;
...
INSERT INTO afi_solicitud VALUES( pc_nss,                  -- n_seguro
                                  ...
                                  pdt_fecha_envio,         -- fecha_elaboracion  CPL-4190 Anteriormente se insertaba VACIO ""
                                  0,                       -- lote
...
--CORREO ELECTRONICO
IF pc_correo_elect IS NOT NULL AND pc_correo_elect <> " " THEN
