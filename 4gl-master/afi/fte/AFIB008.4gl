...
#rv-----------------------
    ...
    DEFINE ban_nunico SMALLINT    ---03032009
    DEFINE vrowid     INTEGER

    LET contador  = 0 
    LET ban_rfc   = 0
    LET ban_unico = 0
    LET vfol_sol  = 0            #04 02 2009
    LET ban_fol   = 0
    LET ban_fena  = 0
    LET ban_sex   = 0
    LET ban_nac   = 0
...

{
        ...
        DECLARE c_lotes2 CURSOR FOR
        SELECT tl.lotes_correlativo
        FROM   tab_lote tl
        WHERE  tl.lotes_cod = 2
        AND    tl.lotes_fecha = HOYDIA
        GROUP BY 1
        FOREACH c_lotes2 INTO   h_corrd
        EXIT FOREACH
        END FOREACH
        ...
        FOREACH curs_1 INTO w_aux.*, vfol_sol
            ...
            LET k_aux.folio_solicitud = vfol_sol
                ...
                IF bnd_mod = 1 THEN
                   ...
                   IF reg_afi_mae.estadon MATCHES w_aux.estadon OR
                      ...
                      reg_afi_mae.estadon =       w_aux.estadon THEN
                       ...
                       LET ban_est = 0       #06 02 2009
                    ELSE
                       LET ban_est = 1
                    END IF

#06 02 2009 hasta linea 559
                    IF reg_afi_mae.paterno      MATCHES w_aux.paterno OR
                       reg_afi_mae.paterno      LIKE    w_aux.paterno OR
                       reg_afi_mae.paterno      =       w_aux.paterno THEN
                      LET ban_pat = 0
                    ELSE
                       LET ban_pat = 1
                    END IF

                    IF reg_afi_mae.materno      MATCHES w_aux.materno OR
                       reg_afi_mae.materno      LIKE    w_aux.materno OR
                       reg_afi_mae.materno      =       w_aux.materno THEN
                        LET ban_mat = 0
                    ELSE
                       LET ban_mat = 1
                    END IF

                    IF reg_afi_mae.nombres      MATCHES w_aux.nombres OR
                       reg_afi_mae.nombres      LIKE    w_aux.nombres OR
                       reg_afi_mae.nombres      =       w_aux.nombres THEN
                       LET ban_nom = 0
                    ELSE
                       LET ban_nom = 1
                    END IF
                        ...
                        (ban_nom  = 0)) THEN
                         ...
                         CASE w_aux.tip_prob
                            ...
                            WHEN 2
                               LET w_aux.fol_prob = w_ant.fol_prob
                               LET w_aux.n_unico  = NULL
                               LET w_aux.doc_prob = '0',w_ant.doc_prob CLIPPED
        --->cambio del lunes 12 abril  INV-140
                            WHEN 3
                               LET w_aux.tip_prob = w_aux.tip_prob
                               LET w_aux.doc_prob = w_aux.doc_prob CLIPPED
        ---<
                            OTHERWISE
                               LET w_aux.tip_prob = NULL
                   ...
                   IF reg_afi_mae.n_unico MATCHES w_aux.n_unico OR
                      ...
                      reg_afi_mae.n_unico[1] = " "              THEN
                       ...
                       IF w_aux.tip_prob = '5' THEN
                          LET w_aux.fol_prob = NULL
                          IF w_aux.n_unico IS NOT NULL THEN
                             LET ban_nunico = 1
                          END IF
                       ELSE
                          LET w_aux.n_unico      = NULL
...
#06 02 2009 hasta linea 590
      ...
      ---<
                       ...
                       ELSE
                             ...
                             ELSE
                                LET w_aux.fol_prob = w_ant.fol_prob
                                INITIALIZE cadena_2 TO NULL
                                LET i = 0
                                FOR i = 1 TO 17
                                   IF i < 10 THEN
                                      LET cadena_2 = cadena_2 CLIPPED, w_aux.cadena_dif[i] CLIPPED
                                   END IF
                                   IF i = 10 THEN
                                      LET cadena_2 = cadena_2 CLIPPED , "1"
                                   END IF
                                   IF i > 10 THEN
                                      LET cadena_2 = cadena_2 CLIPPED
                                      LET cadena_2 = cadena_2 CLIPPED, w_aux.cadena_dif[i] CLIPPED
                                   END IF
                                END FOR
                                LET w_aux.cadena_dif = cadena_2 CLIPPED
                             END IF
                             LET w_aux.doc_prob = w_ant.doc_prob
                ...
                --IF w_aux.cadena_dif <> "00000000000000000" THEN
                --   LET pasa_dat = 1
                --END IF
                ...
                IF bnd_mod THEN     ---OR
                   --pasa_dat THEN
                    ...
                    LET w_aux.tip_prob = w_ant.tip_prob
                    IF  w_aux.tip_prob = 4 OR
                        w_aux.tip_prob = 7 THEN
                          LET w_aux.fol_prob = w_ant.fol_prob
                    END IF
                    LET w_aux.doc_prob = w_ant.doc_prob

                    IF w_aux.tip_prob = 5 AND
                       bnd_mod        = 1 THEN
                        LET w_aux.n_unico = w_ant.n_unico
                    ELSE
                        LET w_aux.n_unico = NULL
        ...
        ---<
                ...
                --- Si se modifica TDP
                IF ban_tdp THEN
                   IF w_ant.tip_prob <> 0 AND
                      w_ant.tip_prob <> 6 THEN
                      LET w_aux.n_unico = w_ant.n_unico
                   END IF
                END IF
                ...
                IF cont_acep = 9998 THEN
                   ...
                   LET cont_acep = 0
                   LET comm = g_paramgrales.ruta_envio CLIPPED,
...
#--------------------
    ...
    DEFINE w_aux  RECORD
        ...
        folio_solicitud  CHAR(10),            #04 02 2009
        sexo             LIKE afi_mae_afiliado.sexo,
        estadon          LIKE afi_mae_afiliado.estadon,
        frecafor         LIKE afi_mae_afiliado.frecafor,
        nacionalidad     LIKE afi_mae_afiliado.nacionalidad,
...
#--------------------
    ...
    ON EVERY ROW
        ...
        PRINT
            ...
            COLUMN 50,a,m,d                                     ,
            COLUMN 63,w_aux.folio_solicitud  USING "&&&&&&&&&&",
