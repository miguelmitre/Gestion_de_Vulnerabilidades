...
#Fecha             => 2de agosto de 2005                                   #
#                     Adecuaciones para independeintes                     #
#Modificado        => FERNANDO HERRERA HERNANDEZ                           #
#Fecha             => 04 de abril de 2006 Circ 7-12                        #
...
GLOBALS
    ...
    DEFINE val_rech                SMALLINT
    DEFINE val_rech_inserta        SMALLINT
    DEFINE vtipo_administracion    CHAR(2)
    DEFINE vdesc_tip_admon         CHAR(20)
    DEFINE vstatus_desc            CHAR(40)

    DEFINE xx_status     SMALLINT
    DEFINE vfol_exis     SMALLINT
    DEFINE vtabla_exis   SMALLINT
    DEFINE vexiste_sol   SMALLINT
    DEFINE tot_afil      SMALLINT
    DEFINE a_yo_act      SMALLINT
    DEFINE a_yo_fena     SMALLINT
    DEFINE a_yo          SMALLINT
    DEFINE m_reenvio     CHAR(1)
    DEFINE d_reenvio     CHAR(25)
    DEFINE rfc_arma      CHAR(10)

    DEFINE vrfc_orig       CHAR(4)
    DEFINE vrfc_si         CHAR(4)
    DEFINE doc_prob_arma   CHAR(4)
    DEFINE vstatus_captura SMALLINT


    DEFINE ban_foledocta SMALLINT
...
#pp-------------------------
    ...
    MENU "Sol"
        ...
        COMMAND "Consulta" "Consulta Solicitud "
            ...
            CALL Consulta()
            CALL Inicializa()
        COMMAND "Modif" "Modificacion Solicitud "
            LET ACCION = "M"
            CALL Modifica()
            CALL Inicializa()
        COMMAND "Folio" "Modificar Folio Solicitud "
            LET ACCION = "F"
            CALL Modifica_folio()
            CALL Inicializa()
        COMMAND "NSS" "Modificar NSS Solicitud "
            LET ACCION = "S"
            CALL Modifica_nss()
            CALL Inicializa()
        COMMAND "CURP" "Modificar CURP Solicitud "
            LET ACCION = "S"
            CALL Modifica_curp()
            CALL Inicializa()
         COMMAND "Prom" "Modificar promotor Solicitud "
            LET ACCION = "S"
            CALL Modifica_pro()
            CALL Inicializa()
        COMMAND "Despliega" "Despliegue Solicitud "
            LET ACCION = "D"
            CALL Inicializa()
            CALL Despliega() 
...
#A---------------
    ...
    DISPLAY "[ Esc ]Graba [ Ctrl-C ]Salir sin Grabar  [W]Observ [N]Tip Admon "
    AT 1,1 ATTRIBUTE(BOLD)

    DISPLAY "CTRL:[E]Dom. [T]Tels [B]Benef [V]Patrones [P]Icefas [Y]Ident"
...
# TIPO SOLICITUD
    AFTER FIELD tipo_solicitud
        ...
        IF (r_afi.tipo_solicitud = 1  OR
            r_afi.tipo_solicitud = 2  OR
            r_afi.tipo_solicitud = 8  OR
            r_afi.tipo_solicitud = 27  -- Menores de edad
            ) THEN
        ...
        ELSE
            ERROR "Tipo Afiliacion solo puede ser 1)Reg 2)Trasp",
                  " 8)Indep"
...
# FOLIO SOLIC
        AFTER FIELD n_folio
            ...
            IF r_afi.n_folio IS NULL THEN
                ...
                NEXT FIELD n_folio
            ELSE
                CALL Valida_existencia_folio(r_afi.n_folio, r_afi.tipo_solicitud)
                RETURNING vfol_exis, vtabla_exis

                IF vfol_exis = 1 THEN
                    CASE vtabla_exis
                        WHEN 1 PROMPT "Folio ya Existe en Maestro de Afiliados, ",
                                      "[Enter] p/salir" FOR enter
                        WHEN 2 PROMPT "Folio ya Existe en solicitudes de afiliacion, ",
                                      "[Enter] p/salir" FOR enter
                        WHEN 3 PROMPT "Folio ya Existe en Maestro de Modificados, ",
                                      "[Enter] p/salir" FOR enter
                        WHEN 4 PROMPT "Solicitud no capturada en control documental, ",
                                      "[Enter] p/salir" FOR enter
                        RETURN
                    END CASE
            ...
            IF FGL_LASTKEY() = FGL_KEYVAL("UP") OR
               FGL_LASTKEY() = FGL_KEYVAL("LEFT") THEN
                IF r_afi.nombres IS NULL OR
                   r_afi.nombres = " "   THEN
                   ERROR "Nombre(s) no puede ser nulo"
                   NEXT FIELD nombres
                ELSE
                   LET v_1 = 0
                   INITIALIZE val_1 TO NULL
...
# FECHA NACIMIENTO
        AFTER FIELD fena
            IF r_afi.fena IS NULL THEN
                ERROR "Fecha de nacimiento NO puede ser NULA"
                NEXT FIELD fena
            END IF
            ...
            IF a_yo > 120 THEN
               ERROR "Esta persona pasa del rango de 120 ayos, verifique fecha de nacimiento"
               NEXT FIELD fena
            END IF
...
---<
          ...
          WHERE  n_unico   = r_afi.n_unico

          IF SQLCA.SQLCODE = 0 THEN
            WHILE TRUE
               PROMPT "CURP ya Ingresado en Maestro de Afiliados, ",
         ...
         #Valida existencia de la solicitud
         CALL Valida_exis_solicitud(r_afi.n_folio, r_afi.tipo_solicitud,
                                    r_afi.n_seguro, r_afi.n_unico)
         RETURNING vexiste_sol, xx

         IF vexiste_sol = 1 THEN
            IF r_afi.tipo_solicitud <> 8 THEN
               ERROR "NSS ya Ingresado en solicitudes de afiliacion"
               NEXT FIELD n_seguro
            ELSE
               ERROR "CURP ya Ingresado en solicitudes de afiliacion"
               NEXT FIELD n_unico
            END IF
         END IF
...
--->FOLIO EDOCTA
        IF r_afi.tipo_solicitud = 2 THEN
            SELECT 'a.X'
            FROM   afi_folio_edocta a
            WHERE  a.n_folio        = r_afi.n_folio
            AND    a.tipo_solicitud = r_afi.tipo_solicitud
            AND    a.nss            = r_afi.n_seguro
            ...
            IF ban_foledocta = 1 THEN
               WHILE TRUE
                  ERROR "Folio de Estado de cuenta debe ser capturado "
                  SLEEP 2
                  ERROR ""
                  CALL ins_foledocta()
...
---<
            ...
            CALL inserta_logico(r_afi.n_folio, r_afi.tipo_solicitud,
                                r_afi.n_seguro, su_estatus, g_usuario,
                                operacion)
        ...
        ON KEY (CONTROL-E,CONTROL-T,CONTROL-B,CONTROL-V,CONTROL-P)
         ...
         ON KEY ( CONTROL-N )
            IF r_afi.tipo_solicitud = 8 THEN
               CALL ftipo_admon()
               RETURNING vtipo_administracion, vdesc_tip_admon
               CALL Act_tipo_admon(r_afi.n_seguro, r_afi.n_unico,
                                   vtipo_administracion, g_usuario)
            END IF
...
#ita-------------------------------
    ...
    CALL ins_afi_ctr_act(r_afi.n_seguro, r_afi.n_folio, r_afi.tipo_solicitud)

END FUNCTION

FUNCTION ins_afi_ctr_act(vn_seguro, vn_folio, vtipo_solicitud)
#iaca---------------------------------------------------------
...
#c-----------------
    ...
    DISPLAY "CTRL:[F]Despliega [G]Esq Comis [O]Motivos Rech [F2]FolEdoCta [C]Salir" AT 1,1 ATTRIBUTE(BOLD)
    DISPLAY "[E]Dom. [T]Tels [B]Benef [V]Patrones [W]Observa [Y]Identif [N]Tip Admon " AT 2,1 ATTRIBUTE(BOLD)

    INITIALIZE g_n_operac           TO NULL   -- CPL-3165
    INITIALIZE r_afi.n_folio        TO NULL   -- CPL-3165
    INITIALIZE r_afi.tipo_solicitud TO NULL   -- CPL-3165

    INPUT BY NAME r_afi.tipo_solicitud,
                  r_afi.n_folio,
                  r_afi.n_seguro,
                  r_afi.n_unico WITHOUT DEFAULTS
        ...
        AFTER FIELD tipo_solicitud
            ...
            IF SQLCA.SQLCODE = 0 THEN
                DISPLAY r_afi.desc_solicitud
            ELSE
                ERROR "Tipo Solic puede ser 1)Reg 2)Trasp 3)Unif 4)Virtual 8)Indep"
                LET r_afi.tipo_solicitud = NULL
                DISPLAY BY NAME r_afi.tipo_solicitud
                NEXT FIELD tipo_solicitud
            END IF
        ...
        AFTER FIELD n_folio
            ...
            IF r_afi.n_folio IS NULL THEN
                NEXT FIELD n_unico
            ELSE
                IF NOT Rescata_datos("N",r_afi.n_folio,r_afi.n_unico) THEN
        ...
        AFTER FIELD n_unico
            IF r_afi.n_unico IS NULL THEN
                NEXT FIELD n_seguro
            ELSE
                SELECT COUNT(*)
                INTO   tot_afil
                FROM   afi_mae_afiliado a
                WHERE  a.n_unico = r_afi.n_unico
                ...
                ELSE
                    ...
                    IF NOT Rescata_datos("U",r_afi.n_seguro,r_afi.n_unico) THEN
                        ERROR "Afiliado NO existe"
                        NEXT FIELD n_unico
                    END IF                  
        ...
        BEFORE FIELD n_seguro
            ...
            DECLARE curs1 CURSOR FOR stmt1
               OPEN curs1 USING r_afi.n_seguro, v_crea_fecha
            ...
            DISPLAY "EDAD: ", v_edad, " SIEFORE: ", v_siefore, 
                    "                 Estructura Comercial                       " AT 18,1 ATTRIBUTE(REVERSE)
            --    
            -- CPL-3044
        ...
        AFTER FIELD n_seguro
            ...
            IF r_afi.n_seguro IS NULL THEN
                NEXT FIELD n_folio
            ELSE
                IF NOT Rescata_datos("C",r_afi.n_seguro,r_afi.n_unico) THEN
                    ERROR "Afiliado NO existe"
                    NEXT FIELD n_seguro
                END IF                
                ...
                PREPARE stmt2 FROM v_sql_1
                
                LET v_crea_fecha = HOY

                DECLARE curs2 CURSOR FOR stmt2
                   OPEN curs2 USING r_afi.n_seguro, v_crea_fecha
                  FETCH curs2 INTO v_existe, v_edad, v_criterio, v_ind_edad, v_curp, v_rfc, v_fena
                  CLOSE curs2
        ...
        ON KEY (F2)
            CALL cons_foledocta(r_afi.n_seguro,r_afi.n_folio,r_afi.tipo_solicitud)
        ...
        ON KEY ( CONTROL-E,CONTROL-T,CONTROL-B,CONTROL-V,
                 CONTROL-P,CONTROL-G,CONTROL-J,CONTROL-N)
            ...
            -- si se captura ICEFAS 
            IF KEY = 16 THEN
               IF r_afi.tipo_solicitud = 27 THEN
                  ERROR "NO SE PUEDE CAPTURAR PARA REG MENORES DE EDAD"
                  RETURN
               END IF	
            END IF  
            CASE KEY
                 ...
                 WHEN 5
                     LET comma = "fglgo AFIM002.4gi ", r_afi.n_folio," ",
                                 ...
                                 r_afi.tipo_solicitud
                 WHEN 14
                     CALL fctipo_admon(r_afi.n_unico)
                 WHEN 16
                     LET comma = "fglgo AFIM025 ",
                                 r_afi.n_folio, " ",
                                 r_afi.tipo_solicitud," ",
...
FUNCTION Rescata_datos(l_aux_val,x_valor,x_valor1)
#rd-----------------------------------------------

    DEFINE l_aux_val  CHAR(1)
    DEFINE x_valor    CHAR(11)
    DEFINE x_valor1   CHAR(18)
    DEFINE xx_num     DECIMAL(11,0)
    DEFINE vmax_fact  DATE
    DEFINE vmax_rowid INTEGER
      ...
      WHEN "U"
        ...
        INTO   r_afi.codven,
               ...
               x_finicta,
	             m_reenvio
        FROM   afi_solicitud a, 
        OUTER  afi_ctr_actividad b
        WHERE  a.n_unico = x_valor1
        AND    a.tipo_solicitud = r_afi.tipo_solicitud
    ...
    SELECT a.tipo_trab_ind, b.desc_tipo_trab_ind
      INTO r_afi.tipo_trab_ind, r_afi.desc_tipo_trab_ind
      FROM cta_ctr_reg_ind a, tab_tipo_trab_ind b
     WHERE a.nti           = r_afi.n_seguro
       AND a.curp          = r_afi.n_unico
    ...
    CASE m_reenvio
       WHEN '0' LET d_reenvio = 'DEVOLUCION A REENVIAR'
       WHEN '1' LET d_reenvio = 'ERROR DE CAPTURA'
       #WHEN '2' LET d_reenvio = 'CONFIRMACION DE DATOS'
    END CASE

    IF m_reenvio IS NOT NULL THEN
       DISPLAY " Indicador de Reenvio : ", d_reenvio AT 13,2
       ATTRIBUTES(REVERSE)
    END IF
...
---<
    ...
    EXECUTE con_act INTO r_afi.fecha_activ
    
    IF SQLCA.SQLCODE = NOTFOUND THEN 
    	 LET r_afi.fecha_activ = ""
    END IF 	        
    -- CPL-3165 
...
#------------------
    ...
    DISPLAY "[E]Dom [T]Tels [B]Benef [V]Pat [Y]Ident [W]Obs [N]Tip Admon [O]Mot.Rech   " AT 2,1 ATTRIBUTE(BOLD)
...
--                ERROR "Tipo Solic. solo puede ser 1)Registro 2)Traspaso 8) Independientes"
            IF (r_afi.tipo_solicitud = 1   OR
                r_afi.tipo_solicitud = 2   OR  
                r_afi.tipo_solicitud = 3   OR  
                r_afi.tipo_solicitud = 6   OR  
                r_afi.tipo_solicitud = 7   OR  
                r_afi.tipo_solicitud = 8   OR
                r_afi.tipo_solicitud = 10  OR
                r_afi.tipo_solicitud = 11  OR
                r_afi.tipo_solicitud = 12  OR
                r_afi.tipo_solicitud = 27                  
                ) THEN 
            ELSE
               ERROR "Tipo Sol. solo 1)Reg 2)Trasp",
                     "3)Uni 6-7)Sep 8)No Afi 10)Act y 11-12)Reg Int 27)Menores"
                LET r_afi.tipo_solicitud = NULL
                DISPLAY BY NAME r_afi.tipo_solicitud
                NEXT FIELD tipo_solicitud
            END IF
...
# FECHA DE NACIMIENTO
        BEFORE FIELD fena
          ...
          IF r_afi.n_unico IS NOT NULL             AND
             r_afi.n_unico <> '                  ' AND
             r_afi.n_unico NOT MATCHES "[ *]"      AND
             r_afi.n_unico NOT LIKE " %"           THEN
             LET x_fecha  = r_afi.n_unico[7,8], "/",
                            r_afi.n_unico[9,10],"/",
                            "19",
                            r_afi.n_unico[5,6]
        ...
        AFTER FIELD fena
           ...
           IF a_yo > 120 THEN
              ERROR "Esta persona pasa del rango de 120 ayos, ",
           ...
           IF xx_fecha <> r_afi.fena THEN
              WHILE TRUE
                 PROMPT "Existen inconsistencias en ",
                        "la Fecha nacimiento, es correcto ¿[S/N]? " FOR enter
                 IF enter MATCHES "[Ss/Nn]" THEN
                    IF enter MATCHES "[Ss]" THEN
                       NEXT FIELD sexo
                       EXIT WHILE
...
# N_RFC
        AFTER FIELD n_rfc
            ...
            CALL arma_clave_rfc(r_afi.paterno,
                                r_afi.materno,
                                r_afi.nombres,
                                r_afi.fena) RETURNING rfc_arma

            IF rfc_arma[6,10] != r_afi.n_rfc[6,10] THEN
               IF bnd_fena = 2 THEN
                  ERROR "Existe diferencia en la edad con RFC"
                  WHILE TRUE
                     PROMPT "Es correcta la inconsistencia ¿[S/N]? " FOR enter
                     IF enter MATCHES "[Ss/Nn]" THEN
                        IF enter MATCHES "[Ss]" THEN
                           EXIT WHILE
                        ELSE
            ...
            IF rfc_arma[1,4] != r_afi.n_rfc[1,4] THEN
               LET rfc_arma = rfc_arma[1,4]
               ERROR "El inicio del rfc debe ser: ", rfc_arma
               WHILE TRUE
                  PROMPT "Dejar inicio de rfc encontrado [S/N]? " FOR enter

                  IF enter MATCHES "[Ss/Nn]" THEN
                     IF enter MATCHES "[Ss]" THEN
                        LET r_afi.n_rfc = rfc_arma CLIPPED,r_afi.n_rfc[5,13]
...
# ERROR ORIGEN
        ...
        WHILE TRUE
            ...
            IF aux_pausa MATCHES "[SsNn]" THEN
            	IF r_afi.tipo_solicitud = 2 or r_afi.tipo_solicitud= 15 THEN
            	            CALL ind_36_mes(r_afi.n_seguro,r_afi.n_folio,r_afi.tipo_solicitud)   
            	END IF             
                EXIT WHILE
            END IF
        END WHILE
        ...
        IF aux_pausa MATCHES "[Nn]" THEN
            RETURN
        ELSE
          --IF r_afi.tipo_solicitud = 2 AND   ---circ unica
             --su_estatus = 40 THEN          ---circ cunica

             {SELECT status_captura
               INTO vstatus_captura
               FROM afi_solicitud
              WHERE n_folio        = r_afi.n_folio
                AND tipo_solicitud = r_afi.tipo_solicitud}

            --WHILE TRUE                    ---circ unica
                {IF vstatus_captura = 442 THEN
                   PROMPT "[1]Error captura" ATTRIBUTES (REVERSE) FOR vreenv
                   IF vreenv  MATCHES "[1]" THEN
                       EXIT WHILE
                   ELSE
                       ERROR "Solo debe presionar [1]"
                       SLEEP 3
                       ERROR " "
                   END IF
                ELSE}
                   {PROMPT "MARCA REENVIO [0]Reenvio dev./[1]Error captura/",
                          "[2]Confirma datos: " ATTRIBUTES (REVERSE) FOR vreenv
...
{ ---circ unica
                   PROMPT "MARCA REENVIO [0]Reenvio dev./[1]Error captura/"
                          ATTRIBUTES (REVERSE) FOR vreenv
                   IF vreenv  MATCHES "[01]" THEN
                       EXIT WHILE
                   ELSE
                       ERROR "Solo debe presionar [0] o [1]"
                       SLEEP 3
                       ERROR " "
                   END IF
                #END IF
            ...
            IF r_afi.tipo_solicitud = 1 OR
               ...
               r_afi.tipo_solicitud = 8 THEN
                ...
                IF SQLCA.SQLCODE = 0 THEN
                    DISPLAY r_afi.desc_solicitud
                ELSE
                    ERROR "Tipo Solic. solo puede ser 1)Reg 2)Trasp 8)Indep."
                    LET r_afi.tipo_solicitud = NULL
                    NEXT FIELD tipo_solicitud
                END IF
            END IF
...
#rs---------------------------
    ...
    SELECT fentcons , finicta, fecha_envio, fecha_1a_afil
    INTO   x_fecha, x_finicta, x_fecha_envio, x_1a_afi
    FROM   afi_solicitud
    WHERE  n_seguro = r_afi.n_seguro
    AND    n_folio  = r_afi.n_folio
    AND    tipo_solicitud = r_afi.tipo_solicitud

    DISPLAY "F.Env. ",x_fecha_envio USING "dd-mm-yyyy" AT 13,1
    ATTRIBUTE(REVERSE)

    IF x_finicta IS NOT NULL THEN
        DISPLAY "F.Ap.Cta ",x_finicta USING "dd-mm-yyyy"
        AT 13,20 ATTRIBUTE(REVERSE)
    END IF

    DISPLAY "F.Cert ",x_fecha USING "dd-mm-yyyy" AT 13,41 ATTRIBUTE(REVERSE)

    DISPLAY "F.1a.Af ",x_1a_afi USING "dd-mm-yyyy" AT 13,60 ATTRIBUTE(REVERSE)
...
#mr----------------------
    ...
    INSERT INTO tmp_mot_rch
    SELECT diag_proceso, desc_rech, cve_ced_cuenta, '', fecha_presentacion,
           ''
      FROM taa_det_devol, 
            tab_dev_taa
     WHERE n_seguro          = r_afi.n_seguro
    ...
    INSERT INTO tmp_mot_rch
    SELECT diag_imagen[1,2], desc_rech, cve_ced_cuenta, '', fecha_presentacion,
           ''
      FROM taa_det_devol, 
            tab_dev_taa
     WHERE n_seguro            = r_afi.n_seguro
    ...
    INSERT INTO tmp_mot_rch
    SELECT diag_imagen[3,4], desc_rech, cve_ced_cuenta, '', fecha_presentacion,
           ''
      FROM taa_det_devol, 
            tab_dev_taa
     WHERE n_seguro            = r_afi.n_seguro
    ...
    INSERT INTO tmp_mot_rch
    SELECT diag_imagen[5,6], desc_rech, cve_ced_cuenta, '', fecha_presentacion,
           ''
      FROM taa_det_devol, 
            tab_dev_taa
     WHERE n_seguro            = r_afi.n_seguro
    ...
    INSERT INTO tmp_mot_rch
    SELECT diag_imagen[7,8], desc_rech, cve_ced_cuenta, '', fecha_presentacion,
           ''
      FROM taa_det_devol, 
            tab_dev_taa
     WHERE n_seguro            = r_afi.n_seguro
    ...
    INSERT INTO tmp_mot_rch
    SELECT diag_imagen[9,10], desc_rech, cve_ced_cuenta, '', fecha_presentacion,
           ''
      FROM taa_det_devol, 
            tab_dev_taa
     WHERE n_seguro            = r_afi.n_seguro
    ...
    SELECT 14, 'NO ATENDIDA', cve_ced_cuenta, '', fecha_presentacion, ''
     ...
     WHERE n_seguro = r_afi.n_seguro
       AND rech_proc_operat[1] = " ";

    INSERT INTO tmp_mot_rch
    SELECT rech_proc_operat, '', cve_ced_cuenta, '', fecha_presentacion,
           'OP 14 NO ATENDIDA'
      FROM taa_det_no_aten
     WHERE n_seguro = r_afi.n_seguro
       AND rech_proc_operat[1] <> " "; 
...
#iec----------------------------------------------------
    ...
    DEFINE aux_pausa   CHAR(1)
    DEFINE x_num_folio DECIMAL(10,0)
            ...
            IF r_afi.tipo_solicitud = 1 OR
               ...
               r_afi.tipo_solicitud = 27 THEN
                ...
                IF SQLCA.SQLCODE = 0 THEN
                    DISPLAY r_afi.desc_solicitud
                ELSE
                    ERROR "Tipo Solic. solo puede ser 1)Registro 2)Trasp 27)Menor"
                    LET r_afi.tipo_solicitud = NULL
                    NEXT FIELD tipo_solicitud
                END IF
            ELSE
            ...
            IF r_afi.tipo_solicitud = 1 OR
               ...
               r_afi.tipo_solicitud = 27 THEN
                ...
                IF SQLCA.SQLCODE = 0 THEN
                    DISPLAY r_afi.desc_solicitud
                ELSE
                    ERROR "Tipo Solic. solo puede ser 1)Registro 2)Trasp 8) Indep 27)Menor"
                    LET r_afi.tipo_solicitud = NULL
                    NEXT FIELD tipo_solicitud
                END IF
            END IF
...
#e----------------
    ...
    ON KEY (CONTROL-E)
            ...
            IF xx_status > 15 THEN
                ERROR "NO PUEDE ELIMINAR,", vstatus_desc
...
#di-------------------------
    ...
    FOREACH cur_cur INTO rec_1[seguro_cnt].n_unico,
...
#mf---------------------
  ...
  DISPLAY "" AT 1,1
  DISPLAY "" AT 2,1
  DISPLAY " MODIFICA CURP " AT 1,63 ATTRIBUTE(REVERSE)


  INPUT BY NAME r_afi.tipo_solicitud, r_afi.n_folio,
                r_afi.n_unico WITHOUT DEFAULTS
    ...
    AFTER FIELD tipo_solicitud
      ...
      IF SQLCA.SQLCODE = 0 THEN
      ELSE
        ERROR "Tipo Solic. solo puede ser 1)Reg. 2)Trasp. 3)Unif.",
              " 4)Virtual 5)Asig 8)Ind "
        LET r_afi.tipo_solicitud = NULL
        NEXT FIELD tipo_solicitud
      END IF

      IF r_afi.tipo_solicitud <> 8 THEN
         ERROR "No puede modificarse el CURP para este tipo de solicitud."
         NEXT FIELD tipo_solicitud
      END IF
    ...
    AFTER FIELD n_folio
      ...
      ELSE
         ...
         DISPLAY BY NAME r_afi.*
         LET curp_ant = r_afi.n_unico
      END IF

      IF su_estatus <= 20 THEN
         PROMPT "Ingrese la nueva clave unica de registro poblacional : "

         ATTRIBUTES (REVERSE) FOR curp_nvo

         IF curp_nvo <> '                  ' THEN
            IF LENGTH(curp_nvo) < 18 AND
               LENGTH(curp_nvo) > 0  THEN
               ERROR "Debe ingresar CURP completa"
               NEXT FIELD n_unico 
            ELSE
               IF curp_nvo[1] <> " " OR
                  curp_nvo IS NOT NULL THEN
                  IF curp_nvo[11] = "H" THEN
                     LET sexo_cur = "1"
                  ELSE
                     LET sexo_cur = "2"
                  END IF
            ...
            CALL valida_est_curp(curp_nvo) RETURNING pasa_curp, desc_err
            ...
            IF doc_prob_arma[1,4] != curp_nvo[1,4] THEN
                LET doc_prob_arma = doc_prob_arma[1,4]
                ERROR "El inicio de la curp debe ser: ", doc_prob_arma
                WHILE TRUE
                  PROMPT "¿Dejar inicio de curp encontrado [S/N]? " FOR enter
                  IF enter MATCHES "[Ss/Nn]" THEN
                     IF enter MATCHES "[Ss]" THEN
                        LET curp_nvo = doc_prob_arma CLIPPED, curp_nvo[5,18]
                        DISPLAY BY NAME r_afi.n_unico
                        EXIT WHILE
                     ELSE
                        LET vrfc_orig = curp_nvo[1,4]
                        ...
                        IF SQLCA.SQLCODE = 0 THEN
                           LET r_afi.n_unico = vrfc_si, curp_nvo[5,18]
                           DISPLAY BY NAME r_afi.n_unico
                        END IF
            ...
            CALL var_dig_curp(curp_nvo) RETURNING pasa, dig_curp
         ...
         LET x_fecha  = curp_nvo[7,8], "/",
                        curp_nvo[9,10],"/",
                        "19",
                        curp_nvo[5,6]

         IF x_fecha <> r_afi.fena THEN
            PROMPT "Existen inconsistencias en la Fecha nacimiento , ",
                   "es correcto ¿[S/N]? "  ATTRIBUTES (REVERSE)  FOR enter
            IF enter MATCHES "[Ss/Nn]" THEN
               IF enter MATCHES "[Ss]" THEN
               ELSE
                  NEXT FIELD n_unico 
         ...
         IF STATUS = NOTFOUND THEN
            ...
            IF STATUS = NOTFOUND THEN
               ...
               SET    curp           = curp_nvo
               WHERE  curp           = r_afi.n_unico
               AND    nti            = r_afi.n_seguro

               WHENEVER ERROR CONTINUE
                 UPDATE afi_recepcion
                 SET    n_seguro       = curp_nvo
                 WHERE  n_seguro       = r_afi.n_unico
                 AND    n_folio        = r_afi.n_folio
                 AND    tipo_solicitud = r_afi.tipo_solicitud
               ...
               LET r_afi.n_seguro = curp_nvo

               LET operacion = 'MODIFICA CURP'

               CALL inserta_logico(r_afi.n_folio, r_afi.tipo_solicitud,
                                   r_afi.n_seguro, su_estatus, g_usuario,
                                   operacion)

               IF NOT Rescata_datos("U",r_afi.n_folio,r_afi.n_unico) THEN
                  ERROR "Afiliado NO existe"
                  NEXT FIELD n_folio
               END IF
            ...
            END IF
         ELSE
            ERROR "CURP ya ingresada anteriormente"
            SLEEP 3
            EXIT INPUT
         END IF
      ELSE
         PROMPT "No se puede modificar la CURP" FOR enter
         EXIT INPUT
      END IF
...
FUNCTION ins_foledocta()

    DEFINE g_edocta RECORD
      folio_edocta CHAR(20),
      cuatrim_emision DECIMAL(5,0)
    END RECORD
--->FOLIO EDO CTA
        IF r_afi.tipo_solicitud = 2 THEN
            LET ban_foledocta = 0
            OPEN WINDOW vent_folio AT 7,8 WITH FORM "AFIM00193" 
            ATTRIBUTE(BORDER)

            DISPLAY "                FOLIO DEL ESTADO DE CUENTA                     " AT 2,2 
            --ATTRIBUTE(REVERSE)

            INPUT BY NAME g_edocta.*
            AFTER FIELD folio_edocta

            IF g_edocta.folio_edocta IS NULL OR
               g_edocta.folio_edocta = '' THEN 
                ERROR "Debe capturar el Folio del Estado de Cuenta..."
                NEXT FIELD folio_edocta
            END IF

            PROMPT "Esta seguro del folio capturado? S/N " FOR enter2

            IF enter2 MATCHES "[Ss]" THEN
{
               INSERT INTO afi_folio_edocta
               VALUES (r_afi.n_seguro,
                       r_afi.n_folio,
                       r_afi.tipo_solicitud,
                       g_edocta.folio_edocta,
                       '',                    #grado_riesgo
                       '',                    #vector_riesgo
                       '',                    #id_reg_env_verif
                       '',                    #cod_operacion
            ...
            AFTER FIELD cuatrim_emision

            IF g_edocta.cuatrim_emision IS NULL OR
               g_edocta.cuatrim_emision = '' THEN
                ERROR "Debe capturar el Cuatrimestrte de Emisión del Estado de Cuenta..."
                NEXT FIELD cuatrim_emision
            END IF

            PROMPT "Esta seguro del Cuatrimestre capturado? S/N " FOR enter2
           ...
           ON KEY(INTERRUPT, CONTROL - C, CONTROL - Z)
            ...
            CLOSE WINDOW vent_folio
        END IF
---<
END FUNCTION

FUNCTION cons_foledocta(nss,folio_sol,ts)
#nic------------------------------------
    ...
    IF SQLCA.SQLCODE <> 0 THEN
        ...
        RETURN
    ELSE
        SELECT a.folio_edo_cta,a.cuatrim_emision
          INTO re_g.*
          FROM afi_folio_edocta a
         WHERE a.nss            = nss
    ...
    OPEN WINDOW v123 AT 6,6 WITH FORM "AFIM00193"
    ATTRIBUTE(BORDER)
    DISPLAY "                                                               " AT 1,1 ATTRIBUTE(REVERSE)
    DISPLAY "              CONSULTA FOLIO DEL ESTADO DE CUENTA              " AT 2,2 
            --ATTRIBUTE(REVERSE)

    DISPLAY BY NAME re_g.*        --AT 8,26 ATTRIBUTE(REVERSE)
...
FUNCTION ind_36_mes(nss,folio_sol,ts)
    ...
    DEFINE
        ...
        ts        SMALLINT
           ...
           OPEN WINDOW ventana_x AT 8,6 WITH 5 rows, 68 COLUMNS ATTRIBUTE (BORDER)
           DISPLAY "            INDICADOR DE 36 MESES                " AT 1,1 
           ATTRIBUTE (REVERSE)
           WHILE TRUE 

             PROMPT "Esta seguro de COLOCAR INDICADOR ? S/N " FOR enter
             IF enter MATCHES "[SsNn]" THEN
      		...
      		IF SQLCA.SQLCODE = 0 THEN
         		LET reg_actualizados = reg_actualizados + 1
         
         		UPDATE afi_ctr_mas_dos_tra
         		SET    usuario =  g_usuario, 
         		       factualiza = HOY
         		WHERE  nss = nss
      		ELSE
         		LET reg_nvos = reg_nvos + 1
                 ...
                 HOY                            )
      		END IF                
                   ERROR "viajará en archivo con INDICADOR  DE MAS DE DOS TRASPASOS EN 36 M"
                   SLEEP 3
                   ERROR ""
                   EXIT WHILE
                ELSE                
