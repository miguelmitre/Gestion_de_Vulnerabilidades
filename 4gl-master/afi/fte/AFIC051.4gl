...
GLOBALS
    ...
    DEFINE fecha_marca DATE
    DEFINE st_marca    SMALLINT
    DEFINE bnd_marca   SMALLINT
END GLOBALS
MAIN
...
#pp-------------------------
    ...
    OPEN WINDOW ventana_1 AT 4,4 WITH FORM "AFIC0511" ATTRIBUTE(BORDER)
    ...
    INPUT BY NAME g_reg.proceso, g_reg.generar
	AFTER FIELD proceso
	    NEXT FIELD generar
...
---- verifica si esta en proceso de modificacion
        ...
        ELSE
	        ...
	        SELECT ctam.fecha_ini
	        INTO   fecha_marca
                FROM   cta_act_marca ctam
                WHERE  ctam.nss = vn_seguro
                AND    ctam.marca_cod = 605

                IF SQLCA.SQLCODE = 0 THEN
                    IF fecha_marca < vfecha_asig THEN
                        LET st_marca = 1
                    END IF
                END IF
            END IF
    ...
    IF NOT bnd_proceso THEN
        ...
        DISPLAY "Registros con CURP aceptada     : ",
                 vaprobados USING "#######&" AT 8,15 ATTRIBUTE ( BOLD )

        DISPLAY "Registros con CURP rechazada    : ",
...
#da--------------------------
    ...
    OPTIONS
        ...
        DISPLAY " AFIC051        CONSULTA ARCHIVOS DE DISPERSION DE CURP                        " AT 3,1 
        ATTRIBUTE(REVERSE) 
