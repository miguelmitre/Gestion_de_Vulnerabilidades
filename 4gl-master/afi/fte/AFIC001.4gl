...
#Propietario       => E.F.P.                                              #
#Programa AFIC001  => CARGA DE ARCHIVO GENERADO POR OPERADORA BDNSAR      #
#Sistema           => AFI.                                                #
...
#Modificado        => MARCO ANTONIO GONZALEZ ROJAS                        #
#Fecha             => 14 MARZO DEL 2007.                                  #
#Obj. de la Mod.   => En la Funcion de Aceptado sustitur la Funcion  de   #
#                     habil_siguiente.                                    #
#                  => EDUARDO JOAQUIN RESENDIZ MEDINA                     #
#                  => 24 AGOSTO 2010 CIRCULAR UNICA                       #
###########################################################################
...
GLOBALS
    ...
    DEFINE reg_det RECORD
        ...
        fecha_elaboracion   CHAR(08)     ,                     ---CIRC UNICA
        ...
        fol_aprob            CHAR(10)     ,
        doc_aprob            CHAR(16)     ,
        cve_afo_nss_asoc     CHAR(03)     ,
        ident_tip_trab       CHAR(01)     ,
...
#pp-------------------------
    ...
    DISPLAY " AFIC001      CARGA ARCHIVO RESPUESTA CERTIF. (SOLIC. REG.)                    " AT 3,1 ATTRIBUTE(REVERSE)
...
#-------------------------
    ...
    FOREACH cursor_1 INTO carga_reg
        ...
        IF cont_reg <> total_reg AND cont_reg <> 1 THEN
            ...
            LET campo_49 = carga_reg[526,527]
            LET campo_491= carga_reg[528,529]
            LET campo_492= carga_reg[530,531]
            LET campo_493= carga_reg[532,533]
            LET campo_50 = carga_reg[534,541]
            LET campo_51 = carga_reg[542,547]
            LET campo_52 = carga_reg[548,554]
            LET campo_53 = carga_reg[555,565]                 ---CIRC UNICA
...
# 29-11-2004
        ...
        IF reg_det.cod_operacion = "02" OR
           ...
           reg_det.cod_operacion = "08" THEN
            ...
            DECLARE cons CURSOR FOR qry_consul
            FOREACH cons INTO x_reg.*
...
#a-----------------
    ...
    CALL cal_fecha_avanza_1(xx_fecha,1)  RETURNING  diaSig

    CALL inserta_afi_ctr(60)

    UPDATE afi_solicitud 
    SET    status_interno     = 60,
           finicta            = diaSig,      #Mod. 14-03-2007 =>  + 1 UNITS DAY,
...
#t-----------------
    ...
    CALL inserta_afi_ctr(65)   #cambia a 65 para que se aperturen mas tardar 2 días desp de la cert 
...
#dr----------------------------
    ...
    ELSE
        ...
        DISPLAY "Registros pendientes asig. : ",
            pend_90 USING "#######&" AT 16,15 ATTRIBUTE ( BOLD ) 
    ...
    LET G_IMPRE = g_paramgrales.ruta_listados CLIPPED,"/",g_usuario CLIPPED,
                  ".CIF1_LIS_",hoy USING "DDMMYYYY", "_",hora CLIPPED
    START REPORT rep_cif_ctrl TO G_IMPRE
       OUTPUT TO REPORT rep_cif_ctrl(total_resp, aceptar, traspasar, rechazar,
                                     pendiente, aclaracion, pend_90)
    FINISH REPORT rep_cif_ctrl
...
#ae---------------------
    ...
    FOR i = 1 TO 6
        CASE i
            ...
            WHEN 5 LET st_int = 65         #cambia a 65 para que se aperturen mas tardar 2 días desp de la cert
                   LET total  = traspasar
...
#iac----------------------
    ...
    INSERT INTO afi_ctr_solicitud VALUES 
{	    (1, reg_det.folio_solicitud, reg_det.nss_solicitud,
	    nom_comp, vstatus_interno, vfecha_envio, hoy,
            reg_det.cve_afo_ced, reg_det.ind_nss_modificado,
            xx_fecha, reg_det.curp_oficial, reg_det.ind_curp_modif,
            reg_det.n_rfc_bd, reg_det.nombre_bd, reg_det.nombre_pcanase,
            fnbd, reg_det.codven_bd, reg_det.sexo_bd, reg_det.estadon_bd,
            fpafil, falta, reg_det.cve_afore, reg_det.nacionalidad_bd,
            reg_det.tip_aprob_bd, reg_det.fol_aprob_bd, reg_det.doc_aprob_bd,
	    vindicador_envio, vind_nombre, reg_det.cod_operacion, 
	    reg_det.diag_proceso, vagenc_cod, g_usuario, hoy);
}
	    (1, 
    ...
    IF reg_det.nss_asoc IS NOT NULL AND 
       reg_det.nss_asoc <> "           " THEN
       SELECT 'X'
...
#################################################################
REPORT rep_cif_ctrl(ltotal_resp, laceptar, ltraspasar, lrechazar, lpendiente,
                    laclaracion, lpend_90)
   ...
   FORMAT
      ...
      FIRST PAGE HEADER
         ...
         PRINT COLUMN 01, lafore CLIPPED,
               COLUMN 68, hoy USING "DD-MM-YYYY"
         SKIP 2 LINE
         PRINT COLUMN 07,"-------------------------------------------------------------------"
         PRINT COLUMN 08,"CIFRAS CONTROL CERTIFICACION DE SOLICITUDES DE AFILIADOS"
         PRINT COLUMN 07,"-------------------------------------------------------------------"
         SKIP 2 LINE

      ON EVERY ROW
         PRINT COLUMN 01, "TOTAL REGISTROS RECIBIDOS"
         SKIP 2 LINE

         PRINT COLUMN 01, "Total de Registros del lote     : ", ltotal_resp
                          USING "#######&"
         PRINT COLUMN 01, "Registros aceptados             : ", laceptar
                          USING "#######&"
         PRINT COLUMN 01, "Registros cert. por trasp.      : ", ltraspasar
                          USING "#######&"
         PRINT COLUMN 01, "Registros rechazados            : ", lrechazar
                          USING "#######&"
         PRINT COLUMN 01, "Registros pendientes            : ", lpendiente
                          USING "#######&"
         PRINT COLUMN 01, "Registros en aclaracion         : ", laclaracion
                          USING "#######&"
         PRINT COLUMN 01, "Registros pendientes asig.      : ", lpend_90
                          USING "#######&"

END REPORT
#################################################################
FUNCTION cal_fecha_avanza_1(x_fecha,ciclo)
#cf-------------------------------
