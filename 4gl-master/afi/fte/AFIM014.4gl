...
#Propietario        > EFP                                                #
#Programa AFIM014  => MANTENIMIENTO DE BENEFICIARIOS                     #
...
#pp-------------------------
    ...
    DISPLAY " AFIM014                B E N E F I C I A R I O S                               " AT 3,1 ATTRIBUTE(REVERSE)
...
#m-----------------
	...
	AFTER FIELD paren_cod
        ...
        ON KEY ( ESC )
            FOR i = 1 TO 10
                IF x_benef[i].paren_cod IS NOT NULL THEN
            	    UPDATE afi_mae_benefici 
                    SET    afi_mae_benefici.parentesco = x_benef[i].paren_cod,
                           afi_mae_benefici.porcentaje = x_benef[i].porcentaje,
                           afi_mae_benefici.paterno    = x_benef[i].pat_benef,
                           afi_mae_benefici.materno    = x_benef[i].mat_benef,
                           afi_mae_benefici.nombres    = x_benef[i].nom_benef,
                           afi_mae_benefici.sexo       = x_benef[i].sexo_cod,
                           afi_mae_benefici.fena       = x_benef[i].fena_benef
                    WHERE  afi_mae_benefici.n_seguro   = g_afili.n_seguro
		    AND    afi_mae_benefici.n_folio    = g_afili.n_folio
		    AND    afi_mae_benefici.tipo_solicitud 
                                                       = g_afili.tipo_solicitud
                    AND    afi_mae_benefici.consec     = x_benef[i].consec  
                END IF
            END FOR
    ...
    IF (a-1) >= 1 THEN
        ...
        ON KEY ( CONTROL-b )
            ...
            IF aux_pausa MATCHES "[Ss]" THEN 
                ...
                WHERE  afi_mae_benefici.n_seguro = g_afili.n_seguro
                AND    afi_mae_benefici.n_folio  = g_afili.n_folio
                AND    afi_mae_benefici.tipo_solicitud = g_afili.tipo_solicitud
                AND    afi_mae_benefici.consec   = x_benef[a].consec
