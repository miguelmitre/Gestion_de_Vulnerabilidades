...
#Propietario       => E.F.P.                                                  #
#Programa SEPM001  => MANTENIMIENTO DE SOLICITUDES DE AFILIACION              #
#Autor             => FERNANDO HERRERA HERNANDEZ (VERSION 2)                  #
#Sistema           => AFI                                                     #
#Modificado        => FERNANDO HERRERA HERNANDEZ (CIRC 28-8)                  #
#Fecha             => 26 DE JULIO DE 2004                                     #
#Modificado        => JESUS YANEZ MORENO                                      #
#Fecha             => 18 DE NOVIEMBRE DE 2004                                 #
#Modificado        => JESUS YANEZ MORENO                                      #
#Fecha             => 25 DE SEPTIEMBRE DE 2004                                #
###############################################################################
#Modificado        => Se comentan campos 'modulo_cod' Feb 2012 --> ACS        #
###############################################################################
# fecha Actuali   => CPL-1897 19/02/2015                                       #
# Autor           => Cristian  Morales Roblero                                 #
# Actualizacion   => se corrigue el query para que regrese regrese un solo     #
#                    renglon y se garega validacion para la correcta salida    #
#                    en caso de NO querer seguir con la captura               #
################################################################################
# fecha Actuali   => CPL-3919 06/12/2022                                       #
# Autor           => Jesus David YaÃnez Moreno                                 #
# Actualizacion   => Se modifica la funciÃon SEPGLOBALS para permitir la captura
#                    del punto como caracter valido en el nombre en la sol de  #
#                    registro o trapaso por separaciÃ³n de cuentas            #
################################################################################
...
GLOBALS
    ...
    DEFINE tipo_consulta              ,
           nss_consulta     CHAR(11)  ,
           id_consulta      DEC(10,0) ,
           id_bitacora      DEC(10,0)

    DEFINE g_idsolicitudseparacion   DEC(10,0)
    DEFINE g_idbitacorawsoperacion3334 DEC(10,0)
    DEFINE g_seq_sep_bitacora_wsoperacion33 DEC(10,0)
    DEFINE g_seq_sep_bitacora_wsoperacion34 DEC(10,0)
...
MAIN
    ...
    LET tipo_consulta = ARG_VAL(1)
    LET nss_consulta  = ARG_VAL(2)
    LET id_consulta   = ARG_VAL(3) --idsolicitudseparacion
    LET id_bitacora   = ARG_VAL(3) --idbitacorawsoperacion33
    LET tipo_solic    = ARG_VAL(4)

    IF tipo_consulta = 1 THEN
       LET g_idsolicitudseparacion   = id_consulta
       LET g_idbitacorawsoperacion3334 = id_bitacora
       LET ACCION = "C"
       CALL Consulta(nss_consulta,id_consulta,id_bitacora)
       EXIT PROGRAM
    END IF

    CALL STARTLOG("SEPM001.log")
    CALL inicio()               #i
    CALL crea_tablas()          #ct
...
#i---------------
     ...
     CREATE TEMP TABLE sep_v_seq(id dec(10,0))
...
#pp-------------------------
      ...
      DISPLAY " SEPM001           MANTENIMIENTO SOLICITUDES AFILIACION SEP                    " AT  3,1 ATTRIBUTE(REVERSE)
      DISPLAY HOY USING "dd-mm-yyyy" AT 3,68 ATTRIBUTE(REVERSE)

   -- IF NOT llama_val THEN
        MENU "Sol reg"
         COMMAND KEY("E") "alta rEgistro" "Agregar Solicitud de Afiliacion x Registro"
               CALL despliega_nss("A",6)
         COMMAND KEY("L") "aLta traspaso" "Agregar Solicitud de Afiliacion x Traspaso"
               CALL despliega_nss("A",7)
         COMMAND KEY("R") "consulta registRo" "Consulta Solicitud Afiliacion x Registro"
               CALL despliega_nss("C",6)
         COMMAND KEY("P") "consulta trasPaso" "Consulta Solicitud Afiliacion x Traspaso"
               CALL despliega_nss("C",7)
       ...
       --         CALL Modifica_nss()
       --         CALL Inicializa()
       --     COMMAND "Promotor" "Modificar promotor Solicitud"
       --         LET ACCION = "S"
       --         CALL Modifica_pro()
       --         CALL Inicializa()
...
#in------------------
   ...
   IF NOT tipo_consulta THEN
     CLEAR FORM
   END IF

END FUNCTION

FUNCTION Agrega(l_nss,l_tipo_sol,l_cve_ced,l_idsolicitudseparacion)
#a-----------------------------------------------------------------
    DEFINE l_cve_ced        CHAR(003)
    DEFINE l_nss            CHAR(011),
	   l_tipo_sol       smallint,
           l_idsolicitudseparacion DEC(10,0) ,
           l_idbitacorawsoperacion33 DEC(10,0)
    ...
    LET g_idsolicitudseparacion  = l_idsolicitudseparacion
    ...
    INPUT BY NAME r_afi.* WITHOUT DEFAULTS
        ...
        AFTER FIELD paterno
           ...
           INITIALIZE val_1 TO NULL
           CALL verifica_nombre(r_afi.paterno) 
           RETURNING v_1, val_1
        ...
        AFTER FIELD materno
            ...
            INITIALIZE val_1 TO NULL
            CALL verifica_nombre(r_afi.materno) 
            RETURNING v_1, val_1
        ...
        AFTER FIELD nombres
            ...
            INITIALIZE val_1 TO NULL
            CALL verifica_nombre(r_afi.nombres) 
            RETURNING v_1, val_1
        ...
        ON KEY ( INTERRUPT, CONTROL-C, CONTROL-Z )
            ...
            IF r_afi.tipo_solicitud = 6 THEN
              DELETE FROM sep_bitacora_wsoperacion33 WHERE idbitacorawsoperacion33 = g_seq_sep_bitacora_wsoperacion33
            END IF
            IF r_afi.tipo_solicitud = 7 THEN
              DELETE FROM sep_bitacora_wsoperacion34 WHERE idbitacorawsoperacion34 = g_seq_sep_bitacora_wsoperacion34
            END IF
           ...
           END IF
            ...
            CASE KEY
                WHEN 2
                    ...
                    LET bnd_comma = TRUE
                WHEN 5
                    LET comma = "fglgo SEPM002.4gi ", r_afi.n_folio," ",
                ...
                WHEN 16
                    ...
                    LET bnd_comma = TRUE
                WHEN 20
                    LET comma = "fglgo SEPM003 ", r_afi.n_seguro," ",
                                ACCION CLIPPED," ", r_afi.tipo_solicitud,
                                r_afi.n_folio

                    LET bnd_comma = TRUE
                WHEN 22
                    LET comma = "fglgo SEPM005 ", r_afi.n_seguro," ",
                                ACCION CLIPPED," ", r_afi.tipo_solicitud,
                                r_afi.n_folio
...
#a-----------------
    ...
    DISPLAY " SEPM001           MODIFICACION  SOLICITUDES AFILIACION SEP                    " AT  3,1 ATTRIBUTE(REVERSE)
...
--
        ...
        ON KEY ( ESC )
            ...
            INITIALIZE g_comision.* TO NULL
            ERROR "REGISTRO MODIFICADO" SLEEP 3 ERROR ""
...
#is-------------------------
    ...
    DEFINE c_fecha          CHAR(8)
    DEFINE v_seq_sep_bitacora_wsoperacion33  DEC(10,0)
    DEFINE v_seq_sep_bitacora_wsoperacion34  DEC(10,0)
    DEFINE v_seq_bitacora    DEC(10,0)
    DEFINE c10_fecha          CHAR(10)
    DEFINE c10_fecha2         CHAR(10)
    DEFINE c2_estadon         CHAR(2)
    DEFINE l_clave_transferente CHAR(3)
    DEFINE ll_txt char(200)
    DEFINE c10_n_folio        CHAR(10)
    DEFINE vtip_prob          char(1)
    ...
    IF r_afi.tipo_solicitud = 6 THEN
       LET v_seq_sep_bitacora_wsoperacion33 = 0
       DELETE FROM sep_v_seq 
       INSERT INTO sep_v_seq VALUES (seq_sep_bitacora_wsoperacion33.nextval);
       SELECT id INTO v_seq_sep_bitacora_wsoperacion33 FROM sep_v_seq 
       LET g_seq_sep_bitacora_wsoperacion33 = v_seq_sep_bitacora_wsoperacion33
       LET v_seq_bitacora                   = v_seq_sep_bitacora_wsoperacion33
    END IF

    IF r_afi.tipo_solicitud = 7 THEN 
       LET v_seq_sep_bitacora_wsoperacion34 = 0
       DELETE FROM sep_v_seq 
       INSERT INTO sep_v_seq VALUES (seq_sep_bitacora_wsoperacion34.nextval);
       SELECT id INTO v_seq_sep_bitacora_wsoperacion34 FROM sep_v_seq 
       LET g_seq_sep_bitacora_wsoperacion34 = v_seq_sep_bitacora_wsoperacion34
       LET v_seq_bitacora                   = v_seq_sep_bitacora_wsoperacion34
    END IF
        ...
        VALUES(r_afi.n_seguro,
               ...
               r_afi.estadon,
               0,
               "0000000000" ,--r_afi.cod_promotor,
               r_afi.sexo,
               "0" ,--r_afi.n_operac,
               r_afi.frecafor,
               "",
               TODAY ,--r_afi.fecha_emision,
               "",
               "",
               0,
               "0" ,--r_afi.agenc_cod,
               ...
               0,
               r_afi.tipo_solicitud,
               TODAY,--r_afi.fecha_elaboracion,
               v_seq_bitacora , -- id de bitacora
               "",
               "0",--g_comision.cod_esq_comision,
               "",
               "",
               "",
               "",
               "",
               cod_err,
               "0",--r_afi.folio_edo_cta,
               r_afi.cod_afore_ced,
               "0",--r_afi.salario_base_comis,
               0,
               "",
               x_coduni,
               0,
               "0" ,--r_afi.cod_promotor,
               "",
               "",
               g_idsolicitudseparacion , --se pone el id de sep en folio_captura
               "")

     #-- Se agrega entrada para solicitar solicitud ws CPL-3611

           IF r_afi.tipo_solicitud = 6 THEN

               LET c_fecha = r_afi.fena using "YYYYMMDD"
               LET c2_estadon = r_afi.estadon USING "&&"
               IF r_afi.tip_prob <> 1 AND
                  r_afi.tip_prob <> 2 AND
                  r_afi.tip_prob <> 3 AND
                  r_afi.tip_prob <> 4 AND
                  r_afi.tip_prob <> 5 THEN
                   LET vtip_prob = ""
               ELSE
                   LET vtip_prob = r_afi.tip_prob
               END IF

               INSERT INTO sep_bitacora_wsoperacion33
                      (fechaalta                         , 
                       origensolicitudREQ                ,
                       claveentidadorigenREQ             ,  
                       idnegocioREQ                      ,
                       rfcREQ                            ,
                       apellidopaternoREQ                ,
                       apellidomaternoREQ                ,
                       ...
                       fechanacimientoREQ                ,
                       sexoREQ                           ,
                       entidadfederativanacimientoREQ    ,
                       nacionalidadREQ                   ,
                       clavetipodocumentoaprobatorioREQ  ,
                       idbitacorawsoperacion33           ,
               ...
               -- se actualizan los opcionales de nulos a vacio ''
                       UPDATE sep_bitacora_wsoperacion33 
                       SET   apellidomaternoREQ = '' 
                       WHERE idbitacorawsoperacion33 = v_seq_bitacora 
                       AND   apellidomaternoREQ is null

                       UPDATE sep_bitacora_wsoperacion33 
                       SET   clavetipodocumentoaprobatorioREQ = '' 
                       WHERE idbitacorawsoperacion33 = v_seq_bitacora 
                       AND   clavetipodocumentoaprobatorioREQ is null

                       UPDATE sep_bitacora_wsoperacion33 
                       SET  folioDocumentoProbatorioreq = '' 
                       WHERE idbitacorawsoperacion33 = v_seq_bitacora 
                       AND   folioDocumentoProbatorioreq is null

                       UPDATE sep_bitacora_wsoperacion33 
                       SET    documentoProbatorioreq = '' 
                       WHERE idbitacorawsoperacion33 = v_seq_bitacora 
                       AND   documentoProbatorioreq is null

                       IF vtip_prob = 1 THEN 
                          UPDATE sep_bitacora_wsoperacion33 
                          SET folioDocumentoProbatorioreq = "0000000000" ,
                              documentoProbatorioreq = "0000000000000000"
                          WHERE idbitacorawsoperacion33 = v_seq_bitacora 
                       END IF
               -- fin actualiza nulos
               END IF
             ...
             IF r_afi.tipo_solicitud = 7 THEN
                ...
                LET c10_fecha = r_afi.fena
                LET c10_fecha2 = c10_fecha[7,10],"/",c10_fecha[1,2],"/",c10_fecha[4,5]
                LET c2_estadon = r_afi.estadon USING"&&"     
                ...
                END IF
                 ...
                 SELECT max(rowid)
                 INTO gdet03_rowid
                 FROM sep_det03_solicitud 
                 WHERE idsolicitudseparacion = g_idsolicitudseparacion
                 AND   resultado_operacion = "01"

                 SELECT clave_entidad_involucrado 
                 INTO   l_clave_transferente 
                 FROM   sep_det03_solicitud 
                 WHERE  idsolicitudseparacion = g_idsolicitudseparacion
                 AND    rowid                 = gdet03_rowid
                 AND    resultado_operacion   = "01"
            
                 LET c10_n_folio = r_afi.n_folio USING "&&&&&&&&&&"
 
                 INSERT INTO sep_bitacora_wsoperacion34
                      ( fechaalta                      ,
                        origensolicitudREQ             ,
                        nsstrabajadorREQ               ,
                        curpREQ                        ,
                        ...
                        apellidomaternoREQ             ,
                        nombretrabajadorREQ            ,
                        claveaforetransferenteREQ      ,
                        claveaforereceptoraREQ         ,
                        fechanacimientotrabajadorREQ   ,
                        foliosolicitudREQ              ,
                        sexoREQ                        ,
                        entidadfederativanacimientoREQ ,
                        nacionalidadREQ                ,
                        clavetipodocumentoprobatorioREQ,
                        idbitacorawsoperacion34        ,
                        idsolicitudseparacion          ,
                        estado                         ,
                        usrsolicitud                  )
                 VALUES
               ...
               -- se actualizan los opcionales de nulos a vacio ''
                       ...
                       UPDATE sep_bitacora_wsoperacion34 
                       SET   apellidomaternoREQ = '' 
                       WHERE idbitacorawsoperacion34 = v_seq_bitacora 
                       AND   apellidomaternoREQ is null

                       UPDATE sep_bitacora_wsoperacion34 
                       SET   foliosolicitudREQ = '' 
                       WHERE idbitacorawsoperacion34 = v_seq_bitacora 
                       AND   foliosolicitudREQ is null

                       UPDATE sep_bitacora_wsoperacion34 
                       SET   claveTipoDocumentoProbatorioreq = '' 
                       WHERE idbitacorawsoperacion34 = v_seq_bitacora 
                       AND   claveTipoDocumentoProbatorioreq is null
...
FUNCTION Consulta(l_nss,l_idsolicitudseparacion,l_idbitacorawsoperacion3334)
#c-------------------------------------------------------------------------

    DEFINE l_nss   CHAR(011) 
    DEFINE l_idsolicitudseparacion   DEC(10,0)
    DEFINE l_idbitacorawsoperacion3334 DEC(10,0)
    ...
    LET g_idsolicitudseparacion   = l_idsolicitudseparacion
    LET g_idbitacorawsoperacion3334 = l_idbitacorawsoperacion3334

    OPEN WINDOW ventana_1 AT 2,2 WITH FORM "SEPM0011" ATTRIBUTE(BORDER)

    DISPLAY " SEPM001           CONSULTA SOLICITUD AFILIACION POR SEPARACION                " AT  3,1 ATTRIBUTE(REVERSE)
...
--    AT 1,1 ATTRIBUTE(CYAN)
    ...
    DISPLAY "CONTROL [E]Dom. [V]Patrones [Y]Identif [F]Modifica                "
...
#                    AND afi_ctr_identif.modulo_cod     = l_modulo
             END IF
    ...
    AFTER FIELD observacion
        ...
        ELSE
            UPDATE afi_ctr_observa
             ...
             WHERE afi_ctr_observa.n_seguro       = vn_seguro
