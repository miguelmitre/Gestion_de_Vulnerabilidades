...
#pp-------------------------
    ...
    DISPLAY " SEPB006     GENERACION ARCHIVO CERTIFICACION TRASPASO                         " AT 3,1 ATTRIBUTE(REVERSE)
    ...
    END INPUT
    ERROR ""
    PROMPT "[Enter] Para integrar el lote ..." ATTRIBUTE(REVERSE)
    FOR enter  
...
#rv-----------------------
    ...
    AND    a.tipo_solicitud = 7 #AFILIADO POR TRASPASO
    ...
    IF HAY_REGISTROS THEN
        ...
        WHERE  A.status_interno  = 10 #CON TODOS LOS DOCTOS
        AND    A.tipo_solicitud  =  7 --solicitud de traspaso
        AND    A.frecafor       <= fecha_hasta
        AND    A.n_seguro           = O.nss
        AND    A.n_folio            = O.n_folio
        AND    A.tipo_solicitud     = O.tipo_solicitud
        ...
        DISPLAY "Archivo : ", comm CLIPPED AT 17,1
        ...
        AND    afi_solicitud.tipo_solicitud = 7
...
#--------------------
    ...
    ON EVERY ROW
        ...
        IF (w_aux.n_unico IS NOT NULL AND 
            w_aux.n_unico <> " ") THEN
             LET w_aux.tip_prob = 5 
        ELSE 
             LET w_aux.tip_prob = " "
        END IF 
...
#ln--------------------
    ...
    LET comm = "cd " ,g_seg_modulo.ruta_envio CLIPPED,
               ...
               consec CLIPPED,".TAA > pso" 
    RUN comm

    LET comm = "cd " ,g_seg_modulo.ruta_envio CLIPPED,
               "/ ; mv pso  ","E",HOYDIA USING "yyyymmdd" CLIPPED,
               consec CLIPPED,".TAA " 
    RUN comm
