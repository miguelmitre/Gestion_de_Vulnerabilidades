...
#Propietario       => E.F.P.                                                    #
#Programa SEPM011  => MANTENIMIENTO DE AFILIADOS (SOLO DATOS DE PROCESAR)       #
...
#Sistema           => AFI.                                                      #
#################################################################################
#Modificado por    => Alejandro Chagoya Salazar  24-Julio-2012                  #
#                  => Se inhiben sentencias CLOSE WINDOW del input              #
#-------------------------------------------------------------------------------#
#Requerimiento     => CPL-1452    25-Nov-2013   Alejandro Chagoya Salazar       #
#Actualizacion     => Se modifica secuencia del input en funcion Modifica       #
#################################################################################
DATABASE safre_af
...
GLOBALS
    ...
    DEFINE g_master3 ARRAY [100] OF RECORD
        codven             LIKE afi_mae_afiliado.codven,
        nip                LIKE pro_mae_promotor.nip,
        desc_codven        CHAR(60),
        agenc_cod          CHAR(10),
        ...
        fecha_elaboracion  LIKE afi_mae_afiliado.fecha_elaboracion,
        folio_edo_cta      CHAR(8),
        cod_afore_ced      LIKE afi_his_afiliado.cod_afore_ced,
        desc_afore         CHAR(50),
        fecha_emision      DATE,
        n_folio            LIKE afi_mae_afiliado.n_folio,
        cod_operacion      SMALLINT,
        ...
        sexo               LIKE afi_mae_afiliado.sexo,
        desc_sexo          CHAR(60),
        edo_civil          LIKE afi_his_afiliado.edo_civil,
        desc_edo_civil     CHAR(60),
        fena               LIKE afi_mae_afiliado.fena,
        salario_base_comis LIKE afi_mae_afiliado.salario_base_comis,
        cod_esq_comision   LIKE afi_mae_afiliado.cod_esq_comision,
        ...
        desc_ind_info      CHAR(25),
        cod_error_origen   LIKE afi_mae_afiliado.cod_error_origen,
        const_curp         LIKE afi_his_afiliado.const_curp,
        folio_nvo          DECIMAL(8,0),
        usuario            CHAR(8),
        desc_03            CHAR(75),
        fechas_his         CHAR(75)
...
MAIN
    ...
    CALL STARTLOG(FGL_GETENV("USER")||".SEPM011.log")  #CPL-1452
    CALL inicio()
...
#pp-------------------------
      ...
      DISPLAY " SEPM001      MODIFICACION AFILIADO POR SEPARACION DE CUENTA                   " AT  3,1 ATTRIBUTE(REVERSE)
      DISPLAY HOY USING "dd-mm-yyyy" AT 3,68 ATTRIBUTE(REVERSE)

    IF NOT llama_val THEN
        MENU "MODIFICACION X SEPARACION DE CUENTAS"
      COMMAND "M antes afi" "Modif. Datos antes de procesos derivados"
                LET x_tipo = 5
                CALL despliega_nss("M","",x_tipo) #nueva para separacion

      COMMAND "C invadido antes afi " "Consulta Mod invadido antes proc afiliacion "
                LET x_tipo = 5
                CALL despliega_nss("D","",x_tipo) #nueva para separacion

      COMMAND "C asociado antes afi " "Consulta Mod asociado antes proc afiliacion"
                LET x_tipo = 5
                CALL despliega_nss("C","",x_tipo) #nueva para separacion

      COMMAND "M despues afi " "Modif. Datos posterior a procesos derivados "
                LET x_tipo = 9
                CALL despliega_nss("M","",x_tipo) #nueva para separacion

      COMMAND "C invadido despues afi" "Consulta Mod invadido posterior a proc afiliacion"
                LET x_tipo = 9
                CALL despliega_nss("C","",x_tipo) #nueva para separacion

      COMMAND "C asociado despues afi" "Consulta Mod asociado posterior a proc afiliacion"
                LET x_tipo = 9
                CALL despliega_nss("D","",x_tipo) #nueva para separacion
...
DEFINE l_nss CHAR(011)
    ...
    OPEN WINDOW ventana_1 AT 2,2 WITH FORM "SEPM0111" ATTRIBUTE(BORDER)

    DISPLAY " SEPM011                 MANTENIMIENTO  AFILIADOS                              " AT 3,1 ATTRIBUTE(REVERSE)

    DISPLAY " CTRL: [p]Mod [c] Salir                                                        " AT 2,1 ATTRIBUTE(CYAN)

    CALL Inicializa()
    LET g_master.n_seguro = l_nss
    ...
    AFTER FIELD n_folio
       ...
       LET g_master.n_folio = g_master1.n_folio
...

{
    ...
    --    CALL motivo_renapo()
    ON KEY ( CONTROL-Y ) 
 # Rescata datos de afi_his_afiliado
        SELECT "X"
        FROM   afi_his_afiliado hi
       WHERE  hi.n_seguro = g_master.n_seguro
        GROUP BY 1

        IF SQLCA.SQLCODE = 0 THEN
            CALL verifica_datos_historico()
        ELSE
            PROMPT "Registro sin historico, [Enter] p/continuar "
            ATTRIBUTES (REVERSE)
            FOR enter
        END IF
        ...
        CASE KEY
            ...
            WHEN 14
                IF g_master.n_seguro IS NULL THEN
                   ERROR "Campo NO puede ser NULO"
                ELSE
                   LET G_VAL = 'fglgo ',g_seg_modulo.ruta_exp CLIPPED, 
                               '/CTAC102.4gi ' ,g_master.n_seguro
...
FUNCTION Modifica(m_n_seguro,m_tipo_solicitud,m_n_folio)
#M-------------------------------------------------------
    DEFINE m_n_seguro       CHAR(011)
    DEFINE m_tipo_solicitud SMALLINT
    DEFINE m_n_folio        DECIMAL(11,0)
    ...
    LET g_master.n_seguro       = m_n_seguro 
    LET g_master.tipo_solicitud = m_tipo_solicitud
    LET g_master.n_folio        = m_n_folio
    ...
    DISPLAY " [ Esc ]Modifica [p]Datos modificados [c]Salir                                 " AT 2,1 ATTRIBUTE(CYAN)
    ...
    LET g_master.cod_error_origen = 0   #ACS -26072012
    INPUT BY NAME g_master.* WITHOUT DEFAULTS
        ...
        AFTER FIELD n_folio
           LET g_master.n_folio = g_master1.n_folio 
           DISPLAY g_master.n_folio 

#Se comentan sentencias 'CLOSE WINDOW ventana_1', esto causa error mientras no finalice el input
#ACS   -  24-julio-2012
        AFTER FIELD n_seguro
            IF g_master.n_seguro IS NULL THEN
                NEXT FIELD tipo_solicitud
            ELSE
                ...
                ELSE
                    ...
                    IF (g_master.tipo_solicitud > 3  AND 
                        g_master.tipo_solicitud < 6 )THEN
        ...
        -- Se agrega validacion
           AFTER FIELD tip_prob
               ...
               IF g_master.tip_prob = 5 THEN
                   IF (g_master1.tip_prob = 1  OR 
                       g_master1.tip_prob = 2  OR
                       g_master1.tip_prob = 3  OR
                       g_master1.tip_prob = 4  OR
                       g_master1.tip_prob = 7)  THEN
               ...
               IF g_master.tip_prob = 6 THEN 
                   LET g_master.fol_prob = ""
                   LET g_master.doc_prob = ""
                   DISPLAY BY NAME g_master1.fol_prob, g_master1.doc_prob
               END IF
    ...
    ON KEY ( CONTROL-E, CONTROL-T, CONTROL-B, CONTROL-V, CONTROL-N )
        ...
        CASE KEY
            ...
            WHEN 5
                LET COMMA = "fglgo AFIM012 "
            WHEN 14
             #   LET COMMA = "fglgo AFIM016 " , g_master.n_seguro
            WHEN 20
                LET COMMA = "fglgo AFIM013 "
            WHEN 22
                LET COMMA = "fglgo AFIM015 "
...
#vdm---------------------------------
   ...
   OPEN WINDOW ventana_dm AT 2,2 WITH FORM "SEPM0113" ATTRIBUTE(BORDER)
...
FUNCTION Modifica_no_cert(mn_n_seguro,mn_tipo_solicitud,mn_n_folio)
#-------------------------------------------------------------------

    DEFINE mn_n_seguro CHAR(011)
    DEFINE mn_tipo_solicitud SMALLINT
    DEFINE mn_n_folio DEC(11,0)
    ...
    LET g_master.n_seguro        = mn_n_seguro
    LET g_master.tipo_solicitud  = mn_tipo_solicitud
    LET g_master.n_folio         = mn_n_folio


    OPEN WINDOW ventana_1 AT 2,2 WITH FORM "SEPM0111" ATTRIBUTE(BORDER)
    DISPLAY " SEPM011     CONSULTA AFILIADOS MODIFICADOS POR SEPARACION                     " AT 3,1 ATTRIBUTE(REVERSE) 
    DISPLAY "                                                                               " AT 6,1 ATTRIBUTE(REVERSE)
    DISPLAY "                                                                               " AT 14,1 ATTRIBUTE(REVERSE)
    DISPLAY "               Nombre del Trabajador segun Documento Probatorio                " AT 10,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "dd-mm-yyyy" AT 3,65 ATTRIBUTE(REVERSE)
    ...
    CALL Inicializa()
            ...
            ELSE
                ...
                INSERT INTO safre_af:afi_mae_modifica
                VALUES (g_master1.tipo_solicitud     ,
                        g_master1.n_folio            ,
                        g_master1.fecha_elaboracion  ,
                        g_master1.folio_edo_cta      ,
...
FUNCTION verifica_datos_historico() # 
#vhi---------------------------------
   OPEN WINDOW ventana_hi AT 2,2 WITH FORM "AFIM0116" ATTRIBUTE(BORDER)

   DISPLAY " AFIM0116                HISTORICO DE AFILIADOS                                 " AT 3,1 ATTRIBUTE(REVERSE) 
   DISPLAY "                                                                                " AT 6,1 ATTRIBUTE(REVERSE) 
   DISPLAY "                                                                                " AT 14,1 ATTRIBUTE(REVERSE) 
   DISPLAY "               Nombre del Trabajador segun Documento Probatorio                 " AT 10,1 ATTRIBUTE(REVERSE) 
   DISPLAY HOY USING "dd-mm-yyyy" AT 3,65 ATTRIBUTE(REVERSE)

   CALL Rescata_datos_historico() #rdm  # aqui estoy ahora


   IF (pos-1) >= 1 THEN
      CALL SET_COUNT(pos-1)
      DISPLAY ARRAY g_master3 TO scr_2.*
         ON KEY (INTERRUPT)
            EXIT DISPLAY
      END DISPLAY
   ELSE
      ERROR "No hay registros para NSS"
      SLEEP 1
      ERROR ""
   END IF
   CLOSE WINDOW ventana_hi
END FUNCTION
FUNCTION Rescata_datos_historico()
#rdm--------------------------------
   ...
   SELECT UNIQUE h.cod_promotor,
    ...
    OUTER  tab_doc_prob c
    WHERE  h.n_seguro = g_master.n_seguro
    AND    h.tip_prob = c.docprob_cod
    ORDER BY h.frecafor DESC

    LET pos = 1
    FOREACH cursor_3 INTO g_master3[pos].codven,
                          g_master3[pos].agenc_cod,
                          g_master3[pos].n_folio,
                          g_master3[pos].frecafor,
                          g_master3[pos].paterno,
                          g_master3[pos].materno,
                          g_master3[pos].nombres,
                          g_master3[pos].n_seguro,
                          g_master3[pos].n_unico,
                          g_master3[pos].n_rfc,
                          g_master3[pos].sexo,
                          g_master3[pos].edo_civil,
                          g_master3[pos].fena,
                          g_master3[pos].salario_base_comis,
                          g_master3[pos].cod_esq_comision,
                          g_master3[pos].estadon,
                          g_master3[pos].tip_prob,
                          g_master3[pos].fol_prob,
                          g_master3[pos].doc_prob,
                          g_master3[pos].ind_infonavit,
                          g_master3[pos].nacionalidad,
                          g_master3[pos].tipo_solicitud,
                          g_master3[pos].fecha_elaboracion,
                          g_master3[pos].cod_error_origen,
                          g_master3[pos].folio_edo_cta,
                          g_master3[pos].cod_afore_ced,
                          g_master3[pos].const_curp,
                          g_master3[pos].fecha_emision,
                          g_master3[pos].usuario



      IF g_master3[pos].n_folio = g_master3[pos].folio_nvo THEN
          LET g_master3[pos].folio_nvo = ''
      END IF
      ...
      INTO   pat,
             ...
             g_master3[pos].nip
      FROM   pro_mae_promotor
      WHERE  cod_promotor = g_master3[pos].codven

      IF SQLCA.SQLCODE <> 0 THEN
         LET g_master3[pos].desc_codven = "NO EXISTE"
      ELSE
         LET g_master3[pos].desc_codven = pat CLIPPED," ",
                                     mat CLIPPED," ",
                                     nom CLIPPED
      END IF

      SELECT UNIQUE finicta,
             fecha_envio,
             fentcons,
             finitmte
      INTO   x_finicta,
      ...
      WHERE  n_seguro = g_master3[pos].n_seguro
        AND  n_folio = g_master3[pos].n_folio
        AND  tipo_solicitud = g_master3[pos].tipo_solicitud

      SELECT nombre_uni_n1
      INTO   g_master3[pos].agenc_desc
      FROM   com_nivel1
      WHERE  coduni_n1 = g_master3[pos].agenc_cod

      IF SQLCA.SQLCODE <> 0 THEN
         LET g_master3[pos].agenc_desc = "NO EXISTE"
      END IF

      SELECT ecivi_desc
      INTO   g_master3[pos].desc_edo_civil
      FROM   tab_edo_civil
      WHERE  ecivi_cod = g_master3[pos].edo_civil

      IF SQLCA.SQLCODE <> 0 THEN
         LET g_master3[pos].desc_edo_civil = "NO EXISTE"
      END IF

      SELECT sexo_desc
      INTO   g_master3[pos].desc_sexo
      FROM   tab_sexo
      WHERE  sexo_cod = g_master3[pos].sexo

      IF SQLCA.SQLCODE <> 0 THEN
         LET g_master3[pos].desc_sexo = "NO EXISTE"
      END IF
      ...
      SELECT estad_desc
      INTO   g_master3[pos].desc_estadon
      FROM   tab_estado
      WHERE  estad_cod = g_master3[pos].estadon

      IF SQLCA.SQLCODE <> 0 THEN
         LET g_master3[pos].desc_estadon = "NO EXISTE"
      END IF

      SELECT pais_desc
      INTO   g_master3[pos].desc_nacionalidad
      FROM   tab_pais
      WHERE  pais_cod = g_master3[pos].nacionalidad

      IF SQLCA.SQLCODE <> 0 THEN
         LET g_master3[pos].desc_nacionalidad = "NO EXISTE"
      END IF

     CASE g_master3[pos].ind_infonavit
        WHEN "N"
           LET g_master3[pos].ind_infonavit = 0
        WHEN "S"
           LET g_master3[pos].ind_infonavit = 1
     END CASE

     SELECT @descripcion
       INTO g_master3[pos].desc_ind_info
       FROM safre_af:tab_ind_cred
      WHERE @codigo = g_master3[pos].ind_infonavit

     IF STATUS = NOTFOUND THEN
        LET g_master3[pos].desc_ind_info = "NO EXISTE"
     END IF
        IF STATUS = NOTFOUND THEN
            LET g_master3[pos].desc_ind_info = "NO EXISTE"
        END IF

        IF g_master3[pos].tipo_solicitud = 5 THEN
            LET l_estado = 'ASIGNADO REGISTRADO'
        ELSE
            LET l_estado = 'AFILIADO REGISTRADO'
        END IF
      ...
      IF finit IS NOT NULL THEN
            LET edo_desc = edo_desc CLIPPED, " ASIG MISMA AFORE"
        END IF
            LET g_master3[pos].desc_03= l_estado , "/" ,l_desc ,
            "/" ,edo_desc

        IF g_master3[pos].tipo_solicitud <> 5 THEN
            LET g_master3[pos].fechas_his = "F.Apert.  ", x_finicta ,
            "   F.Envio  ", x_fecha_envio , "  F.Cert.   " , x_fecha

        ELSE
            LET g_master3[pos].fechas_his = "F.Apert.  ", x_finicta ,
            "   F.Envio  ", x_fecha_envio , "  F.Asig.   " , x_fecha
