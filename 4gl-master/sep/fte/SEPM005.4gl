...
#Propietario       => EFP                                                #
#Programa SEPM005  => MANTENIMIENTO A PATRONES DE SOLICITUDES DE AFILIAC.#
...
#pp-------------------------
    ...
    OPEN WINDOW ventana_1 AT 2,2 WITH FORM "SEPM0051" ATTRIBUTE(BORDER)
    DISPLAY " SEPM005        MANTENIMIENTO A SOLICITUDES DE AFILIACION                      " AT 3,1 ATTRIBUTE(REVERSE)
...
#a---------------
    ...
    INPUT BY NAME g_afili.* WITHOUT DEFAULTS
        ...
        AFTER FIELD reg_fed_contrib
            IF g_afili.reg_fed_contrib IS NULL AND
               g_afili.reg_patronal IS NULL THEN
                ERROR "RFC Patronal NO puede ser NULO"
                NEXT FIELD reg_patronal
            END IF
            ...
            WHERE  afi_patron.n_folio         = g_afili.n_folio
            AND    afi_patron.tipo_solicitud  = g_afili.tipo_solicitud
            AND    afi_patron.reg_fed_contrib    = g_afili.reg_fed_contrib
            AND    afi_patron.reg_fed_contrib    = tab_patron.reg_fed_contrib
            GROUP BY 1

            IF SQLCA.SQLCODE = 0 THEN
                DISPLAY BY NAME g_afili.razon_social
                ERROR "El RFC Patronal ya existe para esta solicitud de afiliacion"
                NEXT FIELD reg_patronal
            END IF
	      ...
	      INTO vcont_rfc_pat
	      FROM tab_patron
             WHERE reg_fed_contrib= g_afili.reg_fed_contrib

            IF vcont_rfc_pat = 1 THEN
            ...
            OUTER  tab_ciudad     c,
            OUTER  tab_estado     d
            WHERE  P.reg_fed_contrib= g_afili.reg_fed_contrib
            AND    B.deleg_cod      = P.delega
            AND    C.ciudad_cod     = P.ciudad
            AND    D.estad_cod      = P.estado
	    ...
	    ELSE
               IF vcont_rfc_pat > 1 THEN
                   CALL Despliega_rfc_patron() 
                    RETURNING g_afili.reg_fed_contrib      ,
                              g_afili.razon_social
               ELSE
        ...
        ON KEY ( ESC )
            ...
            IF (g_afili.reg_patronal IS NOT NULL OR
                g_afili.reg_fed_contrib IS NOT NULL) AND
                ...
                IF SQLCA.SQLCODE <> 0 THEN
                    ...
                    VALUES (g_afili.reg_patronal,
                            g_afili.reg_fed_contrib , --g_afili.rfc_patron 
                            ''                  , --g_afili.no_docto         
...
#c-----------------
    ...
    OR     P.reg_fed_contrib = A.reg_fed_contrib)
