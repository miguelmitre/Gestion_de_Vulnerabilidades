...
#MODULO      : SEP
#PROGRAMA    : SEPF016
#DESCRIPCION : LISTA PAREJAS DE SEPARACION EN ESPERA DE SER PROVISIONADAS
#              SE ASIGNA UN UNICO FOLIO DE SEPARACION DE CUENTAS PARA TODAS
#              LAS PAREJAS A SER PROVISIONADAS EN UNA MISMA FECHA
...
MAIN
 ...
 CALL STARTLOG("SEPF016.log")

 LET hoy = TODAY

    OPEN WINDOW ventana_nss AT 2,2 WITH FORM "SEPF030" ATTRIBUTE(BORDER)

    DISPLAY " [Esc] Provisionar [Ctrl-C] Salir                                               " AT 1,1 ATTRIBUTE(REVERSE)

    DISPLAY " SEPF016    PAREJAS DE SEPARACION EN ESPERA DE SER PROVISIONADAS               " AT 3,1 ATTRIBUTE(REVERSE)
    ...
    DISPLAY HOY USING "DD-MM-YYYY" AT 3,68 ATTRIBUTE(REVERSE)
          ...
          " FROM   sep_det_reg_sol_reclamante a , ",
                ...
                -- " sep_det03_solicitud        c , ",
                 ...
                 " afi_mae_afiliado           d   ",
          " WHERE    a.estado = 54 ", -- en espera de provision
          ...
          --  " AND    a.correlativo = c.idSolicitudSeparacion ",
            ...
            " AND    d.status_interno in (100,110,150,200) "
         ...
         WHILE TRUE
           ...
           IF cont_1 = 1 THEN
              ERROR ""
              CLEAR FORM
              PROMPT " Sin Registros para Provisionar...<ENTER> para Salir "
              ATTRIBUTE(REVERSE)
              FOR CHAR g_enter
	   ...
	   ELSE  
              ...
              " FROM   sep_det_reg_sol_reclamante a , ",
                   ...
                   --  " sep_det03_solicitud        c , ",
                     " sep_estado_separacion      e , ",
                     " afi_mae_afiliado           d   ",
              " WHERE    a.estado = (54) ", -- en espera de provisionar
                " AND    a.correlativo = b.idSolicitudSeparacion ",
              --  " AND    a.correlativo = c.idSolicitudSeparacion ",
                " AND    b.clasifica_separacion in ('A','B','C') ",
                " AND    a.nss = d.n_seguro ",
