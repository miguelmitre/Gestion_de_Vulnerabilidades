...
{           asig   RECORD
    ...
    DEFINE vdiag_proceso            CHAR(3),
           ...
           vdiag_imagen             CHAR(2),
           vdiag_imagen1            CHAR(2),
           vdiag_imagen2            CHAR(2),
           vdiag_imagen3            CHAR(2),
           vdiag_imagen4            CHAR(2),
           vdiag_folio              CHAR(3),
           vtip_rech                SMALLINT,
           vmotivo                  CHAR(3),
           vrechazo                 CHAR(120),
...
FUNCTION detalle_notifica(ref,prog,det_cza)
 ...
 --display "detalle_notifica ", p_mae_afi.folio
--sleep 10 
...
##               WHERE a.fecha_valuacion = TODAY
         ...
         IF prog = 428 THEN
--display "prog 428", p_mae_afi.folio
            ...
            CASE opera
                ...
                OTHERWISE
                     ...
                     IF (vdiag_proceso <> '   ')   THEN
                        ...
                        RETURNING p_rechazo.motivo, vrechazo, vtip_rech, vban_rech
                        IF vban_rech = 0 THEN
                     ...
                     IF (vmotivo_rechazo <> '  ')   THEN
                        ...
                        RETURNING vmotivo, vrechazo, vtip_rech, vban_rech
                     ...
                     IF (vdiag_imagen1 <> '00')   THEN
                        CALL obtiene_desc(vdiag_imagen1, 2, vtip_rech)
                        RETURNING vmotivo, vrechazo, vtip_rech, vban_rech
                     ...
                     IF (vdiag_imagen2 <> '00')   THEN
                        CALL obtiene_desc(vdiag_imagen2, 2, vtip_rech)
                        RETURNING vmotivo, vrechazo, vtip_rech, vban_rech
                     ...
                     IF (vdiag_imagen3 <> '00')   THEN
                        CALL obtiene_desc(vdiag_imagen3, 2, vtip_rech)
                        RETURNING vmotivo, vrechazo, vtip_rech, vban_rech
                     ...
                     IF (vdiag_imagen4 <> '00')   THEN
                        CALL obtiene_desc(vdiag_imagen4, 2, vtip_rech)
                        RETURNING vmotivo, vrechazo, vtip_rech, vban_rech
                     ...
                     END IF
            END CASE
--display p_rechazo.motivo, ' ', p_mae_afi.nss
--sleep 10
...
##genera arch.
--display "call r_report ", p_mae_afi.folio
--sleep 10
...
FUNCTION limpia2()
    ...
    LET vdiag_imagen1    = NULL
    LET vdiag_imagen2    = NULL
    LET vdiag_imagen3    = NULL
    LET vdiag_imagen4    = NULL
...
DEFINE
  ...
  IF STATUS = NOTFOUND THEN
     ...
     LET fdiagnostico = "" 
     CASE ftip_rech
       WHEN 0 LET ftip_rech = ftip_rech
       WHEN 1 LET ftip_rech = ftip_rech
       WHEN 2 LET ftip_rech = ftip_rech
       WHEN 3 LET ftip_rech = ftip_rech
       WHEN 4 LET ftip_rech = ftip_rech
       WHEN 5 LET ftip_rech = ftip_rech
     END CASE
  ELSE
     LET fban_rech = 0
     CASE ftip_rech
       ...
       WHEN 3 LET ftip_rech = 3
       WHEN 4 LET ftip_rech = 4

       {WHEN 0 LET ftip_rech = 1 
       WHEN 1 LET ftip_rech = 2
       WHEN 2 LET ftip_rech = 3 
       WHEN 3 LET ftip_rech = 4
       WHEN 4 LET ftip_rech = 5}
     END CASE
  END IF

  RETURN fdiagnostico, fdes_rech, ftip_rech, fban_rech
...
FUNCTION asigna_var(gmotivo, grechazo, gtip_diag, gtip_rech)
  ...
  CASE gtip_rech
    ...
    WHEN 3
       ...
       LET grechazo = rechazo
       LET rechazo3 = grechazo

    WHEN 4
       LET motivo4 = gmotivo USING "#&&"
       LET rechazo = grechazo        
       LET long = 0
       LET i    = 0
       LET long = LENGTH(rechazo)
