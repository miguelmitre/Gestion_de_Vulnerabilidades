...
GLOBALS
    DEFINE p_tabafore               RECORD LIKE tab_afore_local.*,
           ...
           p_mae_afi  RECORD
                  ...
                  fecha_red_bono    DATE,
                  taa_folio					DECIMAL(10,0) #CPL-1461
           END RECORD,
...
FUNCTION detalle_notifica_09(ref,prog,det_cza)
   ...
   PREPARE apt_09 FROM selec_tab1
   -- DISPLAY selec_tab1
   --SLEEP 4
   DECLARE cur_09 CURSOR WITH HOLD FOR apt_09
   ...
   INITIALIZE tipo_proceso,des_transferente,des_receptora, rfc  TO NULL
...
##domicilio
         ...
         END IF -- DOMICILIO SQLCA.SQLCODE = 0
...
##montos
         ...
         SELECT ROUND(SUM(bb.monto_en_pesos),2) INTO s_rcv FROM dis_cuenta bb
                WHERE bb.nss       = p_mae_afi.nss
                  AND bb.fecha_conversion = p_mae_afi.fecha_mov_banxico
                  AND bb.folio 						= p_mae_afi.taa_folio #CPL-1461
         ...
         SELECT ROUND(SUM(h.monto_en_pesos),2) INTO s_ret92 FROM dis_cuenta h
                WHERE h.nss       = p_mae_afi.nss
                  AND h.fecha_conversion = p_mae_afi.fecha_mov_banxico
                  AND h.folio 					 = p_mae_afi.taa_folio #CPL-1461
                  AND h.subcuenta = 7
         ...
         SELECT ROUND(SUM(n.monto_en_pesos),2) INTO s_ahorro FROM dis_cuenta n
                WHERE n.nss       = p_mae_afi.nss
                  AND n.fecha_conversion = p_mae_afi.fecha_mov_banxico
                  AND n.folio 					 = p_mae_afi.taa_folio #CPL-1461
                  AND n.subcuenta = 13
         ...
         SELECT ROUND(SUM(p.monto_en_pesos),2) INTO s_viv97 FROM dis_cuenta p
                WHERE p.nss         = p_mae_afi.nss
                  ...
                  AND p.folio       = p_mae_afi.taa_folio #CPL-1461
         ...
         SELECT ROUND(SUM(t.monto_en_pesos),2) INTO s_fondo  FROM dis_cuenta t
                   WHERE t.nss       = p_mae_afi.nss
                     AND t.fecha_conversion = p_mae_afi.fecha_mov_banxico
                     AND t.folio            = p_mae_afi.taa_folio #CPL-1461
...
## Entidad Transferente
         ...
         LET v_rep  =  "02"                                , "|",  #01
                       ...
                       des_receptora                       , "|",  #157
                       tpo_dato_dif                        , "|",  #158
                       p_mae_afi.importe_bono              , "|",  #159
                       f_red_bono                          , "|",  #160                      
                       sum_ahorro_ret_issste               , "|",  #161
                       sum_ahorro_viv_issste               , "|",  #162                       
                       barras                              , "|",   #163
...
##genera arch.
     ...
     INSERT INTO int_constancia VALUES ( folio,
                                         ...
                                         TODAY,
                                         p_mae_afi.fecha_mov_banxico,
                                         no_carta )

     SET LOCK MODE TO NOT WAIT
     -- DISPLAY "NSS", p_mae_afi.nss
