...
#Propietario       => E.F.P.                                        #
#Programa          => INTB0152 Y INTB01011                           #
#Descripcion       => NOTIFICACION DE ACLARACION DE NO AFILIADOS    #
#Sistema           => INT .                                         #
...
##   DEFER INTERRUPT       
   CALL STARTLOG ("INTB0152.log")
    ...
    SELECT v.nom_arch, arch_det, arch_cza 
        ...
        FROM  tab_layout v
        WHERE v.layout_cod = 445

    OPEN WINDOW INTB0152 AT 2,2 WITH FORM "INTB01521" ATTRIBUTE(BORDER)
         DISPLAY " < ESC > Procesar                                          <",
		 " Ctrl-C > Salir    " AT 1,1 ATTRIBUTE(REVERSE)
                                                                                
         DISPLAY " INTB0152        ACLARACION DE REG",
		"ISTRO NO AFILIADOS                           "
         ...
         IF ban  = 0 THEN
            LET selec_tab = " SELECT b.nss,   a.n_rfc, a.n_unico,  ",
             ...
             LET v_arch = v_ruta CLIPPED,"/", "30504",
             ...
             LET leyenda_cza = "ACLARACION DE REGISTRO DE NO AFILIADOS"

             START REPORT r_report TO v_det
             CALL detalle_noafiliado("04",445,2)

             START REPORT r_report TO v_cza
             CALL cabeza_noafiliado("04",leyenda_cza,445,1)
