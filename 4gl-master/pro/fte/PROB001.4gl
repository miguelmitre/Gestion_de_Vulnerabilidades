...
#Owner             => E.F.P.                                       #
#Programa PROB001  => Catalogo de Tipo de Examen                   #
...
FUNCTION proceso()
   LET HOY = TODAY
   OPEN WINDOW ventana_1 AT 3,3 WITH FORM "PROB0011" ATTRIBUTE( BORDER)
   DISPLAY " PROB001               CATALOGO DE TIPOS DE ",
...
FUNCTION Agrega()
   ...
   LET g_reg.desc_tipo = NULL
   LET sw_1 = 0

   INPUT BY NAME  g_reg.*

         AFTER FIELD cod_tipo
               IF g_reg.cod_tipo IS NULL THEN
         ...
         BEFORE FIELD desc_tipo
               IF g_reg.cod_tipo IS NULL OR  g_reg.cod_tipo = 0 THEN
                  ERROR "Codigo NO puede ser nulo"
                  NEXT FIELD  cod_tipo
               END IF 

         AFTER FIELD desc_tipo
               IF g_reg.desc_tipo IS NULL THEN
               ...
               SELECT "X" 
               FROM   pro_tipo_examen
               WHERE  desc_tipo = g_reg.desc_tipo
         ...
         ON KEY ( ESC )
               ...
               INSERT INTO pro_tipo_examen VALUES ( g_reg.*,TODAY,seg_usuario ) 
...
FUNCTION Consulta()
   ...
   IF (pos-1) >= 1 THEN
      ...
      OPEN WINDOW ventana_2 AT 6,3 WITH FORM "PROB0012" ATTRIBUTE( BORDER)
      ...
      CONSTRUCT cla_where   ON cod_tipo 
                          FROM cod_tipo
      ...
      LET sel_where = "SELECT * FROM pro_tipo_examen WHERE ",cla_where CLIPPED,
                      "ORDER BY 1,2 "
   ...
   IF (pos-1) >= 1 THEN
      ...
      IF (pos-1) >= 1 THEN
          ...
          DISPLAY ARRAY l_record TO scr_1.* 
             ...
             ON KEY (CONTROL-M)
                LET pos = ARR_CURR()
                LET g_reg.cod_tipo = l_record[pos].codigo
                LET g_reg.desc_tipo = l_record[pos].desc_tipo
                EXIT DISPLAY
       ...
       INPUT BY NAME  g_reg.* WITHOUT DEFAULTS 
          ...
          AFTER FIELD desc_tipo
             ...
             IF aux_pausa MATCHES "[Ss]" THEN
                UPDATE pro_tipo_examen 
                SET    desc_tipo  = g_reg.desc_tipo
                WHERE  cod_tipo  = g_reg.cod_tipo
...
FUNCTION impresion(pos)
   ...
   FOR i=1 TO (pos+1)
       LET g_reg.cod_tipo  = l_record[i].codigo
       LET g_reg.desc_tipo = l_record[i].desc_tipo
...
REPORT rpt_taborigen(g_reg)
   ...
   FORMAT
      PAGE HEADER
         PRINT COLUMN 02," PROB001 ",
               COLUMN 27," LISTADO DE TIPOS DE EXAMEN ",
      ...
      ON EVERY ROW
         ...
         PRINT COLUMN 12,g_reg.cod_tipo USING "#&&&&&",
               COLUMN 30,g_reg.desc_tipo
      PAGE TRAILER
         SKIP 2 LINE
