...
#Observacion       => Se modifico query                                        #
#                  => (v2)                                                     #
#Modificado Por    => Isabel Fonseca Frias                                     #
#Fecha             => 08-10-2008                                               #
#Observacion       => Se actualiza el estado de los registros generados        #
#                  => para REVALIDACION Lote Generado                          #
#                  => (v3)                                                     #
#Modificado Por    => Isabel Fonseca Frias                                     #
#Fecha             => 17-09-2009                                               #
            ...
            IF enter MATCHES "[sSnN]" THEN
                IF enter MATCHES "[sS]" THEN
                    ...
                    ELSE
                       ...
                       IF STATUS <> NOTFOUND THEN
                          PROMPT " EL LOTE YA FUE GENERADO, NO PUEDE GENERAR",
                                 " OTRO...<ENTER> PARA SALIR " FOR CHAR enter
                               EXIT PROGRAM
                       ELSE
...
#sp--------------------
    ...
    FOREACH cur_1 INTO reg_1.*
        ...
        CASE reg_1.tipo_operacion
            ...
            WHEN "REV"                                                   --(v3)
                UPDATE pro_capacitacion                                  --(v3)
                SET    pro_capacitacion.estado = 3,                      --(v3)
                       pro_capacitacion.fecha_genera = reg_1.fecha_genera--(v3)
                WHERE  pro_capacitacion.estado = 2                       --(v3)
        END CASE
    END FOREACH
    ...
    LET cat = "cat ",parametro.ruta_envio CLIPPED,"/CZATRAN ",
                     PRO_DET CLIPPED,
                     " > ",RUTA CLIPPED,"/",HOY USING"YYYYMMDD",".080"
...
#ct--------------
    ...
    FIRST PAGE HEADER
        ...
        -- SELECT sum(nro_de_registros)                            --(v2)
        SELECT NVL(sum(nro_de_registros),0)                        --(v2)
        INTO   tot_301
        FROM   pro_ctr_envio A
        WHERE  A.estado = reg_2.procesado
        AND    A.tipo_operacion IN('ALT','REA','REC')

        --IF tot_301 IS NULL OR tot_301 = 0 THEN                   --(v2)
        --   LET tot_301 = 0                                       --(v2)
        --END IF                                                   --(v2)
        ...
        --IF tot_302 IS NULL OR tot_302 = 0 THEN                   --(v2)
        --   LET tot_302 = 0                                       --(v2)
        --END IF                                                   --(v2)
        ...
        --IF tot_303 IS NULL OR tot_303 = 0 THEN                   --(v2)
        --   LET tot_303 = 0                                       --(v2)
        --END IF                                                   --(v2)
        ...
        --IF tot_304 IS NULL OR tot_304 = 0 THEN                  --(v2)
        --   LET tot_304 = 0                                      --(v2)
        --END IF                                                  --(v2)
