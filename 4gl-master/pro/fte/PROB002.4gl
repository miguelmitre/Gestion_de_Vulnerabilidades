...
#Owner             => E.F.P.                                       #
#Programa PROB002  => Catalogo de Tipo de Evento                   #
#Fecha Elaboracion => 07 de Noviembre del 2006                     #
#Elaborado por     => Laura Eugenia Cortes Guzman                  #
#Fecha Ult.Modif.  => 07 de Noviembre del 2006                     #
...
FUNCTION proceso()
   LET HOY = TODAY
   OPEN WINDOW ventana_1 AT 3,3 WITH FORM "PROB0021" ATTRIBUTE( BORDER)
   DISPLAY " PROB002               CATALOGO DE TIPOS DE ",
           "EVENTOS                            " AT 3,1 ATTRIBUTE(REVERSE) 
   DISPLAY HOY USING "dd-mm-yyyy" AT 3,66 ATTRIBUTE(REVERSE)
   
   MENU "CATALOGO DE EVENTOS"
...
FUNCTION Agrega()
   ...
   LET g_reg.desc_evento = NULL
   LET sw_1 = 0

   INPUT BY NAME  g_reg.*

         AFTER FIELD cod_evento
               IF g_reg.cod_evento IS NULL THEN
               ...
               SELECT "X" 
               FROM   pro_evento
               WHERE  cod_evento = g_reg.cod_evento
         ...
         BEFORE FIELD desc_evento
               IF g_reg.cod_evento IS NULL OR  g_reg.cod_evento = 0 THEN
                  ERROR "Codigo NO puede ser nulo"
                  NEXT FIELD  cod_evento
               END IF 

         AFTER FIELD desc_evento
               IF g_reg.desc_evento IS NULL THEN
                  ERROR "Descripcion NO puede ser nula"
                  NEXT FIELD  desc_evento
               END IF 

               SELECT "X" 
               FROM   pro_evento
               WHERE  desc_evento = g_reg.desc_evento
         ...
         ON KEY ( ESC )
               ...
               INSERT INTO pro_evento VALUES ( g_reg.*,TODAY,seg_usuario ) 
...
FUNCTION Consulta()
   ...
   IF (pos-1) >= 1 THEN
      ...
      LET sel_where = "SELECT * FROM pro_evento WHERE ",cla_where CLIPPED,
                      "ORDER BY 1,2 "
   ...
   IF (pos-1) >= 1 THEN
      ...
      IF (pos-1) >= 1 THEN
          ...
          DISPLAY ARRAY l_record TO scr_1.* 
             ...
             ON KEY (CONTROL-M)
                LET pos = ARR_CURR()
                LET g_reg.cod_evento = l_record[pos].codigo
                LET g_reg.desc_evento = l_record[pos].desc_evento
                EXIT DISPLAY
       ...
       INPUT BY NAME  g_reg.* WITHOUT DEFAULTS 
          ...
          AFTER FIELD desc_evento
             ...
             IF g_reg.desc_evento IS NULL THEN
                ERROR "La desc_evento no puede ser Nula "
                NEXT FIELD  desc_evento
             END IF 
             ...
             IF aux_pausa MATCHES "[Ss]" THEN
                UPDATE pro_evento 
                SET    desc_evento  = g_reg.desc_evento
                WHERE  cod_evento  = g_reg.cod_evento
...
FUNCTION impresion(pos)
   ...
   FOR i=1 TO (pos+1)
       LET g_reg.cod_evento  = l_record[i].codigo
       LET g_reg.desc_evento = l_record[i].desc_evento
...
REPORT rpt_taborigen(g_reg)
   ...
   FORMAT
      PAGE HEADER
         PRINT COLUMN 02," PROB002 ",
               COLUMN 27," LISTADO DE TIPOS DE EVENTO ",
      ...
      ON EVERY ROW
         ...
         PRINT COLUMN 12,g_reg.cod_evento USING "#&&&&&",
               COLUMN 30,g_reg.desc_evento
      PAGE TRAILER
         SKIP 2 LINE
