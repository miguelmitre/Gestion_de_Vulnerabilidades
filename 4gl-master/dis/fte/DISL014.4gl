...
#Owner             => E.F.P.        					                            #
#Programa          => DISL014                                                  #
#Descripcion       => Consulta provision y liquidacion int.trans. RCV          #
#Fecha             => 15 marzo 2002.    	       			                      #
...
MAIN
    ...
    ATTRIBUTE( BORDER)
    DISPLAY " DISL014 " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING " dd-mm-yyyy " AT 3,60 ATTRIBUTE(REVERSE)
	
    MENU "Transito RCV"
      COMMAND "Provision" "Consulta de la provision INT transito RCV" 
        LET opc = "P"
        CALL Provision()
      COMMAND "Liquidacion" "Consulta de la liquidacion INT transito RCV" 
        LET opc = "L"
        CALL Provision()
...
FUNCTION Provision()
    ...
    OPEN WINDOW ventana_21 AT 10,4 WITH FORM "DISL0142" ATTRIBUTE( BORDER)
     ...
     AFTER FIELD vfecha_lote
       ON KEY (ESC)
          ...
          IF STATUS <> NOTFOUND THEN
              ...
              EXIT INPUT
          ELSE     
              ERROR "ARCHIVO CON FECHA DIGITADA NO HA SIDO CARGADO TODAVIA O NO EXISTE"
          END IF
    ...
    IF opc = "P" THEN
      ...
      FOREACH cur_det INTO fec,cons
       ...
       AND    a.subcuenta       in (1,2,3,11,15,17)         --c22-11
    ...
    ELSE
      ...
      FOREACH cur_det2 INTO fec,cons
    ...
    IF (pos-1) >= 1 THEN
      ...
      ERROR ""
      OPEN WINDOW ventana_2 AT 6,4 WITH FORM "DISL0141" ATTRIBUTE( BORDER)
      ...
      DISPLAY " DISL014               Intereses en Transito RCV                             " AT 3,1 ATTRIBUTE(REVERSE,green) 
...
#-----------------------------------------------------------------------------
# Se genera una tabla tempo para reunir a los folios que estan en el periodo
#-----------------------------------------------------------------------------
...
FUNCTION cuentas_archi()
    ...
    LET G_LISTA = "chmod 777 ",g_param_dis.ruta_listados CLIPPED,
                       "/",g_usuario CLIPPED,".INT_TRA_RCV_",
                      HOY USING "DD-MM-YYYY","_",hora CLIPPED
  RUN G_LISTA
...
FUNCTION imprimir()
    ...
    LET G_IMPRE = g_param_dis.ruta_listados CLIPPED,"/",g_usuario CLIPPED,
                              ".INT_TRA_RCV_",HOY USING "DD-MM-YYYY",
                                "_",hora CLIPPED
...
FUNCTION cuentas_impri()
    ...
    LET vsel = " SELECT  d.fecha_conversion ",
               ...
               " AND d.subcuenta IN((1,2,3) AND tipo_movimiento = 1"
...
REPORT rpt_cuenta_imp(g, registros, 
    ...
    FORMAT
      PAGE HEADER
        IF opc = "P" THEN
           PRINT COLUMN 02,"DISL014",
                 COLUMN 12,"PROVISION INTERESES EN TRANSITO RCV",
                 COLUMN 61, TODAY USING "mm/dd/yyyy"
        ELSE
           PRINT COLUMN 02,"DISL014",
                 COLUMN 12,"LIQUIDACION INTERESES EN TRANSITO RCV",
                 COLUMN 61, TODAY USING "mm/dd/yyyy"
        END IF
        ...
        SKIP 1 LINES
{
        ...
        IF opc = "P" THEN
           PRINT COLUMN 2, "Fecha Provision   : ",fecha_pro USING "dd/mm/yyyy"
        ELSE
           LET precio = 0
           ...
           PRINT COLUMN 2, "Fecha Liquidacion : ",fecha_rcv USING "dd/mm/yyyy",
