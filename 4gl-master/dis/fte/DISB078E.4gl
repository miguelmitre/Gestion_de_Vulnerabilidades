...
#                  => liquidar lo provisionado (ISSSTE)                       #
...
GLOBALS
   ...
   DEFINE rsie   SMALLINT

   DEFINE rapido CHAR(30),
          lento  CHAR(30)
...
MAIN
   CALL STARTLOG("DISB078E.log")

   LET vfolio = ARG_VAL(1)

   DISPLAY "INICIO IDENTIFICACION NSS CAMBIO SIEFORE (E)"
   ...
   DISPLAY "FIN IDENTIFICACION NSS CAMBIO SIEFORE (E)"
END MAIN
...
FUNCTION Llena_tablas()
   ...
   CREATE TEMP TABLE tmp_nss_liqe
      (nss CHAR(11),
       sub SMALLINT,
       sie SMALLINT
      )

   INSERT INTO tmp_nss_liqe
   SELECT UNIQUE nss,
          subcuenta,
          siefore
   FROM   dis_cuenta
   ...
   CREATE INDEX tmp_nss_liqe_1 ON tmp_nss_liqe(nss)

   PREPARE lento_e FROM lento
   EXECUTE lento_e

   UPDATE STATISTICS FOR TABLE tmp_nss_liqe
END FUNCTION
...
FUNCTION Identifica()
   ...
   DECLARE cur_liqe CURSOR FOR
   SELECT liq.nss,
          liq.sub,
          liq.sie
   FROM   tmp_nss_liqe liq

   FOREACH cur_liqe INTO r_liq.*
...
{ANTES   SELECT "x"                      --v2
         FROM  tes_solicitud             --v2
         WHERE nss = r_liq.lnss          --v2
         AND   fecha_solicitud = TODAY
         AND   folio_origen    = vfolio
         GROUP BY 1                      --v2

         IF STATUS = NOTFOUND THEN
            INSERT INTO tes_solicitud VALUES
               (
                r_liq.lnss,   --nss
                0,            --folio_solicitud 
                ...
                15,           --medio
                3,            --tipo_traspaso
                r_liq.lsie,   --siefore_cedente    --v2
                rsie,         --siefore_receptora  --v2
                100,          --estado
                null,         --folio
                null,         --fecha_traspaso
                vfolio        --ind_decimos
