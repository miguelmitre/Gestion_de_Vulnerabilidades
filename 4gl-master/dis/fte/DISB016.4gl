...
#Owner             => E.F.P.        					                           #
#Programa DISB016  => Cifras Control Archivo Dispersion  dis_provision / AFORE#
#Fecha             => 26 DE MAYO 1999 . 	       			                     #
#Por               => MIGUEL ANGEL HERNANDEZ MARTINEZ. 			               #
#Sistema           => DIS. 					                                    #
-------------------------------------------------------------------------------
#Fecha ini modif   => 01 diciembre 2004                                       #
#Fecha fni modif   =>                                                         #
...
GLOBALS
    ...
    DEFINE l_record   ARRAY[300] OF RECORD
            ...
            monto_en_acciones     DECIMAL(16,6)   ----jerry
    END RECORD
...
MAIN
    ...
    ATTRIBUTE( BORDER)
    DISPLAY " DISB016 " AT 3,1 ATTRIBUTE(REVERSE)
...
FUNCTION Consulta()
    ...
    OPEN WINDOW ventana_21 AT 10,4 WITH FORM "DISB0162" ATTRIBUTE( BORDER)
    ...
    SELECT siefore,                  --c22-6
           ...
           subcuenta,                                                 
           sum(monto_en_pesos),
           sum(monto_en_acciones)    ----jerry agrego accio
      FROM dis_provision
     WHERE folio = v.folio
     GROUP BY 1,2,3                                                 
     ORDER BY 3,1,2
    ...
    FOREACH cursor_1 INTO l_record[pos].*
         ...
         IF l_record[pos].monto_en_acciones IS NULL THEN
            LET l_record[pos].monto_en_acciones = 0
         END IF
         LET pos = pos + 1
    END FOREACH
    ...
    IF (pos-1) >= 1 THEN
      ...
      ERROR ""
      OPEN WINDOW ventana_2 AT 6,4 WITH FORM "DISB0161" ATTRIBUTE( BORDER)
      ...
      DISPLAY "           Cifras de Control de Archivo de Dispersion / AFORE                  " AT 3,1 ATTRIBUTE(REVERSE,green) 
...
FUNCTION cuentas_archi()
    ...
    FOR i=1 TO 300
      ...
      LET g.monto_en_acciones = l_record[i].monto_en_acciones   ---jerry
   ...
   DEFINE vident_arch CHAR(01),
    ...
    FORMAT
      PAGE HEADER
        ...
        PRINT COLUMN 02,"DISB016",
              COLUMN 12,"CIFRAS CONTROL ARCHIVO DE ",vtipo_arch CLIPPED,
