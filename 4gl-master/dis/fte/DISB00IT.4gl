...
-- Propietario  => E.F.P.                                                    --
-- Programa     => DISB00IT                                                  --
-- Descripcion  => PROCESO BATCH INTERESES EN TRANSITO IMSS                  --
-- Sistema      => DIS                                                       --
-- Por          => DMR                                                       --
-- Fecha        => 25 Marzo 2013                                             --
-- Modificado   =>                                                           --
...
-- Modi.        => DMR 04 Marzo 2014                                         --
-- Descr.       => Se modifico programa para no permitir folios duplicados   --
--              => CPL-1558 y ademas el STARTLOG personalizado               --
-------------------------------------------------------------------------------
DATABASE safre_af
...
MAIN
   ...
   CALL STARTLOG(FGL_GETENV("USER")||".DISB00IT.log")
   ...
   OPEN WINDOW ventana1 AT 2,2 WITH FORM "DISB00IT1" ATTRIBUTE(BORDER)
   DISPLAY " DISB00IT           DISPERSION INTERESES EN TRANSITO IMSS                       " AT 3,1 ATTRIBUTE(REVERSE)
   DISPLAY hoy USING "DD-MM-YYYY" AT 3,67 ATTRIBUTE(REVERSE)
   DISPLAY "    LECTURA DE ARCHIVOS, HISTORICOS, INDIVIDUALIZACION Y GENERACION CONFAF      " AT 5,1 ATTRIBUTE(REVERSE) 

   IF vcodigo_afore = 564 THEN       # METLIFE                
      MENU "PROCESO IT-IMSS"
         COMMAND "Carga" "Carga Archivo Intereses en Transito IMSS"
            CALL Carga()
         COMMAND "Liquidaciones" "Liquidacion Int. tran. IMSS RCV, VIV, EST"
            CALL Liquidacion()
         COMMAND "Reverso Disp." "Reverso de Archivo de Intereses Transito IMSS"
            CALL Reverso_normal("RCV")
         COMMAND "rEportes" "Reporte de Provision o Liquidacion Int. Tran. IMSS"
            CALL Menureportes()
...
FUNCTION Menureportes()
   ...
   MENU "REPORTES IT-IMSS" 
      COMMAND "Provision" "Reporte de Provision Intereses en Transito IMSS"
         LET cadena = "fglgo DISL002.4gi"
         RUN cadena
      COMMAND "Liquidacion" "Reporte de Liquidacion Intereses en Transito IMSS"
         LET cadena = "fglgo DISL003.4gi"
         RUN cadena
      COMMAND "Salir" "Salida del Programa"
         EXIT MENU
   END MENU
...
FUNCTION Reverso_normal(tipo_rev)
   ...
   OPEN WINDOW ventana1r AT 2,2 WITH FORM "DISB00IT1" ATTRIBUTE(BORDER)
   DISPLAY " DISB00IT     REVERSO DE INTERESES EN TRANSITO IMSS RCV, EST Y VIV                 " AT 3,1 ATTRIBUTE(REVERSE)
         ...
         ELSE
            ...
            ELSE
               ...
               IF SQLCA.SQLCODE = 100 THEN
                  ERROR "ARCHIVO INEXISTENTE COMO PROCESO INT. TRANSITO IMSS..."
                  NEXT FIELD nombre_archivo
               END IF
            END IF
         END IF
...
FUNCTION Carga()
   ...
   INPUT BY NAME g_reg.*
      ...
      LET ejecuta = "cd ",g_param.ruta_rescate CLIPPED,"; ls > arch_iti" CLIPPED
      RUN ejecuta

      WHENEVER ERROR CONTINUE
         DATABASE safre_tmp
         DROP TABLE archivos_dis_iti
      WHENEVER ERROR STOP

      CREATE TABLE archivos_dis_iti
         (campo  CHAR(100))

      LET ejecuta = g_param.ruta_rescate CLIPPED,"/arch_iti" CLIPPED

      LOAD FROM ejecuta INSERT INTO archivos_dis_iti

      SELECT "X"
      FROM safre_tmp:archivos_dis_iti
   ...
   IF band = 0 THEN
      ...
      IF opc MATCHES '[Ss]' THEN
         ...
         DISPLAY "FOLIO A PROCESAR : ",vfolio AT 14,18
         PROMPT " <ENTER> PARA CONTINUAR ..." FOR char opc      #CPL-1558
...
FUNCTION Ejecuta_lectura()
   ...
   SLEEP 3
   LET ejecuta = "nohup time fglgo DISB201IT.4gi 0 0 0 ",
...
FUNCTION Ejecuta_reverso(folio_rev,tipo_rev,nom_archivo)
   ...
   DELETE FROM dis_cza_iti
   WHERE folio = folio_rev

   DELETE FROM dis_det_iti
   WHERE folio = folio_rev

   DELETE FROM dis_dep_iti
   WHERE folio = folio_rev

   DELETE FROM dis_sum_iti
   WHERE folio = folio_rev
...
FUNCTION Liquidacion()
   OPEN WINDOW vent10 AT 2,2 WITH FORM "DISB00IT2" ATTRIBUTE(BORDER)
   DISPLAY " DISB00IT              LIQUIDACION INTERESES EN TRANSITO IMSS                  " AT 3,1 ATTRIBUTE(REVERSE)
         ...
         CASE 
            WHEN vsubcuenta = "RCV" OR vsubcuenta = "VIV" AND vtipo = "A"
               LET cla_sel = "SELECT 'X' ",
                             ...
                             " AND   estado = 3 ",
                          "AND ident_pago[14,15] IN ('51','55','56','59','50')",
                            " GROUP BY 1 "
...
FUNCTION Despliega_valores(vfolio,vfecha_acreditacion,vsub)  --c22-6.12
   ...
   CASE
      WHEN vsub = "EST"
         LET aux_sub = "('52')"
      WHEN vsub = "RCV"
         LET aux_sub = "('51','55','56','59','50')"
      OTHERWISE
         LET aux_sub = "('')"
   END CASE

   OPEN WINDOW ventana_2 AT 7,6 WITH FORM "DISB00IT3" ATTRIBUTE(BORDER)
   ...
   WHILE TRUE
      LET x_texto = " SELECT tipo_siefore,ident_pago[14,15],impt_aport_acept ",
                    " FROM dis_dep_iti WHERE folio =",vfolio CLIPPED,
...
FUNCTION Despliega_saldos(tipo,aporte)
   ...
   IF vsub = "REMVIV" THEN                                         --c22-11
      ...
      SELECT folio                                                 --c22-11
      FROM  dis_dep_iti                                            --c22-11
      WHERE ident_pago[14,15] NOT IN ('53','57','58')              --c22-11
      AND   estado = 2                                             --c22-11
      GROUP BY 1                                                   --c22-11
      ...
      IF tipo = "I" THEN                       -- DMR ARCH. ACL ESP ident_arch 2
         LET cla_sel = " SELECT folio,",
                            ...
                            " fecha_archivo ",
                       " FROM  dis_dep_iti ",
                       " WHERE ident_pago[14,15] IN ('53','57','58') ",
                       " AND   estado = 2 ",
                       " AND   folio NOT IN (SELECT folio FROM tmp_folioss) ",
                       " AND   folio IN (SELECT folio FROM dis_cza_iti ",
                                       " WHERE  ident_arch = '2') ",
      ...
      ELSE
         LET cla_sel = " SELECT folio,",
                       ...
                       " AND   estado = 2 ",
                       " AND   folio NOT IN (SELECT folio FROM tmp_folioss) ",
                       " AND   folio NOT IN (SELECT folio FROM dis_cza_iti ",
                                           " WHERE  ident_arch = '2') ",
                       " GROUP BY 1,2,3 " CLIPPED,
                       " ORDER BY 1,2 " CLIPPED
      END IF
   ...
   INPUT ARRAY g_sal WITHOUT DEFAULTS FROM scr_liquida.*
      ...
      ON KEY (control-m)
         ...
         CASE
            WHEN tipo_liquida = "51" 
               CALL Asigna_montos_aporte()
            WHEN tipo_liquida = "52"
               CALL Asigna_montos_aporte()
         END CASE
...
FUNCTION Ejecuta_liquidacion()
   ...
   SLEEP 3
   LET ejecuta = "nohup time fglgo DISB007IT.4gi 0 0 0 ",
                            g_liq.folio CLIPPED," ",
                            vtipo, " ",
                            vsubcuenta," ",
                            g_liq.fecha_proceso," ",
