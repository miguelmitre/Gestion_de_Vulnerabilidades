...
-- MOdulo       => DIS                                                       --
-- Programa     => DISB074F                                                  --
-- Descripcion  => AGRUPA INTERESES RET POR FECHA VALOR Y SEPARA EL REMANENTE--
--              => EN LA TABLA dis_cuenta_retbnx                             --
-- Autor        => GERARDO ALFONSO VEGA PAREDES                              --
-- Fecha        => 2-agosto 2004.
-------------------------------------------------------------------------------
-- Modificacion => se cambia la leyenda de id_aportante de INfonavit a       --
--                 a RENDIM-BNX                                              --
-- Requrimiento => CPL-1744                                                  --
...
FUNCTION Agrupa_interes()
   ...
   LET ins_cursor = "INSERT INTO safre_tmp:dis_cuenta_retbnx values (?,?,0,?,?,?,NULL,0,?,?,?,?,0,0,0,NULL,?,5,TODAY,?,TODAY,1)"
   ...
   FOREACH curint INTO g_int.*
      ...
      IF vremanente = "S" THEN
         IF vmonto_nor <> 0 THEN
            LET vid_aportante = "RENDIM-BNX" # se realiza cambio de leyenda de INFONAVIT a RENDIM-BNX CPL-1744
            PUT ins_cur FROM g_int.tipo_movimiento,
                             g_int.subcuenta, vfolio, g_int.consecutivo_lote,
                             g_int.nss, g_int.fecha_valor, g_int.fecha_valor,
...
FUNCTION Inicializa_tabla()
   WHENEVER ERROR CONTINUE
      DATABASE safre_tmp
         DROP TABLE dis_cuenta_retbnx

         CREATE TABLE dis_cuenta_retbnx
           (
