...
GLOBALS
   ...
   DEFINE vtasa_tot DECIMAL(16,6),
          vtasa_ord DECIMAL(16,6),
          vtasa_rem DECIMAL(16,6)

   DEFINE vfecha_valorx date
...
FUNCTION Llena_tabla_tasa()

   CREATE TEMP TABLE tmp_tasa
      (fecha_tasa DATE,
       tasa_tot   DECIMAL(16,6),
       tasa_ord   DECIMAL(16,6),
       tasa_rem   DECIMAL(16,6))

   CREATE index tmp_tasa_1 ON tmp_tasa(fecha_tasa)

   INSERT INTO tmp_tasa
   SELECT o.tasa_fecha,
          rem.tasa_valor,
          o.tasa_valor,
          (rem.tasa_valor-o.tasa_valor)
   FROM   tab_tasa_ordinaria o,
          tab_tasa_remanente rem
   WHERE  o.tasa_fecha = rem.tasa_fecha
   AND    o.tasa_origen = "VIV"
   AND    o.tasa_origen = rem.tasa_origen
   AND    (rem.tasa_valor-o.tasa_valor <> 0)
...
FUNCTION Integra_interes()
  ...
  LET ins_cursor = "INSERT INTO safre_tmp:dis_cuenta_int values ( 3, ?,0,?,?,?,NULL,0,?,?,?,?,0,0,0,NULL,?,5,TODAY,?,TODAY,1)"
  ...
  FOREACH curint INTO g_int.*
    ...
    IF g_int.fecha_valor = fecha_rem() THEN
       LET vmonto_nor=g_int.monto_en_pesos / vtasa_tot * vtasa_ord
       LET vmonto_rem=g_int.monto_en_pesos / vtasa_tot * vtasa_rem
       LET vremanente="S"
    ELSE
       LET vtasa_tot = 0
       LET vtasa_ord = 0
   ...
   IF vremanente = "S" THEN
      IF vmonto_nor <> 0 THEN
         ...
         PUT ins_cur FROM g_int.subcuenta, vfolio, vconsec, g_int.nss,
...
FUNCTION fecha_rem()
   ...
   INTO   vfecha_valorx,
          ...
          vtasa_rem
   FROM   tmp_tasa
   WHERE  fecha_tasa   = g_int.fecha_valor

   RETURN vfecha_valorx
