...
FUNCTION genera_det()
   ...
   FOREACH curdet INTO reg_det.*
     ...
     SELECT razon_social
     INTO   reg_det.siefore1
     FROM   tab_siefore_local
     LET reg_det.siefore1 = reg_det.siefore1 CLIPPED
         ...
         IF STATUS <> NOTFOUND THEN
            ...
            LET vcontador = vcontador + 1
               ...
               --Para el numero de acciones------------------
               SELECT SUM(monto_en_acciones) * 1000000
               INTO  vnum_acc1
               ...
               AND   tipo_movimiento  in (1,2,3,100,101,102,103,104) 
                                         --1=aporta,2= actualizacio,3=int tran
                                         --100,101,102,103,104 = consecutivos

               IF vnum_acc1 IS NULL THEN
                     LET vnum_acc1   = 0
               END IF
               ...
               IF vinteres_rcv IS NULL THEN
                  LET vinteres_rcv = 0
               END IF

               LET vnum_acc1 = vnum_acc1 + vinteres_rcv

               LET reg_det.num_accion1    = vnum_acc1


               --Para el valor de la accion  -----------------
               LET reg_det.valor_accion1  = vvalor_acc1 

               --Para el importe de la comision de flujo --------------
               SELECT SUM(monto_en_pesos) * 100
               INTO  reg_det.impt_comision
               FROM  safre_tmp:tmp_discuenta
               WHERE nss              = reg_det.nss
               AND   folio_sua        = reg_det.folio_sua
               AND   id_aportante     = reg_det.nrp_patron
               ...
               IF reg_det.impt_comision IS NULL THEN
                     LET reg_det.impt_comision = 0
               END IF

               --Para el importe de la comision cobrada por saldo--------------
               SELECT SUM(monto_en_acciones),fecha_conversion
...
---               AND   tipo_movimiento  in (1,2)
               ...
               RETURNING vimpt_com_saldo

               LET vimpt_com_saldo = vimpt_com_saldo * 100

               IF vimpt_com_saldo IS NULL THEN 
                  LET vimpt_com_saldo = 0 
               END IF

               IF reg_det.impt_total_rcv IS NULL THEN 
                  LET reg_det.impt_total_rcv = 0 
               END IF

               LET vsuma_rcv = reg_det.impt_total_rcv
              
               IF reg_det.impt_total_vol IS NULL THEN 
                  LET reg_det.impt_total_vol = 0 
               END IF

               LET vsuma_vol = reg_det.impt_total_vol

               IF reg_det.impt_total_gub IS NULL THEN 
                  LET reg_det.impt_total_gub = 0 
               END IF

               LET vsuma_gub = reg_det.impt_total_gub

               LET vtotal_rcv = vtotal_rcv +(vsuma_rcv + vsuma_vol + vsuma_gub)
               ...
               LET reg_det.impt_total_viv = vinteres * 100
  
               LET viv = reg_det.impt_total_viv

               LET vtotal_viv = vtotal_viv + viv
               ...
               --INGRESANDO LOS REGISTROS A dis_cuenta POR VIVIENDA
               IF vtotal_viv  >  0 THEN
                  CALL Agrega_a_dis_cuenta(reg_det.nss,0,vtotal_viv,
                                           vvalor_acc1,4,today)
               END IF


               --INGRESANDO LOS REGISTROS A dis_cuenta <> VIVIENDA
               CALL Agrega_a_dis_cuenta(reg_det.nss,vcom_saldo,vtotal_viv,
                                        vvalor_acc1,1,today)



               OUTPUT TO REPORT rep_rcv(reg_det.*)
   ...
   FORMAT
      ON EVERY ROW
         PRINT COLUMN 1,reg_det.tipo_registro,
                        ...
                        reg_det.fecha_pago,
                        reg_det.fecha_valor_rcv,
                        reg_det.fecha_valor_viv,
                        reg_det.folio_sua,
                        reg_det.nrp_patron,
                        reg_det.impt_total_rcv USING "&&&&&&&",  
                        reg_det.impt_total_vol USING "&&&&&&&",
                        reg_det.impt_total_viv USING "&&&&&&&",
                        reg_det.impt_total_gub USING "&&&&&&&",
                        reg_det.impt_comision  USING "&&&&&&&",
                        vimpt_com_saldo        USING "&&&&&&&",
                        reg_det.fecha_pago_gub,
                        reg_det.ident_viv_gara,
                        reg_det.mov_afect_cta,
                        reg_det.fecha_mov,
                        reg_det.afore_disper,
                        reg_det.afore_admin,
                        reg_det.siefore1,        --**
                        reg_det.num_accion1   USING "&&&&&&&&&&&&&&&&",   --**
                        reg_det.valor_accion1 USING "&&&&&&&&&&&",   --**
                        reg_det.fecha_accion1,   --**
                        reg_det.siefore2,        --**
                        reg_det.num_accion2 USING "&&&&&&&&&&&&&&&&",     --**
                        reg_det.valor_accion2 USING "&&&&&&&&&&&",   --**
                        reg_det.fecha_accion2,   --**
                        reg_det.resultado,       --**
                        reg_det.diag1,
                        reg_det.diag2,
                        reg_det.diag3,
...
FUNCTION genera_sum()
  ...
  SELECT *
  INTO   reg_sum.*
  FROM   safre_tmp:tmp_sum_aporte

   LET reg_sum.tipo_registro     = "09"
   LET reg_sum.ident_servicio    = "03"
   LET reg_sum.ident_operacion   = "60"
   LET reg_sum.tipo_ent_origen   = "01"
   LET reg_sum.cve_ent_origen    = vcodigo_afore
   LET reg_sum.tipo_ent_destino  = "03"
   LET reg_sum.cve_ent_destino   = "001"

   LET vsalida = g_param_dis.ruta_envio CLIPPED,"/sum_apo"

   START REPORT rep_sum TO vsalida
   LET vfec_creac_lote  = reg_sum.fecha_creac_lote
   ...
   FORMAT
      ON EVERY ROW
         ...
         PRINT COLUMN 1,reg_sum.tipo_registro,
            ...
            reg_sum.cve_ent_origen,
            reg_sum.tipo_ent_destino,
            reg_sum.cve_ent_destino,
            reg_sum.fecha_creac_lote,
            vtotal_rcv             USING '&&&&&&&&&&&&&&&',
            vtotal_viv USING '&&&&&&&&&&&&&&&',
            vcontador              USING '&&&&&&&&',
            228 SPACES
...
FUNCTION genera_archivo()
   ...
   LET ejecuta="cat ",g_param_dis.ruta_envio CLIPPED,"/cza_apo ",
                      g_param_dis.ruta_envio CLIPPED,"/det_apo ",
                      g_param_dis.ruta_envio CLIPPED,"/sum_apo > ",
                     g_param_dis.ruta_envio CLIPPED,"/",vfec_creac_lote,".ERROR"
...
##------------------------------------------------------------------------
FUNCTION Agrega_a_dis_cuenta(vn_seguro,vmto_acc,vtotal_viv2,vvalor_acc,vsubc,fconver)

  DEFINE vn_seguro      CHAR(11)
  DEFINE vsubc          SMALLINT
  DEFINE vmto_acc       DECIMAL(15,2)
  ...
  IF vsubc  =  4  THEN
       ...
       LET reg_dis.monto_en_pesos    =   vtotal_viv2 *(-1)
       LET reg_dis.monto_en_acciones =   0.0
       LET reg_dis.siefore           =   0      
       LET reg_dis.precio_accion     =   0
    ELSE
       LET reg_dis.subcuenta         =   1
       LET reg_dis.monto_en_pesos    =   vvalor_acc * vmto_acc *(-1)
       LET reg_dis.monto_en_acciones =   vmto_acc * -1
       LET reg_dis.siefore           =   1
       LET reg_dis.precio_accion        =   vvalor_acc
    END IF
    ...
    LET reg_dis.fecha_pago           =   today+1
    LET reg_dis.fecha_valor          =   today+1
    LET reg_dis.fecha_conversion     =   today+1
    LET reg_dis.dias_cotizados = 0
    LET reg_dis.sucursal             =   ' '     --?
    LET reg_dis.id_aportante         =   "AC-"
    LET reg_dis.estado               =   0
    LET reg_dis.fecha_proceso        =   today+1
    LET reg_dis.usuario              =   vusuario 
    LET reg_dis.fecha_archivo        =   today+1
