...
#                  => dis_cza_bono, dis_sum_bono) de BONO REDENCION ANUAL     #
#                  => GENERA ARCHIVO de confirmacion BONOCONF                 #
#Fecha Inicio      => 11 Marzo 2011                                           #
#Elaboro           => DMR                                                     #
#Modificaciones    => (Este programa recibe archivo,redencion anual del bono  #
#                  envia montos por siefore de los pesos generados por UDIS)  #
#Fecha Mod         =>                                                         #
#                  =>                                                         #
###############################################################################
DATABASE safre_af
...
FUNCTION Genera_aportes_nor(vfolio)    -- ETAPA 5
   DEFINE
     ...
     impt_bono36               DECIMAL(22,4),
     bono_det                  DECIMAL(22,4)
   ...
   FOREACH cur_val INTO dcurp
      ...
      ---- validacion que nss exista en cta_regimen y tenga saldo en sub 36 ----
      IF vind_rechazo = 0 THEN
         ...
         IF cont_regi = 0 THEN                              --c22-6.3
            DISPLAY "NSS no existe en cta_regimen ",dnss
            LET vind_rechazo = 1
         ELSE
            ...
            IF impt_bono36 <= 0 THEN 
               DISPLAY "NSS sin saldo en subcuenta 36 ",dnss
               LET vind_rechazo = 1
            ELSE
               SELECT importe_bono/10000
               INTO bono_det
               FROM dis_det_bono
               WHERE folio = vfolio
               AND   n_unico = dcurp

               IF bono_det > impt_bono36 THEN
                  DISPLAY "NSS con saldo en BONO MENOR al SOLICITADO ",dnss
                  LET vind_rechazo = 1
               END IF
            END IF
         END IF
...
FUNCTION Genera_confaf(vfolio)          -- ETAPA 7
   ...
   (consec   INTEGER,
    ...
    idsie    CHAR(08),
    pesos    DECIMAL(18,4),
    acciones DECIMAL(18,4))                                        --c22-6


   ---- APORTE DBMX ----
   INSERT INTO tmp_res_provision                                   --c22-6
   SELECT A.consecutivo_lote,                                      --c22-6
          "41",                                                    --c22-6
          A.siefore,                                               --c22-6
   ...
   SELECT consec FROM tmp_res_provision

   FOREACH cur_act INTO conseca
      SELECT monto_en_acciones
      INTO  mon_acc
      FROM  dis_provision
      WHERE folio = vfolio
      AND   subcuenta = 36
      AND   consecutivo_lote = conseca

      UPDATE tmp_res_provision
      SET   acciones = mon_acc
      WHERE consec   = conseca
   END FOREACH
...
--   WHERE folio = vfolio
   ...
   FOREACH cur_resumen INTO vsub,vsie,vpor,vident_sie,vpesos,vacciones
...
--    AND    ident_pago[14,15] = vsub                          --c22-6
      ...
      IF vcuantos = 1 THEN                                     --c22-6.3
         ...
         SET    importe_bono     = importe_bono / 10000,    --c22-6.5
                ...
                impt_bono_acept  = vacciones,               --c22-6
                impt_bono_dev    = 0,  --vrechazo,          --c22-6.9
                ident_siefore    = vident_sie,              --c22-6
                tipo_siefore     = vsie                     --c22-6
         WHERE  folio            = vfolio                   --c22-6
         AND    ident_pago[14,15]= "49"                     --c22-6
         AND    tipo_siefore     = 0                        --c22-6

         IF vrechazo > 0 THEN
            LET g_dep.importe_dbmx = g_dep.importe_dbmx / 100
      ...
      ELSE                                                  --c22-6.3
         ...
         IF vcontador_sie = 1 THEN                          --c22-6.3
            ...
            SET    importe_bono     = importe_bono / 10000, --c22-6.5
                   importe_dbmx     = importe_dbmx / 100,   --c22-6.5
                   impt_bono_acept  = vacciones,            --c22-6.3
            ...
            AND    ident_pago[14,15]= "49"                  --c22-6.3
            AND    tipo_siefore     = 0                     --c22-6.3

            IF vrechazo > 0 THEN
               LET vimporte = g_dep.importe_bono /10000
               LET vimporte2= g_dep.importe_dbmx /100
        ...
        --  END IF                                          --c22-119
            ...
            IF vcontador_sie >= 2 THEN
               IF vrechazo > 0 THEN                         --c22-119
                  LET vimporte = g_dep.importe_bono         --c22-119
                  LET vimporte2= g_dep.importe_dbmx         --c22-119
               ELSE                                         --c22-119
                  LET vimporte = g_dep.importe_bono /10000  --c22-119
                  LET vimporte2= g_dep.importe_dbmx /100    --c22-119
               END IF                                       --c22-119
            END IF                                          --c22-119
   ...
   FOREACH curctar INTO rident_pago
      UPDATE dis_dep_bono
      SET impt_bono_dev = recha_bono,
          impt_dbmx_dev = recha_dbmx,
