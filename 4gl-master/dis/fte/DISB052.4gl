...
#Proyecto          => safre_af                                                #
#Owner             => E.F.P                                                   #
#Programa DISB052  => EJECUTA VIA BATCH PROCESO DE CALCULO INTERESES EST EN   #
#                  => TRANSITO PROVISION                                      #
...
MAIN
   ...
   ATTRIBUTE( BORDER)
   DISPLAY " DISB052 " AT 3,1 ATTRIBUTE(REVERSE)
   DISPLAY HOY USING " dd-mm-yyyy " AT 3,60 ATTRIBUTE(REVERSE)

   MENU "MENU EST "
     COMMAND "Provision" "Calculo intereses EST en transito Provision"
       CALL   Procesa(1)
     COMMAND "Liquidacion" "Liquidacion intereses EST en transito"
       CALL   Procesa(2)
     COMMAND "Consultas" "Consulta de intereses EST en transito"
       LET ejecuta = "fglgo DISL015.4gi "
       RUN ejecuta
     COMMAND "Reverso" "Reverso de intereses EST en transito"
       LET ejecuta = "fglgo DISB041.4gi "
       RUN ejecuta
...
FUNCTION Procesa(paso)
 ...
 -- DISPLAY "                     Aplicacion de Intereses por Afore                                          " AT 13,1 ATTRIBUTE(REVERSE)
	...
	LET x_precio_accion = 1
	    ...
	    ERROR "Este campo no puede se nulo"
	...
	ELSE
	   ERROR  "PROVISION DE ESTE ARCHIVO, YA REALIZADA ANTERIORMENTE"
	   NEXT FIELD nom_archivo
	END IF
	...
	DECLARE cursor_tmp CURSOR FOR
	  ...
	  WHERE  n_registros[93,93] in ('3')  --ESTATALES
 ...
 ELSE
    ---------------------- PARA LIQUIDACION EST-------------------------
    OPEN WINDOW ventana_3 AT 2,2 WITH FORM "DISB0271" ATTRIBUTE(BORDER)
    DISPLAY " DISB027       LIQUIDACION INTERESES EST, RECURSOS EN TRANSITO                  " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY "                     Intereses recibidos de Operadora                                        " AT 6,1 ATTRIBUTE(REVERSE)
 -- DISPLAY "                     Aplicacion de Intereses por Afore                                            " AT 13,1 ATTRIBUTE(REVERSE)
...
DEFINE
  ...
  LET ejecuta = "nohup time fglgo DISB026.4gi ",
 			                       g_reg.fecha_aplica," ",
					       g_reg.nom_archivo,"  & "
  RUN ejecuta
...
FUNCTION Ejecuta_batch()
  ...
  INSERT INTO dis_ctrl_proceso VALUES
    ...
    ERROR "Ejecutando Calculo de Interes por nohup ..."
    SLEEP 3
    LET ejecuta = "nohup time fglgo DISB027.4gi ",
						 g_reg2.fecha_aplica," ",
						 g_reg2.fecha_accion," ",
