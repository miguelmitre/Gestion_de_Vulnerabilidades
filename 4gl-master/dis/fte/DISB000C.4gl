...
-- Propietario  => E.F.P.                                                    --
-- Programa     => DISB000                                                   --
-- Descripcion  => PROCESO BATCH DISPERSION Y LIQUIDACION                    --
-- Sistema      => DIS                                                       --
...
MAIN
   ...
   OPEN WINDOW ventana1 AT 2,2 WITH FORM "DISB000C1" ATTRIBUTE(BORDER)

   DISPLAY " DISB000C          DISPERSION BATCH LIBERACION TERMINAL                       " AT 3,1 ATTRIBUTE(REVERSE)

   DISPLAY hoy USING "DD-MM-YYYY" AT 3,67 ATTRIBUTE(REVERSE)

   DISPLAY "     CALCULO INTERESES VIVIENDA, INTREGRACION, Y ACTUALIZACION ETIQUETA          " AT 6,1 ATTRIBUTE(REVERSE)

   MENU "BATCH"
      COMMAND "Calculo"                  "Calculo intereses VIV en cta_interes_viv"
         CALL Intereses()
      COMMAND "Agrupacion"               "Agrupa intereses en dis_cuenta_int"
         CALL Agrupacion()
      COMMAND "Integracion"              "Integra intereses a dis_cuenta"
         CALL Integracion()
      COMMAND "Respaldo 888"             "Respaldo 888"
         CALL Respaldo888()
----      COMMAND "Actuliaza etiquetas"      "Actualiza etiquetas"
----         CALL Actualiza_etiquetas()
...
FUNCTION Ejecuta_intereses()
   ...
   INSERT INTO dis_ctrl_proceso VALUES 
      (TODAY,                   -- fecha_proceso
       "DISB009B",              -- proceso_cod
       3,                       -- etapa_cod  -- CALCULO RENDIMIENTOS
   ...
   SLEEP 3
   LET ejecuta = "nohup time fglgo DISB009B.4gi 0 0 0 ",
                                                g_reg.tasa_interes," ",
...
FUNCTION Ejecuta_Agrupacion()
   ...
   INSERT INTO dis_ctrl_proceso VALUES 
      (TODAY,                   -- fecha_proceso
       "DISB034B",              -- proceso_cod
       4,                       -- etapa_cod  -- Agrupacion intereses
       hora_inicial,            -- hora_inicial
       hora_final,              -- hora_final
       "S",                     -- parametro1
       "",                      -- parametro2
   ...
   SLEEP 3
   LET ejecuta = "nohup time fglgo DISB034B.4gi 0 0 0 & "   
   RUN ejecuta
   ERROR "El proceso se ejecuto satisfactoriamente por nohup"
   SLEEP 3
   ERROR ""
...
FUNCTION Ejecuta_Integracion()
   ...
   IF STATUS = NOTFOUND THEN
      ERROR "No existen datos en dis_cuenta_int para integrar en dis_cuenta"
      SLEEP 3
      CLOSE cur_integra
      RETURN
   END IF
   ...
   INSERT INTO dis_ctrl_proceso VALUES 
      (TODAY,                   -- fecha_proceso
       "DISB037B",              -- proceso_cod
       7,                       -- etapa_cod  -- Integra intereses en dis_cuenta
       hora_inicial,            -- hora_inicial
       hora_final,              -- hora_final
       "S",                     -- parametro1
       "",                      -- parametro2
   ...
   SLEEP 3
   LET ejecuta = "nohup time fglgo DISB037B.4gi 0 0 0 & "   
   RUN ejecuta
   ERROR "El proceso se ejecuto satisfactoriamente por nohup"
   SLEEP 3
   ERROR ""
...
FUNCTION Ejecuta_respaldo888()
   ...
   INSERT INTO dis_ctrl_proceso VALUES 
      (TODAY,                   -- fecha_proceso
       "DISB035B",              -- proceso_cod
       5,                       -- etapa_cod  -- Respaldo 888
       hora_inicial,            -- hora_inicial
       hora_final,              -- hora_final
   ...
   SLEEP 3
   LET ejecuta = "nohup time fglgo DISB035B.4gi 0 0 0 ",g_reg.fecha_aplicacion,"  & " 
   RUN ejecuta
   ERROR "El proceso se ejecuto satisfactoriamente por nohup"
   SLEEP 3
   ERROR ""
...

{
   ...
   IF opc MATCHES '[Ss]' THEN
      CALL Ejecuta_actualiza()
   END IF
   CLEAR FORM
   CLEAR SCREEN
END FUNCTION

FUNCTION Ejecuta_actualiza()
   ...
   INSERT INTO dis_ctrl_proceso VALUES 
      (TODAY,                   -- fecha_proceso
       "DISB036B",              -- proceso_cod
       6,                       -- etapa_cod  -- Actualiza etiquetas
       hora_inicial,            -- hora_inicial
       hora_final,              -- hora_final
       g_reg.fecha_aplicacion,  -- parametro1
       NULL,                    -- parametro2
       ...
       0)                 -- usuario
   IF STATUS < 0 THEN
      ERROR "ERROR AL INSERTA EN TABLA dis_ctrl_proceso Actualiza etiquetas ",STATUS
      SLEEP 4
      EXIT PROGRAM
   END IF

   ERROR "Ejecutando Actuazliacion etiquetas por nohup ..."
   SLEEP 3
   LET ejecuta = "nohup time fglgo DISB036B.4gi ",g_reg.fecha_aplicacion," & "  
   RUN ejecuta
   ERROR "El proceso se ejecuto satisfactoriamente por nohup"
   SLEEP 3
   ERROR ""
