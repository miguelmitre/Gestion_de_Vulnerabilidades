...
MAIN
   ...
   DROP TABLE mapa_temp
   CREATE TABLE mapa_temp
   (
    nom_archivo CHAR(16)
   )
   WHENEVER ERROR CONTINUE

   RUN "/safre/dis/fte/gen_arch.sh"

   LOAD FROM "/safre/dis/fte/arch_dis" INSERT INTO mapa_temp

   DECLARE cur_archivo CURSOR FOR
   SELECT dm.folio, dm.nom_archivo
     FROM dis_mapa dm, mapa_temp mt
    WHERE dm.nom_archivo = mt.nom_archivo
      AND dm.estatus = 0

   FOREACH cur_archivo INTO reg_ident.folio,
			    reg_ident.nom_archivo
      LET hora_ini = TIME
      DISPLAY "Inicio : ", reg_ident.nom_archivo, " ",hora_ini 
      CALL carga_archivos()
      LET hora_fin = TIME
      DISPLAY "Fin : ", hora_fin 

      UPDATE dis_mapa
	 SET estatus = 1
       WHERE nom_archivo = reg_ident.nom_archivo
