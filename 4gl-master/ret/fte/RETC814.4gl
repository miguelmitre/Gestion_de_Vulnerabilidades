...
#Programa RETC814  => GENERACION DE SALDOS DEL PROCESO POR LEY 73(OPER.54) "B"  #
#                     TIPO DE RETIRO B                                          #
#By                => DMR                                                       #
#Fecha Elaboracion.=> 04 DE FEBRERO DEL 2004                                    #
#Fecha Actualizacion> 04-Agosto-2004                                            #
#Sistema           => RET                                                       #
#Actualizo         => IJR Jueves 12/Dic/2004                                    #
#                  => IJR 090805 diag302 con decimos y AIV's                    #
#                  =>                                                           #
...
MAIN
    ...
    CALL primer_paso()        #pp --Asignacion de diagnostico

    DISPLAY " PROCESANDO SEGUNDO PASO              " AT 15,10 ATTRIBUTE(REVERSE)
    CALL segundo_paso()       #sp --calc. saldos tipo pension por muerte (OR,AS,VI,VO)

    DISPLAY " PROCESANDO TERCER PASO               " AT 15,10 ATTRIBUTE(REVERSE)
    CALL tercer_paso()        #tp --genera saldos para registros que son por muerte

    DISPLAY " PROCESANDO CUARTO PASO               " AT 15,10 ATTRIBUTE(REVERSE)
    CALL cuarto_paso()        #cp --actualiza saldos en cero para registros rechazados
    ...
    CLOSE WINDOW retc8141
    CALL f_abre_ventana()

    DISPLAY BY NAME reg_3.*

    DISPLAY "TOTAL REGISTROS DE SOLICITUDES : ",cont_det_pe02 AT 12,19
    ...
    DISPLAY "TOTAL REGISTROS PROVISIONADOS  : ",cont_provi    AT 14,19
...
#pp------------------
   ...
   DEFINE
    ...
    --SELECCIONA TODOS LOS REGISTROS RECEPCIONADOS PARA EL TIPO DE RETIRO "B"--
    ...
    FOREACH cur_2 INTO reg_01.*, c1_estado_sub_viv
        ...
        --Se guarda el diagnostico que originalmente tree la solicitud--
        LET v_diag_registro = reg_01.diag_registro

        LET reg_01.diag_registro = 501 --CTA ACEPTADA CON TRANSF. DE RECURSOS
                ...
                ELSE
                    ...
                    ELSE
                        ...
                        IF s_nss_traspasado = 1 THEN
                           ...
                           IF reg_01.tipo_pension <> "VI" AND
                              reg_01.tipo_pension <> "VO" AND
                              reg_01.tipo_pension <> "OR" AND
                              reg_01.tipo_pension <> "AS" THEN
                              --No es por muerte
        ...
        -- PENSIONES ANTERIORES AL INICIO DE OPERACION DE LAS AFORES
        IF (reg_01.fecha_ini_pen < "07/01/1997") AND
           ...
           IF reg_01.tipo_pension <> "VI" AND reg_01.tipo_pension <> "VO" AND
              reg_01.tipo_pension <> "OR" AND reg_01.tipo_pension <> "AS" THEN
...
#sxp-----------------
    ...
    WHERE  A.folio             = reg_3.folio_oper_02
    AND    A.tipo_pension      IN ("VI","VO","OR","AS") --LOS QUE SON POR MUERTE
    AND    A.tipo_retiro       = gs_tipo_retiro
    AND    A.folio             = B.folio
    AND    A.nss               = B.nss
    ...
    --TODOS LOS REGISTROS PARA MUERTE
    FOREACH cur_62 INTO reg_13.*, monto_viv_ori_02, estado_sub_viv
        ...
        DECLARE c_saldo2 CURSOR FOR eje_saldo_dia

        FOREACH c_saldo2 USING reg_13.nss,
                              v_subcuenta,  --v_subcuenta=0 y v_grupo=0 trae todas las subcuentas
                              v_grupo,
                              HOY
                         INTO v_subcuenta,
                              v_siefore,
            ...
            IF v_subcuenta = 2  OR  v_subcuenta = 5  OR  v_subcuenta = 6  OR
               v_subcuenta = 9  OR  v_subcuenta = 4  THEN
                ...
                IF f_monto_acc > 0  THEN
                    ...
                    ELSE
                        ...
                        CALL f_provisiona_subcta(reg_13.nss         ,
                                                 v_siefore          ,
                                                 v_subcuenta        ,
                                                 reg_13.consecutivo ,
                                                 f_monto_acc        ,
        ...
        FOR ls_siefore = 1 TO gs_num_siefores
            ...
            IF arr_siefore[ls_siefore].activo = TRUE THEN
                ...
                LET d6_monto_acc_rcv = arr_siefore[ls_siefore].monto_accion_cv  +
                                       arr_siefore[ls_siefore].monto_accion_es  +
                                       arr_siefore[ls_siefore].monto_accion_esp +
        ...
        IF reg_11.monto_parti_viv <> 0 THEN
            LET reg_11.monto_parti_viv  = reg_11.monto_parti_viv + f_obtiene_vivieda92(reg_13.nss           ,
                                                                                      reg_13.fecha_ini_pen ,
                                                                                      reg_13.diag_registro ,
                                                                                      "M")
        END IF
...
#=============================================================================#
# Objetivo : Calcular saldos para el Tipo de Pension que no son por Muerte    #
#            IV, IP                                                           #
#=============================================================================#
...
#tp------------------
    ...
    DEFINE
        ld_viv_92             DECIMAL(14,2)

    ---------------------------------------------------------------------------
    --VERIFICA SI EXISTEN DATOS PARA EL TIPO DE PENSION Y EL TIPO DE RETIRO "B"
    ---------------------------------------------------------------------------
    SELECT COUNT(*)
    INTO   i_tot_registros
    FROM   ret_transf_rx A , ret_transf_tx B
    WHERE  A.folio             = reg_3.folio_oper_02
    AND    A.tipo_pension NOT IN ("VI","VO","OR","AS")  --LOS QUE NO SON POR MUERTE
    AND    A.tipo_retiro       = gs_tipo_retiro
    AND    A.folio             = B.folio
    AND    A.nss               = B.nss
    AND    A.consecutivo       = B.consecutivo
    ...
    ------------------------------------------------------------------------
    --SELECCIONA LOS DATOS PARA EL TIPO DE PENSION Y EL TIPO DE RETIRO "B"
    ------------------------------------------------------------------------
    ...
    FROM   ret_transf_rx A , ret_transf_tx B
    WHERE  A.folio              = reg_3.folio_oper_02
    AND    A.tipo_pension  NOT IN ("VI","VO","OR","AS")  --LOS QUE NO POR MUERTE
    AND    A.tipo_retiro        = gs_tipo_retiro
    AND    A.folio              = B.folio
    AND    A.nss                = B.nss
    AND    A.consecutivo        = B.consecutivo
    ...
    FOREACH cur_6 INTO reg_01.*, monto_viv_ori_02, estado_sub_viv
        ...
        LET v_diag_procesar         = reg_01.diag_registro  -- Diagnostico de procesar
        LET reg_01.diag_registro    = 501
        ...
        -- Verifica y elimina todos los registros que sean anteriores a un traspaso
        CALL f_verifica_traspasos_afore(reg_01.nss)
        ...
        FOREACH c_saldo_dia  USING reg_01.nss     ,
            ...
            IF reg_14.subcuenta = 2  OR  reg_14.subcuenta = 5  OR  reg_14.subcuenta = 6
            OR reg_14.subcuenta = 9  OR  reg_14.subcuenta = 4 THEN
                ...
                IF reg_14.subcuenta = 4 THEN
                    ...
                    LET ld_viv_92   = f_obtiene_vivieda92(reg_01.nss            ,
                                                          reg_01.fecha_ini_pen  ,
                                                          v_diag_procesar       ,
                                                          "V")
        ...
        IF reg_15.monto_accion_viv <> 0 THEN
            LET reg_15.monto_accion_viv = reg_15.monto_accion_viv + ld_viv_92
        END IF

        IF reg_01.diag_registro = 501 THEN
           --SOLO SE ACTUALIZA VIVIENDA--
           UPDATE ret_transf_tx
...
#cp------------------
    ...
    DEFINE lr_viv RECORD
        nss                   LIKE ret_transf_rx.nss         ,
        consecutivo           LIKE ret_transf_rx.consecutivo ,
        viv_rx                LIKE ret_transf_rx.saldo_viv97 ,
        viv_tx                LIKE ret_transf_tx.saldo_viv97
   END RECORD
    ...
    --Actualizamos el monto de vivienda a reportar y eliminamos las provisiones de vivienda
    DECLARE cur_viv_rec CURSOR FOR
    SELECT A.nss                    ,
           A.consecutivo            ,
           A.saldo_viv97  viv_rx    ,
           B.saldo_viv97  viv_tx
    FROM   ret_transf_rx    A ,
           ret_transf_tx    B
    WHERE  A.folio          = B.folio
    AND    A.nss            = B.nss
    AND    A.folio          = reg_3.folio_oper_02
    AND    A.tipo_retiro    = gs_tipo_retiro

    FOREACH cur_viv_rec INTO lr_viv.*
        UPDATE ret_transf_tx
        SET    saldo_viv97      = lr_viv.viv_rx
        WHERE  nss              = lr_viv.nss
        AND    consecutivo      = lr_viv.consecutivo
        AND    folio            = reg_3.folio_oper_02
        
        DELETE
        FROM   dis_provision
        WHERE  nss              = lr_viv.nss
        AND    consecutivo_lote = lr_viv.consecutivo
        AND    folio            = reg_3.folio_oper_02
...
#sp------------------
    ...
    LET ruta_det_nss = s_modulo.ruta_envio CLIPPED, "/", "DET-NSS-B-tmp"
    LET ruta_det_nss = ruta_det_nss CLIPPED

    LET ruta_det_sie = s_modulo.ruta_envio CLIPPED, "/", "DET-SIE-B-tmp"
    LET ruta_det_sie = ruta_det_sie CLIPPED

    LET ruta_det_viv = s_modulo.ruta_envio CLIPPED, "/", "DET-VIV-B-tmp"
    LET ruta_det_viv = ruta_det_viv CLIPPED

    LET G_LISTA_DET = s_modulo.ruta_envio CLIPPED, "/", "DET-B"
    ...
    FOREACH cur_8 INTO r_transf_tx.*, r_transf_rx.*
...
#dnl----------------------
    ...
    FOREACH cur_des INTO nssa, consec, vdiag
       ...
       LET reg_20.estado_marca = 40 --DESMARCA POR VALIDACION INTERNA--
...
#=============================================================================#
# Funcion  : redondea_val                                                     #
# Objetivo : Dado un monto indicado en p_monto_redondear lo redondea a tantos #
#            decimales como se indica en p_redondea.                          #
#=============================================================================#
...
FUNCTION f_abre_ventana()
    ...
    OPEN WINDOW retc8141 AT 4,4 WITH FORM "RETC8141" ATTRIBUTE(BORDER)
    DISPLAY "   < Ctrl-C > Salir                                        RETIRO 'B'       " AT 1,1 ATTRIBUTE(REVERSE)
    DISPLAY " RETC814           GENERA SALDOS DE DEVOLUCION POR LEY 73                   " AT 3,1 ATTRIBUTE(REVERSE)
...
FUNCTION f_verifica_traspasos_afore(p_nss)

    DEFINE p_nss LIKE ret_transf_rx.nss

    DEFINE #loc #date
        ld_max_fecha_banxico    DATE

    -- Obtiene la fecha del ultimo traspaso
    ...
    IF STATUS <> NOTFOUND THEN
        ...
        FROM   tmp_dis_cuenta
        WHERE  nss              = p_nss
        AND    fecha_conversion < ld_max_fecha_banxico
    END IF


END FUNCTION

FUNCTION f_obtiene_vivieda92(pr_viv92)

    DEFINE pr_viv92 RECORD
        nss             LIKE dis_provision.nss              ,
        fip             LIKE ret_transf_rx.fecha_ini_pen    ,
        diag_procesar   LIKE ret_transf_rx.diag_registro    ,
        tipo_pension    CHAR(1)
    END RECORD

    DEFINE lr_mtos RECORD
        subcuenta           SMALLINT      ,
        siefore             SMALLINT      ,
        monto_acc           DECIMAL(16,6) ,
        monto_pesos         DECIMAL(16,6)
    ...
    DEFINE
        lr_ap_posteriores       ,
        lr_monto_viv92          DECIMAL(10,2)


    LET lr_mtos.subcuenta   = 8
    LET lr_mtos.siefore     = 11
    LET lr_mtos.monto_acc   = 0
    LET lr_mtos.monto_pesos = 0

    SELECT nvl(SUM(monto_en_acciones),0)
    INTO   lr_monto_viv92
    FROM   dis_cuenta
    WHERE  nss          = pr_viv92.nss
    AND    subcuenta    = lr_mtos.subcuenta

    IF pr_viv92.tipo_pension <> "M" THEN
        LET lr_ap_posteriores = f_obten_ap_posterior(pr_viv92.nss              ,
                                                     pr_viv92.fip              ,
                                                     pr_viv92.diag_procesar    ,
                                                     lr_mtos.*)

        IF lr_ap_posteriores IS NULL THEN
            LET lr_ap_posteriores = 0
        END IF


        LET lr_monto_viv92 = lr_monto_viv92 - lr_ap_posteriores
    END IF

    RETURN lr_monto_viv92
