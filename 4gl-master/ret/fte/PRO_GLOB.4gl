...
FUNCTION Verifica_documento_probatorio(x_num,x_param)
	...
	CASE x_num
	     ...
	     WHEN 1 # ACTA DE NACIMIENTO #
		    ...
		    LET numero = op1_uno
		    SELECT "X" FROM safre_af:tab_estado
 ...
 
{
	...
	LET acumulado = acumulado + 
	    (ASCII(rfc[ind_3])) * ind_3
    END FOR

    LET cociente = acumulado / 11
    LET paso     = cociente  * 11
    LET residuo  = acumulado - paso    
    IF  residuo > 0 THEN
    ...
    IF rfc[13] <> digito THEN
       ERROR "digito incorrecto
    END IF
}
