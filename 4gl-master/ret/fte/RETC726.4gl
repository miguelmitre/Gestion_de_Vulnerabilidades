################################################################################
#Owner             => E.F.P.                                                   #
#Programa RETC726  => PRELIQUIDACION DE DISPOSICION                            #
#Fecha creacion    => 16 DE OCTUBRE DE 2009                                    #
#Fecha Nvo Retiro  =>       Noviembre 2011                                     #
#By                => JAVIER GONZALEZ JERONIMO                                 #
#Fecha actualiz.   =>                                                          #
#Actualizacion     =>                                                          #
#Sistema           => RETIROS Ref. punto 2.5   Disposición                     #
################################################################################
DATABASE safre_af
...
MAIN
    ...
    CALL STARTLOG("RETC726.log")

    CALL init() #i

    OPEN WINDOW main_win AT 4,4 WITH 19 ROWS, 75 COLUMNS ATTRIBUTE(BORDER)
    DISPLAY " RETC726    PRE LIQUIDACION DE DISPOSICION EN PESOS                " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING"DD-MM-YYYY" AT 3,65 ATTRIBUTE(REVERSE)

    MENU "PRELIQUIDACION"
      
    ---    Se elimina rutina de ejecucion de la preliquidacion   Feb 2012
    ...
    ---                CLOSE WINDOW RETC7262
    ---                CALL f_bitacora_err(gi_proceso) #-- Muestra la pantalla de errores
    ---            END IF
    ---        END IF

        COMMAND "Detalle X Siefore" "Consulta de Pre Liquidación de Disposición  "
...
#i-------------
    ...
    LET   gc_query  =  ' SELECT  COUNT(*)                                     ',
                       ...
                       '         safre_af:ret_solicitud_tx b                  ',
                       '  WHERE  a.tipo_movimiento  IN ( 820, 830 )           ',
                       '    AND  a.nss                =  b.nss                ',
...
FUNCTION f_despliega_info()
    ...
    OPEN WINDOW RETC7261 AT 4,4 WITH FORM "RETC7261" ATTRIBUTE (BORDER)
    DISPLAY " <CTRL-C> SALIR                                             <ESC> EJECUTAR    " AT 1,1 ATTRIBUTE(REVERSE)
    DISPLAY " RETC726       PRELIQUIDACION DE DISPOSICION EN PESOS                         " AT 3,1 ATTRIBUTE(REVERSE)
...
FUNCTION f_abre_ventana()
    ...
    OPEN WINDOW RETC7262 AT 4,4 WITH FORM "RETC7262" ATTRIBUTE(BORDER)
...
FUNCTION f_inserta_bitacora(pr_error)
    ...
    -- Campos generales
    LET lr_bitacora.programa    = "RETC726"
...
FUNCTION f_bitacora_err(pi_proceso)
    ...
    OPEN WINDOW RETC7263 AT 4,4 WITH FORM "RETC7263" ATTRIBUTE(BORDER)

    DISPLAY "  < Ctrl-C > Salir                                       < ESC > Consulta      " AT 1,1 ATTRIBUTE(REVERSE)
    DISPLAY " RETC726      BITACORA DE ERRORES DE PRELIQUIDACION ISSSTE                     " AT 2,1 ATTRIBUTE(REVERSE)
    DISPLAY "                                                                               " AT 3,1
...
FUNCTION f_genera_detalle(pdt_fec_detalle)
    ...
    CALL   Fnc_Filtro()
    OPEN WINDOW RETC7264 AT 4,4 WITH FORM "RETC7264" ATTRIBUTE (BORDER)
    DISPLAY " <CTRL-C> SALIR                                                               " AT 1,1 ATTRIBUTE(REVERSE)
    DISPLAY " RETC7264 CONSULTA DE PRELIQUIDACION DE DISPOSICION EN PESOS                  " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING"DD-MM-YYYY"                                                            AT 3,65 ATTRIBUTE(REVERSE)
    
    IF gar_tot[5].pesos_s > 0 THEN
        CALL SET_COUNT(4)
    ELSE
        CALL SET_COUNT(4)
    END IF
...
FUNCTION Fnc_Filtro()
   ...
   OPEN    WINDOW RETC7265                                             AT 4,4 WITH FORM "RETC7265"   ATTRIBUTE (BORDER)
   DISPLAY " <CTRL-C> SALIR                                             <ESC> EJECUTAR    " AT 1,1   ATTRIBUTE(REVERSE)
   DISPLAY " RETC7085       PRELIQUIDACION DE DISPOSICION EN PESOS                        " AT 3,1   ATTRIBUTE(REVERSE)
   DISPLAY HOY USING"DD-MM-YYYY"                                                            AT 3,65  ATTRIBUTE(REVERSE)
   ...
   WHILE TRUE
         ...
         ELSE
             ...
             CALL Fnc_Acumula1a(11, 3)
             CALL Fnc_Acumula2a(ld_fecha_conversion, 11)
             LET   ls_sale   = 1
         END IF
         EXIT WHILE
   END WHILE
   CLOSE     WINDOW RETC7265
END FUNCTION
...
FUNCTION  Fnc_Acumula1a(ls_sie, i)
  ...
  FOR i  =  i  TO  k
      CASE i WHEN  1
              ...
              LET  gar_tot[i].des_tip       =  Fnc_Movto(820)
              LET  gar_tot[i].des_tip       =  '820-', gar_tot[i].des_tip CLIPPED 
             WHEN  2
              LET  gar_tot[i].sie           =  ls_sie
              LET  gar_tot[i].des_tip       =  Fnc_Movto(830)
              LET  gar_tot[i].des_tip       =  '830-', gar_tot[i].des_tip CLIPPED 
             WHEN  3
              LET  gar_tot[i].sie           =  ls_sie
              LET  gar_tot[i].des_tip       =  Fnc_Movto(820)
              LET  gar_tot[i].des_tip       =  '820-', gar_tot[i].des_tip CLIPPED 
