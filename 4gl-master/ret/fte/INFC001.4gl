...
GLOBALS
    ...
    DEFINE gr_ret_status             RECORD #glo #gr_ret_status
...
MAIN
	...
	AFTER FIELD fecha_proceso
		    ...
		    NEXT FIELD fecha_proceso
                ...
                SELECT "OK"
                  FROM inf_his_oper97
                 WHERE estado = gr_ret_status.capturado
                 GROUP BY 1
	...
	ON KEY (ESC)
                ...
                IF STATUS = NOTFOUND THEN
	            PROMPT " NO EXISTEN REGISTRO PARA PROCESAR...<ENTER> ",
		...
		IF esta_seguro() THEN #es
		    ...
		    DISPLAY "ARCHIVO GENERADO EN LA RUTA :"  AT 10,23
                    DISPLAY g_seg_modulo.ruta_envio   AT 11,23
...
#i------------- 
    ...
    SELECT  A.status
    INTO   gr_ret_status.capturado
    FROM   ret_status A
    WHERE  descripcion = "CAPTURADO"

    SELECT A.status
    INTO   gr_ret_status.procesado
    FROM   ret_status A
...
#pp-------------------
    ...
    FOREACH cur_od INTO vident_oper    -- Por cada indentificador de oper.
       ...
       LET cabeza_oper.cve_ent_origen   = g_codigo_afore
       ...
       LET sumario_oper.cve_ent_origen    = g_codigo_afore
    ...
    SET    folio             = ultimo_folio        ,
           ...
           estado            = gr_ret_status.procesado
    WHERE  estado            = gr_ret_status.capturado
...
#rd--------------------
	...
	ON EVERY ROW
	    ...
	    PRINT
	        ...
	        COLUMN 166,g_codigo_afore USING"&&&"                 ,
