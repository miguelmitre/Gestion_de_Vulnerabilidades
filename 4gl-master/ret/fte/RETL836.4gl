...
FUNCTION archivo()
   ...
   FOREACH cur_detalle INTO gr_reporte.nss      ,
      ...
      #Saldos
      IF gs_tipo_informe = 14 OR
         gs_tipo_informe = 15 THEN
      ...
      IF gs_tipo_informe = 15 OR   --TRANSFERENCIA regimen 97
         gs_tipo_informe = 17 THEN --DISPOSICION   regimen 97
...
#CPL-1626  INI
   ...
   DECLARE cur_trans CURSOR FOR 
     ...
     FROM   tmp_dis_cuenta
     WHERE  tipo_movimiento IN (800,810,815)
...
#CPL-1626  FIN
   ...
   SELECT consecutivo_lote,
          fecha_conversion
   FROM   tmp_dis_cuenta
   WHERE  tipo_movimiento IN (820,825,830,840,850,860,880)
   AND    fecha_conversion BETWEEN ld_fecha_ini_reporte AND ld_fecha_fin_reporte
   GROUP BY 1,2
   ORDER BY 1,2
...
FUNCTION get_datos_ret(lc_nss, ld_consecutivo_disp, li_folio_trans)
   ...
   IF gs_tipo_informe = 14 OR   --TRANSFERENCIA
      gs_tipo_informe = 15 THEN --TRANSFERENCIA
