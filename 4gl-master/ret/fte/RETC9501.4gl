...
# Owner              => E.F.P.                                                  #
# Programa RETC9501  => REVERZO DE LA PROVISION DE DISPOSICIONES ISSSTE         #
...
FUNCTION init()
    ...
    LET gr_bitacora.programa      = "RETC9501"
    LET gr_bitacora.desc_tramite  = "PROVISION DISP ISSSTE"
...
FUNCTION f_captura_folio()
    ...
    DEFINE folio_prov LIKE ret_sol_issste_tx.folio
    ...
    OPEN WINDOW main_rev AT 4,4 WITH FORM "RETC95011" ATTRIBUTE(BORDER) 
    DISPLAY " <CTRL-C> Salir                                             <ESC> Ejecutar " AT 1,1 ATTRIBUTE(REVERSE)
    DISPLAY " RETC9501    REVERSO DE PROVISION DE DISPOSICIONES ISSSTE                  " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY" AT 3,65 ATTRIBUTE(REVERSE)
    
    --CAPTURA DEL FOLIO A REVERSAR
    INPUT BY NAME folio_prov WITHOUT DEFAULTS
        
        AFTER FIELD folio_prov
            IF folio_prov IS NULL THEN
                ERROR "EL FOLIO NO PUEDE SER NULO"
                NEXT FIELD folio_prov
            END IF
             
            -- Verifica que el folio exista y este provisionado
            SELECT "OK"
            FROM   ret_sol_issste_tx
            WHERE  folio            = folio_prov
