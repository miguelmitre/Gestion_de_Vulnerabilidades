################################################################################
#Owner             => E.F.P.                                                   #
#Programa TABM091  => CATALOGO AGRUPA SUBCUENTA  			       #
...
FUNCTION proceso()
   ...
   LET HOY = TODAY
   OPEN WINDOW ven_1 AT 3,3 WITH FORM "TABM0911" ATTRIBUTE( BORDER)
   DISPLAY " TABM091                CATALOGO  AGRUPA  SUBCUENTA                            " AT 3,1 ATTRIBUTE(REVERSE)
...
--      ELSE 
      ...
      ON KEY ( ESC )
	    ...
	    END IF  
         ...
         INSERT INTO tab_agrupa_subcta VALUES ( g_reg.* )
...
FUNCTION Consulta()
   ...
   IF (pos-1) >= 1 THEN
      ...
      OPEN WINDOW ven_2 AT 6,3 WITH FORM "TABM0912" ATTRIBUTE( BORDER)
      DISPLAY " (ENTER) CONSULTA             (CTRL-P) IMPRIMIR              (CTRL-C) SALIR    " AT 1,1 ATTRIBUTE(REVERSE,BOLD)
      DISPLAY " TABM091                 CATALOGO  AGRUPA  SUBCUENTA                          " AT 3,1 ATTRIBUTE(REVERSE,BOLD)
      DISPLAY HOY USING "dd/mm/yyyy" AT 3,67 ATTRIBUTE(REVERSE)

      LET int_flag = FALSE

      CONSTRUCT cla_where ON grupo
                          FROM grupo
         ON KEY (control-m)
      ...
      LET sel_where = "SELECT * FROM tab_agrupa_subcta WHERE ",
                      cla_where CLIPPED,
                      "ORDER BY 1 "
...
FUNCTION Elimina()
   ...
   IF (pos-1) >= 1 THEN
      ...
      CONSTRUCT cla_where ON grupo,subcuenta
                          FROM grupo,subcuenta
         ON KEY (control-m)
            ERROR "PROCESANDO INFORMACION..."
            LET int_flag = FALSE
            EXIT CONSTRUCT
