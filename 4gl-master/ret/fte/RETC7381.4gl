...
#Owner             => E.F.P.                                                    #
#Programa RETC7381 => REVERSO PROVISION DE SOLICITUD DE SALDO VENTANILLA 2.5    #
#Fecha creacion    => 2 DE MAYO DE 2012                                         #
...
FUNCTION init()
    ...
    LET gr_bitacora.programa      = "RETC7381"
    LET gr_bitacora.desc_tramite  = "PROVISION DERECHO OTORGADO VENT 2.5"
...
FUNCTION f_captura_datos()
    ...
    FROM   ret_trans_imss
    WHERE  estado_solicitud IN (gr_edo.provisionado)

    
    OPEN WINDOW RETC73811 AT 4,4 WITH FORM "RETC73811" ATTRIBUTE(BORDER)
    DISPLAY " <ESC> Ejecutar                                           < CTRL-C > SALIR  " AT 1,1 ATTRIBUTE(REVERSE)
    DISPLAY " RETC7381   REVERSO PROVISION DERECHO OTORGADO - VENTANILLA 2.5             " AT 3,1 ATTRIBUTE(REVERSE)
    ...
    ELSE
        INPUT BY NAME lr_captura.* WITHOUT DEFAULTS
            AFTER FIELD folio
                ...
                ELSE
                    ...
                    ELSE
                        ...
                        DISPLAY "Total de registros del folio   : ", lr_datos_prov.nss AT 07,12
                        DISPLAY "Fecha de provision del folio   : ", lr_datos_prov.fecha_conv USING "DD-MM-YYYY" AT 08,12
                    END IF
                END IF
            ...
            ON KEY (ESC)
                ...
                WHILE TRUE
                    PROMPT "¿EJECUTAR REVERSO DE PROVISION DE DERECHO OTORGADO? (S/N) : " FOR CHAR enter
...
FUNCTION f_reverso(pr_datos)
    ...
    WHERE  id_solicitud_saldo IN (SELECT id_solicitud_saldo                                  
    ...
    DISPLAY "R. ELIMINADOS EN ret_reinversion        : ", li_cont
            USING "<<<,<<&" AT 12,12

    LET gar_tablas_rev[2].tabla     = "ret_reinversion"
    ...
    DELETE
    FROM   ret_monto_ventanilla
    WHERE  folio          = pr_datos.folio

    LET li_cont = SQLCA.SQLERRD[3]
    
    DISPLAY "R. ELIMINADOS EN ret_monto_ventanilla   : ", li_cont
            USING "<<<,<<&" AT 13,12

    LET gar_tablas_rev[3].tabla     = "ret_monto_ventanilla"
    ...
    DISPLAY "R. ELIMINADOS EN ret_trans_imss         : ", li_cont
            USING "<<<,<<&" AT 14,12

    LET gar_tablas_rev[4].tabla     = "ret_trans_imss"
    ...
    SET    folio                    = NULL      ,
           estado                   = gr_edo.derecho_otorgado
    WHERE  folio                    = pr_datos.folio
    AND    estado                   IN (gr_edo.provisionado, gr_edo.rechazado_derecho)
    ...
    LET gar_tablas_rev[5].tabla     = "ret_det_datamart"
...
FUNCTION f_verifica_estado(lr_datos)
    ...
    ELSE
        CASE ls_estado
            WHEN gr_edo.provisionado  
                LET ls_id = 0

            OTHERWISE
                LET lc_error = "    ERROR...EL LOTE DEBE ESTAR EN ESTADO ", gr_edo.provisionado, " - PROVISIONADO"
                LET ls_id = 1
        END CASE
    END IF
