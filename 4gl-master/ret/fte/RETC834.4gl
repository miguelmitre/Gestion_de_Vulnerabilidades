...
#Owner             => E.F.P.                                                    #
#Programa RETC834  => GENERA NOTIFICACION DE DISPOSICION DE RECUSROS SOLICITUD  #
#                     OP.05 POR RETIRO "M"                                      #
#Fecha creacion    => 24 DE SEPTIEMBRE 2004                                     #
#By                => DMR                                                       #
#Modificado por    => FRANCO ULLOA VIDELA                                       #
#Fecha actualiza   => 07 DE FEBRERO DEL 2008                                    #
#Modificado por    => JAVIER GONZALEZ JERONIMO                                  #
#Fecha actualiza   => 29 DE MARZO DE 2008                                       #
#                  => Se corrigio la aplicacion de porcentaje IP cuando este    #
#                     es menor al 50%                                           #
#Modificado por    => JAVIER GONZALEZ JERONIMO                                  #
...
#Modificado por    => JAVIER GONZALEZ JERONIMO                                  #
#Fecha actualiza   => 26 DE JUNIO DE 2013                                       #
#                  => - Mejoras en el codigo fuente                             #
#                     - Se realiza la provision de vivienda 97 si la fecha de   #
#                       resolucion de la solicitud es mayor al 12 de enero de   #
#                       2012 (Req. CPL-1326)                                    #
#Sistema           => RET                                                       #
#################################################################################
...
FUNCTION primer_paso()
    ...
    #DEFINE lar_siefore ARRAY[20] OF RECORD   #CPL-2106
    DEFINE lar_siefore ARRAY[99] OF RECORD
        activo                  SMALLINT        ,
    ...
    FOREACH cur_sol INTO lr_solicitud.*
        ...
        FOREACH cur_grupo INTO ls_subcta_grupo
            ...
            IF lr_mto_prov.monto_en_acciones > 0 THEN
                ...
                IF lr_mto_prov.siefore <> 11 THEN
                    ...
                    END CASE
                ELSE
                    IF lr_mto_prov.subcuenta = 8 THEN
                        LET li_acc_viv92    = lr_mto_prov.monto_en_acciones
                    END IF
                    IF lr_mto_prov.subcuenta = 4 THEN
                ...
                LET ls_provisiona = ls_provisiona + 1
            END IF

        END FOREACH -- Subcuentas
        
        IF ls_provisiona = 0 THEN
        	  UPDATE ret_solicitud_tx
            SET    estado_solicitud = gr_estado.cancelado
                   ,cod_rechazo_ent  = 49    --Sin saldo en subcuentas
            WHERE  nss              = lr_solicitud.nss
            AND    consecutivo      = lr_solicitud.consecutivo
            AND    estado_solicitud = gr_estado.confirmado
        ...
        FOR ls_siefore = 1 TO ms_99_siefores
            ...
            IF lar_siefore[ls_siefore].activo = TRUE THEN
                ...
                IF lar_siefore[ls_siefore].acciones_ret97 < 0 THEN
                    LET lar_siefore[ls_siefore].acciones_ret97 = 0
                END IF

                IF lar_siefore[ls_siefore].acciones_ret92 < 0 THEN
                    LET lar_siefore[ls_siefore].acciones_ret92 = 0
                END IF
...
FUNCTION primer_paso_benef()
   ...
   LET ls_grupo = 0
    ...
    -- Iniciamos ciclo para cada nss
    FOREACH cur_sol_2 INTO lr_solicitud.*
        ...
        DECLARE cur_grupo_2 CURSOR FOR
        ...
        FOREACH cur_grupo_2 INTO lr_mto_prov.siefore,
                                 lr_mto_prov.subcuenta,
                                 lr_mto_prov.monto_en_acciones,
                                 lr_mto_prov.monto_en_pesos
...
FUNCTION segundo_paso()
    ...
    LET lc_ruta_det_nss = gr_seg_modulo.ruta_envio CLIPPED, "/", "DET-NSS-M-tmp"
    LET lc_ruta_det_nss = lc_ruta_det_nss CLIPPED

    LET lc_ruta_det_sie = gr_seg_modulo.ruta_envio CLIPPED, "/", "DET-SIE-M-tmp"
    LET lc_ruta_det_sie = lc_ruta_det_sie CLIPPED

    LET lc_ruta_det_viv = gr_seg_modulo.ruta_envio CLIPPED, "/", "DET-VIV-M-tmp"
    LET lc_ruta_det_viv = lc_ruta_det_viv CLIPPED

    LET gc_ruta_archivo = gr_seg_modulo.ruta_envio CLIPPED, "/", "DET-M"
...
#                      provision de recursos del retiro M                   #
#---------------------------------------------------------------------------#
FUNCTION f_valida_ejecucion()
    ...
    DISPLAY "RETIRO M  : ", lr_ejecuta.num_registros  AT 8,23
    ...
    IF lr_ejecuta.procesa = 1 THEN
        WHILE TRUE
            PROMPT "¿DESEA EJECUTAR LA PROVISION DEL RETIRO M? (S/N) : " FOR enter
...
--        LET pr_provi.acciones           = f_lib_redondea_val(pr_provi.acciones, 2)  -- CPL-3558
...
FUNCTION f_abre_ventana()
    ...
    OPEN WINDOW retc8341 AT 4,4 WITH FORM "RETC8341" ATTRIBUTE(BORDER)
    DISPLAY " RETC834     NOTIFICA Y PROVISIONA DISPOSICION DE RECURSOS                     " AT 3,1 ATTRIBUTE(REVERSE)
...
REPORT rpt_detalle_sol_03(pr_solicitud, pr_info_biometrica)
    ...
    FORMAT
        ON EVERY ROW
            ...
            IF (pr_solicitud.nss[1] = 'I')THEN
                LET pr_solicitud.nss = '00000000000'
            END IF  

            LET pr_solicitud.fecha_ini_pen = pr_solicitud.fecha_ini_pen CLIPPED
            IF pr_solicitud.fecha_ini_pen IS NULL OR pr_solicitud.fecha_ini_pen = "" THEN
                LET  pr_solicitud.fecha_ini_pen = "01/01/0001"
            END IF
