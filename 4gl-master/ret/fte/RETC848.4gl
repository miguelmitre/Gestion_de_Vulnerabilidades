...
#                 => no eran necesarias                                        #
#Modificado por   => JAVIER GONZALEZ JERONIMO                                  #
#Fecha actualiza  => 14 DE SEPTIEMBRE DE 2009                                  #
#                 => Se agregan los retiros issste y los registros que no se   #
#                 => hubieran pagado en bancoppel en la generacion del         #
#                 => archivo de cajas                                          #
#Modificado por   => JAVIER GONZALEZ JERONIMO                                  #
#Fecha actualiza  => 21 DE JUNIO DE 2011                                       #
...
#Sistema          => RETIROS                                                   #
#-----------------------------MODIFICACIONES-----------------------------------#
#Requerimiento    => CPL-1239                                                  #
#Fecha y Autor    => 01-Abril-2013   Alejandro Chagoya S.                      #
#Descripcion      => Se agrega join a la tabla ret_parcial con el consecutivo  #
#Requerimiento    => CPL-2016                                                  #
#Modificado por   => Emmanuel Reyes                                            #
#                 => Se modifica la obtencion del folio que se envia al archivo#
#                 => de cajas para el proceso de PMG unicamente                #
#Fecha actualiza  => 10 JULIO de 2015                                          #
#------------------------------------------------------------------------------#
#Requerimiento    => CPL-2996                                                  #
#Fecha y Autor    => 25-abr-2019 Isai Jimenez Rojas                            #
#Descripcion      => Se Incorpora inclusion de movimiento de complementarias   #
#------------------------------------------------------------------------------#
#Modificacion     => CPL-3021 incorporacion de movimientos de PMG ISSSTE       #
################################################################################
DATABASE safre_af
    ...
    DEFINE VERSION                  CHAR(7)                 --CPL-2996
    DEFINE PROGRAMA                 CHAR(20)                --CPL-2996
...
MAIN
    ...
    LET PROGRAMA = ARG_VAL(0)   --CPL-2996
    --LET VERSION = "v1.2996"     --CPL-2996
    --LET VERSION = "v1.3021"     --CPL-3021
    LET VERSION = "v1.3367"     --CPL-3367
  ...
  --DISPLAY " <ESC> - Ejecutar                                          <CTRL-C> - Salir  " AT 2,1 ATTRIBUTE(REVERSE)
    DISPLAY VERSION AT 2,72                             --CPL-2996
    DISPLAY " RETC848           GENERA INFORMACION RETIROS AFORE-COPPEL                   " AT 3,1 ATTRIBUTE(REVERSE)
    ...
    IF (gs_flag = TRUE) THEN
        ...
        IF f_lib_pregunta("¿DESEA GENERAR EL REPORTE? (S/N) ") = TRUE THEN
            --CPL-2996 se agrega filtro para no continuar si no hay informacion
            CALL f_genera_tablas_tmp(gdt_fec_ini, gdt_fec_fin) RETURNING li_contador
            
            IF li_contador = 0 OR li_contador IS NULL THEN
               CALL f_lib_error_msg("NO HAY INFORMACION CON EL CRITERIO ESPECIFICADO")
            ELSE 
...
FUNCTION init()
    ...
    INTO   gs_caja
    FROM   tab_pago
    WHERE  descripcion = "PAGO EN CAJAS COPPEL"
...
FUNCTION f_genera_caja_coppel()
    ...
    LET gc_nom_arcaja = "Ro", HOY USING"YYMMDD", ".txt"
    LET gc_nom_arcaja = gc_nom_arcaja CLIPPED

    LET lc_nom_arch_gen = gc_ruta CLIPPED, "/", gc_nom_arcaja
    LET lc_nom_arch_gen = lc_nom_arch_gen CLIPPED

    LET cat = "cat ",G_LISTA_1 CLIPPED," ",G_LISTA_2 CLIPPED," ",
              G_LISTA_3 CLIPPED," > ", gc_ruta CLIPPED, "/", gc_nom_arcaja
    RUN cat
    ...
    DISPLAY "CON EL NOMBRE : ", gc_nom_arcaja AT 14,18
...
FUNCTION genera_cza_coppel()
    ...
    START REPORT rpt_encabezado TO G_LISTA_1
        OUTPUT TO REPORT rpt_encabezado(gr_cza_coppel.*)
...
FUNCTION genera_det_coppel()
    ...
    DEFINE ls_tipo_movimiento SMALLINT, --CPL-2016
    ...
    FOREACH cur_1 INTO gr_det_coppel.n_seguro      ,
        ...
        CASE lc_bandera
            ...
            WHEN "pmg"
                CALL f_datos_pmg(gr_det_coppel.n_seguro, gr_det_coppel.consec_retiro)
                   ...
                   --CPL-2016
                   LET gr_det_coppel.num_fol_serv = f_obten_folio_pago_pmg()
            ...
            WHEN "bcp"
                ...
                SELECT fecha_captura,
                       ...
                       monto_neto   ,
                       impuesto_ret ,
                       tipo_movimiento --CPL-2016
                INTO   gr_det_coppel.fecha_captura      ,
                       gr_det_coppel.num_fol_serv       ,
                       gr_det_coppel.imp_doc_neto       ,
                       gr_det_coppel.impuesto_retenido  ,
                       ls_tipo_movimiento               --CPL-2016
                FROM   ret_datos_bancoppel
                WHERE  nss                = gr_det_coppel.n_seguro
                AND    consec_retiro      = gr_det_coppel.consec_retiro
                AND    estatus            <> 2
                AND    fecha_caja_coppel BETWEEN gdt_fec_ini AND gdt_fec_fin
                AND    fecha_recibe IS NOT NULL

                --CPL-2016 --Valida si el tipo de movimiento corresponde a PMG
                IF ls_tipo_movimiento = 841 THEN
                   --Si el movimiento corresponde a PMG cambia el folio de pago
                   LET gr_det_coppel.num_fol_serv = f_obten_folio_pago_pmg()
                END IF
        ...
        OUTPUT TO REPORT rpt_detalle(gr_det_coppel.*)
...
#---------------------------------------------------------------------------#
# f_genera_tablas_tmp : Genera las tablas temporales que se utilizaran      #
#                       en la generacion del archivo de caja coppel         #
#---------------------------------------------------------------------------#
FUNCTION f_genera_tablas_tmp(pr_fechas)
    ...
    DEFINE li_contador   INTEGER   --CPL-2996
    ...
    AND   (   tipo_movimiento = 490               -- Voluntarias
           ...
           OR tipo_movimiento = 897               -- complementarias  --CPL-2996
           OR tipo_movimiento = 10                -- Retencion de ISR
           OR tipo_movimiento BETWEEN 800 AND 899 -- Retiros IMSS e ISSSTE
           )
    ...
    WHENEVER ERROR CONTINUE
        DROP TABLE tmp_datos_caja
    WHENEVER ERROR STOP

    SELECT  unique(a.nss)       , -- Disposiciones
            b.nombres           ,
            b.paterno           ,
            b.materno           ,
            b.tienda_cod        ,
    ...
    AND     a.consecutivo_lote  = b.consecutivo
    AND     a.consecutivo_lote  = d.consecutivo #ACS CPL-1239
    AND     a.nss               = b.nss
    AND     a.nss               = d.nss
    AND     b.tipo_pago         = gs_caja
    UNION
    SELECT  UNIQUE a.nss        , -- Parcialidades  #ACS CPL-2021
            b.nombres           ,
            b.paterno           ,
            b.materno           ,
            b.tienda_cod        ,
    ...
    AND     a.nss               = b.nss
    AND     a.nss               = d.nss
-- Estas lineas se comentan por contingencia, se deben descomentar una vez se normalicen los pagos de parciales
-- Se atendió con el req CPL-3167
--    AND     b.tipo_pago        != gs_caja
--    AND     (b.tipo_pago != gs_caja AND b.tipo_pago != 3)
--  Se discriminan dos tipos de pago más, segú petición CPL-3167
    AND     (b.tipo_pago != gs_caja AND b.tipo_pago != 3 AND b.tipo_pago != 2 AND b.tipo_pago != 4)
    AND     c.consec_pago       > 1
    AND     c.estado            = 8 --liquidado
    UNION
    SELECT  unique(a.nss)       , -- Voluntarias y complementarias   --CPL-2996
            b.nombres           ,
            b.paterno           ,
            b.materno           ,
            b.tienda_cod        ,
    ...
    FROM    tmp_dat_dis_cta a   ,
            ret_beneficiario b
    WHERE   a.tipo_movimiento   IN(490,897,493)                          --CPL-2996
    AND     a.consecutivo_lote  = b.consecutivo
    AND     a.nss               = b.nss
    AND     b.tipo_pago         = gs_caja
    UNION
    SELECT  unique(a.nss)       , -- Retiros Totales ISSSTE (Modulo Antiguo)
            b.nombres           ,
            b.paterno           ,
            b.materno           ,
            b.tienda_cod        ,
    ...
    AND     b.tipo_pago         = gs_caja
    UNION
    SELECT  unique(a.nss)       , -- Retiros Parciales ISSSTE (Modulo Antiguo)
            b.nombres           ,
            b.paterno           ,
            b.materno           ,
            b.tienda_cod        ,
    ...
    WHERE   a.fecha_caja_coppel BETWEEN pr_fechas.fecha_ini AND pr_fechas.fecha_fin
    AND     a.estatus       <> 2
    AND     a.fecha_recibe  IS NOT NULL
    INTO TEMP tmp_datos_caja
    ...
    SELECT COUNT(*)             --CPL-2996
    INTO   li_contador          --CPL-2996
    FROM   tmp_datos_caja       --CPL-2996
    
    RETURN li_contador          --CPL-2996
...
#---------------------------------------------------------------------------#
# f_calc_impuesto_retenido : Obtiene el monto del impuesto retenido a la    #
#                            liquidacion                                    #
#---------------------------------------------------------------------------#
...
FUNCTION f_datos_parcial(pc_nss, pi_consec)
    ...
    --obtencion de folio_solicitud
    LET lr_datos.num_fol_serv = NULL  #CPL-2021
...
#                   Garantizada                                                #
#------------------------------------------------------------------------------#
FUNCTION f_datos_pmg_iss(pc_nss, pi_consec)          --CPL-3021
...
FUNCTION f_datos_voluntarias(pc_nss, pi_consec)
    ...
    WHERE  a.nss              = pc_nss
    AND    a.consecutivo_lote = pi_consec
    AND    a.tipo_movimiento  IN(490,493,897)     --CPL-2996
    AND    a.consecutivo_lote = d.consecutivo
...
#---------------------------------------------------------------------------#
# f_obten_folio_pago_pmg : Obtiene un folio diferente para los pagos de PMG #
#                          utilizando tabla de folios de servicio  CPL-2016 #
#        Implementada por: EMMANUEL REYES                                   #
#---------------------------------------------------------------------------#
FUNCTION f_obten_folio_pago_pmg()

   DEFINE ls_mensaje      CHAR(500)
   DEFINE li_folio        LIKE dis_cuenta.folio
   DEFINE max_folio       INTEGER
   DEFINE c_year          CHAR(10)
   DEFINE n_year          SMALLINT
   ...
   SET LOCK MODE TO WAIT  #Asignacion Folio

    LOCK TABLE rec_folio IN EXCLUSIVE MODE

       SELECT NVL(folio_rec,0)
       INTO   max_folio
       FROM   rec_folio
       WHERE  folio_rec >=  0
       AND    tipo_id   =  rec.tipoid

       IF max_folio = 0 OR max_folio IS NULL THEN
          LET c_year    = HOY USING "dd/mm/yyyy"
          LET c_year    = c_year[9,10]
          LET n_year    = c_year
          LET max_folio = n_year * 1000000 + 1

          IF fgl_getenv("DEBUGEFP") THEN 
             DISPLAY "max_folio   =",max_folio
             DISPLAY "rec.tipoid  =",rec.tipoid
             DISPLAY "HOY         =",HOY
             DISPLAY "Usuario     =",gc_usuario
          END IF 

          INSERT INTO rec_folio
          VALUES (max_folio,      #folio_rec
                  rec.tipoid,     #tipo_id
                  HOY,            #fecha_fol
                  gc_usuario)        #usuario
          UNLOCK TABLE rec_folio
          SET LOCK MODE TO NOT WAIT  #Asignacion Folio
          --LET max_folio = 1
    ELSE
          LET max_folio = max_folio + 1

          UPDATE rec_folio
          SET    rec_folio.folio_rec = max_folio,
                 rec_folio.fecha_fol = TODAY,
                 rec_folio.usuario   = gc_usuario
          WHERE  rec_folio.tipo_id   = rec.tipoid
...
#---------------------------------------------------------------------------#
# rpt_encabezado : Genera el encabezado del archivo de caja coppel          #
#---------------------------------------------------------------------------#
...
#---------------------------------------------------------------------------#
# rpt_detalle : Genera el detalle del archivo de caja coppel                #
#---------------------------------------------------------------------------#
...
#---------------------------------------------------------------------------#
# rpt_sumario : Genera el sumario del archivo de caja coppel                #
#---------------------------------------------------------------------------#
...
FUNCTION f_acerca_de()
    ...
    LET lc_fecha_hora = "28/05/2021 12:00:00 p. m."   --obtenido con F7 en ultraedit
    ...
    OPEN WINDOW w_acerca AT 8,10 WITH 10 rows, 60 COLUMNS ATTRIBUTE(BORDER, PROMPT LINE LAST -1)
       ...
       DISPLAY "Ult cambio  : ", "Incorporacion de movimientos de complementarias" AT 5,2
       DISPLAY "            : ", "Incorporacion de movimientos de PMG ISSSTE     " AT 6,2
...
FUNCTION f_beneficiarios_designados(gr_det_benef)
    ...
    IF STATUS = NOTFOUND THEN 
        ...
        AND    fecha_captura   = gr_det_benef.fecha_captura
