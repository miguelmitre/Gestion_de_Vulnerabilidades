#################################################################################
#Owner             => E.F.P.                                                    #
#Programa RETM816  => MANTENEDOR DE REGISTRO DE CERTIFICADO IMSS - DISPOSICION  #
#                     DE RECURSOS POR UNA RESOLUCION NO REGISTRADA EN EL        #
#                     DATAMART (TIPO RETIRO M)                                  #
#                                  VERSION COPPEL                               #
#Fecha creacion    => 21 DE SEPTIEMBRE DE 2004                                  #
#By                => DAVID MCKAY ROMERO                                        #
...
--        autoridad          CHAR(9)                                 ,
        ...
        cod_rechazo_ent    LIKE ret_solicitud_tx.cod_rechazo_ent   ,  --xavier -
        ...
        porcentaje_val     LIKE ret_Solicitud_tx.porcentaje_val
    END RECORD
...
--        autoridad          CHAR(9)                                 ,
    ...
    DEFINE reg_1 RECORD #glo #reg_1
        ...
        rechazado          LIKE ret_estado.estado_solicitud , --xavier--
        liquidado          LIKE ret_estado.estado_solicitud   --xavier--
    END RECORD
...
MAIN
    ...
    CALL STARTLOG("RETM816.log")
    CALL init()

    OPEN WINDOW retm8161 AT 2,3 WITH FORM "RETM8161" ATTRIBUTE (BORDER)
    DISPLAY " RETM816       SOLICITUD DE DISPOSICION DE RECURSOS TIPO M                     " AT 3,1 ATTRIBUTE(REVERSE)
...
#a---------------
    ...
    OPEN WINDOW retm8162 AT 2,3 WITH FORM "RETM8162" ATTRIBUTE (BORDER)
    DISPLAY " RETM816                  DATOS DEL AFILIADO                                   " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
    DISPLAY " Ctrl-C:Salir     Ctrl-P:Forma de Pago      Ctrl-B:Dom Trab/Benef Esc:Agrega" AT 1,1
    DISPLAY " Ctrl-D:Bitacora  Ctrl-A:Emision de Cartas  Ctrl-R:Liq Retiro     Ctrl:Saldo" AT 2,1
    DISPLAY "                         DATOS DE LA SOLICITUD                                 " AT 7,1 ATTRIBUTE(REVERSE)
    ...
    INPUT BY NAME reg.nss                ,
                  ...
                  reg.porcentaje_val WITHOUT DEFAULTS
        ...
        AFTER FIELD tipo_pension
            ...
            IF reg.tipo_pension IS NULL THEN
               ...
               AND    tipo_pension    = reg.tipo_pension
               AND    tipo_prestacion = reg.tipo_prestacion
               AND    tipo_retiro     = reg.tipo_retiro
               GROUP BY 1

               IF SQLCA.SQLCODE = NOTFOUND  THEN
                  PROMPT " NO EXISTE REGISTRO EN LA RET_MATRIZ_DERECHO "
                  FOR CHAR enter
                  NEXT FIELD tipo_prestacion
               END IF
            ...
            ELSE
               ...
               AND    tipo_pension IN ("AS","OR","VI","VO","CE","VE","IN","IP")
               GROUP BY 1
        ...
        AFTER FIELD folio_solicitud
            ...
            IF reg.folio_solicitud IS NULL THEN
                ERROR " Folio de la Solicitud NO puede ser NULO "
                NEXT FIELD folio_solicitud
            END IF
        ...
        AFTER FIELD porcentaje_val
            IF reg.tipo_pension = "IP" THEN
               IF reg.porcentaje_val < 50 OR reg.porcentaje_val IS NULL THEN
                  ERROR " EL PORCENTAJE DE VALUACION DEBE SER >= 50 "
                  NEXT FIELD porcentaje_val
               END IF
            ELSE
     ...
     END IF
         ...
         IF reg.tipo_pension = "IP" THEN
            IF reg.porcentaje_val < 50 THEN
               ERROR " EL PORCENTAJE DE VALUACION DEBE SER >= 50% "
               NEXT FIELD porcentaje_val
            END IF
     ELSE
        LET reg.porcentaje_val = 100
        DISPLAY reg.porcentaje_val
         END IF
     ...
     WHENEVER ERROR STOP  --CONTINUE
         ...
         VALUES(reg.nss                 ,
                ...
                reg.fecha_resolucion    , #fecha_resolucion
                reg.fecha_solicitud     ,
                ' '                     , #cve_doc_probatorio
      ...
      END IF
          ...
          DISPLAY "  REGISTRO INGRESADO  ","" AT 22,1 ATTRIBUTE(REVERSE)
          SLEEP 3
...
#dtr-----------------------------
    ...
    WHILE TRUE
        LET prepare_1p = " SELECT * FROM tab_prestacion ",
                        " WHERE descripcion MATCHES ",'"',x_buscar CLIPPED,'"',
                        " AND   tipo_prestacion IN (0,1,3) ",
                        " ORDER BY 1 " CLIPPED

        PREPARE pre_1p FROM prepare_1p
        DECLARE cur_1p CURSOR FOR pre_1p

        LET pos = 1

        FOREACH cur_1p INTO ra_reg[pos].*
            LET pos = pos + 1
            IF pos >= 1000 THEN
                ERROR "    Fue Sobrepasada la capacidad maxima del arreglo"
                ATTRIBUTE(NORMAL)
        ...
        IF (pos-1) < 1 THEN
            ERROR "    ARCHIVO TIPO DE PRESTACION VACIO"
            ATTRIBUTE(NORMAL)
        END IF
...
#dtr-----------------------------
    ...
    WHILE TRUE
        LET prepare_1s = " SELECT * FROM tab_seguro ",
                        " WHERE descripcion MATCHES ",'"',x_buscar CLIPPED,'"',
                        " AND   clave IN ('IV','CV','RT','IM') ",
                        " ORDER BY 1 " CLIPPED

        PREPARE pre_1s FROM prepare_1s
        DECLARE cur_1s CURSOR FOR pre_1s

        LET pos = 1

        FOREACH cur_1s INTO ra_reg[pos].*
            LET pos = pos + 1
            IF pos >= 1000 THEN
                ERROR "    Fue Sobrepasada la capacidad maxima del arreglo"
                ATTRIBUTE(NORMAL)
...
#dtr-----------------------------
    ...
    WHILE TRUE
        LET prepare_1r= " SELECT * FROM tab_pension ",
                        " WHERE descripcion MATCHES ",'"',x_buscar CLIPPED,'"',
                        " AND   tipo_pension IN ('AS','OR','VI','VO','CE','VE','IN','IP') ",
                        " ORDER BY 1 " CLIPPED

        PREPARE pre_1r FROM prepare_1r
        DECLARE cur_1r CURSOR FOR pre_1r

        LET pos = 1

        FOREACH cur_1r INTO ra_reg[pos].*
            LET pos = pos + 1
            IF pos >= 1000 THEN
                ERROR "    Fue Sobrepasada la capacidad maxima del arreglo"
                ATTRIBUTE(NORMAL)
        ...
        IF (pos-1) < 1 THEN
            ERROR "    ARCHIVO TIPO DE PENSION VACIO"
            ATTRIBUTE(NORMAL)
        END IF
...
#c-----------------
    ...
    OPEN WINDOW retm8055 AT 2,3 WITH FORM "RETM8163" ATTRIBUTE (BORDER)
    DISPLAY " RETM805                  DATOS DEL AFILIADO                                   " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
    DISPLAY " Ctrl-C:Salir     Ctrl-P:Forma de Pago      Ctrl-B:Dom Trab/Benef Esc:Consul" AT 1,1
    DISPLAY " Ctrl-D:Bitacora  Ctrl-A:Emision de Cartas  Ctrl-R:Liq Retiro     Ctrl:Saldo" AT 2,1
    DISPLAY "                         DATOS DE LA SOLICITUD                                 " AT 7,1 ATTRIBUTE(REVERSE)
    DISPLAY "                        DATOS DE CONTROL INTERNO                               " AT 16,1 ATTRIBUTE(REVERSE)
...
#m-----------------
    ...
    OPEN WINDOW retm8056 AT 2,3 WITH FORM "RETM8163" ATTRIBUTE (BORDER)
    DISPLAY " RETM805                  DATOS DEL AFILIADO                                   " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
    DISPLAY " Ctrl-C:Salir     Ctrl-P:Forma de Pago      Ctrl-B:Dom Trab/Benef Esc:Modifica" AT 1,1
    DISPLAY " Ctrl-D:Bitacora  Ctrl-A:Emision de Cartas  Ctrl-R:Liq Retiro     Ctrl:Saldo" AT 2,1
    DISPLAY "                         DATOS DE LA SOLICITUD                                 " AT 7,1 ATTRIBUTE(REVERSE)
    DISPLAY "                        DATOS DE CONTROL INTERNO                               " AT 16,1    ATTRIBUTE(REVERSE)
...
#c------------------------------------
    DEFINE reg_mod RECORD
        ...
        desc_grupo         LIKE tab_grupo.descripcion              ,
        semanas_cotizadas  LIKE ret_solicitud_tx.semanas_cotizadas ,
        cod_rechazo_ent    LIKE ret_solicitud_tx.rechazo_cod       ,  --xavier--
        rechazo_cod        LIKE ret_solicitud_tx.rechazo_cod       ,
        rechazo_desc       LIKE tab_rch_marca.rechazo_desc         ,
        fecha_captura      LIKE ret_solicitud_tx.fecha_captura     ,
        fecha_modifica     LIKE ret_solicitud_tx.fecha_modifica    ,
    ...
    LET reg.cod_rechazo_ent    = reg_mod.cod_rechazo_ent      --xavier--
    ...
    INPUT BY NAME reg.tipo_prestacion    ,
                  reg.regimen            ,
                  reg.tipo_seguro        ,
                  reg.tipo_pension       ,
                  reg.folio_solicitud    ,
        ...
        AFTER FIELD porcentaje_val
            IF reg.tipo_pension = "IP" THEN
               IF reg.porcentaje_val < 50 THEN
                  ERROR " EL PORCENTAJE DE VALUACION NO PUEDE SER MENOR A 50% "
                  NEXT FIELD porcentaje_val
               END IF
            ELSE
           LET reg.porcentaje_val = 100
           DISPLAY reg.porcentaje_val
            ...
            ON KEY (ESC)
                ...
                IF reg.tipo_pension = "IP" THEN
                   IF reg.porcentaje_val < 50 THEN
                      ERROR " PORCENTAJE DE VALUACION NO PUEDE SER MENOR A 50% "
                      NEXT FIELD porcentaje_val
                   END IF
                ELSE
               LET reg.porcentaje_val = 100
   ...
   IF int_flag <> TRUE THEN
      ...
      IF vmodif2 = 1 THEN
         ...
         SET    ret_solicitud_tx.regimen          =reg.regimen          ,
                ...
                ret_solicitud_tx.fecha_resolucion =reg.fecha_resolucion ,
                ...
                ret_solicitud_tx.porcentaje_val   =reg.porcentaje_val
      ...
      ELSE
         ...
         ELSE
           ...
           IF vmodif2 = 3 THEN
              ...
              -- Se consulta tipo_movimento para retiro M --
              SELECT movimiento
...
#e----------------
    ...
    OPEN WINDOW retm8057 AT 2,3 WITH FORM "RETM8163" ATTRIBUTE (BORDER)
    DISPLAY " RETM805                  DATOS DEL AFILIADO                                   " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
...
#cc----------------
    ...
    DISPLAY " RETM805                  DATOS DEL AFILIADO                                   " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
    DISPLAY " Ctrl-C:Salir     Ctrl-P:Forma de Pago      Ctrl-B:Dom Trab/Benef Esc:Confirma" AT 1,1
    DISPLAY " Ctrl-D:Bitacora  Ctrl-A:Emision de Cartas  Ctrl-R:Liq Retiro     Ctrl:Saldo" AT 2,1
    DISPLAY "                         DATOS DE LA SOLICITUD                                 " AT 7,1 ATTRIBUTE(REVERSE)
    DISPLAY "                        DATOS DE CONTROL INTERNO                               " AT 16,1    ATTRIBUTE(REVERSE)
