#################################################################################
#Owner             => E.F.P.                                                    #
#Programa RETM803  => MANTENEDOR DE REGISTRO DE CERTIFICADO IMSS - RESOLUCION   #
#                     REGIMEN 73 (TIPO RETIRO E)                                #
#                                  VERSION COPPEL                               #
#Fecha creacion    => 20 DE ENERO DE 2004                                       #
#By                => STEFANIE DANIELA VERA PIÑA                                #
#Fecha actualiz.   => 10 DE OCTUBRE DE 2007                                     #
...
MAIN
    ...
    OPEN WINDOW retm8031 AT 2,3 WITH FORM "RETM8031"  ATTRIBUTE (BORDER)
    DISPLAY " RETM803  SOLICITUD DE DISPOSICION DE RECURSOS TIPO E REGIMEN 73               " AT 3,1 ATTRIBUTE(REVERSE)
...
#a---------------
    ...
    OPEN WINDOW retm8032 AT 2,3 WITH FORM "RETM8032" ATTRIBUTE (BORDER)
    DISPLAY " RETM803                  DATOS DEL AFILIADO                                   " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
    ...
    INPUT BY NAME reg.nss                ,
        ...
        IF vaccion = 'C' OR
           vaccion = 'M' THEN
                ...
                AND    diag_datamart    in (101,210,300,301,501)
            ...
            IF sw_1 = 0 THEN
                ...
                IF ult_consecutivo IS NULL OR ult_consecutivo = "" THEN
                    LET ult_consecutivo = 1
                END IF
        ...
        AFTER FIELD nss
            ...
            IF reg.nss IS NULL THEN
                DISPLAY "  CAMPO NO PUEDE SER NULO ","" AT 22,1
        ...
        AFTER FIELD tipo_pension
            ...
            IF reg.tipo_pension IS NULL THEN
                 CALL despliega_tipo_pension(reg.tipo_seguro) #dtp
            ...
            ELSE
                ...
                LET v_busca = 0
                 ...
                 { IF reg.tipo_pension = "IP" THEN
                    ...
                    AND    diag_datamart   IN  (101,210,300,301)
       ...
       ON KEY (ESC)
        ...
        END IF
                ...
                VALUES(reg.nss                 ,
                       ...
                       "S"                     ,#tipo_id
                       reg.curp                ,
                       reg.sec_pension         , #se agrego este campo
                       0                       ,#tipo_documento
                       reg.tipo_retiro         ,
                       reg.regimen             ,
                       reg.tipo_seguro         ,
...
#dts----------------------------------------
    ...
    OPEN WINDOW retm8034 AT 05,12 WITH FORM "RETM8024" ATTRIBUTE(BORDER)
    ...
    WHILE TRUE
        ...
        END IF
    END WHILE
    CLOSE WINDOW retm8034
END FUNCTION

FUNCTION despliega_tipo_pension(v_tipo_seguro)
#dtp------------------------------------------
    DEFINE  ra_reg ARRAY[100] OF RECORD
        codigo            CHAR(02),
        descripcion       CHAR(50)
    ...
    DEFINE #loc #char
        prepare_1     CHAR(240) ,
        x_buscar      CHAR(030) ,
        v_tipo_seguro CHAR(002)

    OPEN WINDOW retm8035 AT 05,12 WITH FORM "RETM8024" ATTRIBUTE(BORDER)
    ...
    WHILE TRUE
        LET prepare_1 = " SELECT A.* FROM tab_pension A,ret_matriz_derecho B ",
                       ...
                       " AND   B.tipo_retiro = 'E'  ",
                       " AND   B.regimen     = 73   ",
                       " AND   B.tipo_seguro = ",'"',v_tipo_seguro,'"',
...
#c-----------------
    ...
    OPEN WINDOW retm8033 AT 2,3 WITH FORM "RETM8033" ATTRIBUTE (BORDER)
    DISPLAY " RETM803                  DATOS DEL AFILIADO                                   " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
...
#c------------------------------------
    ...
    INPUT BY NAME reg.nss     ,
        ...
        AFTER FIELD semanas_cotizadas
            ...
            #ff
        {
            display "num_resolucion ",reg.num_resolucion
            display "reg.folio_solicitud ",reg.folio_solicitud
            display "reg.fecha_solicitud ",reg.fecha_solicitud
            display "reg.semanas_cotizadas ",reg.semanas_cotizadas
        prompt "para " for char enter
        }
            display reg.num_resolucion
            display reg.folio_solicitud
            display reg.fecha_solicitud
            display reg.semanas_cotizadas
       -- prompt "para " for char enter
            ...
            ON KEY (CONTROL-B)
                ...
                IF STATUS = NOTFOUND THEN
                    ERROR "    NO SE HAN CAPTURADO BENEFICIARIOS    "
                    ATTRIBUTE(REVERSE)
            NEXT FIELD num_resolucion
                ELSE
                ...
                ELSE
                    IF vmodif2 = 2 THEN
                        IF reg.estado_solicitud = reg_1.capturado THEN
                            ...
                            END IF
                        ELSE
                            DISPLAY " LA SOLICITUD DEBE ESTAR EN ESTADO CAPTURADO " AT 22,1 
                            ATTRIBUTE(REVERSE) 
                            SLEEP 2
                            CALL inicializa() #i
                            EXIT INPUT                            
               ...
               ELSE
                        ...
                        IF vmodif2 = 3  THEN
                           ...
                           AND ret_solicitud_tx.consecutivo = reg.consecutivo
                            ...
                            -----------------------------------
                        DISPLAY "EL REGISTRO SE CANCELO CON EXITO  ","" AT 22,1
                        ATTRIBUTE(REVERSE)
                        SLEEP 3
                        CALL inicializa() #i
                        INITIALIZE reg_mod.* TO NULL
        ...
        ON KEY ( CONTROL-M )
            ...
            IF enter MATCHES "[Ss]" THEN
               ...
               ELSE
                  DELETE
                  FROM  ret_beneficiario
                  WHERE ret_beneficiario.nss         = arr_1[arr].nss
                  AND  ret_beneficiario.consecutivo = arr_1[arr].consecutivo

                  DELETE
                  FROM  ret_solicitud_tx
                  WHERE ret_solicitud_tx.nss         = arr_1[arr].nss
                  AND   ret_solicitud_tx.consecutivo = arr_1[arr].consecutivo
...
#cc----------------
    ...
    DISPLAY " RETM803                  DATOS DEL AFILIADO                                   " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
    DISPLAY " Ctrl-F:Rechazar  Ctrl-B:Dom Trab/Benef   Esc:Confirma" AT 1,1
    DISPLAY "                         DATOS DE LA SOLICITUD                                 " AT 7,1 ATTRIBUTE(REVERSE)
    DISPLAY "                        DATOS DE CONTROL INTERNO                               " AT 16,1
    ATTRIBUTE(REVERSE)
    ...
    DISPLAY ARRAY arr_1 TO scr_1.*
        ...
        { ON KEY (CONTROL-F)
           LET pos = ARR_CURR()
        ------------------------sil---------------------
           CALL rechazar(arr_1[pos].nss,arr_1[pos].consecutivo,
                arr_1[pos].tipo_prestacion,arr_1[pos].tipo_retiro)
           RETURNING codigo,entidad
           LET arr_1[pos].cod_rechazo_ent = codigo
           DISPLAY arr_1[pos].cod_rechazo_ent TO cod_rechazo_ent
           LET  vmodif = 3
           LET flag = 0
           EXIT DISPLAY }
        -------------------------------------------------
