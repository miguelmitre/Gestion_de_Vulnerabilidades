...
GLOBALS
    ...
    DEFINE v_existe         SMALLINT    -- CPL-3956
    DEFINE v_cad            CHAR(1000)  -- CPL-3956
...
FUNCTION abre_ventana()
    ...
    INPUT BY NAME fliquida, nomarchivo WITHOUT DEFAULTS
      ...
      AFTER FIELD nomarchivo
         ...
         LET v_cad = '',
                     ...
                     '\n  WHERE nom_archivo = "',nomarchivo CLIPPED,'" '
         -- DISPLAY v_cad CLIPPED            
         PREPARE q_01 FROM v_cad
         EXECUTE q_01 INTO v_existe
      ...
      ON KEY (ESC)
         ...
         LET v_cad = '',
                     ...
                     '\n  WHERE nom_archivo = "',nomarchivo CLIPPED,'" '
         -- DISPLAY v_cad CLIPPED            
         PREPARE q_02 FROM v_cad
         EXECUTE q_02 INTO v_existe
    ...
    LET v_cad = '',
                ...
                '\n  WHERE estado_reg  =  2                       ',
                '\n    AND nom_archivo = "',nomarchivo CLIPPED,'" '
    -- DISPLAY v_cad CLIPPED            
    PREPARE q_03 FROM v_cad
    EXECUTE q_03 INTO v_existe
...
FUNCTION lanza_proceso()
    ...
    LET v_cad = '',
                '\n SELECT FIRST 1 *, ROWID                                           ',  --- CPL-3956 - FALLA AQUI MAS DE 1 REGISTRO
    ...
    EXECUTE con_reg INTO reg_ctr_traspaso.*, v_rowid
    ...
    LET v_cad = '',                
                ...
                '\n    AND nom_archivo        = "',nomarchivo CLIPPED,'"              ',
                '\n  GROUP BY 1                                                       '
    PREPARE q_04 FROM v_cad 
    EXECUTE q_04 INTO vcve_ced_cuenta
    ...
    LET v_cad = '', 
                ...
                ''
    -- DISPLAY v_cad
    PREPARE q_upd2 FROM v_cad  
    EXECUTE q_upd2          
    
    IF reg_bat.pid          IS NULL THEN LET reg_bat.pid = 0         END IF
    IF reg_bat.proceso_cod  IS NULL THEN LET reg_bat.proceso_cod = 0 END IF 
    IF reg_bat.opera_cod    IS NULL THEN LET reg_bat.opera_cod = 0   END IF 
...
FUNCTION valida_traspaso()
    ...
    LET v_cad = '',
                ...
                '\n  WHERE estado_reg  =  2                       ',
                '\n    AND nom_archivo = "',nomarchivo CLIPPED,'" '
    PREPARE q_05 FROM v_cad
    EXECUTE q_05 INTO v_existe
