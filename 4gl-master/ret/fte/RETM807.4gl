#################################################################################
#Owner             => E.F.P.                                                    #
#Programa RETM807  => MANTENEDOR DE REGISTRO DE CERTIFICADO IMSS - RETIRO       #
#                     ANEXO A PLAN PRIVADO REGIMEN 73 (TIPO RETIRO F)           #
#                                  VERSION COPPEL                               #
#Fecha creacion    => 16 DE ENERO DE 2004                                       #
#By                => MARCOS GODINEZ JIMENEZ                                    #
#Fecha actualiz.   => 23 DE OCTUBRE DE 2007                                     #
...
MAIN
    ...
    CALL STARTLOG("RETM807.log")   ---omar
    CALL init()
    LET xaccion = " "
    OPEN WINDOW retm8071 AT 2,3 WITH FORM "RETM8071" ATTRIBUTE(BORDER)
    DISPLAY " RETM807                   DATOS DEL AFILIADO                                  " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
    DISPLAY "                    DATOS DE LA SOLICITUD DE PLAN PRIVADO 73                   " AT 7,1 ATTRIBUTE(REVERSE)
...
#a----------------
    ...
    OPEN WINDOW retm8072 AT 2,3 WITH FORM "RETM8071" ATTRIBUTE (BORDER)
    DISPLAY " RETM807                  DATOS DEL AFILIADO                                   " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
    ...
    INPUT BY NAME rg_datamart.nss                ,
        ...
        IF vaccion = 'C' OR
           vaccion = 'M' THEN
            ...
            IF sw_1 = 0 THEN
                ...
                LET rg_datamart.tipo_prestacion = 5
        ...
        CASE vestado_solicitud
            ...
            ELSE
                ...
                IF rg_datamart.fecha_ini_pen >= "01071997" THEN
...
#c-----------------
    ...
    OPEN WINDOW retm8073 AT 2,3 WITH FORM "RETM8071" ATTRIBUTE (BORDER)
    DISPLAY " RETM807                  DATOS DEL AFILIADO                                   " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
    ...
    FOREACH cur_6 INTO ga_solicitud[i].*
        ...
        SELECT sec_pension
        INTO   ga_solicitud[i].sec_pension
        FROM   ret_det_datamart
        WHERE  nss = ga_solicitud[i].nss
        AND    tipo_prestacion  = ga_solicitud[i].tipo_prestacion
        AND    tipo_seguro      = ga_solicitud[i].tipo_seguro
        ...
        GROUP BY 1
            ...
            IF STATUS <> NOTFOUND THEN
                LET v_ejecuta="fglgo RETM810 ",ga_solicitud[pos].nss," ",
                   ga_solicitud[pos].consecutivo," ","C"
                RUN v_ejecuta
            ELSE
...
--  LET rg_datamart.aseguradora        = reg_mod.aseguradora
    ...
    LET  rg_datamart.diag_registro  = reg_mod.diag_registro
    LET rg_datamart.estado_sub_viv   = reg_mod.estado_sub_viv
...
#e-----------------
    ...
    OPEN WINDOW retm8075 AT 2,3 WITH FORM "RETM8071" ATTRIBUTE (BORDER)
    DISPLAY " RETM807                  DATOS DEL AFILIADO                                   " AT 3,1 ATTRIBUTE(REVERSE)
    DISPLAY HOY USING "DD-MM-YYYY"  AT 3,66 ATTRIBUTE(REVERSE)
...
---omar
          IF SQLCA.SQLCODE < 0 THEN
             ...
             PROMPT " ERROR DELETE ret_solicitud_tx AVISE A SISTEMAS "
             FOR enter
             EXIT PROGRAM
                  END IF
                  WHENEVER ERROR STOP
                  ...
                  DELETE
                  FROM  ret_consecutivo
                  WHERE ret_consecutivo.consecutivo = ga_solicitud[arr].consecutivo
...

{
                  ...
                  CALL tipo_movimiento(ga_solicitud[arr].tipo_seguro   ,
                                       ga_solicitud[arr].tipo_pension  ,
                                       ga_solicitud[arr].tipo_prestacion
                                      )#tm
                  RETURNING s_tipo_movimiento

                  LET reg_rev.marca_cod   = s_tipo_movimiento
                  LET reg_rev.nss         = ga_solicitud[arr].n_seguro
                  LET reg_rev.correlativo = ga_solicitud[arr].consecutivo
...
FUNCTION pre_gunta3()
     ...
     WHILE TRUE
         PROMPT "  DESEAS RECAHAZAR  S/N ? " FOR opc
         IF opc NOT MATCHES "[SsNn]" THEN
            CONTINUE WHILE
         ELSE
            EXIT WHILE
           ...
           IF vmodif2 = 3 THEN
              ...
              IF opc MATCHES "[Ss]" THEN
                 ...
                 SET ret_solicitud_tx.estado_solicitud  = reg_1.rechazado,
                     ret_solicitud_tx.cod_rechazo_ent   = rg_datamart.cod_rechazo_ent,
                 ...
                 DISPLAY "  REGISTRO RECHAZADO      " AT 22,1 ATTRIBUTE(REVERSE) SLEEP 3
   ...
   IF int_flag <> TRUE THEN
    IF vmodif2 = 1  THEN
      ...
      WHENEVER ERROR CONTINUE
          ...
          ret_solicitud_tx.fecha_solicitud   = rg_datamart.fecha_solicitud  ,
          ret_solicitud_tx.semanas_cotizadas = rg_datamart.semanas_cotizadas,
          ret_solicitud_tx.usuario_modifica  = rg_datamart.usuario_modifica ,
          ret_solicitud_tx.fecha_modifica    = rg_datamart.fecha_modifica
          WHERE ret_solicitud_tx.consecutivo = rg_datamart.consecutivo
---omar
      IF SQLCA.SQLCODE < 0 THEN
         LET x_error = "UPDATE ret_solicitud_tx:",
                 "nss ",rg_datamart.nss,
    ...
    ELSE
       IF vmodif2 = 2 THEN
          IF rg_datamart.estado_solicitud = reg_1.capturado THEN
            ...
            IF opc MATCHES "[Ss]" THEN
               ...
               ret_solicitud_tx.fecha_solicitud   = rg_datamart.fecha_solicitud  ,
               ret_solicitud_tx.semanas_cotizadas = rg_datamart.semanas_cotizadas,
               ret_solicitud_tx.usuario_confirma  = rg_datamart.usuario_confirma ,
               ret_solicitud_tx.fecha_confirma    = rg_datamart.fecha_confirma   ,
               ret_solicitud_tx.estado_solicitud  = reg_1.confirmado
               WHERE ret_solicitud_tx.consecutivo = rg_datamart.consecutivo
      ...
      ELSE
             DISPLAY "  CONFIRMACION RECHAZADA" AT 22,1
             ATTRIBUTE(REVERSE)
             SLEEP 3
             CALL inicializa() #i
          END IF
