...
#Programa RETL925  => REPORTE DE PRELIQUIACION TRANSFERENCIAS ISSSTE (ACCIONES)#
...
MAIN
    ...
    CALL STARTLOG("RETL925.log")
    CALL init()

    OPEN WINDOW RETL9251 AT 4,4 WITH FORM "RETL9251" ATTRIBUTE (BORDER)
    DISPLAY "                                < Ctrl-C >                                       " AT 1,1 ATTRIBUTE(REVERSE)
    DISPLAY " RETL925   REPORTE PRELIQUIDACION DE TRANSFERENCIAS DE RECURSOS ISSSTE PESOS   " AT 3,1 ATTRIBUTE (REVERSE)
    ...
    INPUT BY NAME gr_captura.* WITHOUT DEFAULTS
    	  AFTER FIELD folio
    	  	IF gr_captura.folio IS NOT NULL THEN
    	  		 ...
    	  		 AND    tipo_movimiento IN (861, --H TRANSF. GOB. FED. PE
                                        862, --I TRANSF. ASEGURADORA
                                        863, --J TRANSF. ISSSTE PRESC
                                        858, --K RET. REINGRESO (ISSS
                                        865) --L TRANSF. POR CESANTIA
             GROUP BY 1
...
FUNCTION init()
    ...
    LET gc_archivo = gr_seg_modulo.ruta_listados CLIPPED,
                     "/", gc_usuario CLIPPED,
                     ".RPT_LIQ_",HOY USING"DDMMYYYY",".925"
END FUNCTION
################################################################################
FUNCTION primer_paso()
   ...
   AND    a.folio            = b.folio
   AND    a.diag_procesar   IN (501, 507)
   ...
   FOREACH cur_liq INTO  gr_transf.nss            ,
                         gr_transf.consecutivo    ,
                         gr_transf.tipo_retiro    ,
                         gr_transf.tipo_seguro    ,
                         gr_transf.tipo_pension   ,
                         gr_transf.tipo_prestacion,
                         gr_transf.siefore        ,
                         gr_transf.regimen        ,
                         gr_transf.diag_procesar  ,
                         gr_transf.fecha_ini_pen

      INITIALIZE lr_montos.* TO NULL

      #Obtener nombre
      EXECUTE get_nombre USING gr_transf.nss INTO gr_transf.nombre

      LET gr_transf.acciones_neto       = 0
      LET gr_transf.acciones_ret08      = 0
      LET gr_transf.acciones_cv_cs      = 0
      LET gr_transf.acciones_ahorro_sol = 0
      LET gr_transf.acciones_fov08      = 0
      LET gr_transf.acciones_rcv        = 0

      FOREACH cur_montos USING gr_transf.nss        ,
                               gr_transf.consecutivo,
                               gr_captura.folio
         ...
         CASE
         	  WHEN lr_montos.subcuenta = 30
         	  	 LET gr_transf.acciones_ret08 = gr_transf.acciones_ret08 + lr_montos.acciones
         	  WHEN lr_montos.subcuenta = 31 OR
         	  	   lr_montos.subcuenta = 32
         	  	 LET gr_transf.acciones_cv_cs = gr_transf.acciones_cv_cs + lr_montos.acciones
         	  WHEN lr_montos.subcuenta = 33 OR
         	  	   lr_montos.subcuenta = 34
         	  	 LET gr_transf.acciones_ahorro_sol = gr_transf.acciones_ahorro_sol + lr_montos.acciones
         	  WHEN lr_montos.subcuenta = 35
         	  	 LET gr_transf.acciones_fov08 = gr_transf.acciones_fov08 + lr_montos.acciones
         END CASE
      END FOREACH

      #Neto
      LET gr_transf.acciones_neto  = gr_transf.acciones_ret08     +
                                     gr_transf.acciones_cv_cs     +
                                     gr_transf.acciones_ahorro_sol

      #RCV
      LET gr_transf.acciones_rcv = gr_transf.acciones_ret08      +
                                gr_transf.acciones_cv_cs

      OUTPUT TO REPORT rpt_liquida(gr_transf.*)
   END FOREACH
   ...
   OUTPUT
    ...
    FIRST PAGE HEADER
       ...
       FOR li_cont = 1 TO 5
       	 ...
       	 CASE li_cont
       	 	  WHEN 1 LET lar_total_retiro[li_cont].tipo_retiro = "H" --DISP.TRAB. BENEF. AMPARO NEGATIVA PENSION
       	 	  WHEN 2 LET lar_total_retiro[li_cont].tipo_retiro = "I" --DISP.TRAB. BENEF. AMPARO PENSION AUT. ISSSTE
       	 	  WHEN 3 LET lar_total_retiro[li_cont].tipo_retiro = "J" --DISP.TRAB. BENEF. AMPARO PPP
       	 	  WHEN 4 LET lar_total_retiro[li_cont].tipo_retiro = "K" --DISP.TRAB. BENEF. INSTRUCCION AUTORIDAD
       	 	  WHEN 5 LET lar_total_retiro[li_cont].tipo_retiro = "L" --DISP.TRAB. AL AMPARO DE SU EDAD
       	 END CASE
      ...
      END FOR
       ...
       PRINT COLUMN 55,"REPORTE DE PRE-LIQUIDACION DE TRANSFERENCIAS DE RECURSOS ISSSTE",
            '\033015'
       SKIP 2 LINES

       PRINT COLUMN 001, "AFORE               :", gr_afore.razon_social CLIPPED,
             COLUMN 155, "PAGINA              :" ,PAGENO USING "####",
             '\033015'

       PRINT COLUMN 001, "PROGRAMA            :RETL925",
             COLUMN 155, "FECHA LIQUIDACION   :",gr_captura.fecha_liquida USING"DD/MM/YYYY",
             '\033015'

       PRINT COLUMN 001, "FOLIO INTERNO       :",gr_captura.folio USING "#########&",
             COLUMN 155, "FECHA GENERACION    :",HOY USING "DD/MM/YYYY",
             '\033015'
       ...
       PRINT COLUMN 001, "VALOR ACCION SB1: ", gar_precios[1].precio USING "###&.&&&&&&",
             ...
             '\033015'
       PRINT COLUMN 001, "VALOR ACCION SB4: ", gar_precios[4].precio USING "###&.&&&&&&",
             COLUMN 047, "VALOR ACCION SB5: ", gar_precios[5].precio USING "###&.&&&&&&",
             '\033015'

    PAGE HEADER
      PRINT COLUMN 001, "FOLIO INTERNO       :", gr_captura.folio USING "#########&",
            COLUMN 055, "REPORTE DE PRE-LIQUIDACION DE TRANSFERENCIAS DE RECURSOS ISSSTE",
            COLUMN 155, "PAGINA              :" ,PAGENO USING "####",
            '\033015'
    ...
    BEFORE GROUP OF lr_rpt.tipo_retiro
    	CASE lr_rpt.tipo_retiro
    		 WHEN "H" LET li_cont = 1
    		 WHEN "I" LET li_cont = 2
    		 WHEN "J" LET li_cont = 3
    		 WHEN "K" LET li_cont = 4
    		 WHEN "L" LET li_cont = 5
    	END CASE
      ...
      PRINT COLUMN 001,"|",
            ...
            COLUMN 055,"|",      --regimen
            COLUMN 092,"MONTOS PRE-LIQUIDADOS DE LA CUENTA INDIVIDUAL (PESOS) DEL TIPO RETIRO ",lr_rpt.tipo_retiro,
            COLUMN 185,"|",
                       '\033015'
     ...
     #BLOQUE TITULOS 3                    --CIERRA
      PRINT COLUMN 001,"|",
            ...
            COLUMN 058,"\302",            --siefore
                       ...
                       "\302", L10,L5,L1, --rcv
                       "\302", L10,L2,    --fecha_ini_pen
                       "\302", L10,L1,    --diag_procesar
                       "\277",
                       '\033015'
      ...
      PRINT COLUMN 001,"|    NSS",
            ...
            COLUMN 055,"|SB",
            COLUMN 058,"|    IMPORTE",
            COLUMN 075,"|  RETIRO  08",
            COLUMN 092,"|    CV / CS",
            COLUMN 109,"|  AHORRO  SOL.",
            COLUMN 126,"| FOVISSSTE 08",
            COLUMN 143,"|      RCV",
            COLUMN 160,"|FECHA INICIO",
            COLUMN 173,"|DIAGNOSTICO",
            COLUMN 185,"|",
            '\033015'
      ...
      PRINT COLUMN 001,"\300",L10,L1,            --nss
                       ...
                       "\301",L10,L5,L1,         --rcv
                       "\301", L10,L2,           --fecha_ini_pen
                       "\301", L10,L1,           --diag_procesar
                       "\331",
                       '\033015'
    ...
    ON EVERY ROW
      PRINT COLUMN 002, lr_rpt.nss                             ,
            ...
            COLUMN 059, lr_rpt.acciones_neto            USING "########&.&&&&&&",
            COLUMN 076, lr_rpt.acciones_ret08           USING "########&.&&&&&&",
            COLUMN 093, lr_rpt.acciones_cv_cs           USING "########&.&&&&&&",
            COLUMN 110, lr_rpt.acciones_ahorro_sol      USING "########&.&&&&&&",
            COLUMN 127, lr_rpt.acciones_fov08           USING "########&.&&&&&&",
            COLUMN 144, lr_rpt.acciones_rcv             USING "########&.&&&&&&",
            COLUMN 162, lr_rpt.fecha_ini_pen            USING "DD-MM-YYYY",
            COLUMN 178, lr_rpt.diag_procesar            USING "##&",
            '\033015'
    ...
    AFTER GROUP OF lr_rpt.tipo_retiro
    	...
    	FOR li_siefore = 1 TO 5
            ...
            PRINT COLUMN 001, "\332",L10,L1,                    --nss
                              ...
                              "\302",L10,L5,L1,                 --cv_cs
                              "\302",L10,L5,L1,                 --ahorro_sol
                              "\302",L10,L5,L1,                 --fov_08
                              "\302",L10,L5,L1,                 --rcv
                              "\302",L10,L10,L2,L2,             --LINEA
                              "\277",
                              '\033015'
            ...
            PRINT COLUMN 001, "|", lar_total_retiro[li_cont].lar_total_siefore[li_siefore].nss_tot  USING "##########&",
                  ...
                  COLUMN 058, "|", lar_total_retiro[li_cont].lar_total_siefore[li_siefore].acciones_neto           USING "########&.&&&&&&",
                  COLUMN 075, "|", lar_total_retiro[li_cont].lar_total_siefore[li_siefore].acciones_ret08          USING "########&.&&&&&&",
                  COLUMN 092, "|", lar_total_retiro[li_cont].lar_total_siefore[li_siefore].acciones_cv_cs          USING "########&.&&&&&&",
                  COLUMN 109, "|", lar_total_retiro[li_cont].lar_total_siefore[li_siefore].acciones_ahorro_sol     USING "########&.&&&&&&",
                  COLUMN 126, "|",
                  COLUMN 143, "|", lar_total_retiro[li_cont].lar_total_siefore[li_siefore].acciones_rcv            USING "########&.&&&&&&",
                  COLUMN 160, "|",
                  COLUMN 185, "|",
                  '\033015'
            ...
            PRINT COLUMN 001, "\300",L10,L1,                    --nss
                              ...
                              "\301",L10,L5,L1,                 --cv_cs
                              "\301",L10,L5,L1,                 --ahorro_sol
                              "\301",L10,L5,L1,                 --fov_08
                              "\301",L10,L5,L1,                 --rcv
                              "\301",L10,L10,L2,L2,             --LINEA
                              "\331",
                              '\033015'
            SKIP 1 LINE
      ...
      IF lar_gran_total_retiro[li_cont].nss_tot > 0 THEN
         ...
         PRINT COLUMN 001, "\332",L10,L10,L10,L10,L10,L2,L1, --titulo
                           "\302",L10,L5,L2,L2,              --nss
                           "\302",L10,L10,L10,L10,L10,       --espacios
                           "\302",L10,L5,L1,                 --acciones_fov08
                           "\302",L10,L10,L10,L10,L1,        --espacios
                           "\277",
                           '\033015'
         ...
         PRINT COLUMN 001, "|     TOTAL DE NSS UNICOS POR TIPO DE RETIRO ", lar_gran_total_retiro[li_cont].tipo_retiro,
               ...
               COLUMN 075, "|",
               COLUMN 126, "|", lar_gran_total_retiro[li_cont].acciones_fov08           USING "########&.&&&&&&",
               COLUMN 143, "|",
               COLUMN 185, "|",
               '\033015'
         ...
         PRINT COLUMN 001, "\300",L10,L10,L10,L10,L10,L2,L1, --titulo
                           "\301",L10,L5,L2,L2,              --nss
                           "\301",L10,L10,L10,L10,L10,       --espacios
                           "\301",L10,L5,L1,                 --acciones_fov08
                           "\301",L10,L10,L10,L10,L1,        --espacios
                           "\331",
                           '\033015'
      END IF
      SKIP 2 LINES
    ...
    ON LAST ROW
       ...
       FOR li_siefore = 1 TO 5
             ...
             PRINT COLUMN 001, "|", lar_gran_total_siefore[li_siefore].nss_tot  USING "##########&",
                   ...
                   COLUMN 055, "|S", li_siefore USING "&",
                   COLUMN 058, "|", lar_gran_total_siefore[li_siefore].acciones_neto       USING "########&.&&&&&&",
                   COLUMN 075, "|", lar_gran_total_siefore[li_siefore].acciones_ret08      USING "########&.&&&&&&",
                   COLUMN 092, "|", lar_gran_total_siefore[li_siefore].acciones_cv_cs      USING "########&.&&&&&&",
                   COLUMN 109, "|", lar_gran_total_siefore[li_siefore].acciones_ahorro_sol USING "########&.&&&&&&",
                   COLUMN 126, "|",
                   COLUMN 143, "|", lar_gran_total_siefore[li_siefore].acciones_rcv        USING "########&.&&&&&&",
                   COLUMN 160, "|",
                   COLUMN 185, "|",
                   '\033015'
       ...
       PRINT COLUMN 001, "|     TOTAL DE NSS UNICOS",
             ...
             COLUMN 075, "|",
             COLUMN 126, "|", lr_total_gral.acciones_fov08           USING "########&.&&&&&&",
             COLUMN 143, "|",
             COLUMN 185, "|",
             '\033015'
