...
#Propietario       => E.F.P.                      
#Programa TCAAB005 => TRASPASO ENTRE SIEFORES
#Sistema           => TES
#Ult Mod           => 30 de Abril del 2012  Se incorporo SIEFORE 10
#Fecha             => 29/11/2004
...
GLOBALS "TESB001S.4gl"  #  Definicion  de  querys
...
MAIN
    ...
    LET command = "fglgo TESL001.4gi"
    RUN command

    DISPLAY "============================="
    DISPLAY "* FIN PROCESO TRANSFERENCIA *"
    DISPLAY "============================="
...
FUNCTION  F_010_inicio()
   ...
   LET g_ruta_log1 = g_ruta_envio CLIPPED,"/TESB001.log"

   CALL    STARTLOG(g_ruta_log1)
   ...
   EXECUTE  sql_03      INTO      g_edo_procedente
   EXECUTE  sql_04      INTO      g_provisionada
   EXECUTE  sql_05      INTO      g_edo_recibido
...
FUNCTION  F_020_crea_folio_pendiente()
    ...
    DISPLAY  "TESB001: TRASPASO ENTRE SIEFORES " 
    DISPLAY  "TESB001: ",g_today USING "DD-MM-YYYY" 
   ...
   CALL F_022_precio_acc_parti()
    ...
    FOREACH cur_56 INTO   g_reg_tes_tipo_id.*
        ...
        IF  (g_registros  = 0 OR g_registros IS NULL) THEN 
           DISPLAY "TESB001: SIN REGISTROS PARA ",
        ...
        ELSE 
            DISPLAY "TESB001: ABORTANDO FOLIO YA SE ENCUENTRA PROVISIONADO ..."
	    EXIT PROGRAM
        END IF
...
FUNCTION  F_022_precio_acc_parti()
   ...
   IF bnd_precio = 1 THEN
      LET bnd_precio = 0
      DISPLAY "ABORTNDO PROCESO...PRECIOS INCONSISTENTES"
      EXIT  PROGRAM
   END IF
...
FUNCTION  F_030_trae_parametros()
    ...
    FOREACH cur_57 USING  g_edo_procedente
       ...
       EXECUTE sql_06 USING g_edo_procedente,
                            g_reg_tes_tipo_id.tipo_traspaso
                      INTO  g_registros

        DISPLAY "TESB001: TIPO TRANSFERENCIA     : ", 
          g_reg_tes_tipo_id.descripcion CLIPPED
        DISPLAY ""
        DISPLAY "TESB001: INICIA PROVISION FOLIO : ", 
          reg_tes_ctr_folio.folio USING "######"
        DISPLAY "TESB001: FECHA DE RECEPCION     : ", 
          reg_tes_ctr_folio.fecha_presentacion USING "DD-MM-YYYY" 
        DISPLAY "TESB001: FECHA DE PROVISION     : ", 
          reg_tes_ctr_folio.fecha_provision    USING "DD-MM-YYYY" 
        DISPLAY "TESB001: FECHA DE LIQUIDACION   : ", 
          reg_tes_ctr_folio.fecha_liquidacion  USING "DD-MM-YYYY" 
        DISPLAY "TESB001: REGS. A PROCESAR       : ",
          g_registros  USING  "######"
        DISPLAY ""
...
FUNCTION  F_100_proceso()
    ...
    DISPLAY "TESB001: PROCESANDO FOLIO " ,
          reg_tes_ctr_folio.folio USING "######"
...
FUNCTION  F_133_arma_saldo_rcv()
   ...
   FOREACH  c_saldo_rcv  USING   reg_tes_solicitud.nss,
                                 v_subcuenta ,
                                 v_ramo      ,
                                 g_today
...
#idp-----------------------------------
    ...
    LET      g_tipo_mov                =  210 #traspaso entre siefores cargo
    ...
    FOREACH  spl_provisiona        USING
                  ...
                  g_scta                            ,      #subcuenta
                  g_tipo_mov                        ,      #tipo_movimiento 210
    ...
    EXECUTE sql_54 USING  g_siefore , 
                          reg_tes_ctr_folio.fecha_provision
                   INTO   l_precio_dia_siguiente

    LET      g_ins_pesos               =  - g_ins_acciones * 
                                          l_precio_dia_siguiente
    LET      g_ins_acciones            =  0
    LET      g_tipo_mov                =  1 #traspaso entre siefores abono
...
FUNCTION  F_250_updates()
    ...
    EXECUTE  sql_15
       USING  g_provisionada,reg_tes_ctr_folio.folio,g_edo_procedente
    DISPLAY "TESB001: PROVISION  FOLIO ",
